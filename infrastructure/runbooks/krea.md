...
**Status: rails ready as of 2026-06-12 (ALC SPR-02).** No live call has
ever been made from this codebase; the first live execution is SPR-09's
capped smoke, and its preflight step zero is section 5 of this runbook
(the rotation written from the 2026-06-12 leak — rotation is still
pending; the account's API balance is funded, operator-reported
2026-06-12).
...
With no token wired, everything degrades gracefully: `/krea/*` answers a
typed `503 {"enabled": false, "reason": "no_key"}` and the UI keeps its
deterministic placeholder.

Prod-state honesty: **no record says a Krea token was ever set on prod**
(inferred-from-ansible — the secrets template only gained the
`KREA_API_TOKEN=` line in this sprint, and no earlier runbook mentions
Krea; we cannot read `/etc/antiek/secrets.env` from a dev machine).
Section 3 therefore starts by checking the live file, never assuming.

Sections:

1. Mint a token
2. Fund the API balance — **the money step; separate from any subscription**
3. Wire the token (local + prod)
...
Fill in the `KREA_API_TOKEN=` line (the ansible template ships it empty
with the optional knobs commented —
...
The optional knobs (`KREA_KILL_SWITCH`, `ANTIEK_KREA_BASE_URL`,
`ANTIEK_KREA_MODEL_PATH`, `KREA_DAILY_UNIT_CAP`, `KREA_RATE_LIMIT_MAX`,
`KREA_POLL_BUDGET_S`, `KREA_CACHE_TTL_S`) all have conservative defaults
in `krea_routes.py` — leave them commented unless section 6 or 7 gives
you a reason.
...
still pending** and is SPR-09's preflight step zero. The failure class
...
- `interfaces/research/api/krea_routes.py` — the proxy: gate order,
  ...
  shapes (live verification pending SPR-09)
