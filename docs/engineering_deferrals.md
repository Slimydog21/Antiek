...
## D17 — Mountain Shell v2 yellow re-tone Tailwind mirror
...
The D17 follow-up now re-tones `apps/reading/tailwind.config.js` to mirror the
weathered day utilities (`sun-deep #9C8636`, `sun-glow #F1E08F`) and night shadow
glow (`#84722F`) while keeping `sun` / `bar-accent` loud. `tokens.contrast.test.ts`
imports Tailwind config and asserts the mirror stays byte-identical to
`tokens.ts`.
...
correct) instead of the Tailwind mirror. The D17 follow-up changed the mirror and
