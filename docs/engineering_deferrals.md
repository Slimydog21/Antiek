...
**Status:** ✅ **LIVE INTEGRATIONS CLOSED by the Living Roadmap (PR #16,
`origin/main` `a94d357`) plus 2026-06-30 follow-ups.** The Physics of Reading substrate shipped *dormant* to prod first
...
run then shipped the feeds three of them were waiting on** — the geometry
...
`main`. The 2026-06-30 follow-ups then closed the SiteSee book-reader anchor,
shared voice-blob storage, `claim.reviewed` resolver, review gesture/v1
scheduler, and persisted Review cues policy toggle. The two tracks dovetailed:
PR #14 shipped the consumers, PR #16 and the follow-ups shipped the feeds and
surface controls. What REMAINS deferred is operator-only canon ratification that
flips the CI guard advisory→blocking. No augmentation invents its own data
(PR-2 / PR-6).
...
- **The `source.read` event** (`docs/decisions/spr-06-source-read-event-gap.md`)
  ...
  SiteSee resolver is live (`modes/Reading/sourceRead.ts`). The 2026-06-30
  follow-up also closed the book-reader chunk anchor:
...
- **The review-state resolver + review gesture + policy toggle**
  ...
  a typed event, the generated TypeScript payload exists, `reviewState.ts`
  resolves latest-event-wins review history into `ReviewDueClaimView[]`, and
  `ResearchWorkstation` hands those due claims to `MasterMdViewer` through a
  persisted Review cues operator toggle. Due claims expose Again/Good/Easy review
  controls that emit deterministic scheduler metadata (30 minutes / 1 day / 7
  days). No review history still produces the honest empty due set; policy off
  keeps the reusable viewer dormant.
...
**What's left:** the live integrations are closed. The canon ratification is an
...
**Status:** ✅ Closed on 2026-06-30. The design *tokens* were already re-toned +
AA-proven (SPR-09 `af21f19`: `apps/reading/src/design/tokens.css` + `tokens.ts`
softened the `--sun-*` ramp, kept `--sun`/`--bar-accent` loud, 12/12 contrast).
The D20 follow-up now re-tones `apps/reading/tailwind.config.js` to mirror the
canonical CSS variables (`var(--sun-deep)`, `var(--sun-glow)`) and night shadow
glow (`var(--sun-deep)`) while keeping `sun` / `bar-accent` loud.
`tokens.contrast.test.ts` imports Tailwind config and asserts the mirror stays
byte-identical to the canonical token surface.
**Unlock criterion:** satisfied (operator-discretion / cheap tidy).
**Spec reference:** `docs/decisions/ams-v2-shell-ship.md` + the verification
report `docs/ams-v2/mountain-shell-v2-verification.md` §4 ("Yellow re-tone
Tailwind mirror closed"); SPR-09 `af21f19`.
**Blocks-what:** nothing — this was chrome-consistency polish, not a functional
gap.

The anti-pattern this entry guarded against: a future agent seeing a loud-yellow
Tailwind utility on screen and **re-softening the CSS vars** (which were already
correct) instead of the Tailwind mirror. The D20 follow-up changed the mirror and
added the CI guard, so future drift reds at the token-test layer.

**Verification:** `tokens.contrast.test.ts` covers the weathered-token contrast
floor and the Tailwind mirror guard; the AMS real-browser gates remain the
full-shell visual proof.
...
**Spec reference:** `docs/decisions/owner-privileged-read-path.md`; CLAUDE.md critical invariant #5 (single-operator until Sprint 22); the SINGLE-OPERATOR ENFORCEMENT comment in `interfaces/research/api/books.py` (~lines 66–90); the `owner-privilege-tag` invariant at `substrate/invariants/owner-privilege-tag.toml`.
**Blocks-what:** nothing today. Single-operator + the fail-closed gate (`_owner_read_policy_tag` returns `attribution_eligible` when `len(operator_allowlist_from_env()) > 1`) + the `tools/lint/owner_privilege_check.py` guard make this a structural no-op until multi-operator configs exist. A deployment with 2+ operators will NOT leak cross-tenant corpus access — the privileged bypass is simply disabled for both tenants (fail-closed-by-architecture, not merely documented).
**Action when unlocked:** when D1 closes and Sprint 22 multi-user activation begins, extend `interfaces/research/api/books.py::_owner_read_policy_tag` to require `request.state.user_id == owner_user_id` rather than relying on the single-operator gate alone (the `owner_user_id` lookup coming from the document's books metadata or a session-scoped user_email→user_id map), and add a `tools/lint/owner_privilege_check.py` rule that any `operator_only` retrieval call site either scopes by user_id or is justified by a comment citing the single-operator gate.
---
...
- **D13 (Physics of Reading live integrations)** — **live integrations closed by
  PR #16 plus the 2026-06-30 follow-ups** (the geometry pass, the `source.read`
  event + resolver, the marginalia note→node write path, the `claim.reviewed`
  signal + resolver + review gesture/v1 scheduler + policy toggle, the SiteSee
...
PR #16 plus the 2026-06-30 review-state/read-tint follow-ups — the geometry pass,
the `source.read` event, the marginalia note→node write path, the
`claim.reviewed` signal + resolver + review gesture/v1 scheduler + policy
toggle, the SiteSee read-tint chunk anchor, and shared marginalia voice-blob
storage are live; only the operator-only canon ratification remains**.
