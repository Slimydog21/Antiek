...
The `no_raise_in_substrate_writers` lint enforces this: a `raise` of a non-system exception in a protected path is flagged (see "Enforcement").
...
Add a variant: define a Pydantic model with `kind: Literal["new_kind"]`, add it to the `SubstrateError` union. Every `match` over `SubstrateError` that uses `assert_exhaustive` will then fail `mypy --strict` until it handles the new arm.
...
| `tools/benchmarks/hot_paths/` | perf harness | `hot_path_language.md` (ARE-12) |
