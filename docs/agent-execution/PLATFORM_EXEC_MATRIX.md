...
| P-16b | Parallel gather mock E2E | `acquisition/search/parallel/` + `make_parallel_gather_loop` | `pytest tests/test_parallel_gather_loop.py -q` | `ANTIEK_DRW_GATHER=parallel` + `PARALLEL_API_KEY` in operator `.env` | Live Parallel index coverage / cost |
...
| P-17b | Pack fidelity | `orchestration/session_evidence_pack.py` | `pytest tests/test_drw_pack_fidelity.py -q` | — | Operator accepts synthesis on real query |
| P-18 | ResearchArtifact HTML transport | `substrate/research_artifact/` + `ArtifactOutlineShelf` (DistillView) | `./scripts/canonical_verify.sh html-transport` | — | Full Write canvas tab-complete; book/EPUB reader HTML ingest |

**Profile:** `./scripts/canonical_verify.sh deep-research` runs P-11..P-17 hermetic gates incl. P-16b Parallel gather + P-17b pack fidelity (ANT-DRL SPR-DRL-02, SPR-DRL-08, SPR-DRL-09).

**Profile:** `./scripts/canonical_verify.sh html-transport` runs P-18 (ANT-AHT SPR-AHT-01..06).
