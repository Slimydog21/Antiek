...
| P-49 | Reading substrate pytest | `.github/workflows/ci.yml` `pytest` job + `docs/decisions/ci-pytest-timeout.md` + `tests/test_ci_pytest_timeout_docs.py` | CI on `main` (full suite) | — | Local hardware parity |
| P-50 | Agent execution gates | `tools/agent/verify_handoff.ts` + `scripts/audit_agent_session.sh` + `apps/reading/src/modes/Settings/Settings.test.tsx` + `apps/reading/src/shared/copyLint.test.ts` + `tests/test_provenance_ref_lint.py` + `tests/test_prompt_autoresearch_readiness.py` + read activation + `tests/test_operator_actions_probe_contracts.py` + operator support probes + Werner paths | `canonical_verify.sh agent-gates` + case study §5 | Werner operator card (htmlspec) | Live Werner p95/fps and operator UX without artifact |
| P-51 | Serve / rights / legal | `docs/OPERATOR_ACTIONS.md` + `infrastructure/runbooks/first-deploy.md` + `tools/ops/retrieval_gate_probe.py` + `tests/test_retrieval_gate_probe.py` | **No** informational CI job alone (F7); `canonical_verify.sh agent-gates` runs the OA-020 support probe tests | Operator deploy checklist + production probe evidence | Jurisdiction-specific legal review and production-VM probe run |
...
| P-54 | Loop 1 E2E (DeepResearchComplete) | `orchestration/loop_one/orchestrator.py` | `pytest tests/test_loop_one_orchestrator.py::test_loop_one_happy_path_emits_completed -q` | — | Live LLM on all 5 roles |
| P-55 | DeepResearchComplete negative | `orchestration/invariants/deep_research_complete.py` | `pytest tests/test_deep_research_complete.py::test_drw_only_trajectory_fails_without_synthesis -q` | — | Production DRW with Exa adapter |
| P-56 | Cascade session reconstruct | `orchestration/cascade_session.py` | `pytest tests/test_cascade_session.py -q` | — | SSE transport reconnect E2E |
| P-57 | PromotionFunnel serialize | `runtime/research_runner/promotion_funnel.py` | `pytest tests/test_research_runner.py::test_promotion_funnel_serialized_no_lock_timeout -q` | — | Remote-exec fan-out under load |
| P-58 | knowledge.reused flywheel | `runtime/research_runner/host_local.py` start path | `pytest tests/test_flywheel_reuse.py::test_two_run_contract_gather_emits_knowledge_reused_on_second_start -q` | — | Live dispatch cost delta > 0 on reuse-consuming loop |
| P-59 | Exa gather mock E2E | `runtime/research_runner/host_local.py make_exa_gather_loop` | `pytest tests/test_exa_gather_loop.py -q` | — | Live EXA_API_KEY discover→ingest cost |
| P-60 | Parent-terminal observability | `interfaces/research/api/cascade_routes.py _run_to_completion + session_status` | `pytest tests/test_drw_parent_terminal.py -q` | `docs/decisions/deep-research-smoke-checklist.md` | Real session-parent DeepResearchComplete on smoke DRW #1 |
| P-61 | ResearchArtifact HTML transport | `substrate/research_artifact/` + `ArtifactOutlineShelf` (DistillView) | `./scripts/canonical_verify.sh html-transport` | — | Full Write canvas tab-complete; book/EPUB reader HTML ingest |

**Profile:** `./scripts/canonical_verify.sh research-bridge-dogfood` runs P-52 bridge closure hermetic gates (ADRB log, readiness, reconcile, report, verdict, draft export).

**Profile:** `./scripts/canonical_verify.sh prompt-autoresearch-wedge1` runs P-53 hermetic gates (CLI, readiness, calibration, verdict, probe, docs).

**Profile:** `./scripts/canonical_verify.sh deep-research` runs P-54..P-60 hermetic gates (ANT-DRL SPR-DRL-02, SPR-DRL-08, SPR-DRL-09).

**Profile:** `./scripts/canonical_verify.sh html-transport` runs P-61 (ANT-AHT SPR-AHT-01..06).
