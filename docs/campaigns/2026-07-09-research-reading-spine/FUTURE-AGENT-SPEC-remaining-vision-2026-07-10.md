# Future-agent execution brief — remaining Antiek vision gaps

**Campaign tip at write:** `447182e2` · branch `campaign/research-reading-spine-2026-07-09-main` · PR **#465** (OPEN)  
**URL:** https://github.com/Slimydog21/Antiek/pull/465  
**Bar:** James Hawkins craftsmanship · five values · HTML-first · propose≠promote · NotDiamond advisory only  
**STOP:** operator only (`.infinite/STOP` or explicit message)

## Do not rebuild

Prefer **SESSION-ARC-tt-uw-infinite-wave.md** + **inventory-vg.txt** + competitive notes. Tip letters **tt–vg** (40 residual ships this infinite wave after tt).

### Closed product map (tt–vg)

| Theme | Residuals |
|---|---|
| Write-seed SSOT | tt–tu · tv · **vd–vg** twin_cross_asset_merge full path (known 16 · dogfood v8) |
| Free STEM (Boole, Heaviside) | tx–tz · ub–ud dogfood v5–v6 |
| Multi-float Select open | ue–uf |
| MO foresight | uh L4 · um–un remaining-after · uw hydrate Settings |
| arxiv/substack honesty | uj–uk · up–uq · us · uu · va–vb |
| Marketplace L5 payment honesty | uy |
| Swarm handoff | … · vf · **vh** |

### P0 — Operator merge PR #465
Agents never merge main/prod.

### P2 — Dual-gate live injectors (operator-only)
L1–L4 offline default · L5 payment deferred · L6 live multi-agent · L7 ND **never** router

### P1 next offline (residual **va+**)
Outside saturated hydrate/MO/catalog/dogfood thrash — inventory twin/note-taker or grounded citation path for real gaps only.

## Doctrine
One residual letter/cycle · inventory-first · green tests · campaign push only · complete work never complete goal · Infinite until STOP.
