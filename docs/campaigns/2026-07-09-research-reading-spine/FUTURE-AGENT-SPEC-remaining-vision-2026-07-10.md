# Future-agent execution brief — remaining Antiek vision gaps

**Campaign tip at write:** `8d924665` · branch `campaign/research-reading-spine-2026-07-09-main` · PR **#465** (OPEN)  
**URL:** https://github.com/Slimydog21/Antiek/pull/465  
**Bar:** James Hawkins craftsmanship · five values · HTML-first · propose≠promote · NotDiamond advisory only  
**STOP:** operator only (`.infinite/STOP` or explicit message)

## Do not rebuild

Prefer **SESSION-ARC-tt-vt-infinite-wave.md** + inventories **tt–vt** + competitive notes. Tip letters **tt–vt** (53 residual ships).

### Closed product (high signal)

| Theme | Residuals |
|---|---|
| Write-seed SSOT | known_count **17** · full hosted Open Write provenance matrix |
| Free STEM books | Boole · Heaviside · dogfood v5–v6 |
| Multi-float Select open | all surfaces |
| MO foresight | L4 · remaining-after rec+custom · hydrate Settings |
| arxiv/substack honesty | full matrix · dogfood v7 |
| Marketplace L5 | manual_receipt_only + Settings dual-gate L5 deferred (vt) |
| Dogfood recursive | v4–v9 |

### P0 — Operator merge PR #465
Agents never merge main/prod. **This is the highest leverage action.**

### P2 — Dual-gate live injectors (operator-only)
L1–L4 offline default · L5 payment deferred · L6 live multi-agent · L7 ND **never** router

### P1 next offline (residual **vn+**)
Outside saturated write-seed/dogfood thrash unless regression. Prefer real product bugs found by inventory.

## Doctrine
One residual letter/cycle · inventory-first · green tests · campaign push only · complete work never complete goal · Infinite until STOP.
