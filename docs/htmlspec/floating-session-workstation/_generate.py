...
SPRINTS: list[dict[str, Any]] = [
    ...
    {
        "id": "FSW-SPR-11",
        "slug": "reviewed-publication-source-acquisition",
        "title": "Reviewed publication-source acquisition",
        "status": "partial",
        "wave": 11,
        "goal": "Let a consented collective execution gather only the exact arXiv and Substack references frozen in the reviewed collective revision, with signed source authority, bounded rights-aware excerpts, canonical receipts, and honest browser scope before spend.",
        "files": "substrate/midnight_oil/{publication_sources,swarm_plan,worker_cli,live_roles,session_flywheel}.py; interfaces/research/api/{engagement_routes,midnight_oil_routes}.py; apps/reading/src/{api/engagement.ts,components/engagement/CollectiveExecutionPanel*}; tests/test_{midnight_oil_publication_sources,midnight_oil_worker_cli,engagement_routes}.py",
        "milestones": [
            ("Freeze reviewed source authority", "Derive one closed, canonical manifest from the immutable collective unit's exact source_references. V1 admits modern arXiv abs identities and HTTPS *.substack.com/p/<slug> posts only; it rejects custom domains, generic URL/unknown references, recomputed ref-id mismatch, duplicates, more than 64 refs, or a manifest above 256 KB. Mutable latest-version identities are explicit: the first acquired bytes become the durable operation revision and changed retry bytes require re-review/reconciliation.", "The manifest hash commits ordered ref id, kind, canonical identity, acquisition mode, excerpt cap, rights-policy id, and connector-capability id; full JSON/hash are included in the execution receipt and owner authority, while the hash is independently included in context binding and JobConsentConfig v4 before enqueue or provider work."),
            ("Acquire bounded evidence through attested connectors", "Run all required publication preflight/acquisition before the planner or any later provider. The connector port has no generic URL method. Its egress transport permits HTTPS:443 to the exact manifest host/path only, disables redirects and subresources, rejects credentials and private/loopback/link-local/reserved DNS answers, and records attempted destinations. arXiv returns metadata/abstract; Substack requires an explicit excerpt injector.", "Known blockers stop before consent/enqueue. A post-ready acquisition failure terminates with zero provider calls and deposited limitations. Persist one result per job+manifest+ref before planning; crash-after-result reuses exact bytes, while claim-without-result or changed bytes requires reconciliation. No partial exact-source execution."),
            ("Extend canonical evidence receipts", "Use a discriminated corpus/publication receipt union. Bind each publication receipt to owner digest, execution/job, manifest, stage key/role, question, reviewed ref, signed plan, connector id/version, canonical URL, extraction mode/range, source length when known, truncation reason, rights tier/use, byte length, and excerpt SHA-256. Treat source text as inert evidence input.", "Gatherer output accounts for every supplied receipt once; cross-owner/job/manifest/stage/ref/question substitution, kind confusion, changed bytes, prompt injection, duplicate identity, and off-manifest material are rejected. Substack fails closed unless the connector proves excerpt scope; a byte cap alone never proves it was not a short full post."),
            ("Show scope and limitations before consent", "Render the exact reviewed publication list, acquisition mode, cap, live/offline connector readiness, and exclusions in the cohesive-execution panel. Require preparation to return the source manifest hash and counts; invalidate prepared authority when the collective revision changes.", "The UI says arXiv abstract versus Substack bounded excerpt, never claims full-text access, never offers generic web, keeps the consent token ephemeral, and projects source failures/limitations into terminal evidence rather than hiding them."),
        ],
        "out": "No arbitrary web search, worker-selected URLs, generic URL acquisition, full-page Substack scraping, paywall bypass, PDF viewing, automatic body rehosting, mutable post-confirmation source edits, or claim that an attached reference was fetched successfully.",
        "gates": "uv run pytest -q tests/test_midnight_oil_publication_sources.py tests/test_midnight_oil_worker_cli.py tests/test_midnight_oil_live_stage_engine.py tests/test_engagement_routes.py; uv run mypy --strict <changed-python>; uv run ruff check <changed-python>; npm --prefix apps/reading test -- src/api/engagement.test.ts src/components/engagement/CollectiveExecutionPanel.test.tsx; npm --prefix apps/reading run typecheck; npm --prefix apps/reading run build; hardenx . --strict --no-color",
        "rigor": {
            "1 · Intellectual honesty": "A source reference proves reviewed intent, not successful acquisition or redistribution permission. The manifest and UI must distinguish arXiv metadata/abstract, bounded Substack excerpt, connector unavailable, truncated/paywalled, and rights-denied; never turn identity-only material into evidence.",
            "2 · Fairness": "Steelman querying the whole operator corpus: it improves recall and remains useful as a separate signed policy, but it does not satisfy the user's exact-source request. Preserve corpus retrieval while ensuring external connector traffic is limited to the confirmed unit; publishers absorb no hidden scrape or paywall-bypass behavior.",
            "3 · Rigor": "Test manifest ordering, ref-id recomputation, custom/generic host rejection, 64-ref/256-KB caps, redirect and DNS/private-IP egress attempts, Unicode byte caps, license unknown, unproven Substack excerpt scope, connector absence, crash checkpoints, changed retry bytes, cross-execution receipt substitution, and provider-call count zero—including planner—for every source failure.",
            "4 · Diligence": "Reuse SourceReference normalization, collective_preview_sha256, JobConsentConfig v3 binding, signed SwarmLivePlan authority, acquisition.arxiv metadata client, explicit Substack injector seam, arxiv_tiers deny-by-default resolution, and live-stage receipt joins. Do not reuse hydrate's exception-swallowing identity fallback in a paid gather path.",
            "5 · Defensibility": "Persist the manifest JSON/hash, connector identity, canonical URL, use mode, rights/truncation fields, exact excerpt hash/bytes, question lineage, and terminal limitations. A future auditor must prove which reviewed source was requested and what bounded bytes reached a model without retaining secrets or an unauthorized full body.",
        },
        "pattern": "adversarial-verification",
        "lenses": "signed manifest drift and off-list network access; rights/truncation/redirect fail-closed behavior; receipt substitution and UI scope honesty",
    },
]
...
<footer class="spec-footer">{esc(row['id'])} · generated {DATE} · source: active Antiek /goal Cycle 16</footer>"""
...
<footer class="spec-footer">Generated by htmlspec · source: active Antiek /goal Cycle 16 · {DATE}</footer>"""
