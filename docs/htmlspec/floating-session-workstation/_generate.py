...
<footer class="spec-footer">{esc(row['id'])} · generated {DATE} · source: active Antiek /goal Cycle 10</footer>"""
...
<section class="block"><h2>Rejected alternatives</h2><table class="spec"><thead><tr><th>Alternative</th><th>Why rejected</th><th>Reconsider if</th></tr></thead><tbody><tr><td>New session/merge service</td><td>Duplicates shipped engagement_spine writers and breaks recursive twin provenance.</td><td>The canonical spine is deliberately retired through a migration.</td></tr><tr><td>Silent cross-asset collective</td><td>A multi-selection mistake can mix unrelated or future foreign-owner context.</td><td>Ownership policy explicitly defines shared workspaces and UI communicates scope.</td></tr><tr><td>Automatic into-parent merge</td><td>Destroys the draft-review boundary the product vision explicitly asks for.</td><td>Never; confirmation may become a richer review transaction, not disappear.</td></tr><tr><td>API-only graph ownership</td><td>An owner-verified route still deposits private note labels into the shared graph, where global readers or identical-text dedup can cross account boundaries.</td><td>The graph schema and every read surface gain a proven row-level owner policy; physical per-owner graphs remain the safer current contract.</td></tr><tr><td>Content hash as the whole receipt</td><td>Stable node ids prevent duplicate rows but do not pin the reviewed selection, prevent idempotency-key reuse, make a multi-note batch atomic, or replay the exact response after a crash.</td><td>Never for explicit user confirmation; content addressing remains one layer inside the receipt transaction.</td></tr><tr><td>Replace canonical retrieval with the private graph</td><td>Personal partitions currently contain promoted notes, not the shared documents/chunks needed for professional research; replacing rather than fusing would make authenticated users less informed.</td><td>Documents and rights-aware corpus retrieval migrate to a fully owner-native store with an explicit public federation layer.</td></tr><tr><td>Fall back to canonical when an owner graph is absent</td><td>Absence would silently change the meaning and confidentiality scope of a personal-memory request; it also makes routing bugs look like valid empty-user behavior.</td><td>Never implicitly. Canonical public retrieval remains a separately declared input to the fusion.</td></tr></tbody></table></section>
...
<footer class="spec-footer">Generated by htmlspec · source: active Antiek /goal Cycle 10 · {DATE}</footer>"""
