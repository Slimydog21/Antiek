...
|---|---|---|---|---|
| `Library.openWork` | Library mode `open` callback / `BookCard onOpen` | `modes/Library/index.tsx:150` (`openDocument(documentId)`); card wired from the shelf | **`openDocument` → `/read/:id`** | **SPR-05** swapped direct `navigate('/read/…')` for the one resolver; **SPR-09** asserts. |
...
| `DRW.citeSource` | "cite source" affordance on a canvas BlockCard / its detail | door: `modes/DeepResearchWorkspace/Canvas/BlockCard.tsx:120` (`onCiteSource(node)`); wired from `index.tsx:197`–204; provenance resolved in `BlockDetail.tsx:48` (`documentId: node.source_document_id`) | **`openDocument` → `/read/:id` with optional chunk** | **SPR-07** resolves provenance to a real source open via `openDocument`; **SPR-09** asserts. (`EXPECTED_OPEN_DOORS` attributes it to `index.tsx (BlockCard detail)`; the button lives in `Canvas/BlockCard.tsx`, the host wiring in `index.tsx` — see §4.) |
| `Write.traceToSource` | citation-chip trace-to-source in the Write editor | emit: `modes/Write/Editor/Citation.tsx:38` (`emitTraceIntent`); bus: `modes/Write/Editor/traceIntent.ts:28`; resolve+open: `modes/Write/WriteHome.tsx:116` (`openDocument(target.document_id, { chunkId })` when servable); provenance chain view: `modes/Write/Xray.tsx:3` (`getTraceTarget`) | **`openDocument` → `/read/:id` when servable, honest fallback when gated** | **SPR-07** routes through `openDocument` with the §9.0 gate; **SPR-09** asserts. (`EXPECTED_OPEN_DOORS` lists `Citation.tsx + Xray.tsx + WriteHome.tsx`; the OPEN action is `WriteHome.tsx:116`. `Xray.tsx` is the provenance-chain VIEW, not the open call — see §4.) |
| `MetaReading.openCitation` | click a citation in a meta-reading deliverable | `modes/Reading/MetaReading/index.tsx:100` (`openCitation` cb) → `:102` `openDocument(c.document_id, { page, chunkId })`; chip wired below | **`openDocument` → `/read/:id` with citation locator** | **SPR-05/07** routes through `openDocument`; **SPR-09** asserts. |
| `Route./read/:documentId` | the canonical Reader route | `App.tsx:173` (`<Route path="/read/:documentId" element={<BookReader />}>`) | mounts `BookReader`, the one Reader host | **SPR-03** mounts the one `<Reader>` here; **SPR-05** makes it `openDocument`'s mount target; **SPR-09** asserts every OPEN door lands here. |
...
3. **`Write.traceToSource`'s OPEN action is `WriteHome.tsx:116`; `Xray.tsx` is a VIEW, not the open call.**
   ...
   call (the actual open).
   `Xray.tsx:3` renders the paragraph↔blocks provenance chain (`getTraceTarget`);
   it does not itself open a document. SPR-07 routes `WriteHome.tsx:116` through
   `openDocument`.
