...
| `source_gate` wired; DB-backed census producer present; live corpus artifact deferred | `tools.source_census` + `tools.lint.source_gate` | `tools/source_census.py`, `tools/lint/source_gate.py:1-21` | `tests/test_source_gate.py` | `tools/lint/source_gate.py` (CI) | P3 merged (**SR-10** for live census) | **PARTIAL** |
...
| Adapters migrate to register; allowlist → empty | `acquisition.*.adapter` | per-adapter insert paths | `tests/test_register_source.py` + acquisition adapter cluster | `tools/lint/register_check.py` | **SR-05** | **CLOSED** (see `docs/decisions/asr-sr05-adapter-migration.md`; allowlist empty) |
| NULL backfill on prod DB (box) | `tools.backfill_null_content_class` | `tools/backfill_null_content_class.py` | `tests/test_backfill_null_content_class.py` | dry-run/apply CLI; prod guard | **SR-06** | **PARTIAL** (tooling landed; prod DB run still operator-gated) |
