...
| `GET /chunks/{chunk_id}` withholds `personal_reading` body | `interfaces.research.api.app` | `interfaces/research/api/app.py:2147-2219` | — (no `personal_reading` chunk fixture) | — | **SR-02** | **OPEN** |
| NULL `content_class` grandfather passes public retrieval (legacy carve-out) | `substrate.graph.search` | `substrate/graph/search.py:285-296` | `tests/test_retrieval_time_gate.py` (NULL legacy) | — | **SR-06** / **SR-07** | **OPEN** |
...
| `serve_full_text_guarded` is sole serving-boundary caller | `substrate.books.serve_guard` | `substrate/books/serve_guard.py:1-50` | `tests/test_serve_guard.py` | `tools/lint/serve_guard_check.py` | PR #43 | **CLOSED** |
