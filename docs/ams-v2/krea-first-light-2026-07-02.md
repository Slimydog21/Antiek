...
- **RUN B, local:** uvicorn (`--workers 1`, port 8123) booted from the adapter branch's
  tree; on main that same live-correct model-in-path adapter is now present via PR
  #110 / commit `ce6178ec` (`interfaces/research/api/krea_routes.py`) with
  `KREA_DAILY_UNIT_CAP=3`; server healthy in 128 s (cold-boot compile of a fresh
  worktree — budget ≥180 s for first boot). Then `tools/krea_smoke.py --base`
  (lands via ALC-Revival SPR-02; this branch's own first-contact procedure), then two
  timed fresh `/krea/scene` calls.
...
- **The three stream-spike rows (now measured, flux baseline):**
  ...
  - (iii) $/min at cap: **$0.33 at $0.04/unit** or **$0.057 at the runbook's
    $0.007/gen** (`infrastructure/runbooks/krea.md`, lands via ALC-Revival SPR-02)
    — the unit price is the one number this run cannot see; the
