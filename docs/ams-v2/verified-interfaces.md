...
| Regression-anchor spec | `NEW: apps/reading/e2e/ams-shell.spec.ts` | `NEW-to-build` | SPR-01 (M4) | Initially five skipped placeholders (scene/window/igloo/hotkey/penguin), each naming the owning sprint that flips it green. SPR-10 reconciled them to ordinary green `ams-real` tests (§11). |
...
| Tokens soften the yellow (SPR-09 + D17 follow-up) | SPR-09 softened the var-driven tokens (`tokens.css`/`tokens.ts`): `--sun`/`--bar-accent` kept loud (`#F5DF24`), accents re-toned + a new `--sun-light` ramp, AA preserved. The D17 follow-up then re-toned the `tailwind.config.js` mirror for `sun-deep`, `sun-glow`, and night shadows, with `tokens.contrast.test.ts` guarding the mirror. | RECONCILED |
