...
These come from the throwaway Playwright gate `apps/reading/e2e/_ams/stream-spike.spec.ts` (closed spike artifact at `1494d53b`, not landed on main; the `chromium`/Storybook project), driving the real `<Scene/>` compositor via the `ProceduralFloor` story (`apps/reading/src/scene/StreamSpike.stories.tsx`, closed spike artifact at `1494d53b`; story id `ams-v2-stream-spike--procedural-floor`). The spec ran with `contextOptions: { reducedMotion: "no-preference" }` so `useSceneClock` was **not** frozen (asserted live: `data-scene-frozen="false"`). Real values are persisted to `apps/reading/e2e/_ams/.artifacts/stream-spike-metrics.json` (closed spike artifact at `1494d53b`, not landed on main).
...
| (ii′) sustained **generative** fps | **0.136 gen fps** (measured 2026-07-02) | measured, 1/avg submit→complete, n=2 | doc-derived ceiling was ~0.25 (Flux ~4 s/img); measured reality is ~half of it (7.33 s/img) |
| (i′) time-to-first-**generated**-frame | **7.17 s min / 7.33 s avg** (measured 2026-07-02) | measured, s, `/krea/scene` fresh path (submit→poll→complete through the L411 adapter), n=2 + smoke's 1 | doc-derived was ~4 s/img + ≤12 s poll budget — inside the poll budget, ~1.8× the per-image estimate |
| (iii) $/minute @ configured cap | **$0.33/min at $0.04/unit** (measured latency × doc unit price); **$0.057/min if bfl/flux-1-dev bills $0.007/gen** per `infrastructure/runbooks/krea.md` (lands via ALC-Revival SPR-02) | measured 8.2 img/min ceiling (60/7.33 s) × unit price; **unit-price ground truth is the operator's dashboard decrement** | doc-derived was ~$0.60/min @ 4 s/img; slower measured latency cuts the burn rate proportionally |
...
- *Generative fps* — required **≥ 10 gen fps** for "near-real-time"; doc-derived ceiling was **~0.25 gen fps** (Flux ~4 s/image). **Measured 2026-07-02: 0.136 gen fps** (7.33 s/img avg through the live L411 adapter) — miss by **~74×**, worse than the estimate. The not-live caveat is retired; the miss is now measured.
- *$/minute at the configured cap* — doc-derived was **~$0.60/min**. **Measured ceiling: 8.2 img/min ⇒ $0.33/min at $0.04/unit** (cap exhausted in ~6 min of one session), or **$0.057/min if the runbook's $0.007/gen flux price is the real unit** — either way the fps bar above already kills the stream; the cost bar is only survivable under the cheaper unit price, which the operator's dashboard decrement must confirm.
