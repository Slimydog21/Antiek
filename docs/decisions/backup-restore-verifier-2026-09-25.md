...
The Parquet component passes 92 relevant tests on each reviewed DuckDB version, including a normal GNU-tar success fixture, adversarial archive/report/content cases, and the real-schema refusal. The native candidate adds 18 focused tests, including an exact positive 54-table restore, an uncheckpointed-file negative, and a sequence-state negative. Exact-head CI and independent review must pass before the native component evidence is complete. This is **55/100 integration readiness**: source authority, sandbox, guard binding, R2 proof, and a production real-schema capture/restore remain open. Production release evidence is **0/100**.
