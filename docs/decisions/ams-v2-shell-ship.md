...
- **The yellow re-tone shipped in two steps.** SPR-09 softened the design
  *tokens* (`apps/reading/src/design/tokens.css` + `tokens.ts`) and proved AA;
  the Tailwind mirror was a disclosed follow-up at ship time. D20 closed that
  follow-up on 2026-06-30 by mirroring `sun-deep` / `sun-glow` and night
  shadows through the canonical CSS variables in `apps/reading/tailwind.config.js`,
  with `tokens.contrast.test.ts` guarding future drift.
...
2. **D20 closed 2026-06-30** — `apps/reading/tailwind.config.js` now mirrors the
   softened vars, with `tokens.contrast.test.ts` guarding drift (see
   `engineering_deferrals.md` D20).
