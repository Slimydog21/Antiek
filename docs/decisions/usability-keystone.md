...
| 2 | **launch** | A research started via the canonical entrypoint (`POST /research/plans` → `/approve` → `/launch`), **authenticated**, reaches the terminal `investigation.completed` event within a bounded poll (reported as a `launch` finding if not — never a silent pass). | `cascade_routes.py:577-583, 591-638`; `events.py:83` |
| 3 | **compound** | `knowledge_reuse_count > 0` from **this** investigation's per-investigation `knowledge.reused` event (`action_counts` scoped to the leaf iid on the temp events dir), **not** a global `/health` counter. | `events.py:180` `KNOWLEDGE_REUSED`; `host_local.py` (emit site) |
| 4 | **read** | A real artifact comes back through `GET /chunks/{id}` — a real authenticated HTTP fetch through the production read surface; `document_id` + `chunk_id` resolve. | `app.py:2155-2257` |
...
3. **the LIVE run against `api.antiek.ai` is OPERATOR-GATED — not executed by
   this sprint.**
   *Closest real path:* the **same** probe, parameterized by
   ...
   routes + assertions (no forked second implementation). The in-process mode is
   the pre-merge gate; the live mode is the operator's post-deploy step.
   ...
   **in-process** (forcing the compound leg's real surface to 0 → `[BLOCKED]
   ...
   but does **not** hit `api.antiek.ai`, mint a prod credential, or POST a research
   to prod. The live run is the operator's step
   (`infrastructure/runbooks/usability-keystone-verify-live.md`), mirroring SPR-06's
   operator-gated prod measurement.
