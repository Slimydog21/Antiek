...
def _cost(token: float) -> CostToResolve:
...
def test_point_is_mean_of_paired_diffs() -> None:
...
def test_bootstrap_deterministic_on_seed() -> None:
...
def test_degenerate_constant_diffs_have_zero_width_ci() -> None:
...
def test_t_interval_brackets_point() -> None:
