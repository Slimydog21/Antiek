...
class TurbopufferSubstrate:
    ...
    def rebuild_shadow(self, *, dry_run: bool = False, batch_size: int = 500) -> dict[str, Any]:
        ...
        digest_input = json.dumps({"rows": payload, "fingerprint": fingerprint,
                                   "model": model_name, "dimension": dimension},
        ...
        verified_hash = hashlib.sha256(json.dumps(
            {"rows": verified_payload, "fingerprint": fingerprint, "model": model_name,
             "dimension": dimension}, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
