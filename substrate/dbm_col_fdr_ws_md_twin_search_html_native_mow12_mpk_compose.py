"""Draft-before-full-merge residual over collective FDR MD mow12 (pure).

Short residual moniker dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
    ColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
    ColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError,
    compose_col_fdr_ws_md_twin_search_html_native_mow12_mpk,
)
...
AUTHORITY = "dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for draft-before-merge + collective FDR MD mow12."""


@dataclass(frozen=True)
class DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    collective_pack: ColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    draft_gate: object,
    collective_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    """Draft-before-merge + collective FDR MD mow12. Never writes/merges."""
    ...
    try:
        ...
        raise DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[draft_gate] {n}" for n in dg.notes)

    try:
        cp = compose_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    ...
    except ColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    if pack_ready:
        notes.append(
            "pack_ready=true — draft-before-merge + collective FDR MD mow12 ready; still pure"
        )
    else:
        notes.append(
    ...
    return DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    "DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose",
    "DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError",
    "compose_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk",
    "format_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
