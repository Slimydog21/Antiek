...
Short residual moniker nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk.
...
AUTHORITY = (
    "nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_compose_advisory"
)
...
from substrate.rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_compose import (
...
class NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(ValueError):
    """Fail-closed validation for ND shadow + twin presentation weekly pack."""


@dataclass(frozen=True)
class NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    twin_presentation: RtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose
    pack_ready: bool
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk(
    *,
    nd_shadow: object,
    ...
    try:
        twin = compose_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk(
    ...
    except RtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError as e:
        raise NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(
            str(e)
        ) from e
    ...
    return NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose(
        week_id=week,
        session_id=session,
...
def format_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_summary(
    c: NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_summary",
]
