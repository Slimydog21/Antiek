...
class ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackComposeError(ValueError):
    """Fail-closed validation for model decision + twin search HTML-native MO."""


@dataclass(frozen=True)
class ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_model_decision_twin_search_html_native_mo_weekly_src_write_pack(
    *,
    decision: object,
    ...
    try:
        tsp = compose_twin_search_html_native_marketplace_mo_weekly_src_write_pack(
    ...
    return ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackCompose(
        week_id=week,
        session_id=session,
...
def format_model_decision_twin_search_html_native_mo_weekly_src_write_pack_summary(
...
__all__ = [
    ...
    "format_model_decision_twin_search_html_native_mo_weekly_src_write_pack_summary",
]
