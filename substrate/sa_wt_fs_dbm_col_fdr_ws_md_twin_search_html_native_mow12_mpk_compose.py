...
Short residual moniker sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
    WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
    WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError,
    compose_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk,
)

AUTHORITY = "sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for source-attach + write-twin FDR MD mow12 residual."""


@dataclass(frozen=True)
class SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    write_pack: WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    sources: object,
    write_pack: object,
    operator_ack: object,
    ...
    try:
        ...
        raise SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[sources] {n}" for n in src.notes)

    try:
        wp = compose_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
            write=write_pack.get("write"),
            fullscreen_pack=write_pack.get("fullscreen_pack"),
            operator_ack=operator_ack,
            require_both=write_pack.get("require_both"),
        )
    except WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    return SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
