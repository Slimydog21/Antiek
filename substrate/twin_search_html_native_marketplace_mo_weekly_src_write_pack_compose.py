...
from substrate.html_native_marketplace_mo_weekly_src_write_pack_compose import (
    HtmlNativeMarketplaceMoWeeklySrcWritePackCompose,
    HtmlNativeMarketplaceMoWeeklySrcWritePackComposeError,
    compose_html_native_marketplace_mo_weekly_src_write_pack,
)
...
class TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(ValueError):
    """Fail-closed validation for twin search + HTML-native marketplace MO weekly src write."""


@dataclass(frozen=True)
class TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    html_pack: HtmlNativeMarketplaceMoWeeklySrcWritePackCompose
    pack_ready: bool
    hit_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "twin_search_html_native_marketplace_mo_weekly_src_write_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_twin_search_html_native_marketplace_mo_weekly_src_write_pack(
    *,
    search_query: object,
    twin_records: object,
    html_pack: object,
...
) -> TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackCompose:
    ...
    try:
        ...
        raise TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(str(e)) from e
    notes.extend(f"[search] {n}" for n in search.notes)

    try:
        hp = compose_html_native_marketplace_mo_weekly_src_write_pack(
            html_view=html_pack.get("html_view"),
            market_pack=html_pack.get("market_pack"),
            operator_ack=operator_ack,
            require_both=html_pack.get("require_both"),
        )
    except HtmlNativeMarketplaceMoWeeklySrcWritePackComposeError as e:
        raise TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(str(e)) from e
    ...
    return TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackCompose(
        week_id=week,
        session_id=session,
...
def format_twin_search_html_native_marketplace_mo_weekly_src_write_pack_summary(
    c: TwinSearchHtmlNativeMarketplaceMoWeeklySrcWritePackCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_twin_search_html_native_marketplace_mo_weekly_src_write_pack_summary",
]
