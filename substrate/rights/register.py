...
see ``.caffenagent/REFRAME-P1-diligence.md``) — this module makes that
migration a one-line call per adapter, not a per-adapter reinvention.
...
def register_source_document(
    ...
    *,
    ...
    3. ``ip_holder_id`` is used directly if given; else, if
       ``rights_holder_name`` is given AND ``source_kind`` is escrow-eligible,
       a ``pre_onboarded`` account is found-or-created. A ``user_content``
       source never mints escrow, even with a name (defense-in-depth).
    ...
    if (
        resolved_ip is None
        and rights_holder_name
        and source_kind not in _NON_ESCROW_SOURCE_KINDS
    ):
