"""Floating DR highlight launch over workstation record model decision MO weekly pack (pure).
...
from substrate.workstation_record_model_decision_twin_search_mo_weekly_pack_compose import (
...
class FloatingDrWorkstationRecordModelDecisionMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for floating DR + workstation record model decision."""


@dataclass(frozen=True)
class FloatingDrWorkstationRecordModelDecisionMoWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise FloatingDrWorkstationRecordModelDecisionMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_floating_dr_workstation_record_model_decision_mo_weekly_pack(
    *,
    ...
    try:
        ...
        raise FloatingDrWorkstationRecordModelDecisionMoWeeklyPackComposeError(str(e)) from e
    notes.extend(f"[highlight_launch] {n}" for n in hl.notes)

    try:
        rp = compose_workstation_record_model_decision_twin_search_mo_weekly_pack(
    ...
    except WorkstationRecordModelDecisionTwinSearchMoWeeklyPackComposeError as e:
        raise FloatingDrWorkstationRecordModelDecisionMoWeeklyPackComposeError(str(e)) from e
    ...
    return FloatingDrWorkstationRecordModelDecisionMoWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_floating_dr_workstation_record_model_decision_mo_weekly_pack_summary(
...
__all__ = [
    "FloatingDrWorkstationRecordModelDecisionMoWeeklyPackCompose",
    "FloatingDrWorkstationRecordModelDecisionMoWeeklyPackComposeError",
    "compose_floating_dr_workstation_record_model_decision_mo_weekly_pack",
    "format_floating_dr_workstation_record_model_decision_mo_weekly_pack_summary",
]
