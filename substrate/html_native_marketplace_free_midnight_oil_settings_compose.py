...
from substrate.marketplace_free_midnight_oil_settings_decision_compose import (
...
class HtmlNativeMarketplaceFreeMidnightOilSettingsComposeError(ValueError):
    """Fail-closed validation for HTML-native + marketplace free MO settings."""


@dataclass(frozen=True)
class HtmlNativeMarketplaceFreeMidnightOilSettingsCompose:
    session_id: str
    parent_asset_id: str
    asset_id: str
    title: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise HtmlNativeMarketplaceFreeMidnightOilSettingsComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_html_native_marketplace_free_midnight_oil_settings(
    *,
    html_view: object,
    market_pack: object,
    operator_ack: object,
    ...
    try:
        mp = compose_marketplace_free_midnight_oil_settings_decision(
            market=market_pack.get("market"),
            mo_pack=market_pack.get("mo_pack"),
            operator_ack=operator_ack,
            require_both=market_pack.get("require_both"),
        )
    except MarketplaceFreeMidnightOilSettingsDecisionComposeError as e:
        raise HtmlNativeMarketplaceFreeMidnightOilSettingsComposeError(
            str(e)
        ) from e
    ...
    return HtmlNativeMarketplaceFreeMidnightOilSettingsCompose(
        session_id=session,
...
def format_html_native_marketplace_free_midnight_oil_settings_summary(
...
__all__ = [
    ...
    "format_html_native_marketplace_free_midnight_oil_settings_summary",
]
