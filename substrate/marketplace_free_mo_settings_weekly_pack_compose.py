...
from substrate.midnight_oil_settings_decision_weekly_pack_compose import (
    MidnightOilSettingsDecisionWeeklyPackCompose,
    MidnightOilSettingsDecisionWeeklyPackComposeError,
    compose_midnight_oil_settings_decision_weekly_pack,
)


class MarketplaceFreeMoSettingsWeeklyPackComposeError(ValueError):
    """Fail-closed validation for marketplace free + MO settings decision."""


@dataclass(frozen=True)
class MarketplaceFreeMoSettingsWeeklyPackCompose:
    title: str
    account_id: str
    week_id: str
    session_id: str
    ...
    mo_pack: MidnightOilSettingsDecisionWeeklyPackCompose
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "marketplace_free_mo_settings_weekly_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MarketplaceFreeMoSettingsWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_marketplace_free_mo_settings_weekly_pack(
    *,
    market: object,
    mo_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> MarketplaceFreeMoSettingsWeeklyPackCompose:
    ...
    try:
        mop = compose_midnight_oil_settings_decision_weekly_pack(
            mo=mo_pack.get("mo"),
            settings_pack=mo_pack.get("settings_pack"),
            operator_ack=operator_ack,
            require_both=mo_pack.get("require_both"),
        )
    except MidnightOilSettingsDecisionWeeklyPackComposeError as e:
        raise MarketplaceFreeMoSettingsWeeklyPackComposeError(
            str(e)
        ) from e
    ...
    return MarketplaceFreeMoSettingsWeeklyPackCompose(
        title=title,
        account_id=account,
        week_id=week,
        session_id=session,
...
def format_marketplace_free_mo_settings_weekly_pack_summary(
    c: MarketplaceFreeMoSettingsWeeklyPackCompose,
) -> str:
    return (
...
__all__ = [
    "MarketplaceFreeMoSettingsWeeklyPackCompose",
    "MarketplaceFreeMoSettingsWeeklyPackComposeError",
    "compose_marketplace_free_mo_settings_weekly_pack",
    "format_marketplace_free_mo_settings_weekly_pack_summary",
]
