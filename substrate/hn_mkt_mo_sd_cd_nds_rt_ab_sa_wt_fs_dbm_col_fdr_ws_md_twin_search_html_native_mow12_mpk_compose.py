...
Short residual moniker hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
AUTHORITY = (
    "hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"
)
...
from substrate.mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
    MktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
    MktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError,
    compose_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk,
)


class HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for HTML-native + marketplace free residual."""


@dataclass(frozen=True)
class HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    session_id: str
    parent_asset_id: str
    asset_id: str
    title: str
    ...
    market_pack: MktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(f"{field} must be a non-empty string")
    return value.strip()


def compose_hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    html_view: object,
    market_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    """HTML-native view + marketplace free residual. Never PDF primary."""
    if not isinstance(operator_ack, bool):
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError("operator_ack must be an explicit boolean")
    if not isinstance(html_view, dict):
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError("html_view must be an object")
    if not isinstance(market_pack, dict):
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError("market_pack must be an object")

    require = True if require_both is None else require_both
    if not isinstance(require, bool):
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError("require_both must be boolean when set")
    ...
    try:
        ...
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[html_view] {n}" for n in hv.notes)

    try:
        mp = compose_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
            market=market_pack.get("market"),
            mo_pack=market_pack.get("mo_pack"),
            operator_ack=operator_ack,
            require_both=market_pack.get("require_both"),
        )
    except MktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    if (
        ...
        raise HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError("invariant: honesty flags must remain false / REJECT")
    ...
    return HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        session_id=session,
...
def format_hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(c: HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose) -> str:
    return (
...
__all__ = [
    "AUTHORITY",
    "HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose",
    "HnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError",
    "compose_hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk",
    "format_hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
]
