"""Model decision-tree over twin search HTML-native marketplace MO weekly (pure).
...
from substrate.twin_search_html_native_marketplace_mo_weekly_pack_compose import (
    TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose,
    TwinSearchHtmlNativeMarketplaceMoWeeklyPackComposeError,
    compose_twin_search_html_native_marketplace_mo_weekly_pack,
)


class ModelDecisionTwinSearchHtmlNativeMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for model decision + twin search HTML-native MO."""


@dataclass(frozen=True)
class ModelDecisionTwinSearchHtmlNativeMoWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    twin_search_pack: TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose
    pack_ready: bool
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "model_decision_twin_search_html_native_mo_weekly_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ModelDecisionTwinSearchHtmlNativeMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_model_decision_twin_search_html_native_mo_weekly_pack(
    *,
    decision: object,
    ...
    """Model decision + twin search HTML-native marketplace MO weekly. Never live-routes."""
    ...
    try:
        tsp = compose_twin_search_html_native_marketplace_mo_weekly_pack(
    ...
    except TwinSearchHtmlNativeMarketplaceMoWeeklyPackComposeError as e:
        raise ModelDecisionTwinSearchHtmlNativeMoWeeklyPackComposeError(
            str(e)
        ) from e
    ...
    if pack_ready:
        notes.append(
            ...
            "marketplace MO weekly ready; still pure"
        )
    else:
        notes.append(
    ...
    return ModelDecisionTwinSearchHtmlNativeMoWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_model_decision_twin_search_html_native_mo_weekly_pack_summary(
    c: ModelDecisionTwinSearchHtmlNativeMoWeeklyPackCompose,
) -> str:
    exceed = (
        "would_exceed=null"
...
__all__ = [
    ...
    "format_model_decision_twin_search_html_native_mo_weekly_pack_summary",
]
