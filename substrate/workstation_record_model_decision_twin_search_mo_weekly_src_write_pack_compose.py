...
from substrate.model_decision_twin_search_html_native_mo_weekly_src_write_pack_compose import (
    ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackCompose,
    ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackComposeError,
    compose_model_decision_twin_search_html_native_mo_weekly_src_write_pack,
)
...
class WorkstationRecordModelDecisionTwinSearchMoWeeklySrcWritePackComposeError(ValueError):
    """Fail-closed validation for workstation records + model decision twin search."""


@dataclass(frozen=True)
class WorkstationRecordModelDecisionTwinSearchMoWeeklySrcWritePackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WorkstationRecordModelDecisionTwinSearchMoWeeklySrcWritePackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_workstation_record_model_decision_twin_search_mo_weekly_src_write_pack(
    *,
    session_id: object,
    items: object,
    ...
    try:
        ...
        raise WorkstationRecordModelDecisionTwinSearchMoWeeklySrcWritePackComposeError(str(e)) from e
    notes.extend(f"[records] {n}" for n in rec.notes)

    try:
        dp = compose_model_decision_twin_search_html_native_mo_weekly_src_write_pack(
    ...
    except ModelDecisionTwinSearchHtmlNativeMoWeeklySrcWritePackComposeError as e:
        raise WorkstationRecordModelDecisionTwinSearchMoWeeklySrcWritePackComposeError(str(e)) from e
    ...
    return WorkstationRecordModelDecisionTwinSearchMoWeeklySrcWritePackCompose(
        week_id=week,
        session_id=session,
...
def format_workstation_record_model_decision_twin_search_mo_weekly_src_write_pack_summary(
...
__all__ = [
    ...
    "format_workstation_record_model_decision_twin_search_mo_weekly_src_write_pack_summary",
]
