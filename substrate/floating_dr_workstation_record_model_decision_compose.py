"""Floating DR highlight launch over workstation record model decision pack (pure).
...
from substrate.workstation_record_model_decision_twin_search_compose import (
...
class FloatingDrWorkstationRecordModelDecisionComposeError(ValueError):
    """Fail-closed validation for floating DR + workstation record model decision."""


@dataclass(frozen=True)
class FloatingDrWorkstationRecordModelDecisionCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise FloatingDrWorkstationRecordModelDecisionComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_floating_dr_workstation_record_model_decision(
    *,
    ...
    try:
        ...
        raise FloatingDrWorkstationRecordModelDecisionComposeError(str(e)) from e
    notes.extend(f"[highlight_launch] {n}" for n in hl.notes)

    try:
        rp = compose_workstation_record_model_decision_twin_search(
    ...
    except WorkstationRecordModelDecisionTwinSearchComposeError as e:
        raise FloatingDrWorkstationRecordModelDecisionComposeError(str(e)) from e
    ...
    return FloatingDrWorkstationRecordModelDecisionCompose(
        week_id=week,
        session_id=session,
...
def format_floating_dr_workstation_record_model_decision_summary(
...
__all__ = [
    ...
    "format_floating_dr_workstation_record_model_decision_summary",
]
