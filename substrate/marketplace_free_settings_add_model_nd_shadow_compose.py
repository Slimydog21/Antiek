...
class MarketplaceFreeSettingsAddModelNdShadowComposeError(ValueError):
    """Fail-closed validation for marketplace free + competition DR pack."""


@dataclass(frozen=True)
class MarketplaceFreeSettingsAddModelNdShadowCompose:
    title: str
    account_id: str
    session_id: str
    week_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MarketplaceFreeSettingsAddModelNdShadowComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_marketplace_free_settings_add_model_nd_shadow(
    *,
    market: object,
    ...
    try:
        cp = compose_settings_add_model_nd_shadow_competition_dr_mo(
    ...
    return MarketplaceFreeSettingsAddModelNdShadowCompose(
        title=title,
        account_id=account,
        session_id=session,
        week_id=week,
...
def format_marketplace_free_settings_add_model_nd_shadow_summary(
