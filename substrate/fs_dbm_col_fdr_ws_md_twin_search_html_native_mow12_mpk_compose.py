...
Short residual moniker fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
...
AUTHORITY = "fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for fullscreen + draft-before-merge collective FDR MD mow12."""


@dataclass(frozen=True)
class FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    draft_pack: DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    fullscreen: object,
    draft_pack: object,
    operator_ack: object,
    ...
    try:
        ...
        raise FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[fullscreen] {n}" for n in fs.notes)

    try:
        dp = compose_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    ...
    except DbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    return FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
