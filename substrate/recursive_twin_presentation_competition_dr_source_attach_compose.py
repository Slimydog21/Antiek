"""Recursive twin note-taker presentation over competition DR source-attach pack.
...
class RecursiveTwinPresentationCompetitionDrSourceAttachComposeError(ValueError):
...
@dataclass(frozen=True)
class RecursiveTwinPresentationCompetitionDrSourceAttachCompose:
    parent_asset_id: str
    session_id: str
    title: str
    account_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise RecursiveTwinPresentationCompetitionDrSourceAttachComposeError(
...
def compose_recursive_twin_presentation_competition_dr_source_attach(
    *,
    twin: object,
    presentation: object,
    ...
    """Twin presentation + competition DR source-attach pack. Never writes/merges."""
    ...
    try:
        ...
        raise RecursiveTwinPresentationCompetitionDrSourceAttachComposeError(str(e)) from e
    ...
    try:
        comp = compose_competition_dr_source_attach_antiek_bench_recommend(
            competition=competition_pack.get("competition"),
            source_pack=competition_pack.get("source_pack"),
            operator_ack=operator_ack,
    ...
    focus = _require_nonempty(comp.focus_task, field="focus_task")
    op = _require_nonempty(comp.operator_id, field="operator_id")
    ...
    if require:
        pack_ready = (
            ...
            and comp.live_router_authorized is False
            and comp.remote_index_queried is False
    ...
    if pack_ready:
        notes.append(
            "pack_ready=true — twin presentation + competition DR source-attach "
    ...
    if (
        ...
        or comp.live_router_authorized is not False
        or comp.remote_index_queried is not False
    ...
    return RecursiveTwinPresentationCompetitionDrSourceAttachCompose(
...
def format_recursive_twin_presentation_competition_dr_source_attach_summary(
