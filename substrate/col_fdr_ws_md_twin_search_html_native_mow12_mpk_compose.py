"""Collective multiselect residual over floating DR MD mow12 (pure).

Short residual moniker col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
...
AUTHORITY = "col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class ColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for collective + floating DR MD mow12 residual."""


@dataclass(frozen=True)
class ColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    floating_pack: FdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    multiselect: object,
    ...
    try:
        ...
        raise ColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[multiselect] {n}" for n in ms.notes)

    try:
        fp = compose_fdr_ws_md_twin_search_html_native_mow12_mpk(
    ...
    except FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise ColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    return ColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: ColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
