...
from substrate.floating_dr_workstation_record_model_decision_compose import (
    FloatingDrWorkstationRecordModelDecisionCompose,
    FloatingDrWorkstationRecordModelDecisionComposeError,
    compose_floating_dr_workstation_record_model_decision,
)
...
class CollectiveMultiselectFloatingDrWorkstationRecordComposeError(ValueError):
    """Fail-closed validation for collective multiselect + floating DR pack."""


@dataclass(frozen=True)
class CollectiveMultiselectFloatingDrWorkstationRecordCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise CollectiveMultiselectFloatingDrWorkstationRecordComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_collective_multiselect_floating_dr_workstation_record(
    *,
    multiselect: object,
    ...
    try:
        fp = compose_floating_dr_workstation_record_model_decision(
    ...
    except FloatingDrWorkstationRecordModelDecisionComposeError as e:
        raise CollectiveMultiselectFloatingDrWorkstationRecordComposeError(
            str(e)
        ) from e
    ...
    return CollectiveMultiselectFloatingDrWorkstationRecordCompose(
        week_id=week,
        session_id=session,
...
def format_collective_multiselect_floating_dr_workstation_record_summary(
...
__all__ = [
    ...
    "format_collective_multiselect_floating_dr_workstation_record_summary",
]
