...
from substrate.write_mode_twin_fullscreen_draft_collective_mo_weekly_pack_compose import (
    WriteModeTwinFullscreenDraftCollectiveMoWeeklyPackCompose,
    WriteModeTwinFullscreenDraftCollectiveMoWeeklyPackComposeError,
    compose_write_mode_twin_fullscreen_draft_collective_mo_weekly_pack,
)


class SourceAttachWriteTwinFsDraftMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for source attach + write twin collective pack."""


@dataclass(frozen=True)
class SourceAttachWriteTwinFsDraftMoWeeklyPackCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise SourceAttachWriteTwinFsDraftMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_source_attach_write_twin_fs_draft_mo_weekly_pack(
    *,
    sources: object,
    write_pack: object,
    operator_ack: object,
    ...
    try:
        wp = compose_write_mode_twin_fullscreen_draft_collective_mo_weekly_pack(
            write=write_pack.get("write"),
            fullscreen_pack=write_pack.get("fullscreen_pack"),
            operator_ack=operator_ack,
            require_both=write_pack.get("require_both"),
        )
    except WriteModeTwinFullscreenDraftCollectiveMoWeeklyPackComposeError as e:
        raise SourceAttachWriteTwinFsDraftMoWeeklyPackComposeError(
            str(e)
        ) from e
    ...
    return SourceAttachWriteTwinFsDraftMoWeeklyPackCompose(
        session_id=session,
...
def format_source_attach_write_twin_fs_draft_mo_weekly_pack_summary(
...
__all__ = [
    ...
    "format_source_attach_write_twin_fs_draft_mo_weekly_pack_summary",
]
