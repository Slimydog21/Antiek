"""Workstation recursive records residual over model-decision mow12 (pure).

Short residual moniker ws_md_twin_search_html_native_mow12_mpk (NAME_MAX-safe).
Grafts workstation recursive records on md_twin_search_html_native_mow12_mpk tip.
...
from substrate.md_twin_search_html_native_mow12_mpk_compose import (
    AUTHORITY as MD_AUTHORITY,
...
AUTHORITY = "ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class WsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for workstation + model-decision mow12 residual."""


@dataclass(frozen=True)
class WsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    decision_pack: MdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_ws_md_twin_search_html_native_mow12_mpk(
    *,
    session_id: object,
    items: object,
...
) -> WsMdTwinSearchHtmlNativeMow12MpkCompose:
    """Workstation records + model-decision mow12 residual. Never persists/injects."""
    ...
    try:
        ...
        raise WsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[records] {n}" for n in rec.notes)

    try:
        dp = compose_md_twin_search_html_native_mow12_mpk(
    ...
    except MdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise WsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    if pack_ready:
        notes.append(
            "pack_ready=true — workstation records + model-decision mow12 ready; still pure"
        )
    else:
        notes.append(
    ...
    return WsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: WsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    "WsMdTwinSearchHtmlNativeMow12MpkCompose",
    "WsMdTwinSearchHtmlNativeMow12MpkComposeError",
    "compose_ws_md_twin_search_html_native_mow12_mpk",
    "format_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
