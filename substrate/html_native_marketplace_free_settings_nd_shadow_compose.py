"""HTML-native view over marketplace free settings ND shadow pack (pure).
...
from substrate.marketplace_free_settings_add_model_nd_shadow_compose import (
    MarketplaceFreeSettingsAddModelNdShadowCompose,
    MarketplaceFreeSettingsAddModelNdShadowComposeError,
    compose_marketplace_free_settings_add_model_nd_shadow,
)


class HtmlNativeMarketplaceFreeSettingsNdShadowComposeError(ValueError):
    """Fail-closed validation for HTML-native + marketplace free settings ND pack."""


@dataclass(frozen=True)
class HtmlNativeMarketplaceFreeSettingsNdShadowCompose:
    session_id: str
    parent_asset_id: str
    asset_id: str
    title: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise HtmlNativeMarketplaceFreeSettingsNdShadowComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_html_native_marketplace_free_settings_nd_shadow(
    *,
    html_view: object,
    market_pack: object,
    operator_ack: object,
    ...
    """HTML-native on marketplace free settings ND shadow. Never PDF-primary."""
    ...
    try:
        tp = compose_marketplace_free_settings_add_model_nd_shadow(
            market=market_pack.get("market"),
            settings_pack=market_pack.get("settings_pack"),
            operator_ack=operator_ack,
            require_both=market_pack.get("require_both"),
        )
    except MarketplaceFreeSettingsAddModelNdShadowComposeError as e:
        raise HtmlNativeMarketplaceFreeSettingsNdShadowComposeError(
            str(e)
        ) from e
    ...
    if pack_ready:
        notes.append(
            "pack_ready=true — HTML-native view + marketplace free settings ND shadow "
            "ready; still pure"
        )
    else:
        notes.append(
    ...
    return HtmlNativeMarketplaceFreeSettingsNdShadowCompose(
        session_id=session,
...
def format_html_native_marketplace_free_settings_nd_shadow_summary(
