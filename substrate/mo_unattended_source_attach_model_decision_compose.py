...
class MoUnattendedSourceAttachModelDecisionComposeError(ValueError):
    """Fail-closed validation for MO unattended + source attach model decision."""


@dataclass(frozen=True)
class MoUnattendedSourceAttachModelDecisionCompose:
    session_id: str
    parent_asset_id: str
    asset_id: str
    week_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MoUnattendedSourceAttachModelDecisionComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_mo_unattended_source_attach_model_decision(
    *,
    mo: object,
    ...
    try:
        ...
        raise MoUnattendedSourceAttachModelDecisionComposeError(str(e)) from e
    notes.extend(f"[mo] {n}" for n in mo_c.notes)

    try:
        rp = compose_source_attach_model_decision_twin_search(
            sources=research_pack.get("sources"),
            decision_pack=research_pack.get("decision_pack"),
            operator_ack=operator_ack,
    ...
    return MoUnattendedSourceAttachModelDecisionCompose(
        session_id=session,
...
def format_mo_unattended_source_attach_model_decision_summary(
...
__all__ = [
    ...
    "format_mo_unattended_source_attach_model_decision_summary",
]
