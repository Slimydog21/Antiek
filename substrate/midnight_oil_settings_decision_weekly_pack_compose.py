...
from substrate.settings_decision_competition_weekly_pack_compose import (
    SettingsDecisionCompetitionWeeklyPackCompose,
    SettingsDecisionCompetitionWeeklyPackComposeError,
    compose_settings_decision_competition_weekly_pack,
)


class MidnightOilSettingsDecisionWeeklyPackComposeError(ValueError):
    """Fail-closed validation for MO + settings decision competition DR."""


@dataclass(frozen=True)
class MidnightOilSettingsDecisionWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    settings_pack: SettingsDecisionCompetitionWeeklyPackCompose
    pack_ready: bool
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "midnight_oil_settings_decision_weekly_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MidnightOilSettingsDecisionWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_midnight_oil_settings_decision_weekly_pack(
    *,
    mo: object,
    ...
    try:
        ...
        raise MidnightOilSettingsDecisionWeeklyPackComposeError(str(e)) from e
    notes.extend(f"[mo] {n}" for n in mo_c.notes)

    try:
        sp = compose_settings_decision_competition_weekly_pack(
    ...
    except SettingsDecisionCompetitionWeeklyPackComposeError as e:
        raise MidnightOilSettingsDecisionWeeklyPackComposeError(str(e)) from e
    ...
    return MidnightOilSettingsDecisionWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_midnight_oil_settings_decision_weekly_pack_summary(
    c: MidnightOilSettingsDecisionWeeklyPackCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_midnight_oil_settings_decision_weekly_pack_summary",
]
