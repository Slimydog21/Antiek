...
Short residual moniker cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk.
...
AUTHORITY = (
    "cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_compose_advisory"
)
...
from substrate.nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_compose import (
    NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose,
    NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError,
    compose_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk,
)


class CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(ValueError):
    """Fail-closed validation for competition DR + ND shadow twin presentation weekly."""


@dataclass(frozen=True)
class CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose:
    session_id: str
    week_id: str
    parent_asset_id: str
    asset_id: str
    ...
    nd_pack: NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk(
    *,
    competition: object,
    nd_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose:
    ...
    try:
        ...
        raise CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(str(e)) from e
    notes.extend(f"[competition] {n}" for n in comp.notes)

    try:
        nd = compose_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk(
            nd_shadow=nd_pack.get("nd_shadow"),
            twin_presentation=nd_pack.get("twin_presentation"),
            operator_ack=operator_ack,
            require_both=nd_pack.get("require_both"),
        )
    except NdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError as e:
        raise CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError(str(e)) from e
    ...
    return CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose(
        session_id=session,
        week_id=week,
...
def format_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_summary(
    c: CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose,
) -> str:
    return (
...
__all__ = [
    "AUTHORITY",
    "CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkCompose",
    "CdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsMow13MpkComposeError",
    "compose_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk",
    "format_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_mow13_mpk_summary",
]
