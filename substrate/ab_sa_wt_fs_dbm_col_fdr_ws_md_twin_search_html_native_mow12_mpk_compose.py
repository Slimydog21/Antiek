...
Short residual moniker ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
    SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
    SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError,
    compose_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk,
)

AUTHORITY = (
    "ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"
)


class AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for weekly learn + source-attach residual."""


@dataclass(frozen=True)
class AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    source_pack: SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    weekly_learn: object,
    source_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    ...
    try:
        ...
        raise AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[weekly_learn] {n}" for n in wl.notes)

    try:
        sp = compose_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
            sources=source_pack.get("sources"),
            write_pack=source_pack.get("write_pack"),
            operator_ack=operator_ack,
            require_both=source_pack.get("require_both"),
        )
    except SaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    return AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    "AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose",
    "AbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError",
    "compose_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk",
    "format_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
