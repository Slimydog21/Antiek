...
class SourceAttachWriteTwinCollectiveFullscreenComposeError(ValueError):
    """Fail-closed validation for source attach + write twin collective pack."""


@dataclass(frozen=True)
class SourceAttachWriteTwinCollectiveFullscreenCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise SourceAttachWriteTwinCollectiveFullscreenComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_source_attach_write_twin_collective_fullscreen(
    *,
    sources: object,
    write_pack: object,
    operator_ack: object,
    ...
    try:
        wp = compose_write_mode_twin_collective_fullscreen_collective_multiselect(
    ...
    return SourceAttachWriteTwinCollectiveFullscreenCompose(
        session_id=session,
...
def format_source_attach_write_twin_collective_fullscreen_summary(
...
__all__ = [
    ...
    "format_source_attach_write_twin_collective_fullscreen_summary",
]
