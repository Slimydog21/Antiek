...
from substrate.midnight_oil_settings_decision_weekly_src_write_mo_weekly_pack_compose import (
    MidnightOilSettingsDecisionWeeklySrcWriteMoWeeklyPackCompose,
    MidnightOilSettingsDecisionWeeklySrcWriteMoWeeklyPackComposeError,
    compose_midnight_oil_settings_decision_weekly_src_write_mo_weekly_pack,
)


class MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for marketplace free + MO settings decision."""


@dataclass(frozen=True)
class MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackCompose:
    title: str
    account_id: str
    week_id: str
    session_id: str
    ...
    mo_pack: MidnightOilSettingsDecisionWeeklySrcWriteMoWeeklyPackCompose
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack(
    *,
    market: object,
    mo_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackCompose:
    ...
    try:
        mop = compose_midnight_oil_settings_decision_weekly_src_write_mo_weekly_pack(
            mo=mo_pack.get("mo"),
            settings_pack=mo_pack.get("settings_pack"),
            operator_ack=operator_ack,
            require_both=mo_pack.get("require_both"),
        )
    except MidnightOilSettingsDecisionWeeklySrcWriteMoWeeklyPackComposeError as e:
        raise MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackComposeError(
            str(e)
        ) from e
    ...
    return MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackCompose(
        title=title,
        account_id=account,
        week_id=week,
        session_id=session,
...
def format_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack_summary(
    c: MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackCompose,
) -> str:
    return (
...
__all__ = [
    "MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackCompose",
    "MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackComposeError",
    "compose_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack",
    "format_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack_summary",
]
