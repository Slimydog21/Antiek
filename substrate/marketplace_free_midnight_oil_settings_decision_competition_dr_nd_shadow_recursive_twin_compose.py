...
from substrate.midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_compose import (
    MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose,
    MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError,
    compose_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin,
)


class MarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(ValueError):
    """Fail-closed validation for marketplace free + MO settings decision."""


@dataclass(frozen=True)
class MarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose:
    title: str
    account_id: str
    week_id: str
    session_id: str
    ...
    mo_pack: MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin(
    *,
    market: object,
    mo_pack: object,
    operator_ack: object,
    ...
    try:
        mop = compose_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin(
            mo=mo_pack.get("mo"),
            settings_pack=mo_pack.get("settings_pack"),
            operator_ack=operator_ack,
            require_both=mo_pack.get("require_both"),
        )
    except MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError as e:
        raise MarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(
            str(e)
        ) from e
    ...
    return MarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose(
        title=title,
        account_id=account,
        week_id=week,
        session_id=session,
...
def format_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_summary(
    c: MarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_summary",
]
