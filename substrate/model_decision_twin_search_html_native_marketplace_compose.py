"""Model decision-tree + usage bar over twin search HTML-native marketplace (pure).
...
class ModelDecisionTwinSearchHtmlNativeMarketplaceComposeError(ValueError):
    """Fail-closed validation for model decision + twin search marketplace pack."""


@dataclass(frozen=True)
class ModelDecisionTwinSearchHtmlNativeMarketplaceCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ModelDecisionTwinSearchHtmlNativeMarketplaceComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_model_decision_twin_search_html_native_marketplace(
    *,
    decision: object,
    ...
    """Model decision + twin search HTML-native marketplace. Never live-routes."""
    ...
    try:
        tsp = compose_twin_search_html_native_recursive_twin_marketplace(
    ...
    focus = _require_nonempty(tsp.focus_task, field="focus_task")
    ...
    return ModelDecisionTwinSearchHtmlNativeMarketplaceCompose(
        week_id=week,
        session_id=session,
...
def format_model_decision_twin_search_html_native_marketplace_summary(
