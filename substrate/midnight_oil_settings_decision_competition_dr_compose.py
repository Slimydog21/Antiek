...
from substrate.settings_decision_competition_dr_nd_shadow_twin_weekly_compose import (
    ...
    SettingsDecisionCompetitionDrNdShadowTwinWeeklyComposeError,
    compose_settings_decision_competition_dr_nd_shadow_twin_weekly,
)


class MidnightOilSettingsDecisionCompetitionDrComposeError(ValueError):
    """Fail-closed validation for MO + settings decision competition DR."""


@dataclass(frozen=True)
class MidnightOilSettingsDecisionCompetitionDrCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    settings_pack: SettingsDecisionCompetitionDrNdShadowTwinWeeklyCompose
    pack_ready: bool
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "midnight_oil_settings_decision_competition_dr_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MidnightOilSettingsDecisionCompetitionDrComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_midnight_oil_settings_decision_competition_dr(
    *,
    mo: object,
    ...
    try:
        ...
        raise MidnightOilSettingsDecisionCompetitionDrComposeError(str(e)) from e
    notes.extend(f"[mo] {n}" for n in mo_c.notes)

    try:
        sp = compose_settings_decision_competition_dr_nd_shadow_twin_weekly(
    ...
    except SettingsDecisionCompetitionDrNdShadowTwinWeeklyComposeError as e:
        raise MidnightOilSettingsDecisionCompetitionDrComposeError(str(e)) from e
    ...
    return MidnightOilSettingsDecisionCompetitionDrCompose(
        week_id=week,
        session_id=session,
...
def format_midnight_oil_settings_decision_competition_dr_summary(
    c: MidnightOilSettingsDecisionCompetitionDrCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_midnight_oil_settings_decision_competition_dr_summary",
]
