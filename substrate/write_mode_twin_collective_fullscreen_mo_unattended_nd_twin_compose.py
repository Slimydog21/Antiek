"""Write twin collective analysis over fullscreen MO unattended ND twin (pure).
...
class WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinComposeError(ValueError):
    """Fail-closed validation for write twin collective + fullscreen MO ND twin pack."""


@dataclass(frozen=True)
class WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
    ...
    fullscreen_pack: FullscreenMoUnattendedDraftMultiselectNdTwinCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_write_mode_twin_collective_fullscreen_mo_unattended_nd_twin(
    *,
    write: object,
    ...
    """Write twin collective + fullscreen MO unattended ND twin. Never writes/dispatches."""
    ...
    try:
        fs = compose_fullscreen_mo_unattended_draft_multiselect_nd_twin(
            fullscreen=fullscreen_pack.get("fullscreen"),
            mo_pack=fullscreen_pack.get("mo_pack"),
            operator_ack=operator_ack,
    ...
    session_aligned = fs.session_id == session
    parent_aligned = fs.parent_asset_id == parent or fs.asset_id == parent
    ...
    if require:
        pack_ready = (
            ...
            and fs.live_execution_authorized is False
        ...
        pack_ready = (
            ...
            and fs.production_router_verdict == "REJECT"
            and fs.pdf_primary is False
            and (w.pack_ready is True or fs.pack_ready is True)
        )

    if pack_ready:
        notes.append(
            "pack_ready=true — write twin collective + fullscreen MO unattended ND twin "
            "ready; still pure"
        )
    else:
        notes.append(
    ...
    if (
        ...
        or fs.live_execution_authorized is not False
    ...
    return WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinCompose(
        session_id=session,
...
def format_write_mode_twin_collective_fullscreen_mo_unattended_nd_twin_summary(
    c: WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinCompose,
) -> str:
    return (
