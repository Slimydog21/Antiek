...
from substrate.marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack_compose import (
...
class HtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(ValueError):
    """Fail-closed validation for HTML-native + marketplace free MO settings."""


@dataclass(frozen=True)
class HtmlNativeMarketplaceMoWeeklySrcWritePackCompose:
    session_id: str
    parent_asset_id: str
    asset_id: str
    title: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise HtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_html_native_marketplace_mo_weekly_src_write_pack(
    *,
    html_view: object,
    market_pack: object,
    operator_ack: object,
    ...
    try:
        mp = compose_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack(
            market=market_pack.get("market"),
            mo_pack=market_pack.get("mo_pack"),
            operator_ack=operator_ack,
            require_both=market_pack.get("require_both"),
        )
    except MarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackComposeError as e:
        raise HtmlNativeMarketplaceMoWeeklySrcWritePackComposeError(
            str(e)
        ) from e
    ...
    return HtmlNativeMarketplaceMoWeeklySrcWritePackCompose(
        session_id=session,
...
def format_html_native_marketplace_mo_weekly_src_write_pack_summary(
...
__all__ = [
    ...
    "format_html_native_marketplace_mo_weekly_src_write_pack_summary",
]
