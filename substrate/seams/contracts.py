...
* ``WriteToSpeakSeam``    write → speak     (commission interviews from an
  outline gap)
...
class WriteToSpeakSeam(_SeamBase):
    ...
    Implemented by ``substrate.write.speak_commission`` and the Write REST
    route ``/write/sections/{section_id}/commission-speak``. Write promotes the
    gap to a stable ``question`` node; Speak creates a private project whose
    interview guide references that same question id. The seam carries the
    ``question_node`` reference and section provenance, never the question body."""

    from_workflow: Literal["write"] = "write"
    to_workflow: Literal["speak"] = "speak"
    entity_kind: Literal["question_node"] = "question_node"
    # The outline section whose gap commissions the interview.
