...
from substrate.recursive_twin_presentation_antiek_bench_weekly_source_attach_write_twin_compose import (
...
class NdShadowRecursiveTwinPresentationWeeklySourceAttachComposeError(ValueError):
    """Fail-closed validation for ND shadow + twin presentation weekly pack."""


@dataclass(frozen=True)
class NdShadowRecursiveTwinPresentationWeeklySourceAttachCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise NdShadowRecursiveTwinPresentationWeeklySourceAttachComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_nd_shadow_recursive_twin_presentation_weekly_source_attach(
    *,
    nd_shadow: object,
    ...
    try:
        twin = compose_recursive_twin_presentation_antiek_bench_weekly_source_attach_write_twin(
    ...
    except RecursiveTwinPresentationAntiekBenchWeeklySourceAttachWriteTwinComposeError as e:
        raise NdShadowRecursiveTwinPresentationWeeklySourceAttachComposeError(
            str(e)
        ) from e
    ...
    return NdShadowRecursiveTwinPresentationWeeklySourceAttachCompose(
        week_id=week,
        session_id=session,
...
def format_nd_shadow_recursive_twin_presentation_weekly_source_attach_summary(
...
__all__ = [
    "NdShadowRecursiveTwinPresentationWeeklySourceAttachCompose",
    "NdShadowRecursiveTwinPresentationWeeklySourceAttachComposeError",
