...
class ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsComposeError(ValueError):
    """Fail-closed validation for model decision + twin search free settings pack."""


@dataclass(frozen=True)
class ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_model_decision_twin_search_html_native_marketplace_free_settings(
    *,
    decision: object,
    ...
    try:
        tsp = compose_twin_search_html_native_marketplace_free_settings(
    ...
    return ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsCompose(
        week_id=week,
        session_id=session,
...
def format_model_decision_twin_search_html_native_marketplace_free_settings_summary(
