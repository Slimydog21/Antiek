"""Floating DR residual over workstation MD twin-search mow12 (pure).

Short residual moniker fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.ws_md_twin_search_html_native_mow12_mpk_compose import (
...
AUTHORITY = "fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for floating DR + workstation MD mow12 residual."""


@dataclass(frozen=True)
class FdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    record_pack: WsMdTwinSearchHtmlNativeMow12MpkCompose
    parent_aligned: bool
    pack_ready: bool
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    highlight_launch: object,
    record_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> FdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    """Floating DR + workstation MD mow12 residual. Never dispatches/merges."""
    ...
    try:
        ...
        raise FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[highlight_launch] {n}" for n in hl.notes)

    try:
        rp = compose_ws_md_twin_search_html_native_mow12_mpk(
    ...
    except WsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    if pack_ready:
        notes.append(
            "pack_ready=true — floating DR + workstation MD mow12 ready; still pure"
        )
    else:
        notes.append(
    ...
    return FdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: FdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    "FdrWsMdTwinSearchHtmlNativeMow12MpkCompose",
    "FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError",
    "compose_fdr_ws_md_twin_search_html_native_mow12_mpk",
    "format_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
