...
from substrate.source_attach_write_twin_fs_draft_mo_weekly_pack_compose import (
    SourceAttachWriteTwinFsDraftMoWeeklyPackCompose,
    SourceAttachWriteTwinFsDraftMoWeeklyPackComposeError,
    compose_source_attach_write_twin_fs_draft_mo_weekly_pack,
)


class AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for weekly learn + source attach write twin."""


@dataclass(frozen=True)
class AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_antiek_bench_weekly_src_write_fs_draft_mo_weekly_pack(
    *,
    weekly_learn: object,
    source_pack: object,
    operator_ack: object,
    ...
    try:
        ...
        raise AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackComposeError(str(e)) from e
    notes.extend(f"[weekly_learn] {n}" for n in wl.notes)

    try:
        sp = compose_source_attach_write_twin_fs_draft_mo_weekly_pack(
            sources=source_pack.get("sources"),
            write_pack=source_pack.get("write_pack"),
            operator_ack=operator_ack,
            require_both=source_pack.get("require_both"),
        )
    except SourceAttachWriteTwinFsDraftMoWeeklyPackComposeError as e:
        raise AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackComposeError(str(e)) from e
    ...
    return AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_antiek_bench_weekly_src_write_fs_draft_mo_weekly_pack_summary(
...
__all__ = [
    "AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackCompose",
    "AntiekBenchWeeklySrcWriteFsDraftMoWeeklyPackComposeError",
    "compose_antiek_bench_weekly_src_write_fs_draft_mo_weekly_pack",
    "format_antiek_bench_weekly_src_write_fs_draft_mo_weekly_pack_summary",
]
