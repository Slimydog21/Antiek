"""Twin intelligent search over HTML-native marketplace MO weekly pack (pure).
...
from substrate.html_native_marketplace_mo_weekly_pack_compose import (
    HtmlNativeMarketplaceMoWeeklyPackCompose,
    HtmlNativeMarketplaceMoWeeklyPackComposeError,
    compose_html_native_marketplace_mo_weekly_pack,
)
...
class TwinSearchHtmlNativeMarketplaceMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for twin search + HTML-native marketplace MO weekly."""


@dataclass(frozen=True)
class TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    html_pack: HtmlNativeMarketplaceMoWeeklyPackCompose
    pack_ready: bool
    hit_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "twin_search_html_native_marketplace_mo_weekly_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise TwinSearchHtmlNativeMarketplaceMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_twin_search_html_native_marketplace_mo_weekly_pack(
    *,
    search_query: object,
    twin_records: object,
    html_pack: object,
    ...
    """Twin substrate search + HTML-native marketplace MO weekly. Never remote-indexes."""
    ...
    try:
        ...
        raise TwinSearchHtmlNativeMarketplaceMoWeeklyPackComposeError(str(e)) from e
    notes.extend(f"[search] {n}" for n in search.notes)

    try:
        hp = compose_html_native_marketplace_mo_weekly_pack(
            html_view=html_pack.get("html_view"),
            market_pack=html_pack.get("market_pack"),
            operator_ack=operator_ack,
            require_both=html_pack.get("require_both"),
        )
    except HtmlNativeMarketplaceMoWeeklyPackComposeError as e:
        raise TwinSearchHtmlNativeMarketplaceMoWeeklyPackComposeError(str(e)) from e
    ...
    if pack_ready:
        notes.append(
            "pack_ready=true — twin search + HTML-native marketplace MO weekly "
            "ready; still pure"
        )
    else:
        notes.append(
    ...
    return TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_twin_search_html_native_marketplace_mo_weekly_pack_summary(
    c: TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_twin_search_html_native_marketplace_mo_weekly_pack_summary",
]
