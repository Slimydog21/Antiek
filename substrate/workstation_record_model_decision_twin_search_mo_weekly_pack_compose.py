"""Workstation recursive records over model decision twin search MO weekly pack (pure).
...
from substrate.model_decision_twin_search_html_native_mo_weekly_pack_compose import (
    ModelDecisionTwinSearchHtmlNativeMoWeeklyPackCompose,
    ModelDecisionTwinSearchHtmlNativeMoWeeklyPackComposeError,
    compose_model_decision_twin_search_html_native_mo_weekly_pack,
)
...
class WorkstationRecordModelDecisionTwinSearchMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for workstation records + model decision twin search."""


@dataclass(frozen=True)
class WorkstationRecordModelDecisionTwinSearchMoWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WorkstationRecordModelDecisionTwinSearchMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_workstation_record_model_decision_twin_search_mo_weekly_pack(
    *,
    session_id: object,
    items: object,
    ...
    try:
        ...
        raise WorkstationRecordModelDecisionTwinSearchMoWeeklyPackComposeError(str(e)) from e
    notes.extend(f"[records] {n}" for n in rec.notes)

    try:
        dp = compose_model_decision_twin_search_html_native_mo_weekly_pack(
    ...
    except ModelDecisionTwinSearchHtmlNativeMoWeeklyPackComposeError as e:
        raise WorkstationRecordModelDecisionTwinSearchMoWeeklyPackComposeError(str(e)) from e
    ...
    return WorkstationRecordModelDecisionTwinSearchMoWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_workstation_record_model_decision_twin_search_mo_weekly_pack_summary(
...
__all__ = [
    "WorkstationRecordModelDecisionTwinSearchMoWeeklyPackCompose",
    "WorkstationRecordModelDecisionTwinSearchMoWeeklyPackComposeError",
    "compose_workstation_record_model_decision_twin_search_mo_weekly_pack",
    "format_workstation_record_model_decision_twin_search_mo_weekly_pack_summary",
]
