...
class BedrockBatchRecoveryAdapter:
    ...
    adapter_contract = "aws-bedrock-model-invocation-job-v1"
    ...
    def prepare(
        ...
        model: str, request: BedrockBatchRequest, projected_max_cents: int,
        projection_digest: str, rate_snapshot: str,
    ) -> ProviderSubmissionSnapshot:
        ...
        hold = PaidHoldIntent(
            ...
            operation_digest=_digest(seed), projection_digest=projection_digest,
            rate_snapshot=rate_snapshot, provider_idempotency_key=token,
        )
        ...
        return self.ledger.prepare_provider_submission(
            command_key, binding, hold, projected_max_cents, intent
        )
