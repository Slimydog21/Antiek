...
def _cost_gate(
    ...
    actual = actual_cost_usd if actual_cost_usd is not None else round(sum(call.cost_usd for call in calls), 4)
    ...
    if budget_usd is not None:
        compare = actual if actual_cost_usd is not None else estimated
        if compare > budget_usd:
            findings.append(
                GateFinding(
                    code="budget_exceeded",
                    severity="error",
                    message=f"Multimedia spend ${compare:.4f} exceeds budget ${budget_usd:.4f}.",
                )
            )
    return GateResult(
