...
class ResearchSpendLedger:
    ...
    def register_fallback_manifest(
        ...
        with self._write("register_fallback_manifest") as connection:
            ...
            connection.executemany(
                ...
                "provider_idempotency_key) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                    (
                        manifest.chain_id,
    ...
    def integrity_check(self) -> str:
        ...
        try:
            ...
            fallback_chains = connection.execute(
                "SELECT * FROM research_fallback_chains ORDER BY chain_id"
            ).fetchall()
            for row in fallback_chains:
    ...
    def _fallback_history_row(
        ...
        for route in manifest.routes:
            hold_rows = connection.execute(
                "SELECT hold_id FROM research_spend_holds WHERE run_id=? AND reservation_key=?",
            ...
            if not hold_rows:
                ...
                hold = self._load_hold(connection, str(hold_rows[0]["hold_id"]))
                ...
                if state is FallbackRouteState.SETTLED:
                    if settled:
                        raise LedgerIntegrityError("fallback chain has duplicate settlements")
                    receipt = self._settled_hold_receipt_in_context(
                        connection, hold.hold_id, owner_id
                    )
                    evidence_sha256 = receipt.evidence_sha256
                    intent_sha256 = receipt.resolution_intent_sha256
                    settled = True
                public = FallbackRouteHistory(
                    route.fallback_index,
                    route.provider,
