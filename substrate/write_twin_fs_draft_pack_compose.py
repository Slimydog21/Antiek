"""Write twin FS draft pack residual over fullscreen draft stack tip (pure).

Short module name for macOS/GitHub 255-byte basename limit.
Parent: fullscreen_open_draft_before_merge...nd_shadow_recursive_twin (c530).
draft_written / analysis_written / merge_executed / live_dispatched always False.
...
class WriteTwinFsDraftPackComposeError(
    ValueError
):
...
class WriteTwinFsDraftPackCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "write_twin_fs_draft_pack_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WriteTwinFsDraftPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_write_twin_fs_draft_pack(
    *,
    write: object,
    fullscreen_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> WriteTwinFsDraftPackCompose:
    ...
    try:
        fp = compose_fullscreen_open_draft_before_merge_collective_multiselect_floating_dr_workstation_record_model_decision_twin_search_html_native_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin(
    ...
    return WriteTwinFsDraftPackCompose(
        session_id=session,
...
def format_write_twin_fs_draft_pack_summary(
    c: WriteTwinFsDraftPackCompose,
) -> str:
    return (
...
__all__ = [
    "WriteTwinFsDraftPackCompose",
    "WriteTwinFsDraftPackComposeError",
    "compose_write_twin_fs_draft_pack",
    "format_write_twin_fs_draft_pack_summary",
]
