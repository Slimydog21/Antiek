"""Write twin collective over fullscreen collective multiselect floating DR (pure).
...
class WriteModeTwinCollectiveFullscreenCollectiveMultiselectComposeError(
    ValueError
):
    """Fail-closed validation for write twin collective + fullscreen pack."""


@dataclass(frozen=True)
class WriteModeTwinCollectiveFullscreenCollectiveMultiselectCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WriteModeTwinCollectiveFullscreenCollectiveMultiselectComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_write_mode_twin_collective_fullscreen_collective_multiselect(
    *,
    write: object,
    ...
    try:
        fp = compose_fullscreen_open_collective_multiselect_floating_dr(
            fullscreen=fullscreen_pack.get("fullscreen"),
            collective_pack=fullscreen_pack.get("collective_pack"),
            operator_ack=operator_ack,
    ...
    return WriteModeTwinCollectiveFullscreenCollectiveMultiselectCompose(
        session_id=session,
...
def format_write_mode_twin_collective_fullscreen_collective_multiselect_summary(
...
__all__ = [
    ...
    "format_write_mode_twin_collective_fullscreen_collective_multiselect_summary",
]
