...
| escape-hatch observability | guarded | `tests/test_escape_hatch.py::test_escape_hatch_observability_report_surface` | ARE Wave 1 follow-on. Proves the audited bypass marker's exported `counter_snapshot()` exposes exact per-reason hit counts, so bypass activity can feed audit/invariant report surfaces instead of disappearing after the first warning |
...
| role-parser provenance validation | guarded | `tests/test_provenance_ref_lint.py::test_lint_passes_on_current_tree` | Karpathy deep-lens survivor. Every parser that emits model-provided provenance ids must route through `substrate.provenance.validate_ref(s)`; planted bypass tests cover raw id outputs, constructor refs, mixed trusted/raw refs, and local `known_`/`canonical_` id membership checks |
