...
from substrate.settings_decision_competition_dr_nd_shadow_recursive_twin_presentation_weekly_compose import (
...
class MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(ValueError):
    """Fail-closed validation for MO + settings decision competition DR."""


@dataclass(frozen=True)
class MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    settings_pack: SettingsDecisionCompetitionDrNdShadowRecursiveTwinPresentationWeeklyCompose
    pack_ready: bool
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin(
    *,
    mo: object,
    ...
    try:
        ...
        raise MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(str(e)) from e
    notes.extend(f"[mo] {n}" for n in mo_c.notes)

    try:
        sp = compose_settings_decision_competition_dr_nd_shadow_recursive_twin_presentation_weekly(
    ...
    except SettingsDecisionCompetitionDrNdShadowRecursiveTwinPresentationWeeklyComposeError as e:
        raise MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinComposeError(str(e)) from e
    ...
    return MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose(
        week_id=week,
        session_id=session,
...
def format_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_summary(
    c: MidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_summary",
]
