...
class CompetitionDrMoUnattendedSourceAttachRewriteComposeError(ValueError):
    """Fail-closed validation for competition DR + free marketplace bench MO."""


@dataclass(frozen=True)
class CompetitionDrMoUnattendedSourceAttachRewriteCompose:
    session_id: str
    title: str
    account_id: str
    week_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise CompetitionDrMoUnattendedSourceAttachRewriteComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_competition_dr_mo_unattended_source_attach_rewrite(
    *,
    competition: object,
    mo_pack: object,
    operator_ack: object,
    ...
    try:
        ...
        raise CompetitionDrMoUnattendedSourceAttachRewriteComposeError(str(e)) from e
    notes.extend(f"[competition] {n}" for n in comp.notes)

    try:
        fp = compose_mo_unattended_source_attach_antiek_bench_rewrite(
            mo=mo_pack.get("mo"),
        ...
        raise CompetitionDrMoUnattendedSourceAttachRewriteComposeError(str(e)) from e
    notes.extend(f"[mo_pack] {n}" for n in fp.notes)

    session = _require_nonempty(comp.session_id, field="session_id")
    title = _require_nonempty(fp.research_pack.title, field="title")
    account = _require_nonempty(fp.research_pack.account_id, field="account_id")
    ...
    if pack_ready:
        ...
        notes.append(
            "pack_ready=false — competition, mo_pack, alignment, or operator_ack "
            "gate open"
        )
    ...
    return CompetitionDrMoUnattendedSourceAttachRewriteCompose(
        session_id=session,
        title=title,
        account_id=account,
        week_id=week,
...
def format_competition_dr_mo_unattended_source_attach_rewrite_summary(
    ...
    return (
        ...
        f"ceiling_approved={c.mo_pack.mo.ceiling_approved} · "
        f"week={c.week_id} · "
