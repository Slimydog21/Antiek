...
from substrate.midnight_oil_settings_decision_competition_dr_compose import (
    MidnightOilSettingsDecisionCompetitionDrCompose,
    MidnightOilSettingsDecisionCompetitionDrComposeError,
    compose_midnight_oil_settings_decision_competition_dr,
)


class MarketplaceFreeMidnightOilSettingsDecisionComposeError(ValueError):
    """Fail-closed validation for marketplace free + MO settings decision."""


@dataclass(frozen=True)
class MarketplaceFreeMidnightOilSettingsDecisionCompose:
    title: str
    account_id: str
    week_id: str
    session_id: str
    ...
    mo_pack: MidnightOilSettingsDecisionCompetitionDrCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise MarketplaceFreeMidnightOilSettingsDecisionComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_marketplace_free_midnight_oil_settings_decision(
    *,
    market: object,
    mo_pack: object,
    operator_ack: object,
    require_both: object | None = None,
) -> MarketplaceFreeMidnightOilSettingsDecisionCompose:
    ...
    try:
        mop = compose_midnight_oil_settings_decision_competition_dr(
            mo=mo_pack.get("mo"),
            settings_pack=mo_pack.get("settings_pack"),
            operator_ack=operator_ack,
            require_both=mo_pack.get("require_both"),
        )
    except MidnightOilSettingsDecisionCompetitionDrComposeError as e:
        raise MarketplaceFreeMidnightOilSettingsDecisionComposeError(
            str(e)
        ) from e
    ...
    return MarketplaceFreeMidnightOilSettingsDecisionCompose(
        title=title,
        account_id=account,
        week_id=week,
        session_id=session,
...
def format_marketplace_free_midnight_oil_settings_decision_summary(
    c: MarketplaceFreeMidnightOilSettingsDecisionCompose,
) -> str:
    return (
...
__all__ = [
    "MarketplaceFreeMidnightOilSettingsDecisionCompose",
    "MarketplaceFreeMidnightOilSettingsDecisionComposeError",
    "compose_marketplace_free_midnight_oil_settings_decision",
    "format_marketplace_free_midnight_oil_settings_decision_summary",
]
