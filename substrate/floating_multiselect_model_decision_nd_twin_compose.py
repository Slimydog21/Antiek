"""Floating multi-select over model decision + twin search HTML-native ND twin (pure).
...
from substrate.model_decision_twin_search_html_native_nd_twin_compose import (
...
class FloatingMultiselectModelDecisionNdTwinComposeError(ValueError):
    """Fail-closed validation for multi-select + model decision ND twin pack."""


@dataclass(frozen=True)
class FloatingMultiselectModelDecisionNdTwinCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
    ...
    decision_pack: ModelDecisionTwinSearchHtmlNativeNdTwinCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise FloatingMultiselectModelDecisionNdTwinComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_floating_multiselect_model_decision_nd_twin(
    *,
    multiselect: object,
    ...
    """Multi-select cohesive + model decision ND twin. Never dispatches."""
    ...
    try:
        dp = compose_model_decision_twin_search_html_native_nd_twin(
    ...
    except ModelDecisionTwinSearchHtmlNativeNdTwinComposeError as e:
        raise FloatingMultiselectModelDecisionNdTwinComposeError(
            str(e)
        ) from e
    ...
    return FloatingMultiselectModelDecisionNdTwinCompose(
        session_id=session,
...
def format_floating_multiselect_model_decision_nd_twin_summary(
...
__all__ = [
    ...
    "format_floating_multiselect_model_decision_nd_twin_summary",
]
