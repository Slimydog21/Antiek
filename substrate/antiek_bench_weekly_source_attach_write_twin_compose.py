"""Antiek-bench weekly learn over source-attach write twin collective fullscreen (pure).
...
class AntiekBenchWeeklySourceAttachWriteTwinComposeError(ValueError):
    """Fail-closed validation for weekly learn + source attach write twin."""


@dataclass(frozen=True)
class AntiekBenchWeeklySourceAttachWriteTwinCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority": (
                "antiek_bench_weekly_source_attach_write_twin_compose_advisory"
            ),
        }
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AntiekBenchWeeklySourceAttachWriteTwinComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_antiek_bench_weekly_source_attach_write_twin(
    *,
    weekly_learn: object,
    source_pack: object,
    operator_ack: object,
    ...
    try:
        ...
        raise AntiekBenchWeeklySourceAttachWriteTwinComposeError(str(e)) from e
    notes.extend(f"[weekly_learn] {n}" for n in wl.notes)

    try:
        sp = compose_source_attach_write_twin_collective_fullscreen(
    ...
    return AntiekBenchWeeklySourceAttachWriteTwinCompose(
        week_id=week,
        session_id=session,
...
def format_antiek_bench_weekly_source_attach_write_twin_summary(
...
__all__ = [
    ...
    "format_antiek_bench_weekly_source_attach_write_twin_summary",
]
