...
``palette`` colors and ``ink`` must be ``#RRGGBB``; ``width``/``height``
...
__all__ = ["DEFAULT_INK", "SeededRng", "sketch_svg"]
...
# Caption/label ink. Separate from DEFAULT_PALETTE because it is NOT a
# voice: it never joins the ring cycle and it is not indexed by position,
# it is simply the color of text drawn on top of the ground. It used to be
# a bare literal inside ``_data_body``, which meant a caller could recolor
# the ground through ``palette`` and end up with near-invisible text on it
# — the first real caller (the ``sketch`` projection widget, whose ground
# is the pale LEMON_NEUTRALS[0]) hit exactly that. It is now an argument
# with this value as its default, so standalone output is byte-unchanged.
DEFAULT_INK = "#e8e8f0"
...
def sketch_svg(
    *,
    ...
    palette:
        ...
        voice colors.
    ink:
        ``#RRGGBB`` color for the caption and bar labels in data mode
        (they are drawn over ``palette[0]``, so a caller that darkens or
        lightens the ground must be able to move the text with it).
        Defaults to ``DEFAULT_INK``; unused in generative mode, which
        emits no text.
...
MANIFEST = SkillManifest(
    ...
    parameters=(
        ...
        SkillParameter("ink", "str | None", "#RRGGBB for caption/label text drawn over the ground (data mode only).", required=False, default="DEFAULT_INK"),
    ),
    returns=(
