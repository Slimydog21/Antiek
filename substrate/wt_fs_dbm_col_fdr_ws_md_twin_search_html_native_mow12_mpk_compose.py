...
Short residual moniker wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk.
...
from substrate.fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose import (
    FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
    FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError,
    compose_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk,
)
...
AUTHORITY = "wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_compose_advisory"


class WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(ValueError):
    """Fail-closed validation for write twin + fullscreen FDR MD mow12 residual."""


@dataclass(frozen=True)
class WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
    ...
    fullscreen_pack: FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    *,
    write: object,
    ...
    try:
        ...
        raise WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    notes.extend(f"[write] {n}" for n in wr.notes)

    try:
        fp = compose_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk(
    ...
    except FsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError as e:
        raise WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkComposeError(str(e)) from e
    ...
    return WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose(
        week_id=week,
        session_id=session,
...
def format_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(
    c: WtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpkCompose,
) -> str:
    return (
...
__all__ = [
    ...
    "format_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpk_summary",
    "AUTHORITY",
]
