...
class SettingsAddModelNdShadowCompetitionDrMoComposeError(ValueError):
    """Fail-closed validation for settings + draft fullscreen weekly ND pack."""


@dataclass(frozen=True)
class SettingsAddModelNdShadowCompetitionDrMoCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise SettingsAddModelNdShadowCompetitionDrMoComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_settings_add_model_nd_shadow_competition_dr_mo(
    *,
    settings: object,
    nd_pack: object,
    operator_ack: object,
    ...
    try:
        rp = compose_nd_shadow_competition_dr_mo_unattended_rewrite(
            nd_shadow=nd_pack.get("nd_shadow"),
            competition_pack=nd_pack.get("competition_pack"),
            operator_ack=operator_ack,
    ...
    return SettingsAddModelNdShadowCompetitionDrMoCompose(
        session_id=session,
...
def format_settings_add_model_nd_shadow_competition_dr_mo_summary(
