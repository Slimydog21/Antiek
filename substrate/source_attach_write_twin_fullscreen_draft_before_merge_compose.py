...
from substrate.write_mode_twin_collective_fullscreen_draft_before_merge_compose import (
...
class SourceAttachWriteTwinFullscreenDraftBeforeMergeComposeError(ValueError):
    """Fail-closed validation for source attach + write twin collective pack."""


@dataclass(frozen=True)
class SourceAttachWriteTwinFullscreenDraftBeforeMergeCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise SourceAttachWriteTwinFullscreenDraftBeforeMergeComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_source_attach_write_twin_fullscreen_draft_before_merge(
    *,
    sources: object,
    write_pack: object,
    operator_ack: object,
    ...
    try:
        wp = compose_write_mode_twin_collective_fullscreen_draft_before_merge(
            write=write_pack.get("write"),
            fullscreen_pack=write_pack.get("fullscreen_pack"),
            operator_ack=operator_ack,
            require_both=write_pack.get("require_both"),
        )
    except WriteModeTwinCollectiveFullscreenDraftBeforeMergeComposeError as e:
        raise SourceAttachWriteTwinFullscreenDraftBeforeMergeComposeError(
            str(e)
        ) from e
    ...
    return SourceAttachWriteTwinFullscreenDraftBeforeMergeCompose(
        session_id=session,
...
def format_source_attach_write_twin_fullscreen_draft_before_merge_summary(
...
__all__ = [
    ...
    "format_source_attach_write_twin_fullscreen_draft_before_merge_summary",
]
