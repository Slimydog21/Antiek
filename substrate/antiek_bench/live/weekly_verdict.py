    ...
    def __post_init__(self) -> None:
        ...
        for item in self.items:
            ...
            if (
                ...
                or any(
                    not candidate.live_response_hash
...
def build_weekly_verdict(
    *,
    ...
    if judged_manifest is not None:
        manifest_digest = _digest(asdict(judged_manifest))
...
def _build_judged_task_layer(
    *,
    ...
    for join in joins:
        ...
        live_responses = {
            row.requested_model: row.response_hash for row in matching_live_rows
        }
        candidate_models = tuple(candidate.model_id for candidate in join.candidates)
        candidate_responses = {
            candidate.model_id: candidate.live_response_hash for candidate in join.candidates
        }
        if (
            suite_item is None
            ...
            or live_responses != candidate_responses
        ...
        for judge in join.allowed_judges:
            identity = (
                week_id,
                suite.suite_version,
                join.item_id_hash,
            ...
            matches = [
                ...
                if (
                    ...
                    row.task_class,
                    row.rubric_version,
                    row.judge_model.strip().casefold(),
                    row.schema_version,
                )
                == identity
            ]
            forward = [row for row in matches if row.candidate_hashes == pair]
            reverse = [row for row in matches if row.candidate_hashes == tuple(reversed(pair))]
            if len(forward) != 1 or len(reverse) != 1:
                reasons.append("incomplete_judge_panel")
                continue
            first, reversed_row = forward[0], reverse[0]
            try:
    ...
    for join in joins:
        ...
        disagreement_payload.append(asdict(disagreement))
        calibration_payload.append(asdict(calibration))
