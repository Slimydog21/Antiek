...
from substrate.recursive_twin_presentation_weekly_src_write_mo_weekly_pack_compose import (
...
class NdShadowRecursiveTwinWeeklySrcWriteMoWeeklyPackComposeError(ValueError):
    """Fail-closed validation for ND shadow + twin presentation weekly pack."""


@dataclass(frozen=True)
class NdShadowRecursiveTwinWeeklySrcWriteMoWeeklyPackCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise NdShadowRecursiveTwinWeeklySrcWriteMoWeeklyPackComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_nd_shadow_recursive_twin_weekly_src_write_mo_weekly_pack(
    *,
    nd_shadow: object,
    ...
    try:
        twin = compose_recursive_twin_presentation_weekly_src_write_mo_weekly_pack(
    ...
    except RecursiveTwinPresentationWeeklySrcWriteMoWeeklyPackComposeError as e:
        raise NdShadowRecursiveTwinWeeklySrcWriteMoWeeklyPackComposeError(
            str(e)
        ) from e
    ...
    return NdShadowRecursiveTwinWeeklySrcWriteMoWeeklyPackCompose(
        week_id=week,
        session_id=session,
...
def format_nd_shadow_recursive_twin_weekly_src_write_mo_weekly_pack_summary(
...
__all__ = [
    ...
    "format_nd_shadow_recursive_twin_weekly_src_write_mo_weekly_pack_summary",
]
