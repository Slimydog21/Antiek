...
class SignedCutoverLifecycleStateV2(_Closed):
    ...
    target_store_id: str
    target_basename: str
    target_parent_device: int = Field(ge=0, le=MAX_I63)
    target_parent_inode: int = Field(ge=1, le=MAX_I63)
...
class Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1(_Closed):
    schema_version: Literal[1] = 1
    target_store_id: str
    root_id: str
    root_manifest_sha256: str
    ...
    target_dev: int = Field(ge=0, le=MAX_I63)
    target_ino: int = Field(ge=1, le=MAX_I63)
    lifecycle_phase: Literal["abort_rename_fsynced"] = "abort_rename_fsynced"
    phase_version: int = Field(ge=3, le=7)
    issuer_sequence: int = Field(ge=3, le=7)
    state_sha256: str
    barrier_id: str | None
    freeze_nonce: str | None
    source_manifest_sha256: str | None
    ...
    @model_validator(mode="after")
    def _closed_abort_rename_fsynced_recovery_pins(
        self,
        ...
        if (
            ...
            raise ValueError("epoch0 recovery abort rename fsynced authority pins")
        return self


class Epoch0RecoveryAbortRenameFsyncCompletionV1(_Closed):
    schema_version: Literal[1] = 1
    abort_renamed_to_tombstone_state: SignedMigrationLifecycleStateV1
    abort_rename_fsynced_state: SignedMigrationLifecycleStateV1
    synthetic_fixture_eligibility_only: Literal[True] = True
    live_migration_verified: Literal[False] = False
    user_accounting_effect: Literal[False] = False
    transport_reachable: Literal[False] = False
...
class Epoch0RecoveryAbortTombstoneUnlinkedAuthorityPinsV1(_Closed):
    schema_version: Literal[1] = 1
    target_store_id: str
    root_id: str
    root_manifest_sha256: str
    ...
    target_dev: int = Field(ge=0, le=MAX_I63)
    target_ino: int = Field(ge=1, le=MAX_I63)
    lifecycle_phase: Literal["abort_tombstone_unlinked"] = "abort_tombstone_unlinked"
    phase_version: int = Field(ge=4, le=8)
    issuer_sequence: int = Field(ge=4, le=8)
    state_sha256: str
    barrier_id: str | None
    freeze_nonce: str | None
    source_manifest_sha256: str | None
    ...
    @model_validator(mode="after")
    def _closed_abort_tombstone_unlinked_recovery_pins(
        self,
        ...
        if (
            ...
            raise ValueError("epoch0 recovery abort tombstone unlinked authority pins")
        return self


class Epoch0RecoveryAbortTombstoneUnlinkCompletionV1(_Closed):
    schema_version: Literal[1] = 1
    abort_rename_fsynced_state: SignedMigrationLifecycleStateV1
...
class Epoch0RecoveryAbortSourcesRevalidatedAuthorityPinsV1(_Closed):
    schema_version: Literal[1] = 1
    target_store_id: str
    root_id: str
    root_manifest_sha256: str
    ...
    target_dev: int = Field(ge=0, le=MAX_I63)
    target_ino: int = Field(ge=1, le=MAX_I63)
    lifecycle_phase: Literal["abort_sources_revalidated"] = "abort_sources_revalidated"
    phase_version: int = Field(ge=6, le=10)
    issuer_sequence: int = Field(ge=6, le=10)
    state_sha256: str
    barrier_id: str | None
    freeze_nonce: str | None
    source_manifest_sha256: str | None
    ...
    @model_validator(mode="after")
    def _closed_abort_sources_revalidated_recovery_pins(
        self,
        ...
        if (
            ...
            raise ValueError("epoch0 recovery abort sources revalidated authority pins")
        return self


class Epoch0RecoveryAbortSourcesRevalidationCompletionV1(_Closed):
    schema_version: Literal[1] = 1
    abort_deletion_fsynced_state: SignedMigrationLifecycleStateV1
    abort_sources_revalidated_state: SignedMigrationLifecycleStateV1
    synthetic_fixture_eligibility_only: Literal[True] = True
    live_migration_verified: Literal[False] = False
    user_accounting_effect: Literal[False] = False
    transport_reachable: Literal[False] = False
...
class Epoch0RecoveryAbortBarrierReleasedAuthorityPinsV1(_Closed):
    schema_version: Literal[1] = 1
    target_store_id: str
    root_id: str
    root_manifest_sha256: str
    ...
    target_dev: int = Field(ge=0, le=MAX_I63)
    target_ino: int = Field(ge=1, le=MAX_I63)
    lifecycle_phase: Literal["abort_barrier_released"] = "abort_barrier_released"
    phase_version: int = Field(ge=7, le=11)
    issuer_sequence: int = Field(ge=7, le=11)
    state_sha256: str
    barrier_id: str | None
    freeze_nonce: str | None
    source_manifest_sha256: str | None
    ...
    @model_validator(mode="after")
    def _closed_abort_barrier_released_recovery_pins(
        self,
        ...
        if (
            ...
            raise ValueError("epoch0 recovery abort barrier released authority pins")
        return self


class Epoch0RecoveryAbortBarrierReleaseCompletionV1(_Closed):
    schema_version: Literal[1] = 1
    abort_sources_revalidated_state: SignedMigrationLifecycleStateV1
    abort_barrier_released_state: SignedMigrationLifecycleStateV1
...
def _verify_cutover_lifecycle_origin_v2(
    ...
    *,
    ...
    if (
        ...
        or (successor.target_parent_device, successor.target_parent_inode)
        != (prior.target_parent_dev, prior.target_parent_ino)
...
def _verify_epoch0_recovery_abort_rename_fsync_completion_v1(
    ...
    *,
    ...
    if (
        type(completion) is not Epoch0RecoveryAbortRenameFsyncCompletionV1
        or type(issuer_verification_key) is not VerificationKeyV1
        or type(expected_renamed_pins) is not Epoch0RecoveryAbortRenamedAuthorityPinsV1
        or expected_renamed_pins.lifecycle_phase != "abort_renamed_to_tombstone"
    ):
        raise ValueError("epoch0 recovery abort rename fsync completion type")
    completion = Epoch0RecoveryAbortRenameFsyncCompletionV1.model_validate(
        completion.model_dump(mode="python")
    )
    renamed = completion.abort_renamed_to_tombstone_state
    fsynced = completion.abort_rename_fsynced_state
    _verify_signed_migration_lifecycle_state(renamed, issuer_verification_key)
    _verify_migration_lifecycle_transition(renamed, fsynced, issuer_verification_key)
    renamed_pins = Epoch0RecoveryAbortRenamedAuthorityPinsV1.model_validate(
        {
            "target_store_id": renamed.target_store_id,
            "root_id": renamed.root_id,
            "root_manifest_sha256": renamed.root_manifest_sha256,
            "target_parent_dev": renamed.target_parent_dev,
            "target_parent_ino": renamed.target_parent_ino,
            "target_basename": renamed.target_basename,
            "target_dev": renamed.target_dev,
            "target_ino": renamed.target_ino,
            "lifecycle_phase": renamed.lifecycle_phase,
            "phase_version": renamed.phase_version,
            "issuer_sequence": renamed.issuer_sequence,
            "state_sha256": renamed.state_sha256,
            "barrier_id": renamed.barrier_id,
            "freeze_nonce": renamed.freeze_nonce,
            "source_manifest_sha256": renamed.source_manifest_sha256,
            "copy_audit_sha256": renamed.copy_audit_sha256,
    ...
    if (
        renamed_pins != expected_renamed_pins
        or renamed.lifecycle_phase != "abort_renamed_to_tombstone"
        or fsynced.lifecycle_phase != "abort_rename_fsynced"
        or fsynced.phase_version != renamed.phase_version + 1
        or fsynced.issuer_sequence != renamed.issuer_sequence + 1
        or fsynced.previous_state_sha256 != renamed.state_sha256
        or (
        ...
        != (
            renamed.target_store_id,
            renamed.root_id,
            renamed.root_manifest_sha256,
            renamed.target_parent_dev,
            renamed.target_parent_ino,
            renamed.target_basename,
            renamed.target_dev,
            renamed.target_ino,
            renamed.tombstone_basename,
        )
        or inherited_pins != renamed_pin_tuple
        ...
        raise ValueError("epoch0 recovery abort rename fsync completion mismatch")


def _verify_epoch0_recovery_abort_tombstone_unlink_completion_v1(
    completion: Epoch0RecoveryAbortTombstoneUnlinkCompletionV1,
    *,
    issuer_verification_key: VerificationKeyV1,
    expected_fsynced_pins: Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1,
) -> None:
    if (
        type(completion) is not Epoch0RecoveryAbortTombstoneUnlinkCompletionV1
        or type(issuer_verification_key) is not VerificationKeyV1
        or type(expected_fsynced_pins) is not Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1
        or expected_fsynced_pins.lifecycle_phase != "abort_rename_fsynced"
    ):
        raise ValueError("epoch0 recovery abort tombstone unlink completion type")
    completion = Epoch0RecoveryAbortTombstoneUnlinkCompletionV1.model_validate(
        completion.model_dump(mode="python")
    )
    fsynced = completion.abort_rename_fsynced_state
    unlinked = completion.abort_tombstone_unlinked_state
    _verify_signed_migration_lifecycle_state(fsynced, issuer_verification_key)
    _verify_migration_lifecycle_transition(fsynced, unlinked, issuer_verification_key)
    fsynced_pins = Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1.model_validate(
        {
            "target_store_id": fsynced.target_store_id,
            "root_id": fsynced.root_id,
            "root_manifest_sha256": fsynced.root_manifest_sha256,
            "target_parent_dev": fsynced.target_parent_dev,
            "target_parent_ino": fsynced.target_parent_ino,
            "target_basename": fsynced.target_basename,
            "target_dev": fsynced.target_dev,
            "target_ino": fsynced.target_ino,
            "lifecycle_phase": fsynced.lifecycle_phase,
            "phase_version": fsynced.phase_version,
            "issuer_sequence": fsynced.issuer_sequence,
            "state_sha256": fsynced.state_sha256,
    ...
    if (
        fsynced_pins != expected_fsynced_pins
        or fsynced.lifecycle_phase != "abort_rename_fsynced"
        or unlinked.lifecycle_phase != "abort_tombstone_unlinked"
        or unlinked.phase_version != fsynced.phase_version + 1
        or unlinked.issuer_sequence != fsynced.issuer_sequence + 1
        or unlinked.previous_state_sha256 != fsynced.state_sha256
        or (
        ...
        or inherited_pins != fsynced_pin_tuple
        or completion.synthetic_fixture_eligibility_only is not True
        or completion.live_migration_verified is not False
        or completion.user_accounting_effect is not False
        or completion.transport_reachable is not False
...
def _verify_epoch0_recovery_abort_sources_revalidation_completion_v1(
    ...
    *,
    ...
    if (
        type(completion) is not Epoch0RecoveryAbortSourcesRevalidationCompletionV1
        or type(issuer_verification_key) is not VerificationKeyV1
        or type(expected_fsynced_pins) is not Epoch0RecoveryAbortDeletionFsyncedAuthorityPinsV1
        or expected_fsynced_pins.lifecycle_phase != "abort_deletion_fsynced"
    ):
        raise ValueError("epoch0 recovery abort sources revalidation completion type")
    completion = Epoch0RecoveryAbortSourcesRevalidationCompletionV1.model_validate(
        completion.model_dump(mode="python")
    )
    fsynced = completion.abort_deletion_fsynced_state
    revalidated = completion.abort_sources_revalidated_state
    _verify_signed_migration_lifecycle_state(fsynced, issuer_verification_key)
    _verify_migration_lifecycle_transition(fsynced, revalidated, issuer_verification_key)
    fsynced_pins = Epoch0RecoveryAbortDeletionFsyncedAuthorityPinsV1.model_validate(
        {
            "target_store_id": fsynced.target_store_id,
            "root_id": fsynced.root_id,
            "root_manifest_sha256": fsynced.root_manifest_sha256,
    ...
    inherited_pins = (
        revalidated.barrier_id,
        revalidated.freeze_nonce,
        revalidated.source_manifest_sha256,
        revalidated.copy_audit_sha256,
        revalidated.witness_sha256,
    )
    fsynced_pin_tuple = (
        fsynced.barrier_id,
        fsynced.freeze_nonce,
    ...
    if (
        fsynced_pins != expected_fsynced_pins
        or fsynced.lifecycle_phase != "abort_deletion_fsynced"
        or revalidated.lifecycle_phase != "abort_sources_revalidated"
        or revalidated.phase_version != fsynced.phase_version + 1
        or revalidated.issuer_sequence != fsynced.issuer_sequence + 1
        or revalidated.previous_state_sha256 != fsynced.state_sha256
        or (
            revalidated.target_store_id,
            revalidated.root_id,
            revalidated.root_manifest_sha256,
            revalidated.target_parent_dev,
            revalidated.target_parent_ino,
            revalidated.target_basename,
            revalidated.target_dev,
            revalidated.target_ino,
            revalidated.tombstone_basename,
        )
        != (
            fsynced.target_store_id,
            fsynced.root_id,
        ...
        raise ValueError("epoch0 recovery abort sources revalidation completion mismatch")


def _verify_epoch0_recovery_abort_barrier_release_completion_v1(
    ...
    *,
    ...
    if (
        type(completion) is not Epoch0RecoveryAbortBarrierReleaseCompletionV1
        or type(issuer_verification_key) is not VerificationKeyV1
        or type(expected_revalidated_pins)
        is not Epoch0RecoveryAbortSourcesRevalidatedAuthorityPinsV1
        or expected_revalidated_pins.lifecycle_phase != "abort_sources_revalidated"
    ):
        raise ValueError("epoch0 recovery abort barrier release completion type")
    completion = Epoch0RecoveryAbortBarrierReleaseCompletionV1.model_validate(
        completion.model_dump(mode="python")
    )
    revalidated = completion.abort_sources_revalidated_state
    released = completion.abort_barrier_released_state
    abort_result = completion.abort_result
    _verify_signed_migration_lifecycle_state(revalidated, issuer_verification_key)
    _verify_migration_lifecycle_transition(revalidated, released, issuer_verification_key)
    revalidated_pins = Epoch0RecoveryAbortSourcesRevalidatedAuthorityPinsV1.model_validate(
        {
            "target_store_id": revalidated.target_store_id,
            "root_id": revalidated.root_id,
            "root_manifest_sha256": revalidated.root_manifest_sha256,
            "target_parent_dev": revalidated.target_parent_dev,
            "target_parent_ino": revalidated.target_parent_ino,
            "target_basename": revalidated.target_basename,
            "target_dev": revalidated.target_dev,
            "target_ino": revalidated.target_ino,
            "lifecycle_phase": revalidated.lifecycle_phase,
            "phase_version": revalidated.phase_version,
            "issuer_sequence": revalidated.issuer_sequence,
            "state_sha256": revalidated.state_sha256,
    ...
    if (
        revalidated_pins != expected_revalidated_pins
        or revalidated.lifecycle_phase != "abort_sources_revalidated"
        or released.lifecycle_phase != "abort_barrier_released"
        or released.phase_version != revalidated.phase_version + 1
        or released.issuer_sequence != revalidated.issuer_sequence + 1
        or released.previous_state_sha256 != revalidated.state_sha256
        or (
            released.target_store_id,
            released.root_id,
            released.root_manifest_sha256,
            released.target_parent_dev,
            released.target_parent_ino,
            released.target_basename,
            released.target_dev,
            released.target_ino,
            released.tombstone_basename,
        )
        != (
            revalidated.target_store_id,
            revalidated.root_id,
        ...
        or inherited_pins != revalidated_pin_tuple
        or abort_result.store_id != released.target_store_id
        or abort_result.source_manifest_sha256 != released.source_manifest_sha256
        or abort_result.copy_audit_sha256 != released.copy_audit_sha256
        or abort_result.aborted_at_ms != released.updated_at_ms
        or completion.synthetic_fixture_eligibility_only is not True
        or completion.live_migration_verified is not False
        or completion.user_accounting_effect is not False
        or completion.transport_reachable is not False
        ...
        raise ValueError("epoch0 recovery abort barrier release completion mismatch")
...
_MIGRATION_ABORT_RENAME_FSYNCED_TARGET_LAYOUT = Literal["pre_unlink", "post_unlink"]
...
def _classify_migration_abort_rename_fsynced_target_layout(
    *,
    parent_fd: int,
    target_fd: int,
    target_basename: str,
    tombstone_basename: str,
    expected_target_identity: tuple[int, int],
) -> _MIGRATION_ABORT_RENAME_FSYNCED_TARGET_LAYOUT:
    if type(parent_fd) is not int or type(target_fd) is not int or parent_fd < 0 or target_fd < 0:
        raise ValueError("migration abort rename fsynced target layout descriptor")
    target_info = os.fstat(target_fd)
    if (
        not stat.S_ISREG(target_info.st_mode)
        or target_info.st_uid != os.getuid()
        ...
        raise ValueError("migration abort rename fsynced target identity")
    target_entry = _migration_lifecycle_entry_identity(parent_fd, target_basename)
    tombstone_entry = _migration_lifecycle_entry_identity(parent_fd, tombstone_basename)
    if (
        target_entry is None
        and tombstone_entry == expected_target_identity
        and target_info.st_nlink == 1
    ):
        return "pre_unlink"
    if target_entry is None and tombstone_entry is None and target_info.st_nlink == 0:
        return "post_unlink"
    raise ValueError("migration abort rename fsynced target layout mismatch")


def _verify_migration_abort_unlinked_target_layout(
    *,
    parent_fd: int,
    target_fd: int,
    target_basename: str,
    ...
    if (
        _classify_migration_abort_rename_fsynced_target_layout(
            parent_fd=parent_fd,
            target_fd=target_fd,
            target_basename=target_basename,
            tombstone_basename=tombstone_basename,
        ...
        raise ValueError("migration abort unlinked target layout")
...
def _authenticate_epoch0_recovery_abort_rename_fsynced_state_v1(
    *,
    parent_fd: int,
    target_fd: int,
    verification_key: VerificationKeyV1,
    expected: Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1,
) -> SignedMigrationLifecycleStateV1:
    if (
        type(parent_fd) is not int
        or type(target_fd) is not int
        or type(expected) is not Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1
    ):
        raise ValueError("epoch0 recovery abort rename fsynced descriptor type")
    expected = Epoch0RecoveryAbortRenameFsyncedAuthorityPinsV1.model_validate(
        expected.model_dump(mode="python")
    )
    if _migration_lifecycle_parent_identity(parent_fd) != (
        expected.target_parent_dev,
        ...
        raise ValueError("epoch0 recovery abort rename fsynced parent identity")
    expected_target_identity = (expected.target_dev, expected.target_ino)
    target_info = os.fstat(target_fd)
    if (
        not stat.S_ISREG(target_info.st_mode)
        ...
        raise ValueError("epoch0 recovery abort rename fsynced target identity")
    tombstone_basename = f".{expected.target_basename}.abort-v1"
    _require_migration_abort_target_sidecars_absent(
        parent_fd=parent_fd,
        target_basename=expected.target_basename,
    ...
    if any(getattr(state, field) != getattr(expected, field) for field in fields):
        raise ValueError("epoch0 recovery abort rename fsynced signed state mismatch")
    target_after = os.fstat(target_fd)
    if (target_after.st_dev, target_after.st_ino) != expected_target_identity:
        raise ValueError(
            "epoch0 recovery abort rename fsynced target changed during authentication"
        )
    _classify_migration_abort_rename_fsynced_target_layout(
        parent_fd=parent_fd,
        target_fd=target_fd,
...
def _authenticate_epoch0_recovery_abort_tombstone_unlinked_state_v1(
    *,
    parent_fd: int,
    target_fd: int,
    verification_key: VerificationKeyV1,
    expected: Epoch0RecoveryAbortTombstoneUnlinkedAuthorityPinsV1,
) -> SignedMigrationLifecycleStateV1:
    if (
        type(parent_fd) is not int
        or type(target_fd) is not int
        or type(expected) is not Epoch0RecoveryAbortTombstoneUnlinkedAuthorityPinsV1
    ):
        raise ValueError("epoch0 recovery abort tombstone unlinked descriptor type")
    expected = Epoch0RecoveryAbortTombstoneUnlinkedAuthorityPinsV1.model_validate(
        expected.model_dump(mode="python")
    )
    if _migration_lifecycle_parent_identity(parent_fd) != (
        expected.target_parent_dev,
        ...
        raise ValueError("epoch0 recovery abort tombstone unlinked parent identity")
    expected_target_identity = (expected.target_dev, expected.target_ino)
    tombstone_basename = f".{expected.target_basename}.abort-v1"
    _require_migration_abort_target_sidecars_absent(
        parent_fd=parent_fd,
    ...
    if any(getattr(state, field) != getattr(expected, field) for field in fields):
        raise ValueError("epoch0 recovery abort tombstone unlinked signed state mismatch")
    target_after = os.fstat(target_fd)
    if (target_after.st_dev, target_after.st_ino) != expected_target_identity:
        raise ValueError(
...
def _authenticate_epoch0_recovery_abort_sources_revalidated_state_v1(
    *,
    parent_fd: int,
    target_fd: int,
    verification_key: VerificationKeyV1,
    expected: Epoch0RecoveryAbortSourcesRevalidatedAuthorityPinsV1,
) -> SignedMigrationLifecycleStateV1:
    if (
        type(parent_fd) is not int
        or type(target_fd) is not int
        or type(expected) is not Epoch0RecoveryAbortSourcesRevalidatedAuthorityPinsV1
    ):
        raise ValueError("epoch0 recovery abort sources revalidated descriptor type")
    expected = Epoch0RecoveryAbortSourcesRevalidatedAuthorityPinsV1.model_validate(
        expected.model_dump(mode="python")
    )
    if _migration_lifecycle_parent_identity(parent_fd) != (
        expected.target_parent_dev,
        ...
        raise ValueError("epoch0 recovery abort sources revalidated parent identity")
    expected_target_identity = (expected.target_dev, expected.target_ino)
    tombstone_basename = f".{expected.target_basename}.abort-v1"
    _require_migration_abort_target_sidecars_absent(
        parent_fd=parent_fd,
    ...
    if any(getattr(state, field) != getattr(expected, field) for field in fields):
        raise ValueError("epoch0 recovery abort sources revalidated signed state mismatch")
    target_after = os.fstat(target_fd)
    if (
        not stat.S_ISREG(target_after.st_mode)
        or target_after.st_uid != os.getuid()
        ...
        raise ValueError(
            "epoch0 recovery abort sources revalidated target changed during authentication"
        )
    _verify_migration_abort_unlinked_target_layout(
        parent_fd=parent_fd,
        target_fd=target_fd,
...
def _authenticate_epoch0_recovery_abort_barrier_released_state_v1(
    *,
    parent_fd: int,
    target_fd: int,
    verification_key: VerificationKeyV1,
    expected: Epoch0RecoveryAbortBarrierReleasedAuthorityPinsV1,
) -> SignedMigrationLifecycleStateV1:
    if (
        type(parent_fd) is not int
        or type(target_fd) is not int
        or type(expected) is not Epoch0RecoveryAbortBarrierReleasedAuthorityPinsV1
    ):
        raise ValueError("epoch0 recovery abort barrier released descriptor type")
    expected = Epoch0RecoveryAbortBarrierReleasedAuthorityPinsV1.model_validate(
        expected.model_dump(mode="python")
    )
    if _migration_lifecycle_parent_identity(parent_fd) != (
        expected.target_parent_dev,
        ...
        raise ValueError("epoch0 recovery abort barrier released parent identity")
    expected_target_identity = (expected.target_dev, expected.target_ino)
    tombstone_basename = f".{expected.target_basename}.abort-v1"
    _require_migration_abort_target_sidecars_absent(
        parent_fd=parent_fd,
    ...
    if any(getattr(state, field) != getattr(expected, field) for field in fields):
        raise ValueError("epoch0 recovery abort barrier released signed state mismatch")
    target_after = os.fstat(target_fd)
    if (
        not stat.S_ISREG(target_after.st_mode)
        or target_after.st_uid != os.getuid()
        ...
        raise ValueError(
            "epoch0 recovery abort barrier released target changed during authentication"
        )
    _verify_migration_abort_unlinked_target_layout(
        parent_fd=parent_fd,
        target_fd=target_fd,
...
def _cutover_lifecycle_state_document_v2(state: SignedCutoverLifecycleStateV2) -> bytes:
    if type(state) is not SignedCutoverLifecycleStateV2:
        raise ValueError("cutover lifecycle document type")
    state = SignedCutoverLifecycleStateV2.model_validate(state.model_dump(mode="python"))
    material = state.model_dump(mode="python")
    material["signature_ed25519"] = state.signature_ed25519.hex()
    encoded = _canonical_json(material)
    if len(encoded) > _MAX_MIGRATION_LIFECYCLE_DOCUMENT_BYTES:
        raise ValueError("cutover lifecycle document bound")
    return encoded


def _parse_cutover_lifecycle_state_document_v2(
    document: bytes, verification_key: VerificationKeyV1
) -> SignedCutoverLifecycleStateV2:
    parsed = _parse_strict_json(document, _MAX_MIGRATION_LIFECYCLE_DOCUMENT_BYTES)
    signature = parsed.get("signature_ed25519")
    if type(signature) is not str or not re.fullmatch(r"[0-9a-f]{128}", signature):
        raise ValueError("cutover lifecycle signature encoding")
    parsed["signature_ed25519"] = bytes.fromhex(signature)
    state = SignedCutoverLifecycleStateV2.model_validate(parsed)
    if document != _cutover_lifecycle_state_document_v2(state):
        raise ValueError("cutover lifecycle document not canonical")
    _verify_signed_cutover_lifecycle_state_v2(state, verification_key)
    return state


def _parse_lifecycle_journal_document_v2(
    document: bytes, verification_key: VerificationKeyV1
) -> SignedMigrationLifecycleStateV1 | SignedCutoverLifecycleStateV2:
    parsed = _parse_strict_json(document, _MAX_MIGRATION_LIFECYCLE_DOCUMENT_BYTES)
    schema_version = parsed.get("schema_version")
    if schema_version == 1:
        return _parse_migration_lifecycle_state_document(document, verification_key)
    if schema_version == 2:
        return _parse_cutover_lifecycle_state_document_v2(document, verification_key)
    raise ValueError("lifecycle journal schema version")
...
def _verify_migration_lifecycle_target_state(
    ...
    elif state.lifecycle_phase == "abort_tombstone_unlinked":
...
def _verify_cutover_lifecycle_target_state_v2(
    parent_fd: int, state: SignedCutoverLifecycleStateV2
) -> None:
    if (
        _migration_lifecycle_parent_identity(parent_fd)
        != (state.target_parent_device, state.target_parent_inode)
        or _migration_lifecycle_entry_identity(parent_fd, state.target_basename)
        != (state.target_device, state.target_inode)
        or _migration_lifecycle_entry_identity(
            parent_fd, f".{state.target_basename}.abort-v1"
        )
        is not None
    ):
        raise ValueError("cutover lifecycle target identity mismatch")
...
def _read_signed_lifecycle_journal_v2(
    *, parent_fd: int, target_basename: str, verification_key: VerificationKeyV1
) -> SignedMigrationLifecycleStateV1 | SignedCutoverLifecycleStateV2:
    parent_identity = _migration_lifecycle_parent_identity(parent_fd)
    state_basename = _migration_lifecycle_state_basename(target_basename)
    ...
    if (
        ...
        raise ValueError("lifecycle journal file identity")
    descriptor = os.open(
        state_basename, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0), dir_fd=parent_fd
    )
    try:
        opened = os.fstat(descriptor)
        if (opened.st_dev, opened.st_ino) != (path_info.st_dev, path_info.st_ino):
            raise ValueError("lifecycle journal changed during open")
        chunks: list[bytes] = []
        remaining = _MAX_MIGRATION_LIFECYCLE_DOCUMENT_BYTES + 1
        while remaining:
            chunk = os.read(descriptor, min(65_536, remaining))
        ...
        if (
            not document
            or len(document) != opened.st_size
            or len(document) > _MAX_MIGRATION_LIFECYCLE_DOCUMENT_BYTES
            or (after.st_dev, after.st_ino, after.st_size)
            != (opened.st_dev, opened.st_ino, opened.st_size)
        ):
            raise ValueError("lifecycle journal changed during read")
    finally:
        os.close(descriptor)
    path_after = os.stat(state_basename, dir_fd=parent_fd, follow_symlinks=False)
    if (
        ...
        raise ValueError("lifecycle journal changed after read")
    state = _parse_lifecycle_journal_document_v2(document, verification_key)
    if state.target_basename != target_basename:
        raise ValueError("lifecycle journal target basename mismatch")
    if isinstance(state, SignedMigrationLifecycleStateV1):
        if (state.target_parent_dev, state.target_parent_ino) != parent_identity:
            raise ValueError("lifecycle journal V1 parent mismatch")
        _verify_migration_lifecycle_target_state(parent_fd, state)
    else:
...
def _persist_signed_cutover_lifecycle_state_v2(
    *,
    parent_fd: int,
    state: SignedCutoverLifecycleStateV2,
    verification_key: VerificationKeyV1,
    expected_prior_state_sha256: str,
    expected_issuer_generation_nonce: str,
    locked_parent_fd: int,
    _fault_hook: Callable[[Literal["after_rename", "after_parent_fsync", "after_reread"]], None]
    | None = None,
) -> SignedCutoverLifecycleStateV2:
    parent_identity = _migration_lifecycle_parent_identity(parent_fd)
    if (
        type(state) is not SignedCutoverLifecycleStateV2
        or (state.target_parent_device, state.target_parent_inode) != parent_identity
        or not _HEX64.fullmatch(expected_prior_state_sha256)
        or not _HEX64.fullmatch(expected_issuer_generation_nonce)
        or state.issuer_generation_nonce != expected_issuer_generation_nonce
    ):
        raise ValueError("cutover lifecycle persistence authority mismatch")
    _verify_signed_cutover_lifecycle_state_v2(state, verification_key)
    state_basename = _migration_lifecycle_state_basename(state.target_basename)
    temporary_basename: str | None = None
    try:
        if _migration_lifecycle_parent_identity(locked_parent_fd) != parent_identity:
            raise ValueError("cutover lifecycle locked parent mismatch")
        _cleanup_migration_lifecycle_temporaries(locked_parent_fd, state.target_basename)
        _verify_cutover_lifecycle_target_state_v2(locked_parent_fd, state)
        prior = _read_signed_lifecycle_journal_v2(
            parent_fd=locked_parent_fd,
            target_basename=state.target_basename,
            verification_key=verification_key,
        )
        if prior == state:
            if state.previous_state_sha256 != expected_prior_state_sha256:
                raise ValueError("cutover lifecycle replay predecessor mismatch")
            os.fsync(locked_parent_fd)
            confirmed = _read_signed_lifecycle_journal_v2(
                parent_fd=locked_parent_fd,
                target_basename=state.target_basename,
                verification_key=verification_key,
            )
            if confirmed != state:
                raise ValueError("cutover lifecycle replay durability mismatch")
            return state
        if prior.state_sha256 != expected_prior_state_sha256:
            raise ValueError("cutover lifecycle compare-and-swap mismatch")
        if isinstance(prior, SignedMigrationLifecycleStateV1):
            raise ValueError("cutover lifecycle origin requires root interlock")
        _verify_cutover_lifecycle_transition_v2(prior, state, verification_key)
        encoded = _cutover_lifecycle_state_document_v2(state)
        temporary_basename = (
            f".{state.target_basename}.migration-state-v1.{secrets.token_hex(12)}.tmp"
        )
        temporary_fd = os.open(
        ...
        try:
            ...
            while view:
                written = os.write(temporary_fd, view)
                if written <= 0:
                    raise OSError("short cutover lifecycle state write")
                view = view[written:]
            os.fsync(temporary_fd)
        finally:
            os.close(temporary_fd)
        ...
        if _fault_hook is not None:
            _fault_hook("after_parent_fsync")
        persisted = _read_signed_lifecycle_journal_v2(
            parent_fd=locked_parent_fd,
            target_basename=state.target_basename,
            verification_key=verification_key,
        )
        if persisted != state:
            raise ValueError("cutover lifecycle persistence mismatch")
        if _fault_hook is not None:
            _fault_hook("after_reread")
        return state
    finally:
