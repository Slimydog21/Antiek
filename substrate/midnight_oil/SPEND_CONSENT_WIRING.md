...
snapshot the persisted job into `JobConsentConfig`, convert the approved ceiling
to integer cents, issue a short-lived receipt, and require an exact successful
...
return the prior claim as idempotent, but may not authorize a second operation;
the route/operation ledger must bind `receipt_id` to its single operation ID.
