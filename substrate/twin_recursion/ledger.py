...
class SourceRevision:
    ...
    def source_hash(self) -> str:
        ...
        return source_asset_receipt_hash(self.asset)
...
class TwinSnapshot:
    ...
    def body(self) -> ResearchArtifactBody | None:
        return None if self.body_json is None else ResearchArtifactBody.model_validate_json(self.body_json)
...
def _completion_json(model_id: str, proposal: TwinProposal, receipt: TwinGenerationReceipt) -> str:
    return _canonical_json({"model_id": model_id, "proposal": asdict(proposal), "receipt": asdict(receipt)})
...
class TwinRecursionLedger:
    ...
    def __init__(self, path: str | Path, *, before_commit: Callable[[], None] | None = None,
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            existing = con.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='twin_ledger_meta'").fetchone()
            if existing is None:
                for statement in TABLES.values():
                    con.execute(statement)
                for statement in TRIGGERS.values():
                    con.execute(statement)
                con.execute("INSERT INTO twin_ledger_meta VALUES(1,?,?)", (SCHEMA_VERSION, _schema_digest()))
    ...
    def _verify_schema(self, con: sqlite3.Connection) -> None:
        meta = con.execute("SELECT schema_version,schema_digest FROM twin_ledger_meta WHERE singleton=1").fetchone()
        ...
        rows = con.execute("SELECT name,sql FROM sqlite_master WHERE name LIKE 'twin_%' AND type IN ('table','trigger')").fetchall()
    ...
    def _append_event(self, con: sqlite3.Connection, *, account_id: str, asset_id: str,
                      source_hash: str, event_type: str, data: object) -> None:
        prior = con.execute("SELECT sequence,event_hash FROM twin_events ORDER BY sequence DESC LIMIT 1").fetchone()
        ...
        event_hash = _sha(_canonical_json([sequence, account_id, asset_id, source_hash,
                                           event_type, event_data, previous]))
        con.execute("INSERT INTO twin_events VALUES(?,?,?,?,?,?,?,?,?)",
                    (sequence, _digest("event", event_hash), account_id, asset_id, source_hash,
                     event_type, event_data, previous, event_hash))
    ...
    def apply_completion(self, revision: SourceRevision, *, model_id: str, proposal: TwinProposal,
                         receipt: TwinGenerationReceipt) -> TwinSnapshot:
        ...
        try:
            ...
            existing = con.execute("SELECT * FROM twin_bindings WHERE account_id=? AND asset_id=? AND source_hash=?",
                                   (revision.account_id, revision.asset.asset_id, revision.source_hash)).fetchone()
            if existing is not None:
                if existing["completion_digest"] != completion_digest:
                    raise TwinConflictError("completion substitution conflicts with canonical binding")
                con.commit()
                return self._snapshot(con, row)
            if row["state"] != "pending_authorization":
                raise TwinConflictError(f"source revision is {row['state']}")
            document = generate_twin(revision.asset, model_id=model_id,
                                     authenticated_account_id=revision.account_id,
                                     proposal=proposal, receipt=receipt)
            body_json = _canonical_json(document.body.model_dump(mode="json"))
            body_hash = _sha(body_json)
            twin_id = _digest("twin", revision.account_id, revision.asset.asset_id, revision.source_hash)
            binding_id = _digest("binding", row["job_id"], twin_id)
            proposal_hash = proposal_receipt_hash(revision.asset, proposal)
            con.execute("INSERT INTO twin_bindings VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (binding_id, revision.account_id, revision.asset.asset_id, revision.source_hash,
                         row["job_id"], twin_id, model_id, proposal_hash, receipt.receipt_id,
                         completion_json, completion_digest, body_json, body_hash))
            con.execute("UPDATE twin_sources SET state='ready',failure_code=NULL WHERE account_id=? AND asset_id=? AND source_hash=?",
                        (revision.account_id, revision.asset.asset_id, revision.source_hash))
            self._append_event(con, account_id=revision.account_id, asset_id=revision.asset.asset_id,
                               source_hash=revision.source_hash, event_type="completion_bound",
    ...
    def register_materialized_twin(self, binding_id: str) -> TwinSnapshot:
        ...
        try:
            ...
            row = con.execute("SELECT * FROM twin_sources WHERE account_id=? AND asset_id=? AND source_hash=?",
                              (revision.account_id, asset.asset_id, revision.source_hash)).fetchone()
            if row is None:
    ...
    def _transition_failure(self, revision: SourceRevision, failure: str) -> TwinSnapshot:
        ...
        try:
            ...
            con.execute("UPDATE twin_sources SET state='failed',failure_code=? WHERE account_id=? AND asset_id=? AND source_hash=?",
                        (failure, revision.account_id, revision.asset.asset_id, revision.source_hash))
            self._append_event(con, account_id=revision.account_id, asset_id=revision.asset.asset_id,
                               source_hash=revision.source_hash, event_type="failure_recorded",
    ...
    def reset_failed(self, revision: SourceRevision) -> TwinSnapshot:
        ...
        try:
            ...
            con.execute("UPDATE twin_sources SET state='pending_authorization',failure_code=NULL WHERE account_id=? AND asset_id=? AND source_hash=?",
                        (revision.account_id, revision.asset.asset_id, revision.source_hash))
            self._append_event(con, account_id=revision.account_id, asset_id=revision.asset.asset_id,
                               source_hash=revision.source_hash, event_type="failure_reset",
    ...
    def get(self, account_id: str, asset_id: str, source_hash: str) -> TwinSnapshot:
        with self._connect() as con:
            row = con.execute("SELECT * FROM twin_sources WHERE account_id=? AND asset_id=? AND source_hash=?",
                              (account_id, asset_id, source_hash)).fetchone()
            if row is None:
    ...
    def _snapshot(self, con: sqlite3.Connection, source: sqlite3.Row) -> TwinSnapshot:
        ...
        return TwinSnapshot(source["account_id"], source["asset_id"], source["source_hash"],
                            source["state"], source["parent_binding_id"] is None, source["job_id"],
                            ...
                            None if binding is None else binding["receipt_id"], source["failure_code"])

    def render_twin_html(self, binding_id: str) -> str:
        with self._connect() as con:
            row = con.execute("SELECT body_json FROM twin_bindings WHERE binding_id=?", (binding_id,)).fetchone()
            if row is None:
    ...
    def verify_integrity(self) -> None:
        with self._connect() as con:
            ...
            for event in con.execute("SELECT * FROM twin_events ORDER BY sequence"):
                ...
                expected = _sha(_canonical_json([event["sequence"], event["account_id"], event["asset_id"],
                    event["source_hash"], event["event_type"], event["event_data"], previous]))
                if event["previous_hash"] != previous or event["event_hash"] != expected or event["event_id"] != _digest("event", expected):
                ...
                elif event["event_type"] == "failure_recorded" and current == ("pending_authorization", None):
                    lifecycle[key] = ("failed", data.get("failure_code"))
                elif event["event_type"] == "failure_reset" and current is not None and current[0] == "failed" and data.get("previous_failure_code") == current[1]:
                    lifecycle[key] = ("pending_authorization", None)
                elif event["event_type"] == "completion_bound" and current == ("pending_authorization", None):
            ...
            for source in con.execute("SELECT * FROM twin_sources"):
                ...
                expected_marker = source["parent_binding_id"] if source["parent_binding_id"] is not None else (
                    binding["binding_id"] if binding is not None else source["failure_code"])
                ...
                if binding is not None:
                    ...
                    twin_id = _digest("twin", source["account_id"], source["asset_id"], source["source_hash"])
                    ...
                    if canonical_body != binding["body_json"] or _sha(canonical_body) != binding["body_hash"]:
                        raise TwinIntegrityError("canonical body mismatch")
                    if binding["twin_id"] != twin_id or binding["binding_id"] != _digest("binding", source["job_id"], twin_id):
                        raise TwinIntegrityError("binding identity mismatch")
                    if (receipt.receipt_id != binding["receipt_id"] or
                            receipt.account_id != source["account_id"] or
                            receipt.asset_id != source["asset_id"] or
                    ...
                    if (_canonical_json(completion_value) != binding["completion_json"] or
                            ...
                            completion_value["model_id"] != binding["model_id"]):
    ...
    def universality_report(self, account_id: str) -> UniversalityReport:
        with self._connect() as con:
            row = con.execute("""SELECT count(*) total,
                ...
                FROM twin_sources WHERE account_id=?""", (account_id, account_id)).fetchone()
        total, twinnable, bound = int(row["total"] or 0), int(row["twinnable"] or 0), int(row["bound"] or 0)
        verdict: Verdict = "unknown" if twinnable == 0 else ("universal" if bound == twinnable else "partial")
        return UniversalityReport(account_id, total, twinnable, bound, int(row["twins"] or 0),
                                  int(row["pending"] or 0), int(row["failed"] or 0), verdict)
