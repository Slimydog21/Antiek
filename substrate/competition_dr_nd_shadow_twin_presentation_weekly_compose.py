...
class CompetitionDrNdShadowTwinPresentationWeeklyComposeError(ValueError):
    """Fail-closed validation for competition DR + ND shadow twin presentation weekly."""


@dataclass(frozen=True)
class CompetitionDrNdShadowTwinPresentationWeeklyCompose:
    session_id: str
    week_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise CompetitionDrNdShadowTwinPresentationWeeklyComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_competition_dr_nd_shadow_twin_presentation_weekly(
    *,
    competition: object,
    nd_pack: object,
    operator_ack: object,
    ...
    try:
        ...
        raise CompetitionDrNdShadowTwinPresentationWeeklyComposeError(str(e)) from e
    notes.extend(f"[competition] {n}" for n in comp.notes)

    try:
        nd = compose_nd_shadow_recursive_twin_presentation_weekly_source_attach(
            nd_shadow=nd_pack.get("nd_shadow"),
            twin_presentation=nd_pack.get("twin_presentation"),
            operator_ack=operator_ack,
            require_both=nd_pack.get("require_both"),
        )
    except NdShadowRecursiveTwinPresentationWeeklySourceAttachComposeError as e:
        raise CompetitionDrNdShadowTwinPresentationWeeklyComposeError(str(e)) from e
    ...
    return CompetitionDrNdShadowTwinPresentationWeeklyCompose(
        session_id=session,
        week_id=week,
...
def format_competition_dr_nd_shadow_twin_presentation_weekly_summary(
