"""Draft-before-full-merge over floating multi-select model decision free settings (pure).
...
class DraftBeforeMergeFloatingMultiselectModelDecisionComposeError(ValueError):
    """Fail-closed validation for draft-before-merge + multi-select model decision pack."""


@dataclass(frozen=True)
class DraftBeforeMergeFloatingMultiselectModelDecisionCompose:
    session_id: str
    parent_asset_id: str
    week_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise DraftBeforeMergeFloatingMultiselectModelDecisionComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_draft_before_merge_floating_multiselect_model_decision(
    *,
    draft_gate: object,
    multi_pack: object,
    operator_ack: object,
    ...
    try:
        mp = compose_floating_multiselect_model_decision_twin_search_free_settings(
    ...
    return DraftBeforeMergeFloatingMultiselectModelDecisionCompose(
        session_id=session,
...
def format_draft_before_merge_floating_multiselect_model_decision_summary(
