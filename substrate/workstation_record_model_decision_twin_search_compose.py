...
from substrate.model_decision_twin_search_html_native_marketplace_free_mo_compose import (
    ModelDecisionTwinSearchHtmlNativeMarketplaceFreeMoCompose,
    ModelDecisionTwinSearchHtmlNativeMarketplaceFreeMoComposeError,
    compose_model_decision_twin_search_html_native_marketplace_free_mo,
)
...
class WorkstationRecordModelDecisionTwinSearchComposeError(ValueError):
    """Fail-closed validation for workstation records + model decision twin search."""


@dataclass(frozen=True)
class WorkstationRecordModelDecisionTwinSearchCompose:
    week_id: str
    session_id: str
    parent_asset_id: str
    asset_id: str
...
def _require_nonempty(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise WorkstationRecordModelDecisionTwinSearchComposeError(
            f"{field} must be a non-empty string"
        )
    return value.strip()


def compose_workstation_record_model_decision_twin_search(
    *,
    session_id: object,
    items: object,
    ...
    try:
        ...
        raise WorkstationRecordModelDecisionTwinSearchComposeError(str(e)) from e
    notes.extend(f"[records] {n}" for n in rec.notes)

    try:
        dp = compose_model_decision_twin_search_html_native_marketplace_free_mo(
    ...
    except ModelDecisionTwinSearchHtmlNativeMarketplaceFreeMoComposeError as e:
        raise WorkstationRecordModelDecisionTwinSearchComposeError(str(e)) from e
    ...
    return WorkstationRecordModelDecisionTwinSearchCompose(
        week_id=week,
        session_id=session,
...
def format_workstation_record_model_decision_twin_search_summary(
...
__all__ = [
    "WorkstationRecordModelDecisionTwinSearchCompose",
    "WorkstationRecordModelDecisionTwinSearchComposeError",
    "compose_workstation_record_model_decision_twin_search",
    "format_workstation_record_model_decision_twin_search_summary",
]
