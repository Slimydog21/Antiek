...
audit, not in a deterministic source-onboarding stop-rule.
"""
...
_OPEN_TIER_VALUES = _T1_VALUES | {"T2", "T2_NON_COMMERCIAL", "t2", "non_commercial"}
...
def _row_source(document_id: str, source_uri: str | None, meta: Mapping[str, Any]) -> str | None:
...
def compute_source_census(con: Any, source: str) -> SourceCensus:
    ...
    from the connector metadata, arXiv ids, ``doc-arxiv-*`` ids, or arxiv.org
    origin links. Other sources must declare ``metadata.source == source``. This
    avoids silently sweeping unrelated documents into a source's gate row.
    """
    ...
    measured: list[tuple[str, str | None, str | None, str | None, str | None, dict[str, Any], str | None]] = []
    for row in rows:
