...
def render_verdict_markdown(verdict: Verdict) -> str:
    ...
    lines.append("")
    lines.append(f"_Generated by `tools.dispatch_tier_verdict` at {datetime.now(UTC).isoformat()}_")
    lines.append("")
    return "\n".join(lines)
...
def _validate_verdict(verdict: Verdict) -> None:
    if verdict.decision not in ALLOWED_DECISIONS:
        raise ValueError(
            "dispatch verdict decision must be one of: "
            + ", ".join(sorted(ALLOWED_DECISIONS))
        )
    if not isinstance(verdict.rationale, str) or not verdict.rationale.strip():
        raise ValueError("dispatch verdict rationale must be a non-empty string")
    if not isinstance(verdict.measurement_window_started, str) or not verdict.measurement_window_started:
        raise ValueError("dispatch verdict measurement_window_started must be non-empty")
    if not isinstance(verdict.measurement_window_ended, str) or not verdict.measurement_window_ended:
        raise ValueError("dispatch verdict measurement_window_ended must be non-empty")
    if verdict.gap_pp is not None and (
        isinstance(verdict.gap_pp, bool)
        or not isinstance(verdict.gap_pp, int | float)
        or not math.isfinite(float(verdict.gap_pp))
    ):
        raise ValueError("dispatch verdict gap_pp must be finite when present")
    for score in verdict.scores:
        _validate_provider_score(score)


def _validate_provider_score(score: ProviderScore) -> None:
    if not isinstance(score.provider, str) or not score.provider.strip():
    ...
    for field in ("synthesis_count", "verified_count", "passed_count"):
        ...
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise ValueError(f"provider score {field} must be a non-negative integer")
    if score.verified_count > score.synthesis_count:
        raise ValueError("provider score verified_count cannot exceed synthesis_count")
    if score.passed_count > score.verified_count:
        raise ValueError("provider score passed_count cannot exceed verified_count")
    if (
        isinstance(score.total_cost_usd, bool)
        or not isinstance(score.total_cost_usd, int | float)
        or not math.isfinite(float(score.total_cost_usd))
        or score.total_cost_usd < 0.0
    ):
        raise ValueError("provider score total_cost_usd must be non-negative and finite")
