...
def detect_flappers(
    ...
    *,
    ...
    for nodeid in nodeids:
        ...
        # Alone-run is expensive; run only when suite outcomes suggest composability risk.
        alone: bool | None = None
        if suite_any_fail or nondeterministic:
            alone = _run_pytest(
                [nodeid], repo=repo, shuffle_seed=None, timeout_s=timeout_s
            ).get(nodeid)
