...
1. **Scene layers** — `apps/reading/src/scene/scene.css`, `Scene.tsx` (the living background; SPR-05/06 terrain).
2. **Shell controls** — `apps/reading/src/index.css` + AppShell + `src/shell/*` (chrome, launcher windows, panels; SPR-08 terrain).
3. **Werner** — `apps/reading/src/werner/*.css`, `src/brand/werner/*` (mascot motion/state — graded for craft only, NEVER for hedgehog-resemblance; see firewall).
4. **Reading-mode states** — reading-surface components and their authored states (`apps/reading/src/components/*`, `src/modes/*`).
...
- **Visual crispness:** crispness must HOLD when the scene is off (`tokens.css --glass-bg-solid` opaque fallbacks).
