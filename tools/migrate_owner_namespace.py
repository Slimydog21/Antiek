...
def _plan_tool_connections(report: MigrationReport) -> None:
    """Connected BYO tools (X, YouTube, Polygon, FMP, EDGAR).

    ``registry._record_key`` hashes the owner into the dict key, so re-owning a
    connection means moving the row to a new key AND rewriting
    ``owner_user_id`` — leaving either alone trips the registry's own integrity
    check (``key != _record_key(record.owner_user_id, record.vendor)``).
    """
    from runtime.connectors import registry as connectors
    ...
    for key, record in records.items():
        if record.owner_user_id != LEGACY_OWNER:
            continue
        new_key = connectors._record_key(report.target_owner, record.vendor)
...
def _apply_tool_connections(report: MigrationReport) -> None:
    ...
    with connectors._guard(exclusive=True):
        ...
        migrated: dict[str, connectors.ToolConnectionRecord] = {}
        for key, record in records.items():
            if record.owner_user_id != LEGACY_OWNER:
                migrated[key] = record
                continue
            ...
            # The credential must move too. ``_metadata_matches`` requires
            # metadata.owner_user_id == record.owner_user_id, and
            # ``_byok_reown_candidates`` deliberately skips ``connector_*``
            # pipeline kinds, so re-owning the row alone would leave the
            # connection reading as tampered. Re-seal it here, exactly as the
            # user-model records do: the SecretBox key is bound to
            # (owner, handle), so decrypt-then-restore is the only route.
            #
            # account_handle is kept AS IS. It embeds the old record key, which
            # is cosmetically stale, but the registry only length-validates it
            # and ``_metadata_matches`` compares it between record and
            # metadata. Keeping both on the old value keeps them equal; moving
            # one would not.
            plaintext: str | None = None
            new_cred_id = record.cred_id
            new_fingerprint = record.credential_fingerprint
            resealed = False
            try:
                ...
                new_meta = {m.cred_id: m for m in list_credentials()}[new_ref]
                delete_credential(record.cred_id)
                new_cred_id = new_ref
                new_fingerprint = new_meta.artifact_fingerprint
                resealed = True
            except (KeyError, CredentialIntegrityError):
                logger.warning(
                    "tool_connections.json: %s has no stored credential; "
                    "re-owning the row without re-sealing", key,
                )
            finally:
                plaintext = None

            migrated[new_key] = replace(
                record,
                owner_user_id=report.target_owner,
                cred_id=new_cred_id,
            ...
            logger.info(
                "tool_connections.json: %s -> %s (owner %s -> %s%s)",
                key, new_key, LEGACY_OWNER, report.target_owner,
                ", cred re-sealed" if resealed else ", cred missing",
            )
        connectors._write_unlocked(migrated, pending)


_PLANNERS = (_plan_user_models, _plan_byok, _plan_byot_usage, _plan_lineup, _plan_duckdb,
             _plan_telemetry, _plan_tool_connections)
_APPLIERS = (_apply_user_models, _apply_byok, _apply_byot_usage, _apply_lineup, _apply_duckdb,
