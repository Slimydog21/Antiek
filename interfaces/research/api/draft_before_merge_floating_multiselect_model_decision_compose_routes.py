...
from interfaces.research.api.floating_multiselect_model_decision_twin_search_free_settings_compose_routes import (
    DecisionPackBody,
    MultiSelectBody,
)
...
draft_before_merge_floating_multiselect_model_decision_compose_router = APIRouter(
    prefix="/research/draft-before-merge-floating-multiselect-model-decision",
    tags=["draft-before-merge-floating-multiselect-model-decision-compose"],
)
...
@draft_before_merge_floating_multiselect_model_decision_compose_router.post(
    "/compose"
)
def post_compose(req: ComposeRequest) -> dict[str, Any]:
    try:
        result = compose_draft_before_merge_floating_multiselect_model_decision(
...
def register_draft_before_merge_floating_multiselect_model_decision_compose_routes(
    app: FastAPI,
) -> None:
    app.include_router(
        draft_before_merge_floating_multiselect_model_decision_compose_router
    )
