"""Registerable HTTP surface for settings add-model + bench decision compose."""
...
settings_add_model_bench_decision_compose_router = APIRouter(
    prefix="/research/settings-add-model-bench-decision",
    tags=["settings-add-model-bench-decision-compose"],
)
...
class TaskFamilySeedBody(BaseModel):
    model_config = {"extra": "forbid"}
...
@settings_add_model_bench_decision_compose_router.post("/compose")
def post_compose(req: ComposeRequest) -> dict[str, Any]:
    try:
        result = compose_settings_add_model_bench_decision(
...
def register_settings_add_model_bench_decision_compose_routes(
    app: FastAPI,
) -> None:
    app.include_router(settings_add_model_bench_decision_compose_router)


__all__ = [
    "settings_add_model_bench_decision_compose_router",
