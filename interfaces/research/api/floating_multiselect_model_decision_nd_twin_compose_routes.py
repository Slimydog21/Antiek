"""Registerable HTTP surface for floating multi-select + model decision ND twin."""
...
floating_multiselect_model_decision_nd_twin_compose_router = (
    APIRouter(
        prefix=(
            "/research/floating-multiselect-model-decision-nd-twin"
        ),
        tags=[
            "floating-multiselect-model-decision-nd-twin-compose"
        ],
    )
)
...
@floating_multiselect_model_decision_nd_twin_compose_router.post(
    "/compose"
)
def post_compose(req: ComposeRequest) -> dict[str, Any]:
    try:
        result = compose_floating_multiselect_model_decision_nd_twin(
...
def register_floating_multiselect_model_decision_nd_twin_compose_routes(
    app: FastAPI,
) -> None:
    app.include_router(
        floating_multiselect_model_decision_nd_twin_compose_router
