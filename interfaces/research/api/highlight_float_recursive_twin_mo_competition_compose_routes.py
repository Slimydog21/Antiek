"""Registerable HTTP surface for highlight float → twin MO competition."""
...
highlight_float_recursive_twin_mo_competition_compose_router = APIRouter(
    prefix="/research/highlight-float-recursive-twin-mo-competition",
    tags=["highlight-float-recursive-twin-mo-competition-compose"],
)
...
class HighlightSurfaceBody(BaseModel):
    ...
    include_twin_feed: bool | None = Field(default=None, strict=True)
...
class ComposeRequest(BaseModel):
    ...
    seed_excerpt_from_highlight: bool = Field(default=True, strict=True)
    require_both: bool = Field(default=True, strict=True)


@highlight_float_recursive_twin_mo_competition_compose_router.post("/compose")
def post_compose(req: ComposeRequest) -> dict[str, Any]:
    try:
        result = compose_highlight_float_recursive_twin_mo_competition(
            highlight_surface=req.highlight_surface.model_dump(),
            mo_competition=req.mo_competition.model_dump(),
            operator_ack=req.operator_ack,
            seed_excerpt_from_highlight=req.seed_excerpt_from_highlight,
...
def register_highlight_float_recursive_twin_mo_competition_compose_routes(
    app: FastAPI,
) -> None:
    app.include_router(
        highlight_float_recursive_twin_mo_competition_compose_router
    )
