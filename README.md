...
sources at speed; almost every tool that does so throws away the one
thing that makes research trustworthy (per-source provenance). Antiek
is the workstation that refuses that trade; Cursor for knowledge work,
built on a substrate where every claim can show its receipts.
...
They produce bulletized slop that you skim rather than absorb; they
cite nothing you can verify; they forget everything between sessions.
We think that's three separate failures, and Antiek is an existence
proof that all three are choices, not laws of nature.
...
scaffolding; shipped modules): the reading geometry pass and layout
map, the notebook block-canvas with auto-notebook, the float menu, the
...
2. **The provenance chain.** Claim, then chunk, then document, then IP
   holder. Anything that breaks this chain breaks both the trust story
   and the future economics layer in one move.
...
and Stripe activation, the multi-user pivot) and **explicit deferrals**
(each documented with what unlocks it):
...
is the contract; the companion docs are the status report. Updating the
spec on every shipped PR would couple implementation to spec-editing
(friction we chose not to pay).
