...
> **Landing status (2026-06-12 — transport note, not a format change):**
> this section is ratified spec (binding decision #2,
> `docs/decisions/wrestle-evolution-spec-2026-05-23.md`) but its
> implementation (`sidecar_writer.py` / `sidecar_reader.py` /
> `tests/test_sidecar_e2e.py`) is **not yet on main**. The sidecar
> overlay is functionally wired to the voice-anchor + behavior-taxonomy
> substrate, which has not landed; it is **sequenced behind that
> substrate landing, not dropped**. See
> `docs/decisions/antiek-html-shell-amendment-DRAFT.md`.
