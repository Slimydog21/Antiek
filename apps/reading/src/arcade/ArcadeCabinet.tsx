...
export function ArcadeCabinet() {
  ...
  // Shared factory with LoadingGameHost — the host entry path for cartridges.
  ...
  if (!wernerArcade) {
    return (
      <div data-testid="arcade-cabinet-disabled" className="p-6 text-sm">
        Arcade is off (VITE_WERNER_ARCADE=0).
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Session brand marks rendered in-product (celebrate + thinking) —
          not inventory-only; the cabinet header shows the refined poses. */}
      <div
      ...
      >
        ...
        <span className="text-xs text-ink-mute dark:text-moonlight">
          Session-refined Werner poses drive the brand chrome.
        </span>
      </div>
