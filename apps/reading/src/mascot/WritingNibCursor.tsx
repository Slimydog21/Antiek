 ...
 * no authority over Brain, navigation, networking, or spend.
 */
export function WritingNibCursor({
  ...
  useEffect(() => {
    ...
    if (!disabled) root.classList.add("mascot-writing-nib-active");
    else root.classList.remove("mascot-writing-nib-active");
    return () => root.classList.remove("mascot-writing-nib-active");
  }, [disabled]);
