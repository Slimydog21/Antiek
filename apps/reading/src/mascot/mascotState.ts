/**
 * Brain steering state machine (SPR-05) — the pure core.
 *
 * This is the engine's only authority on "what is Brain doing right now".
 * It is deliberately framework-free: no React, no DOM, no timers. The
 * imperative seam (MascotStage.ts) owns the side effects (moving the
 ...
 *
 * One mascot, one state. MascotStage never tracks a parallel notion of
 ...
 * mascot used to run. Both were removed (Brain is now fixed; the cursor is the
...
/** The ambient state Brain falls back to after a directed action ends.
...
export const INITIAL_MASCOT_STATE: MascotState = { name: "idle", resume: "idle" };
...
function ambientOf(s: MascotState): AmbientState {
...
export function mascotReducer(
  ...
  if (state.name === "frozen") {
    return event.type === "unfreeze" ? INITIAL_MASCOT_STATE : state;
  }
...
/** True while Brain is doing something directed (not just ambient roam). */
export function isBusy(state: MascotState): boolean {
 ...
 *
 * Operator ask #3: when left alone, Brain runs a comic gag of trying to fish
 * and never quite catching — forever. This is his POINTER-IDLE behaviour: the
 * cursor is the bait, so when the pointer MOVES Brain reels toward it (SPR-03);
 ...
 * whole point of this sprint is what Brain does when nobody is moving the
 * mouse"). DO NOT "fix" the loop to run during follow — that re-opens that
 * decision. The loop is gated OFF whenever the reel owns Brain.
 *
 * ── STANDALONE ADAPTATION (read before editing) ──────────────────────────
 * The spec's SPR-05 assumed SPR-01 had turned mascotState.ts into a typed POSE
 * machine (cast/wait/nibble/yank/miss/slump). SPR-01 IS DEFERRED. The real
 * machine above is a STEERING machine (it decides WHAT Brain does, not pose
 ...
 *      loop OWNS Brain — only while the ambient state is `idle` (never
...
/**
 * THE GATE. Does the fishing loop own Brain right now? This is the XOR that
 * keeps the loop and the SPR-03 reel from ever fighting: the loop runs ONLY
 * while Brain is at ambient rest (`idle`) AND the pointer is idle. The instant
 * the pointer moves, `pointerIdle` flips false here and the reel takes over; the
 * instant Brain is following / waddling / emoting / frozen, the loop stands
 ...
 */
export function shouldFish(state: MascotState, pointerIdle: boolean): boolean {
