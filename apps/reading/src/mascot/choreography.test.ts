 ...
 *    proven in MascotStage.test.ts) — the listener never drops or batches;
...
function fakeStage(): {
  ...
  const stage = {
    ...
    dispose: vi.fn(),
  } as unknown as MascotStageController;
...
describe("installChoreography — interruption + teardown", () => {
  it("rapid activations each reach the stage (latest-wins handled downstream)", () => {
    ...
    // latest-wins (proven in MascotStage.test.ts) is what cancels the
