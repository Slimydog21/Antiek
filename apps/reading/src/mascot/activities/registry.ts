 ...
 *      given a home: activities enter Brain's world ONLY as a StationActivity,
 ...
 *      MascotStation. This sprint builds the spine those three hang from.
 * See docs/htmlspec/werner-fixed-station/DESIGN.md and the Brain Station spec.
 *
 ...
 * replacement would let an import-order accident change Brain's behavior.
...
export function registerActivity(
  ...
  if (existing && existing !== activity) {
    throw new Error(
      `Brain station: activity "${activity.id}" is already registered`,
    );
  }
...
/**
 * The default activity — the one Brain runs when nothing else is switched in.
...
export function getDefaultActivity(): StationActivity {
  if (defaultId === null) {
    throw new Error("Brain station: no default activity registered");
  }
  const activity = catalog.get(defaultId);
  if (!activity) {
    throw new Error(
      `Brain station: default activity "${defaultId}" is not in the catalog`,
    );
  }
  return activity;
}
