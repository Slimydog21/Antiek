...
export function StationInstrumentAtlas() {
  ...
  useEffect(() => {
    ...
    if (active) root.classList.add("mascot-ice-cursor-hidden");
    else root.classList.remove("mascot-ice-cursor-hidden");
    return () => root.classList.remove("mascot-ice-cursor-hidden");
  }, [active]);
    ...
    <main className="min-h-screen bg-card-soft p-8 text-ink dark:text-bright">
      <header className="mx-auto max-w-6xl border-b-2 border-rule pb-6">
        <p className="font-mono text-xs uppercase tracking-widest text-shadow-1">
          Brain station · instrument atlas
        </p>
        <h1 className="mt-3 max-w-4xl font-serif text-3xl leading-tight">
          Brain stays home. The cursor changes jobs.
        </h1>
...
const meta = {
  title: "Mascot / Station instruments / Complete atlas",
