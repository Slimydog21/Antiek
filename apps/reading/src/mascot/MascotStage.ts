...
/**
 * MascotStage (SPR-05) — the imperative command controller.
 *
 * This is the SPR-10 seam: a small object exposing moveTo / waddleToEl /
 * emote / follow / idle / freeze. It owns the STATE MACHINE (mascotState.ts)
 ...
 * passes in (`StageHost`). One mascot, one position.
 *
 ...
 * thing the task forbids. So the stage asks the host to "walk Brain to (x,y)
...
export interface StageHost {
   ...
   * with the walking gait visibly on (the host swaps in mascot-waddle). The
  ...
  /** Show/clear an emote mark overlaid on the Brain mark. null clears. */
  setEmote: (kind: EmoteKind | null) => void;
  /** Toggle ambient cursor-follow on the host. DORMANT under the fixed station:
   *  the mascot implements this as a no-op (Brain never chases the cursor). */
...
export interface MascotStageController {
...
export function createMascotStage(
  host: StageHost,
  timers: Timers = defaultTimers,
): MascotStageController {
  let state: MascotState = INITIAL_MASCOT_STATE;
  ...
  const dispatch = (event: Parameters<typeof mascotReducer>[1]) => {
    state = mascotReducer(state, event);
  };
  ...
  const emote = (kind: EmoteKind) => {
    ...
    if (state.name === "frozen") {
      ...
      // after the beat so Brain returns to his frozen idle frame.
      host.setEmote(kind);
      actionTimer = timers.setTimeout(() => {
        actionTimer = null;
        host.setEmote(null);
