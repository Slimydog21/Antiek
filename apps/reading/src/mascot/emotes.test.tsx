 ...
 *  - reduced motion renders a STILL variant (a Brain mark with no animating
...
describe("EmoteView", () => {
  ...
  it("reduced motion renders a STILL canonical mark (an <img>, no animating role=status)", () => {
    for (const kind of EMOTE_KINDS) {
      ...
      // The still path is the canonical <Brain> mark → a role=img span with
      // an inner <img>, NOT the animated BrainThinking/CaughtAFish wrappers
  ...
  it("keeps sleeping single-layered when motion is reduced", () => {
    ...
    expect(container.querySelector(".mascot-sleep-still")).toBeTruthy();
    expect(container.querySelector(".mascot-pose--empty")).toBeNull();
  });
