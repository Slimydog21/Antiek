/**
 * mascotState.test.ts — the steering state machine (SPR-05).
 *
...
const idle: MascotState = { name: "idle", resume: "idle" };

describe("mascotReducer", () => {
  it("starts idle, resuming idle", () => {
    expect(INITIAL_MASCOT_STATE).toEqual(idle);
  });

  it("follow on → following; off → idle (ambient floor moves)", () => {
    const following = mascotReducer(idle, { type: "follow", on: true });
    expect(following).toEqual({ name: "following", resume: "following" });
    const back = mascotReducer(following, { type: "follow", on: false });
  ...
  it("waddle from following resumes following on arrival (mouse-follow not silently cancelled)", () => {
    const following: MascotState = { name: "following", resume: "following" };
    const waddling = mascotReducer(following, { type: "waddle" });
    expect(waddling).toEqual({ name: "waddling", resume: "following" });
    const arrived = mascotReducer(waddling, { type: "arrived" });
    expect(arrived).toEqual({ name: "following", resume: "following" });
  });

  it("emote from idle resumes idle when the beat ends", () => {
    const emoting = mascotReducer(idle, { type: "emote" });
    expect(emoting).toEqual({ name: "emoting", resume: "idle" });
    expect(mascotReducer(emoting, { type: "emoteDone" })).toEqual(idle);
  });

  it("emote mid-follow resumes following (carries the ambient)", () => {
    const following: MascotState = { name: "following", resume: "following" };
    const emoting = mascotReducer(following, { type: "emote" });
    expect(emoting.resume).toBe("following");
    expect(mascotReducer(emoting, { type: "emoteDone" })).toEqual({
      name: "following",
      resume: "following",
    });
  });
  ...
  it("latest-wins: a new waddle supersedes an in-flight emote, keeping the ambient", () => {
    const following: MascotState = { name: "following", resume: "following" };
    const emoting = mascotReducer(following, { type: "emote" });
    const waddling = mascotReducer(emoting, { type: "waddle" });
    expect(waddling).toEqual({ name: "waddling", resume: "following" });
  });

  it("follow toggled DURING a directed action only updates the resume, not the state", () => {
    const waddling: MascotState = { name: "waddling", resume: "idle" };
    const next = mascotReducer(waddling, { type: "follow", on: true });
  ...
  it("stale arrived/emoteDone after supersession are ignored (no crash, no flip)", () => {
    ...
    expect(mascotReducer(waddling, { type: "emoteDone" })).toBe(waddling);
    const emoting: MascotState = { name: "emoting", resume: "idle" };
    // arrived doesn't belong to an emote → ignored.
    expect(mascotReducer(emoting, { type: "arrived" })).toBe(emoting);
  });

  it("freeze is a hard floor: every command is a no-op except unfreeze", () => {
    const frozen = mascotReducer(idle, { type: "freeze" });
    ...
    for (const event of [
      ...
      expect(mascotReducer(frozen, event)).toBe(frozen);
    }
    expect(mascotReducer(frozen, { type: "unfreeze" })).toEqual(INITIAL_MASCOT_STATE);
  });

  it("idle is a hard return to rest from any non-frozen state", () => {
    const emoting: MascotState = { name: "emoting", resume: "following" };
    expect(mascotReducer(emoting, { type: "idle" })).toEqual(idle);
  });
 ...
 *  - the GATE (shouldFish): the loop OWNS Brain only at ambient rest + pointer
...
describe("fishing loop — the gate (shouldFish)", () => {
  ...
  const waddling: MascotState = { name: "waddling", resume: "idle" };
  const emoting: MascotState = { name: "emoting", resume: "idle" };
  const frozen: MascotState = { name: "frozen", resume: "idle" };

  it("idle + pointer idle → the loop owns Brain", () => {
  ...
  it("clean hand-off BOTH directions: idle⇄follow flips ownership with the same predicate", () => {
    ...
    const wentIdle = mascotReducer(following, { type: "follow", on: false });
