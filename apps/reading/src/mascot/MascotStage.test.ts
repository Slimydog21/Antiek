/**
 * MascotStage.test.ts — the imperative command controller (SPR-05 / SPR-10).
 *
...
let stage: MascotStageController;
...
beforeEach(() => {
  ...
  timers = makeTimers();
  host = makeHost();
  stage = createMascotStage(host.host, timers.api);
});

describe("createMascotStage", () => {
  ...
  it("emote durations come from the emote table", () => {
    ...
    const s = createMascotStage(host.host, t);
    s.emote("sleeping");
