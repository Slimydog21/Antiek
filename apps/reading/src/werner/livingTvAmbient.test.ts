...
describe("installLivingTvAmbient", () => {
  ...
  it("recovers fail installer densify with soft idle then silence", () => {
    // Living-TV densify: generic product fail recovers with curtain idle, then silence.
    vi.useFakeTimers();
    const emit = vi.fn();
    const listeners = new Map<string, Set<EventListener>>();
    const target = {
    ...
    // Idle densify: no re-arm after fail recover.
    now = 3_000;
    vi.advanceTimersByTime(1_000);
    expect(emit).toHaveBeenCalledTimes(1);
  ...
  it("curtain-calls product note_saved installer densify (pride beat → idle → silence)", () => {
    // Living-TV densify: product pride beat (note_saved) curtain-calls once to
    // idle sleep, then silence — end credits for invent product pride.
    vi.useFakeTimers();
    const emit = vi.fn();
    const listeners = new Map<string, Set<EventListener>>();
    const target = {
    ...
    // Idle densify: no re-arm after product pride curtain.
    now = 3_000;
    vi.advanceTimersByTime(1_000);
    expect(emit).toHaveBeenCalledTimes(1);
