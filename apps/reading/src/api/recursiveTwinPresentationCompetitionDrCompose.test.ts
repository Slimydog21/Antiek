...
import {
  composeRecursiveTwinPresentationCompetitionDr,
  formatRecursiveTwinPresentationCompetitionDrSummary,
...
describe("composeRecursiveTwinPresentationCompetitionDr", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("twin presentation + competition DR pack ready", () => {
    const c = composeRecursiveTwinPresentationCompetitionDr({
      twin: twin_input,
    ...
    expect(c.presentation.view_mode).toBe("side_panel");
    expect(c.presentation.presented_insight_count).toBe(1);
    expect(c.presentation.presented_question_count).toBe(1);
    ...
    expect(
      formatRecursiveTwinPresentationCompetitionDrSummary(c),
    ).toMatch(/twin_written=false/);
    expect(c.presentation.presentation_sections.length).toBeGreaterThan(3);
  });
  ...
  it("operator_ack false blocks", () => {
    ...
    expect(c.presentation.presentation_ready).toBe(false);
    expect(c.twin_written).toBe(false);
    expect(c.merge_executed).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });

  it("open_requested false blocks presentation and pack", () => {
    const c = composeRecursiveTwinPresentationCompetitionDr({
      twin: twin_input,
      presentation: { ...presentation_input, open_requested: false },
      competition_pack: competition_pack_input,
      operator_ack: true,
    });
    expect(c.presentation.presentation_ready).toBe(false);
  ...
  it("parent_asset_id mismatch blocks", () => {
    const c = composeRecursiveTwinPresentationCompetitionDr({
      twin: { ...twin_input, parent_asset_id: "book-other" },
      presentation: presentation_input,
      competition_pack: competition_pack_input,
      operator_ack: true,
    });
  ...
  it("overlay view_mode + merge preview still pure", () => {
    const c = composeRecursiveTwinPresentationCompetitionDr({
      twin: twin_input,
      presentation: {
        ...presentation_input,
    ...
    expect(c.presentation.view_mode).toBe("overlay");
    expect(c.presentation.merge_to_parent_preview).toBe(true);
    expect(c.merge_executed).toBe(false);
    expect(c.twin_written).toBe(false);
    expect(c.pack_ready).toBe(true);
    expect(c.notes.some((n) => n.includes("merge_to_parent_preview=true"))).toBe(
      true,
    );
  });
});
