...
import {
  composeWriteModeTwinFullscreenDraftCollectiveMoWeeklyPack,
  formatWriteModeTwinFullscreenDraftCollectiveMoWeeklyPackSummary,
} from "./writeModeTwinFullscreenDraftCollectiveMoWeeklyPackCompose";

describe("composeWriteModeTwinFullscreenDraftCollectiveMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_wt_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_wt_outer_decision_pack = {
    decision: residual_wt_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_wt_outer_twin_records,
      html_pack: residual_wt_outer_html_pack,
    },
  };

  const residual_wt_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_wt_outer_record_pack = {
    session_id: "sess-1",
    items: residual_wt_outer_items,
    decision_pack: residual_wt_outer_decision_pack,
  };

  const residual_wt_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_wt_outer_floating_pack = {
    highlight_launch: residual_wt_outer_highlight_launch,
    record_pack: residual_wt_outer_record_pack,
  };

  const residual_wt_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_wt_outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_wt_outer_collective_pack = {
    multiselect: residual_wt_outer_multiselect,
    floating_pack: residual_wt_outer_floating_pack,
  };

  const residual_wt_outer_fullscreen = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    highlight: "Scaling laws claim from page 12",
    prompt: "What evidence supports this?",
  ...
  const residual_wt_outer_draft_pack = {
    draft_gate: residual_wt_outer_draft_gate,
    collective_pack: residual_wt_outer_collective_pack,
  };

  const residual_wt_outer_fullscreen_pack = {
    fullscreen: residual_wt_outer_fullscreen,
    draft_pack: residual_wt_outer_draft_pack,
  };
  ...
  it("write-mode twin + fullscreen draft collective MO weekly pack ready", () => {
    const c = composeWriteModeTwinFullscreenDraftCollectiveMoWeeklyPack({
      write: residual_wt_outer_write,
      fullscreen_pack: residual_wt_outer_fullscreen_pack,
      operator_ack: true,
    });
    expect(c.write.pack_ready).toBe(true);
    expect(c.fullscreen_pack.pack_ready).toBe(true);
    ...
    expect(c.authority).toBe(
      "write_mode_twin_fullscreen_draft_collective_mo_weekly_pack_compose_advisory",
    );
    expect(
      formatWriteModeTwinFullscreenDraftCollectiveMoWeeklyPackSummary(c),
    ).toMatch(/draft_written=false/);
  });
  ...
  it("session mismatch on write blocks pack under require_both", () => {
    const c = composeWriteModeTwinFullscreenDraftCollectiveMoWeeklyPack({
      write: {
        ...residual_wt_outer_write,
  ...
  it("parent mismatch on write blocks pack under require_both", () => {
    const c = composeWriteModeTwinFullscreenDraftCollectiveMoWeeklyPack({
      write: {
        ...residual_wt_outer_write,
        ...
        twin_slices: [
          {
            ...residual_wt_outer_write.twin_slices[0],
            parent_asset_id: "book-other",
          },
          {
            ...residual_wt_outer_write.twin_slices[1],
            parent_asset_id: "book-other-slice-2",
          },
        ],
        chase_slots: residual_wt_outer_write.chase_slots.map((s) => ({
          ...s,
          parent_asset_id: "book-other",
        })),
      },
