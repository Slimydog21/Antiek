 ...
 * budget + twin search HTML-native marketplace free settings pack (pure).
 *
 ...
 * rides twin intelligent search + HTML-native free marketplace — without live
 * dispatch, pack execution, or budget overage.
 *
...
import {
  composeModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettings,
  type ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsCompose,
  type ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsInput,
} from "./modelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsCompose";

export interface FloatingMultiselectModelDecisionTwinSearchFreeSettingsInput {
  multiselect: Omit<
    FloatingMultiSelectCollectiveCohesiveInput,
    "operator_ack"
  >;
  decision_pack: Omit<
    ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsInput,
    "operator_ack"
  >;
...
export interface FloatingMultiselectModelDecisionTwinSearchFreeSettingsCompose {
  session_id: string;
  ...
  notes: string[];
  authority: "floating_multiselect_model_decision_twin_search_free_settings_compose_advisory";
}
 ...
 * search HTML-native free settings. Never live-dispatches or over-budgets.
 */
export function composeFloatingMultiselectModelDecisionTwinSearchFreeSettings(
  input: FloatingMultiselectModelDecisionTwinSearchFreeSettingsInput,
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — floating multi-select + model decision twin search free settings ready; still pure",
    );
  } else {
    notes.push(
...
export function formatFloatingMultiselectModelDecisionTwinSearchFreeSettingsSummary(
