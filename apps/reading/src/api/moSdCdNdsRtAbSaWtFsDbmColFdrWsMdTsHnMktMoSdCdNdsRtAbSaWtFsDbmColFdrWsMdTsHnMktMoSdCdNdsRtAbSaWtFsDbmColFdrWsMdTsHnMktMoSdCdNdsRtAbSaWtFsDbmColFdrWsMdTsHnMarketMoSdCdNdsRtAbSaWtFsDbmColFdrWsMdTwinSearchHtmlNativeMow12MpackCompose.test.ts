...
describe("composeMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack", () => {
const WEEKLY_ND = {
  weekly_learn: {
    week_id: "2026-W28",
...
const residual_ts10_tip_twin_records = [
  ...
  it("Midnight Oil + settings residual pack ready", () => {
    const c = composeMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
      mo: residual_mo_tip,
      settings_pack: residual_mo_settings_pack,
      operator_ack: true,
    });
    expect(c.pack_ready).toBe(true);
    expect(c.mo.pack_ready).toBe(true);
    ...
    expect(formatMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpackSummary(c)).toMatch(/REJECT|pack_ready=true/);
  });
  ...
  it("price_ceiling_ack false blocks pack under require_both", () => {
    const c = composeMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
      mo: { ...residual_mo_tip, price_ceiling_ack: false },
      settings_pack: residual_mo_settings_pack,
      operator_ack: true,
    });
    expect(c.mo.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.charge_executed).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  ...
  it("production router remains REJECT", () => {
    ...
    expect(c.production_router_verdict).toBe("REJECT");
    expect(c.settings_pack.production_router_verdict).toBe("REJECT");
    expect(c.live_execution_authorized).toBe(false);
    expect(c.charge_executed).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.live_dispatch_authorized).toBe(false);
