...
import {
  composeCompetitionDrNdShadowWeeklySrcWriteMoWeeklyPack,
  formatCompetitionDrNdShadowWeeklySrcWriteMoWeeklyPackSummary,
} from "./competitionDrNdShadowWeeklySrcWriteMoWeeklyPackCompose";

describe("composeCompetitionDrNdShadowWeeklySrcWriteMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_cd_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_cd_outer_decision_pack = {
    decision: residual_cd_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_cd_outer_twin_records,
      html_pack: residual_cd_outer_html_pack,
    },
  };

  const residual_cd_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_cd_outer_record_pack = {
    session_id: "sess-1",
    items: residual_cd_outer_items,
    decision_pack: residual_cd_outer_decision_pack,
  };

  const residual_cd_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_cd_outer_floating_pack = {
    highlight_launch: residual_cd_outer_highlight_launch,
    record_pack: residual_cd_outer_record_pack,
  };

  const residual_cd_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_cd_outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_cd_outer_collective_pack = {
    multiselect: residual_cd_outer_multiselect,
    floating_pack: residual_cd_outer_floating_pack,
  };

  const residual_cd_outer_fullscreen = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    highlight: "Scaling laws claim from page 12",
    prompt: "What evidence supports this?",
  ...
  const residual_cd_outer_draft_pack = {
    draft_gate: residual_cd_outer_draft_gate,
    collective_pack: residual_cd_outer_collective_pack,
  };

  const residual_cd_outer_fullscreen_pack = {
    fullscreen: residual_cd_outer_fullscreen,
    draft_pack: residual_cd_outer_draft_pack,
  };

  const residual_cd_outer_write = {
    session_id: "sess-1",
    draft_id: "draft-1",
    parent_asset_id: "book-1",
    twin_slices: [
  ...
  const residual_cd_outer_sources = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    requested_families: ["arxiv" as const, "substack" as const],
    sources: [
  ...
  const residual_cd_outer_write_pack = {
    write: residual_cd_outer_write,
    fullscreen_pack: residual_cd_outer_fullscreen_pack,
  };

  const residual_cd_outer_weekly_learn = {
    week_id: "2026-W28",
    min_events_per_task: 2,
    events: [
      {
  ...
  const residual_cd_outer_source_pack = {
    sources: residual_cd_outer_sources,
    write_pack: residual_cd_outer_write_pack,
  };

  const residual_cd_outer_twin = {
    parent_asset_id: "book-1",
    source_excerpt:
      "<p>Scaling laws hold under noise in compute-optimal regimes.</p>",
    focus_questions: ["Where does it break?", "What residual gaps?"],
  ...
  const residual_cd_outer_presentation = {
    view_mode: "side_panel" as const,
    open_requested: true,
    merge_to_parent_preview: false,
    presented_insights: [
  ...
  const residual_cd_outer_weekly_pack = {
    weekly_learn: residual_cd_outer_weekly_learn,
    source_pack: residual_cd_outer_source_pack,
  };

  const residual_cd_outer_nd_shadow = {
    selected_model_id: "gpt-5.5",
    nd_recommended_model_id: "claude-opus",
    kill_switch_on: true,
    confidence: 0.72,
  ...
  const residual_cd_outer_twin_presentation = {
    twin: residual_cd_outer_twin,
    presentation: residual_cd_outer_presentation,
    weekly_pack: residual_cd_outer_weekly_pack,
  };
  ...
  const residual_cd_outer_nd_pack = {
    nd_shadow: residual_cd_outer_nd_shadow,
    twin_presentation: residual_cd_outer_twin_presentation,
  };

  it("competition DR + ND shadow weekly src write MO weekly pack ready", () => {
    const c = composeCompetitionDrNdShadowWeeklySrcWriteMoWeeklyPack({
    ...
    expect(c.authority).toBe(
      "competition_dr_nd_shadow_weekly_src_write_mo_weekly_pack_compose_advisory",
    );
    expect(
      formatCompetitionDrNdShadowWeeklySrcWriteMoWeeklyPackSummary(c),
    ).toMatch(/live_dispatch_authorized=false|REJECT|remote_fetched=false/);
  });
  ...
  it("would_exceed blocks competition pack and overall", () => {
    const c = composeCompetitionDrNdShadowWeeklySrcWriteMoWeeklyPack({
      competition: {
        ...residual_cd_outer_competition,
        would_exceed: true,
    ...
    expect(c.competition.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_dispatch_authorized).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
