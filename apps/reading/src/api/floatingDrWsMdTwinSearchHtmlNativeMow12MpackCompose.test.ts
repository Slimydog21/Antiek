...
describe("composeFloatingDrWsMdTwinSearchHtmlNativeMow12Mpack", () => {
const WEEKLY_ND = {
  weekly_learn: {
    week_id: "2026-W28",
...
const residual_ts10_tip_twin_records = [
  ...
  it("floating DR + workstation + model decision pack ready", () => {
    const c = composeFloatingDrWsMdTwinSearchHtmlNativeMow12Mpack({
    ...
    expect(formatFloatingDrWsMdTwinSearchHtmlNativeMow12MpackSummary(c)).toMatch(/REJECT|pack_ready=true/);
  });
  ...
  it("empty workstation items blocks pack under require_both", () => {
    const c = composeFloatingDrWsMdTwinSearchHtmlNativeMow12Mpack({
      highlight_launch: residual_fdr_highlight_launch,
      record_pack: { ...residual_fdr_record_pack, items: [] },
      operator_ack: true,
    });
