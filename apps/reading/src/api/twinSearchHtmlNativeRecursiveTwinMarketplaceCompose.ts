/**
 * Twin intelligent search over HTML-native recursive twin marketplace free pack (pure).
 *
 * Operator vision: intelligent search over the twin substrate of the infinite
 * information platform, surfaced beside HTML-native view authority + recursive
 * twin note-taker + free-first marketplace + competition DR honesty — without
 * remote index, twin writes, purchases, or PDF-primary views.
 *
...
import {
  ...
  type HtmlNativeRecursiveTwinMarketplaceFreeInput,
} from "./htmlNativeRecursiveTwinMarketplaceFreeCompose";

export interface TwinSearchHtmlNativeRecursiveTwinMarketplaceInput {
  search_query: string;
  twin_records: TwinSearchRecord[];
  search_limit?: number;
  html_pack: Omit<
    HtmlNativeRecursiveTwinMarketplaceFreeInput,
    "operator_ack"
  >;
...
export interface TwinSearchHtmlNativeRecursiveTwinMarketplaceCompose {
  session_id: string;
  ...
  notes: string[];
  authority: "twin_search_html_native_recursive_twin_marketplace_compose_advisory";
}
...
/**
 * Twin substrate search stacked on HTML-native recursive twin marketplace free.
 * Never remote-indexes; never PDF-primary; never writes twins; ND REJECT.
 */
export function composeTwinSearchHtmlNativeRecursiveTwinMarketplace(
  input: TwinSearchHtmlNativeRecursiveTwinMarketplaceInput,
  ...
  const html_pack = composeHtmlNativeRecursiveTwinMarketplaceFree({
    ...input.html_pack,
  ...
  const focus_task = requireNonEmpty(html_pack.focus_task, "focus_task");
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — twin search + HTML-native recursive twin marketplace free ready; still pure",
    );
  } else {
    notes.push(
...
export function formatTwinSearchHtmlNativeRecursiveTwinMarketplaceSummary(
  ...
  return (
    ...
    `remote_index_queried=false · pdf_view_authorized=false · twin_written=false · purchase_executed=false`
  );
}
