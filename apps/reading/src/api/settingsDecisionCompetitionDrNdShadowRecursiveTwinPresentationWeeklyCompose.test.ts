...
describe("composeSettingsDecisionCompetitionDrNdShadowRecursiveTwinPresentationWeekly", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("settings decision + competition DR ND shadow pack ready", () => {
    const c = composeSettingsDecisionCompetitionDrNdShadowRecursiveTwinPresentationWeekly({
    ...
    expect(
      formatSettingsDecisionCompetitionDrNdShadowRecursiveTwinPresentationWeeklySummary(c),
    ).toMatch(/secrets_stored=false|REJECT|inventory_mutated=false/);
  });
  ...
  it("would_exceed on nested competition blocks overall", () => {
    const c = composeSettingsDecisionCompetitionDrNdShadowRecursiveTwinPresentationWeekly({
      settings: residual_outer_settings,
      competition_pack: {
        ...residual_outer_competition_pack,
        competition: {
          ...residual_outer_competition,
          would_exceed: true,
        },
      },
      operator_ack: true,
    });
    expect(c.competition_pack.competition.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.live_dispatch_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
