...
describe("composeSettingsAddModelFullscreenMoDraftMulti", () => {
  const fullscreen_mo = {
    fullscreen: {
      session_id: "sess-1",
      parent_asset_id: "book-1",
  ...
  it("settings add-model + fullscreen MO pack ready", () => {
    const c = composeSettingsAddModelFullscreenMoDraftMulti({
      settings,
      fullscreen_mo,
      operator_ack: true,
    });
    expect(c.settings.pack_ready).toBe(true);
    expect(c.fullscreen_mo.pack_ready).toBe(true);
    ...
    expect(formatSettingsAddModelFullscreenMoDraftMultiSummary(c)).toMatch(
  ...
  it("propose_add with new model remains pure", () => {
    const c = composeSettingsAddModelFullscreenMoDraftMulti({
      settings: {
        ...settings,
        action: "propose_add",
    ...
    expect(c.settings.proposed_new_count).toBeGreaterThan(0);
    expect(c.pack_ready).toBe(true);
    expect(c.inventory_mutated).toBe(false);
    expect(c.secrets_stored).toBe(false);
  });

  it("fullscreen gated fails closed", () => {
    expect(() =>
      composeSettingsAddModelFullscreenMoDraftMulti({
        settings,
        fullscreen_mo: {
          ...fullscreen_mo,
          fullscreen: { ...fullscreen_mo.fullscreen, gated: true },
        },
        operator_ack: true,
      }),
    ).toThrow(/gated/i);
