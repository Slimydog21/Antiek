...
describe("composeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("marketplace free + MO settings decision pack ready", () => {
    const c = composeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      market: residual_outer_market,
      mo_pack: residual_marketplace_mo_pack,
      operator_ack: true,
    });
    expect(c.market.port_ready).toBe(true);
    expect(c.market.path).toBe("prefer_free_html");
    expect(c.mo_pack.pack_ready).toBe(true);
    expect(c.account_aligned).toBe(true);
    ...
    expect(
      formatMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinSummary(c),
    ).toMatch(/purchase_executed=false|hosted=false|REJECT/);
  });
  ...
  it("purchase_ack without free still never executes purchase", () => {
    const c = composeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      market: {
        ...residual_outer_market,
        free_copy_available: false,
        free_html_projection_sha: null,
        purchase_ack: true,
  ...
  it("MO unattended_ack false blocks overall pack_ready", () => {
    const c = composeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      market: residual_outer_market,
      mo_pack: {
        ...residual_marketplace_mo_pack,
        mo: {
          ...residual_outer_mo,
    ...
    expect(c.mo_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.purchase_executed).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
