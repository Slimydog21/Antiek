...
describe("composeAntiekBenchWeeklyLearnRecursiveTwinPresentationWriteCollective", () => {
...
const COMPETITION_CITATIONS = [
  ...
  // min_events_per_task=2 with 2 failed deep_research + 2 worked twin_notes
  ...
  it("weekly learn + recursive twin presentation write collective pack ready", () => {
    const c = composeAntiekBenchWeeklyLearnRecursiveTwinPresentationWriteCollective({
    ...
    expect(c.twin_presentation_pack.pack_ready).toBe(true);
    ...
    expect(
      formatAntiekBenchWeeklyLearnRecursiveTwinPresentationWriteCollectiveSummary(c),
  ...
  it("operator_ack false blocks", () => {
    ...
    expect(c.learn_ready).toBe(false);
    expect(c.backlog_mutated).toBe(false);
    expect(c.twin_written).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });

  it("sparse events block learn readiness", () => {
    const c = composeAntiekBenchWeeklyLearnRecursiveTwinPresentationWriteCollective({
      weekly_learn: {
        ...pack_outer_weekly_learn,
  ...
  it("open_requested false blocks twin presentation overall", () => {
    const c = composeAntiekBenchWeeklyLearnRecursiveTwinPresentationWriteCollective({
      weekly_learn: pack_outer_weekly_learn,
      twin_presentation_pack: {
        ...pack_outer_twin_presentation_pack,
        presentation: {
          ...pack_outer_presentation,
          open_requested: false,
        },
      },
      operator_ack: true,
    });
    expect(c.twin_presentation_pack.presentation.presentation_ready).toBe(false);
    expect(c.twin_presentation_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.backlog_mutated).toBe(false);
    expect(c.twin_written).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
