...
describe("composeWriteTwinCollectiveHighlightFloatTwinSearch", () => {
  const highlight_pack = {
    highlight: {
      parent_asset_id: "book-1",
  ...
  it("write twin + highlight float twin-search ready", () => {
    const c = composeWriteTwinCollectiveHighlightFloatTwinSearch({
      write,
      highlight_pack,
      operator_ack: true,
    });
    ...
    expect(formatWriteTwinCollectiveHighlightFloatTwinSearchSummary(c)).toMatch(
      /draft_written=false/,
    );
  });
  ...
  it("parent mismatch blocks", () => {
    const c = composeWriteTwinCollectiveHighlightFloatTwinSearch({
      write: {
        ...
        chase_slots: write.chase_slots.map((s) => ({
          ...s,
          parent_asset_id: "book-other",
        })),
      },
