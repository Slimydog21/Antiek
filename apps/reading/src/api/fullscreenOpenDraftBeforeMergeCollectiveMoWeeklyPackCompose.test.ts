...
import {
  composeFullscreenOpenDraftBeforeMergeCollectiveMoWeeklyPack,
  formatFullscreenOpenDraftBeforeMergeCollectiveMoWeeklyPackSummary,
} from "./fullscreenOpenDraftBeforeMergeCollectiveMoWeeklyPackCompose";

describe("composeFullscreenOpenDraftBeforeMergeCollectiveMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_fs_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_fs_outer_decision_pack = {
    decision: residual_fs_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_fs_outer_twin_records,
      html_pack: residual_fs_outer_html_pack,
    },
  };

  const residual_fs_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_fs_outer_record_pack = {
    session_id: "sess-1",
    items: residual_fs_outer_items,
    decision_pack: residual_fs_outer_decision_pack,
  };

  const residual_fs_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_fs_outer_floating_pack = {
    highlight_launch: residual_fs_outer_highlight_launch,
    record_pack: residual_fs_outer_record_pack,
  };

  const residual_fs_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_fs_outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_fs_outer_collective_pack = {
    multiselect: residual_fs_outer_multiselect,
    floating_pack: residual_fs_outer_floating_pack,
  };
  ...
  const residual_fs_outer_draft_pack = {
    draft_gate: residual_fs_outer_draft_gate,
    collective_pack: residual_fs_outer_collective_pack,
  };

  it("fullscreen + draft-before-merge collective MO weekly pack ready", () => {
    const c = composeFullscreenOpenDraftBeforeMergeCollectiveMoWeeklyPack({
    ...
    expect(c.authority).toBe(
      "fullscreen_open_draft_before_merge_collective_mo_weekly_pack_compose_advisory",
    );
    expect(
      formatFullscreenOpenDraftBeforeMergeCollectiveMoWeeklyPackSummary(c),
    ).toMatch(/merge_executed=false/);
  });
