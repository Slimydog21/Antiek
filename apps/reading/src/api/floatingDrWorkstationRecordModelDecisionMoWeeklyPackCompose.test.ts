...
import {
  ...
  formatFloatingDrWorkstationRecordModelDecisionMoWeeklyPackSummary,
} from "./floatingDrWorkstationRecordModelDecisionMoWeeklyPackCompose";

describe("composeFloatingDrWorkstationRecordModelDecisionMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_fd_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_fd_outer_decision_pack = {
    decision: residual_fd_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_fd_outer_twin_records,
      html_pack: residual_fd_outer_html_pack,
    },
  };

  const residual_fd_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_fd_outer_record_pack = {
    session_id: "sess-1",
    items: residual_fd_outer_items,
    decision_pack: residual_fd_outer_decision_pack,
  };
  ...
  it("floating DR + workstation record model decision MO weekly pack ready", () => {
    const c = composeFloatingDrWorkstationRecordModelDecisionMoWeeklyPack({
    ...
    expect(c.highlight_launch.live_dispatched).toBe(false);
    expect(c.highlight_launch.merge_executed).toBe(false);
    expect(c.record_pack.pack_ready).toBe(true);
    ...
    expect(
      formatFloatingDrWorkstationRecordModelDecisionMoWeeklyPackSummary(c),
  ...
  it("parent mismatch blocks pack_ready", () => {
    const c = composeFloatingDrWorkstationRecordModelDecisionMoWeeklyPack({
      highlight_launch: {
        ...residual_fd_outer_highlight_launch,
  ...
  it("gated highlight throws fail-closed", () => {
    expect(() =>
      composeFloatingDrWorkstationRecordModelDecisionMoWeeklyPack({
        highlight_launch: {
          ...residual_fd_outer_highlight_launch,
