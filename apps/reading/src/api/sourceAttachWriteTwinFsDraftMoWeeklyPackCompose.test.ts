...
import {
  ...
  formatSourceAttachWriteTwinFsDraftMoWeeklyPackSummary,
} from "./sourceAttachWriteTwinFsDraftMoWeeklyPackCompose";

describe("composeSourceAttachWriteTwinFsDraftMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_sa_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_sa_outer_decision_pack = {
    decision: residual_sa_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_sa_outer_twin_records,
      html_pack: residual_sa_outer_html_pack,
    },
  };

  const residual_sa_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_sa_outer_record_pack = {
    session_id: "sess-1",
    items: residual_sa_outer_items,
    decision_pack: residual_sa_outer_decision_pack,
  };

  const residual_sa_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_sa_outer_floating_pack = {
    highlight_launch: residual_sa_outer_highlight_launch,
    record_pack: residual_sa_outer_record_pack,
  };

  const residual_sa_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_sa_outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_sa_outer_collective_pack = {
    multiselect: residual_sa_outer_multiselect,
    floating_pack: residual_sa_outer_floating_pack,
  };

  const residual_sa_outer_fullscreen = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    highlight: "Scaling laws claim from page 12",
    prompt: "What evidence supports this?",
  ...
  const residual_sa_outer_draft_pack = {
    draft_gate: residual_sa_outer_draft_gate,
    collective_pack: residual_sa_outer_collective_pack,
  };

  const residual_sa_outer_fullscreen_pack = {
    fullscreen: residual_sa_outer_fullscreen,
    draft_pack: residual_sa_outer_draft_pack,
  };

  const residual_sa_outer_write = {
    session_id: "sess-1",
    draft_id: "draft-1",
    parent_asset_id: "book-1",
    twin_slices: [
  ...
  const residual_sa_outer_write_pack = {
    write: residual_sa_outer_write,
    fullscreen_pack: residual_sa_outer_fullscreen_pack,
  };

  it("source attach + write twin fullscreen draft MO weekly pack ready", () => {
    const c = composeSourceAttachWriteTwinFsDraftMoWeeklyPack({
      sources: residual_sa_outer_sources,
      write_pack: residual_sa_outer_write_pack,
      operator_ack: true,
    });
    expect(c.sources.attach_ready).toBe(true);
    ...
    expect(c.authority).toBe(
      "source_attach_write_twin_fs_draft_mo_weekly_pack_compose_advisory",
    );
    expect(
      formatSourceAttachWriteTwinFsDraftMoWeeklyPackSummary(c),
    ).toMatch(/remote_fetched=false/);
  });
