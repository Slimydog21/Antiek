 ...
 * write twin FS draft pack tip (short residual: AntiekBenchWeeklySrcWritePack — FS 255).
...
import {
  ...
  type SourceAttachWriteTwinFsDraftPackInput,
} from "./sourceAttachWriteTwinFsDraftPackCompose";

export interface AntiekBenchWeeklySrcWritePackInput {
  weekly_learn: Omit<AntiekBenchWeeklyUsageLearnInput, "operator_ack">;
  source_pack: Omit<
    SourceAttachWriteTwinFsDraftPackInput,
    "operator_ack"
  >;
...
export interface AntiekBenchWeeklySrcWritePackCompose {
  week_id: string;
  session_id: string;
  ...
  source_pack: SourceAttachWriteTwinFsDraftPackCompose;
  ...
  notes: string[];
  authority: "antiek_bench_weekly_src_write_pack_compose_advisory";
}
 ...
 */
export function composeAntiekBenchWeeklySrcWritePack(
  input: AntiekBenchWeeklySrcWritePackInput,
): AntiekBenchWeeklySrcWritePackCompose {
  ...
  const source_pack = composeSourceAttachWriteTwinFsDraftPack({
    ...input.source_pack,
    operator_ack: input.operator_ack,
  });
...
export function formatAntiekBenchWeeklySrcWritePackSummary(
  c: AntiekBenchWeeklySrcWritePackCompose,
): string {
  return (
