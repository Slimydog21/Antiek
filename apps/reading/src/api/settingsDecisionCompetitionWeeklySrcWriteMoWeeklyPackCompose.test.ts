...
import {
  composeSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPack,
  formatSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPackSummary,
} from "./settingsDecisionCompetitionWeeklySrcWriteMoWeeklyPackCompose";

describe("composeSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_sd_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_sd_outer_decision_pack = {
    decision: residual_sd_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_sd_outer_twin_records,
      html_pack: residual_sd_outer_html_pack,
    },
  };

  const residual_sd_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_sd_outer_record_pack = {
    session_id: "sess-1",
    items: residual_sd_outer_items,
    decision_pack: residual_sd_outer_decision_pack,
  };

  const residual_sd_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_sd_outer_floating_pack = {
    highlight_launch: residual_sd_outer_highlight_launch,
    record_pack: residual_sd_outer_record_pack,
  };

  const residual_sd_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_sd_outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_sd_outer_collective_pack = {
    multiselect: residual_sd_outer_multiselect,
    floating_pack: residual_sd_outer_floating_pack,
  };

  const residual_sd_outer_fullscreen = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    highlight: "Scaling laws claim from page 12",
    prompt: "What evidence supports this?",
  ...
  const residual_sd_outer_draft_pack = {
    draft_gate: residual_sd_outer_draft_gate,
    collective_pack: residual_sd_outer_collective_pack,
  };

  const residual_sd_outer_fullscreen_pack = {
    fullscreen: residual_sd_outer_fullscreen,
    draft_pack: residual_sd_outer_draft_pack,
  };

  const residual_sd_outer_write = {
    session_id: "sess-1",
    draft_id: "draft-1",
    parent_asset_id: "book-1",
    twin_slices: [
  ...
  const residual_sd_outer_sources = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    requested_families: ["arxiv" as const, "substack" as const],
    sources: [
  ...
  const residual_sd_outer_write_pack = {
    write: residual_sd_outer_write,
    fullscreen_pack: residual_sd_outer_fullscreen_pack,
  };

  const residual_sd_outer_weekly_learn = {
    week_id: "2026-W28",
    min_events_per_task: 2,
    events: [
      {
  ...
  const residual_sd_outer_source_pack = {
    sources: residual_sd_outer_sources,
    write_pack: residual_sd_outer_write_pack,
  };

  const residual_sd_outer_twin = {
    parent_asset_id: "book-1",
    source_excerpt:
      "<p>Scaling laws hold under noise in compute-optimal regimes.</p>",
    focus_questions: ["Where does it break?", "What residual gaps?"],
  ...
  const residual_sd_outer_presentation = {
    view_mode: "side_panel" as const,
    open_requested: true,
    merge_to_parent_preview: false,
    presented_insights: [
  ...
  const residual_sd_outer_weekly_pack = {
    weekly_learn: residual_sd_outer_weekly_learn,
    source_pack: residual_sd_outer_source_pack,
  };

  const residual_sd_outer_nd_shadow = {
    selected_model_id: "gpt-5.5",
    nd_recommended_model_id: "claude-opus",
    kill_switch_on: true,
    confidence: 0.72,
  ...
  const residual_sd_outer_twin_presentation = {
    twin: residual_sd_outer_twin,
    presentation: residual_sd_outer_presentation,
    weekly_pack: residual_sd_outer_weekly_pack,
  };

  const residual_sd_outer_competition = {
    session_id: "sess-1",
    competitor_decisions: [
      {
        competitor: "Perplexity",
  ...
  const residual_sd_outer_nd_pack = {
    nd_shadow: residual_sd_outer_nd_shadow,
    twin_presentation: residual_sd_outer_twin_presentation,
  };
  ...
  const residual_sd_outer_competition_pack = {
    competition: residual_sd_outer_competition,
    nd_pack: residual_sd_outer_nd_pack,
  };

  it("settings decision + competition DR ND shadow weekly pack ready", () => {
    const c = composeSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPack({
    ...
    expect(c.week_aligned).toBe(true);
    ...
    expect(c.authority).toBe(
      "settings_decision_competition_weekly_src_write_mo_weekly_pack_compose_advisory",
    );
    expect(
      formatSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPackSummary(c),
  ...
  it("week mismatch blocks pack_ready", () => {
    const c = composeSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPack({
      settings: {
        ...residual_sd_outer_settings,
        week_id: "2026-W99",
      },
      competition_pack: residual_sd_outer_competition_pack,
      operator_ack: true,
    });
    expect(c.week_aligned).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });

  it("would_exceed on competition blocks pack", () => {
    const c = composeSettingsDecisionCompetitionWeeklySrcWriteMoWeeklyPack({
      settings: residual_sd_outer_settings,
      competition_pack: {
        ...residual_sd_outer_competition_pack,
        competition: {
          ...residual_sd_outer_competition,
          would_exceed: true,
        },
      },
      operator_ack: true,
