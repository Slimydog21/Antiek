...
describe("composeMultiSelectWorkstationRecordWriteTwin", () => {
  const record_write = {
    record_prompt: {
      session_id: "sess-1",
      parent_asset_id: "book-1",
  ...
  it("multiselect + record write twin ready", () => {
    const c = composeMultiSelectWorkstationRecordWriteTwin({
      multiselect,
      record_write,
      operator_ack: true,
    });
    expect(c.multiselect.pack_ready).toBe(true);
    expect(c.record_write.pack_ready).toBe(true);
    ...
    expect(formatMultiSelectWorkstationRecordWriteTwinSummary(c)).toMatch(
  ...
  it("parent mismatch blocks", () => {
    const c = composeMultiSelectWorkstationRecordWriteTwin({
      multiselect: {
        ...
        members: multiselect.members.map((m) => ({
          ...m,
          parent_asset_id: "book-other",
        })),
      },
