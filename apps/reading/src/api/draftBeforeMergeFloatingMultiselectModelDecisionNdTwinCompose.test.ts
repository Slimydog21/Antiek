...
describe("composeDraftBeforeMergeFloatingMultiselectModelDecisionNdTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("draft-before-merge + floating multi-select model decision ND twin pack ready", () => {
    const c = composeDraftBeforeMergeFloatingMultiselectModelDecisionNdTwin({
      draft_gate: outer_draft_gate,
      multi_pack: outer_multi_pack,
      operator_ack: true,
    });
    expect(c.draft_gate.gate_ready).toBe(true);
    expect(c.multi_pack.pack_ready).toBe(true);
    ...
    expect(
      formatDraftBeforeMergeFloatingMultiselectModelDecisionNdTwinSummary(c),
  ...
  it("would_exceed on nested decision blocks overall", () => {
    ...
    expect(c.multi_pack.decision_pack.decision.would_exceed).toBe(true);
    expect(c.multi_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.draft_written).toBe(false);
    expect(c.merge_executed).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
