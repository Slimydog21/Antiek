...
describe("composeWorkstationRecordWriteTwinHighlight", () => {
  const write_pack = {
    write: {
      session_id: "sess-1",
      draft_id: "draft-1",
  ...
  it("record prompt + write twin highlight ready", () => {
    const c = composeWorkstationRecordWriteTwinHighlight({
      record_prompt,
      write_pack,
      operator_ack: true,
    });
    expect(c.record_prompt.pack_ready).toBe(true);
    ...
    expect(formatWorkstationRecordWriteTwinHighlightSummary(c)).toMatch(
  ...
  it("parent mismatch blocks", () => {
    const c = composeWorkstationRecordWriteTwinHighlight({
      record_prompt: { ...record_prompt, parent_asset_id: "book-other" },
      write_pack,
      operator_ack: true,
    });
    expect(c.pack_ready).toBe(false);
