...
import {
  ...
  formatMarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackSummary,
} from "./marketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackCompose";

describe("composeMarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("marketplace free + MO settings weekly pack ready", () => {
    const c = composeMarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPack({
    ...
    expect(
      formatMarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPackSummary(c),
    ).toMatch(/purchase_executed=false/);
  });
  ...
  it("account mismatch blocks pack_ready", () => {
    const c = composeMarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPack({
      market: {
        ...residual_mk2_outer_market,
        account_id: "acct-other",
      },
      mo_pack: residual_mk2_outer_mo_pack,
      operator_ack: true,
    });
    expect(c.account_aligned).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.purchase_executed).toBe(false);
    expect(c.hosted).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  ...
  it("unknown free availability blocks port_ready and pack", () => {
    const c = composeMarketplaceFreeMoSettingsWeeklySrcWriteMoWeeklyPack({
      market: {
        ...residual_mk2_outer_market,
    ...
    expect(c.market.port_ready).toBe(false);
    expect(c.market.path).toBe("blocked_unknown_free");
    expect(c.pack_ready).toBe(false);
    expect(c.purchase_executed).toBe(false);
    expect(c.hosted).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
