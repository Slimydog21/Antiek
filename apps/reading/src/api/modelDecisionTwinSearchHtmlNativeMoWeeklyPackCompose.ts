 ...
 * intelligent search + HTML-native + HTML-native marketplace MO weekly pack (pure).
 *
 ...
 * while twin search + HTML-native marketplace MO weekly honesty remain pure —
...
import {
  composeTwinSearchHtmlNativeMarketplaceMoWeeklyPack,
  type TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose,
  type TwinSearchHtmlNativeMarketplaceMoWeeklyPackInput,
} from "./twinSearchHtmlNativeMarketplaceMoWeeklyPackCompose";

export interface ModelDecisionTwinSearchHtmlNativeMoWeeklyPackInput {
  decision: Omit<SettingsDecisionTreeUsageBarInput, "operator_ack">;
  twin_search_pack: Omit<
    TwinSearchHtmlNativeMarketplaceMoWeeklyPackInput,
    "operator_ack"
  >;
...
export interface ModelDecisionTwinSearchHtmlNativeMoWeeklyPackCompose {
  week_id: string;
  session_id: string;
  ...
  notes: string[];
  authority: "model_decision_twin_search_html_native_mo_weekly_pack_compose_advisory";
}
...
/**
 * Model decision/usage/projection stacked on twin search HTML-native marketplace MO weekly.
 * Never live-routes; never purchases; never PDF-primary; ND REJECT.
 */
export function composeModelDecisionTwinSearchHtmlNativeMoWeeklyPack(
  input: ModelDecisionTwinSearchHtmlNativeMoWeeklyPackInput,
  ...
  const twin_search_pack = composeTwinSearchHtmlNativeMarketplaceMoWeeklyPack({
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — model decision + twin search HTML-native marketplace MO weekly ready; still pure",
    );
  } else {
    notes.push(
...
export function formatModelDecisionTwinSearchHtmlNativeMoWeeklyPackSummary(
