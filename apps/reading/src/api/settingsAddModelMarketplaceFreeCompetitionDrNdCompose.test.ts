...
describe("composeSettingsAddModelMarketplaceFreeCompetitionDrNd", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("settings add-model + marketplace free competition DR ND pack ready", () => {
    const c = composeSettingsAddModelMarketplaceFreeCompetitionDrNd({
    ...
    expect(
      formatSettingsAddModelMarketplaceFreeCompetitionDrNdSummary(c),
  ...
  it("no free copy without purchase blocks market pack overall", () => {
    const c = composeSettingsAddModelMarketplaceFreeCompetitionDrNd({
      settings: pack_outer_settings,
      market_pack: {
        ...pack_outer_market_pack,
        market: {
          ...pack_outer_market,
    ...
    expect(c.market_pack.market.port_ready).toBe(false);
    expect(c.market_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.inventory_mutated).toBe(false);
    expect(c.purchase_executed).toBe(false);
  ...
  it("propose_add with no new ids blocks settings readiness", () => {
    const c = composeSettingsAddModelMarketplaceFreeCompetitionDrNd({
      settings: {
        ...pack_outer_settings,
        pending_add_model_ids: ["gpt-5.5"], // already inventoried
        action: "propose_add",
      },
      market_pack: pack_outer_market_pack,
      operator_ack: true,
    });
    expect(c.settings.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.inventory_mutated).toBe(false);
    expect(c.purchase_executed).toBe(false);
