/**
 * Antiek-bench task→model recommendation residual over Midnight Oil unattended
 * + fullscreen draft-before-merge multi-select model decision pack (pure).
 *
 * Operator vision: weekly usage learn surfaces which models are best for which
 * tasks into the decision tree, while MO unattended (time+goals+price ceiling)
 * + fullscreen draft + multi-select remain pure — never auto-routes.
 *
...
import {
  ...
  type MoUnattendedFullscreenDraftBeforeMergeMultiselectInput,
} from "./moUnattendedFullscreenDraftBeforeMergeMultiselectCompose";

export interface AntiekBenchRecommendMoUnattendedFullscreenDraftInput {
  bench: Omit<AntiekBenchTaskModelRecommendationInput, "operator_ack">;
  mo_pack: Omit<
    MoUnattendedFullscreenDraftBeforeMergeMultiselectInput,
    "operator_ack"
  >;
...
export interface AntiekBenchRecommendMoUnattendedFullscreenDraftCompose {
  week_id: string;
  focus_task: string;
  session_id: string;
  ...
  hosted: false;
  notes: string[];
  authority: "antiek_bench_recommend_mo_unattended_fullscreen_draft_compose_advisory";
}
...
/**
 * Antiek-bench task→model rec stacked on MO unattended fullscreen draft pack.
 * Never live-routes; never rewrites suite; never charges.
 */
export function composeAntiekBenchRecommendMoUnattendedFullscreenDraft(
  input: AntiekBenchRecommendMoUnattendedFullscreenDraftInput,
  ...
  const mo_pack = composeMoUnattendedFullscreenDraftBeforeMergeMultiselect({
    ...input.mo_pack,
    operator_ack: input.operator_ack,
  });
  ...
  const operator_id = requireNonEmpty(
    mo_pack.mo.entry_readiness.entry.operator_id,
    "operator_id",
  );
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — Antiek-bench recommend + MO unattended fullscreen draft ready; still pure",
    );
  } else {
    notes.push(
...
export function formatAntiekBenchRecommendMoUnattendedFullscreenDraftSummary(
