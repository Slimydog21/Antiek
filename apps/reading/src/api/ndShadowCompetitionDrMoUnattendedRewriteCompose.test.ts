...
describe("composeNdShadowCompetitionDrMoUnattendedRewrite", () => {
  ...
  it("ND shadow REJECT re-affirmed on competition DR MO pack ready", () => {
    const c = composeNdShadowCompetitionDrMoUnattendedRewrite({
    ...
    expect(
      formatNdShadowCompetitionDrMoUnattendedRewriteSummary(c),
  ...
  it("ND kill switch on still allows pack when competition ready (shadow suppressed)", () => {
    const c = composeNdShadowCompetitionDrMoUnattendedRewrite({
      nd_shadow: { ...nd_shadow_input, kill_switch_on: true },
      competition_pack,
      operator_ack: true,
    });
    ...
    expect(c.nd_shadow.production_router_verdict).toBe("REJECT");
    expect(c.live_router_authorized).toBe(false);
    // pack can still be ready — ND never routes live
    expect(c.competition_pack.pack_ready).toBe(true);
    expect(c.pack_ready).toBe(true);
  });
});
