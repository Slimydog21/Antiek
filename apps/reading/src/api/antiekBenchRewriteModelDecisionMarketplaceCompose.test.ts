...
describe("composeAntiekBenchRewriteModelDecisionMarketplace", () => {
  ...
  it("rewrite residual + model decision marketplace pack ready", () => {
    const c = composeAntiekBenchRewriteModelDecisionMarketplace({
      rewrite: rewrite_ready,
      model_decision_pack,
      operator_ack: true,
    });
    expect(c.proposal_count).toBeGreaterThanOrEqual(1);
    expect(c.rewrite.applied).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.applied).toBe(false);
    expect(c.model_decision_pack.pack_ready).toBe(true);
    expect(c.model_decision_pack.decision.would_exceed).toBe(false);
    ...
    expect(
      formatAntiekBenchRewriteModelDecisionMarketplaceSummary(c),
    ).toMatch(/suite_rewritten=false/);
  });
  ...
  it("zero rewrite proposals blocks pack_ready under require_both", () => {
    const c = composeAntiekBenchRewriteModelDecisionMarketplace({
      rewrite: rewrite_empty,
      model_decision_pack,
      operator_ack: true,
    });
    expect(c.proposal_count).toBe(0);
    expect(c.pack_ready).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.applied).toBe(false);
    expect(c.live_router_authorized).toBe(false);
  ...
  it("would_exceed on nested decision blocks overall", () => {
    ...
    expect(c.model_decision_pack.decision.would_exceed).toBe(true);
    expect(c.pack_ready).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.applied).toBe(false);
    expect(c.live_router_authorized).toBe(false);
