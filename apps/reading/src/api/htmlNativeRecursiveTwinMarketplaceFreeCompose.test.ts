...
describe("composeHtmlNativeRecursiveTwinMarketplaceFree", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("html-native + recursive twin marketplace free pack ready", () => {
    const c = composeHtmlNativeRecursiveTwinMarketplaceFree({
    ...
    expect(
      formatHtmlNativeRecursiveTwinMarketplaceFreeSummary(c),
    ).toMatch(/pdf_view_authorized=false/);
  });
  ...
  it("claimed pdf format blocks html pack and overall", () => {
    const c = composeHtmlNativeRecursiveTwinMarketplaceFree({
      html_view: {
        ...html_view_input,
        claimed_format: "pdf",
