...
describe("fetchComposerProjection", () => {
  ...
  it("rejects route costs whose exact decimal and reservation disagree", async () => {
    mockFetch.mockResolvedValue(
      response({
        ...projection,
        fallback_plan: {
          ...
          routes: [
            {
              ...fallbackPlan.routes[0],
              projection: {
                ...fallbackPlan.routes[0].projection,
                maximum_cost_usd: "0.201",
              },
            },
          ],
          maximum_chain_exposure_cents: 20,
    ...
    ).rejects.toThrow("invalid fallback cost");
  });

  it("rejects a fallback budget verdict that contradicts remaining budget", async () => {
    mockFetch.mockResolvedValue(
      response({
        ...projection,
        remaining_usd: 0.19,
