...
describe("composeNdShadowAntiekBenchWeeklyMarketplace", () => {
  it("ND REJECT + weekly marketplace pack ready", () => {
    const c = composeNdShadowAntiekBenchWeeklyMarketplace({
      nd_shadow: ND_SHADOW,
    ...
    expect(c.weekly_market.pack_ready).toBe(true);
    ...
    expect(formatNdShadowAntiekBenchWeeklyMarketplaceSummary(c)).toMatch(
  ...
  it("kill switch off still never live-routes", () => {
    const c = composeNdShadowAntiekBenchWeeklyMarketplace({
