...
describe("composeFullscreenDraftCollectivePresentedTwins", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("fullscreen + draft collective pack ready", () => {
    const c = composeFullscreenDraftCollectivePresentedTwins({
      fullscreen,
      draft_collective,
      operator_ack: true,
    });
    expect(c.fullscreen.fullscreen_ready).toBe(true);
    expect(c.draft_collective.pack_ready).toBe(true);
    ...
    expect(
      formatFullscreenDraftCollectivePresentedTwinsSummary(c),
  ...
  it("gated spawn blocks fullscreen readiness", () => {
    // gated true throws or blocks - check behavior
    let threw = false;
    try {
      composeFullscreenDraftCollectivePresentedTwins({
    ...
    if (!threw) {
      // if it doesn't throw, pack_ready must be false
      const c = composeFullscreenDraftCollectivePresentedTwins({
        fullscreen: { ...fullscreen, gated: true },
        draft_collective,
        operator_ack: true,
