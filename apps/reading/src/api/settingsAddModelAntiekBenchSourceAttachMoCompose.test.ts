...
import {
  ...
  formatSettingsAddModelAntiekBenchSourceAttachMoSummary,
...
describe("composeSettingsAddModelAntiekBenchSourceAttachMo", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("settings add-model + bench source attach MO pack ready", () => {
    const c = composeSettingsAddModelAntiekBenchSourceAttachMo({
    ...
    expect(c.settings.action).toBe("propose_add");
    expect(c.settings.proposed_new_count).toBeGreaterThanOrEqual(1);
    expect(c.settings.inventory_mutated).toBe(false);
    expect(c.settings.secrets_stored).toBe(false);
    expect(c.bench_pack.pack_ready).toBe(true);
    expect(c.bench_pack.bench.recommendation?.recommended_model_id).toBe(
      "gpt-5.5",
    );
    expect(c.inventory_vs_bench).toBe("agree");
    ...
    expect(
      formatSettingsAddModelAntiekBenchSourceAttachMoSummary(c),
    ).toMatch(/inventory_mutated=false/);
  });
  ...
  it("preview with no new ids still pure when inventory present", () => {
    const c = composeSettingsAddModelAntiekBenchSourceAttachMo({
      settings: {
        ...settings_input,
        action: "preview",
    ...
    expect(c.settings.pack_ready).toBe(true);
    expect(c.settings.proposed_new_count).toBe(0);
    expect(c.bench_pack.pack_ready).toBe(true);
    expect(c.pack_ready).toBe(true);
    expect(c.inventory_mutated).toBe(false);
    expect(c.secrets_stored).toBe(false);
  ...
  it("high min_events_for_recommendation yields bench_none vs and still pure", () => {
    const c = composeSettingsAddModelAntiekBenchSourceAttachMo({
      settings: settings_input,
      bench_pack: {
        bench: {
          ...bench_input,
          min_events_for_recommendation: 100,
        },
        source_pack,
      },
      operator_ack: true,
    });
    expect(c.bench_pack.bench.recommendation).toBeNull();
    expect(c.inventory_vs_bench).toBe("bench_none");
    expect(c.inventory_mutated).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.suite_rewritten).toBe(false);
