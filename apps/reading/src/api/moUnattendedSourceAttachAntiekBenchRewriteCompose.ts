...
import {
  ...
  type SourceAttachAntiekBenchRewriteModelDecisionInput,
} from "./sourceAttachAntiekBenchRewriteModelDecisionCompose";

export interface MoUnattendedSourceAttachAntiekBenchRewriteInput {
  mo: Omit<MidnightOilPriceCeilingApprovalInput, "operator_ack">;
  research_pack: Omit<
    SourceAttachAntiekBenchRewriteModelDecisionInput,
    "operator_ack"
  >;
...
export interface MoUnattendedSourceAttachAntiekBenchRewriteCompose {
  session_id: string;
  ...
  hosted: false;
  notes: string[];
  authority: "mo_unattended_source_attach_antiek_bench_rewrite_compose_advisory";
}
...
/**
 * MO unattended price-ceiling package stacked on source attach + Antiek-bench
 * rewrite model decision. Never live-executes; never charges.
 */
export function composeMoUnattendedSourceAttachAntiekBenchRewrite(
  input: MoUnattendedSourceAttachAntiekBenchRewriteInput,
  ...
  const research_pack = composeSourceAttachAntiekBenchRewriteModelDecision({
    ...input.research_pack,
    operator_ack: input.operator_ack,
  });
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — MO unattended + source attach Antiek-bench rewrite ready; still pure",
    );
  } else {
    notes.push(
...
export function formatMoUnattendedSourceAttachAntiekBenchRewriteSummary(
