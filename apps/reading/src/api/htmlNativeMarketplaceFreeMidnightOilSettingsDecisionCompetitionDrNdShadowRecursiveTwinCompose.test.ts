...
describe("composeHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("HTML-native + marketplace free MO settings pack ready", () => {
    const c = composeHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      html_view: residual_outer_html_view,
      market_pack: residual_html_native_market_pack,
      operator_ack: true,
    });
    expect(c.html_view.pack_ready).toBe(true);
    expect(c.market_pack.pack_ready).toBe(true);
    ...
    expect(
      formatHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinSummary(c),
    ).toMatch(/pdf_primary=false|REJECT|purchase_executed=false/);
  });
  ...
  it("claimed_format pdf fails closed (never pdf_primary)", () => {
    const c = composeHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      html_view: {
        ...residual_outer_html_view,
        claimed_format: "pdf",
