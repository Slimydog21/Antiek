...
describe("composeAntiekBenchWeeklyMarketplaceFreeSource", () => {
  it("weekly learn + marketplace free source pack ready", () => {
    const c = composeAntiekBenchWeeklyMarketplaceFreeSource({
      weekly_learn: WEEKLY_LEARN,
      market_research: MARKET_RESEARCH,
      operator_ack: true,
    });
    expect(c.weekly_learn.learn_ready).toBe(true);
    expect(c.market_research.pack_ready).toBe(true);
    ...
    expect(formatAntiekBenchWeeklyMarketplaceFreeSourceSummary(c)).toMatch(
  ...
  it("sparse weekly events not learn_ready still honesty", () => {
    const c = composeAntiekBenchWeeklyMarketplaceFreeSource({
      weekly_learn: {
        week_id: "2026-W28",
