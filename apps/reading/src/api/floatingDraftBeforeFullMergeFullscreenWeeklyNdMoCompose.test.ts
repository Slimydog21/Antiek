...
describe("composeFloatingDraftBeforeFullMergeFullscreenWeeklyNdMo", () => {
  it("draft_only + fullscreen weekly ND ready", () => {
    const c = composeFloatingDraftBeforeFullMergeFullscreenWeeklyNdMo({
    ...
    expect(c.week_id).toBe("2026-W28");
    ...
    expect(
      formatFloatingDraftBeforeFullMergeFullscreenWeeklyNdMoSummary(c),
  ...
  it("session/parent mismatch blocks pack", () => {
    const c = composeFloatingDraftBeforeFullMergeFullscreenWeeklyNdMo({
      draft_gate: DRAFT_GATE,
      fullscreen_pack: {
        ...FULLSCREEN_PACK,
        fullscreen: {
          ...FULLSCREEN_PACK.fullscreen,
  ...
  it("promote_full_merge with full_merge_ack still never merges", () => {
    const c = composeFloatingDraftBeforeFullMergeFullscreenWeeklyNdMo({
      draft_gate: {
        ...DRAFT_GATE,
    ...
    expect(c.draft_gate.full_merge_intent_ready).toBe(true);
    expect(c.pack_ready).toBe(true);
    expect(c.draft_written).toBe(false);
    expect(c.merge_executed).toBe(false);
    expect(c.live_dispatched).toBe(false);
  ...
  it("gated highlight throws on fullscreen spawn", () => {
    expect(() =>
      composeFloatingDraftBeforeFullMergeFullscreenWeeklyNdMo({
        draft_gate: DRAFT_GATE,
        fullscreen_pack: {
