...
describe("composeMarketplaceFreeCompetitionDrNdShadowSourceAttach", () => {
...
const COMPETITION_CITATIONS = [
  ...
  // Free-first path: free available → port_ready without purchase
  ...
  it("marketplace free + competition DR ND shadow source-attach pack ready", () => {
    const c = composeMarketplaceFreeCompetitionDrNdShadowSourceAttach({
    ...
    expect(
      c.competition_pack.nd_pack.nd_shadow.production_router_verdict,
    ).toBe("REJECT");
    expect(c.authority).toBe(
      "marketplace_free_competition_dr_nd_shadow_source_attach_compose_advisory",
    );
    expect(
      formatMarketplaceFreeCompetitionDrNdShadowSourceAttachSummary(c),
  ...
  it("no free copy without purchase_ack blocks port_ready", () => {
    const c = composeMarketplaceFreeCompetitionDrNdShadowSourceAttach({
      market: {
        ...pack_outer_market,
  ...
  it("would_exceed on competition blocks competition_pack overall", () => {
    const c = composeMarketplaceFreeCompetitionDrNdShadowSourceAttach({
      market: pack_outer_market,
      competition_pack: {
        ...pack_outer_competition_pack,
        competition: {
          ...pack_outer_competition,
          would_exceed: true,
        },
      },
      operator_ack: true,
