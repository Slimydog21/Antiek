...
describe("composeRecursiveTwinMoPriceCeilingWritePack", () => {
  it("twin propose + mo write ready", () => {
    const c = composeRecursiveTwinMoPriceCeilingWritePack({
      twin: TWIN,
      mo_write: {
        mo: MO,
        research_write: {
    ...
    expect(c.mo_write.pack_ready).toBe(true);
    ...
    expect(formatRecursiveTwinMoPriceCeilingWritePackSummary(c)).toMatch(
      /twin_written=false/,
    );
  });
