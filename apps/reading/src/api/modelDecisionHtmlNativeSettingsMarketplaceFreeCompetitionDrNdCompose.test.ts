...
describe("composeModelDecisionHtmlNativeSettingsMarketplaceFreeCompetitionDrNd", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("model decision + HTML-native settings marketplace free competition pack ready", () => {
    const c = composeModelDecisionHtmlNativeSettingsMarketplaceFreeCompetitionDrNd({
      decision: pack_outer_decision_input,
      html_native_pack,
      operator_ack: true,
    });
    ...
    expect(c.html_native_pack.pack_ready).toBe(true);
    ...
    expect(
      formatModelDecisionHtmlNativeSettingsMarketplaceFreeCompetitionDrNdSummary(c),
  ...
  it("html_native pack session mismatch blocks overall", () => {
    const c = composeModelDecisionHtmlNativeSettingsMarketplaceFreeCompetitionDrNd({
      decision: pack_outer_decision_input,
      html_native_pack: {
        ...html_native_pack,
        html_view: {
          ...pack_outer_html_view,
          session_id: "sess-other",
        },
      },
      operator_ack: true,
    });
    expect(c.html_native_pack.session_aligned).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.pdf_primary).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
