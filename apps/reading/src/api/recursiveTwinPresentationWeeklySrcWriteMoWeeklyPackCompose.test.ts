...
import {
  composeRecursiveTwinPresentationWeeklySrcWriteMoWeeklyPack,
  formatRecursiveTwinPresentationWeeklySrcWriteMoWeeklyPackSummary,
} from "./recursiveTwinPresentationWeeklySrcWriteMoWeeklyPackCompose";

describe("composeRecursiveTwinPresentationWeeklySrcWriteMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_rt_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_rt_outer_decision_pack = {
    decision: residual_rt_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_rt_outer_twin_records,
      html_pack: residual_rt_outer_html_pack,
    },
  };

  const residual_rt_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_rt_outer_record_pack = {
    session_id: "sess-1",
    items: residual_rt_outer_items,
    decision_pack: residual_rt_outer_decision_pack,
  };

  const residual_rt_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_rt_outer_floating_pack = {
    highlight_launch: residual_rt_outer_highlight_launch,
    record_pack: residual_rt_outer_record_pack,
  };

  const residual_rt_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_rt_outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_rt_outer_collective_pack = {
    multiselect: residual_rt_outer_multiselect,
    floating_pack: residual_rt_outer_floating_pack,
  };

  const residual_rt_outer_fullscreen = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    highlight: "Scaling laws claim from page 12",
    prompt: "What evidence supports this?",
  ...
  const residual_rt_outer_draft_pack = {
    draft_gate: residual_rt_outer_draft_gate,
    collective_pack: residual_rt_outer_collective_pack,
  };

  const residual_rt_outer_fullscreen_pack = {
    fullscreen: residual_rt_outer_fullscreen,
    draft_pack: residual_rt_outer_draft_pack,
  };

  const residual_rt_outer_write = {
    session_id: "sess-1",
    draft_id: "draft-1",
    parent_asset_id: "book-1",
    twin_slices: [
  ...
  const residual_rt_outer_sources = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    requested_families: ["arxiv" as const, "substack" as const],
    sources: [
  ...
  const residual_rt_outer_write_pack = {
    write: residual_rt_outer_write,
    fullscreen_pack: residual_rt_outer_fullscreen_pack,
  };

  const residual_rt_outer_weekly_learn = {
    week_id: "2026-W28",
    min_events_per_task: 2,
    events: [
      {
  ...
  const residual_rt_outer_source_pack = {
    sources: residual_rt_outer_sources,
    write_pack: residual_rt_outer_write_pack,
  };
  ...
  const residual_rt_outer_weekly_pack = {
    weekly_learn: residual_rt_outer_weekly_learn,
    source_pack: residual_rt_outer_source_pack,
  };

  it("twin presentation + weekly source attach write twin MO weekly pack ready", () => {
    const c = composeRecursiveTwinPresentationWeeklySrcWriteMoWeeklyPack({
    ...
    expect(c.weekly_pack.learn_ready).toBe(true);
    ...
    expect(c.authority).toBe(
      "recursive_twin_presentation_weekly_src_write_mo_weekly_pack_compose_advisory",
    );
    expect(
      formatRecursiveTwinPresentationWeeklySrcWriteMoWeeklyPackSummary(c),
  ...
  it("session mismatch via nested source blocks pack_ready", () => {
    const c = composeRecursiveTwinPresentationWeeklySrcWriteMoWeeklyPack({
      twin: residual_rt_outer_twin,
      presentation: residual_rt_outer_presentation,
      weekly_pack: {
        ...residual_rt_outer_weekly_pack,
        source_pack: {
          ...residual_rt_outer_source_pack,
          sources: {
            ...residual_rt_outer_sources,
