...
describe("composePaidPurchaseNdShadowTwinPresentation", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("free-first + ND twin pack ready", () => {
    const c = composePaidPurchaseNdShadowTwinPresentation({
      purchase: purchase_free,
      nd_twin,
      operator_ack: true,
    });
    expect(c.purchase.gate_ready).toBe(true);
    expect(c.purchase.purchase_executed).toBe(false);
    expect(c.nd_twin.pack_ready).toBe(true);
    ...
    expect(
      formatPaidPurchaseNdShadowTwinPresentationSummary(c),
  ...
  it("paid path purchase_ready still never executes", () => {
    const c = composePaidPurchaseNdShadowTwinPresentation({
      purchase: {
        ...purchase_free,
    ...
    expect(c.purchase.purchase_ready).toBe(true);
    expect(c.purchase.purchase_executed).toBe(false);
    expect(c.charge_executed).toBe(false);
    expect(c.hosted).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
    // gate_ready depends on port sha for paid path
    expect(c.purchase_executed).toBe(false);
  });

  it("nd twin open false blocks overall when require_both", () => {
    const c = composePaidPurchaseNdShadowTwinPresentation({
      purchase: purchase_free,
      nd_twin: {
        ...nd_twin,
        twin_presentation: {
          ...nd_twin.twin_presentation,
          presentation: {
            ...presentation_input,
    ...
    expect(c.nd_twin.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.purchase_executed).toBe(false);
  });
});
