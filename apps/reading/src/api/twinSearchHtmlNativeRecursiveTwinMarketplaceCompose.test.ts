...
import {
  ...
  formatTwinSearchHtmlNativeRecursiveTwinMarketplaceSummary,
...
describe("composeTwinSearchHtmlNativeRecursiveTwinMarketplace", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("twin search + html-native recursive twin marketplace pack ready", () => {
    const c = composeTwinSearchHtmlNativeRecursiveTwinMarketplace({
    ...
    expect(c.search.remote_index_queried).toBe(false);
    ...
    expect(
      formatTwinSearchHtmlNativeRecursiveTwinMarketplaceSummary(c),
    ).toMatch(/remote_index_queried=false/);
  });
  ...
  it("zero hits blocks overall under require_both", () => {
    const c = composeTwinSearchHtmlNativeRecursiveTwinMarketplace({
  ...
  it("session mismatch on html_pack blocks overall", () => {
    const c = composeTwinSearchHtmlNativeRecursiveTwinMarketplace({
      search_query: "scaling noise",
      twin_records: search_twin_records,
      html_pack: {
        html_view: { ...html_view_input, session_id: "sess-other" },
        twin_pack: {
          twin: twin_note_input,
          market_pack: {
            market: market_input,
    ...
    expect(c.html_pack.session_aligned).toBe(false);
    expect(c.html_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_index_queried).toBe(false);
    expect(c.pdf_view_authorized).toBe(false);
    expect(c.twin_written).toBe(false);
