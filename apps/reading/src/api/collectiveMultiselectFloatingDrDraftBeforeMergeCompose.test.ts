...
describe("composeCollectiveMultiselectFloatingDrDraftBeforeMerge", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("collective multiselect + floating DR draft-before-merge pack ready", () => {
    const c = composeCollectiveMultiselectFloatingDrDraftBeforeMerge({
      multiselect: residual_multiselect,
      floating_dr_pack: residual_floating_dr_pack,
      operator_ack: true,
    });
    expect(c.multiselect.pack_ready).toBe(true);
    expect(c.floating_dr_pack.pack_ready).toBe(true);
    ...
    expect(
      formatCollectiveMultiselectFloatingDrDraftBeforeMergeSummary(c),
    ).toMatch(/pack_dispatched=false/);
  });
  ...
  it("single selection fails closed at multiselect gate", () => {
    expect(() =>
      composeCollectiveMultiselectFloatingDrDraftBeforeMerge({
        multiselect: {
          ...residual_multiselect,
    ...
    ).toThrow(/selected_instance_ids|at least 2/i);
  });
});
