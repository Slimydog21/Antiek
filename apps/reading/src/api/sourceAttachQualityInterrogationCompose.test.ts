...
describe("composeSourceAttachQualityInterrogation", () => {
  it("source quality + interrogation ready", () => {
    const c = composeSourceAttachQualityInterrogation({
      session_id: "sess-1",
    ...
    expect(c.interrogation.chase.planned_slots[0].source_families).toContain(
      "arxiv",
    );
    expect(c.authority).toBe(
      "source_attach_quality_interrogation_compose_advisory",
    );
    expect(formatSourceAttachQualityInterrogationSummary(c)).toMatch(
  ...
  it("low quality blocks source pack", () => {
    const c = composeSourceAttachQualityInterrogation({
      session_id: "sess-3",
      parent_asset_id: "a",
      requested_families: ["arxiv"],
