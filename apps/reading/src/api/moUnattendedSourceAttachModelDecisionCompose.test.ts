...
describe("composeMoUnattendedSourceAttachModelDecision", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("MO unattended + source attach model decision pack ready", () => {
    const c = composeMoUnattendedSourceAttachModelDecision({
      mo,
      research_pack: {
        sources,
        decision_pack: {
    ...
    expect(
      formatMoUnattendedSourceAttachModelDecisionSummary(c),
  ...
  it("recommend_only stage does not fully ready unattended when require_both", () => {
    const c = composeMoUnattendedSourceAttachModelDecision({
      mo: {
        ...mo,
        stage: "recommend_only" as const,
        // recommend_only may still pack_ready on MO side alone
    ...
    // research may be ready; overall depends on mo.pack_ready at recommend_only
    expect(c.live_execution_authorized).toBe(false);
    expect(c.charge_executed).toBe(false);
    expect(c.remote_fetched).toBe(false);
    if (c.mo.pack_ready === false) {
      expect(c.pack_ready).toBe(false);
    }
  });

  it("session mismatch in sources blocks research and overall", () => {
    const c = composeMoUnattendedSourceAttachModelDecision({
      mo,
      research_pack: {
