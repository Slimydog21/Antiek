...
describe("composeDraftBeforeMergeMultiSelectRecordWrite", () => {
  const multi_pack = {
    multiselect: {
      session_id: "sess-1",
      parent_asset_id: "book-1",
  ...
  it("draft-before-merge + multi-select pack ready", () => {
    const c = composeDraftBeforeMergeMultiSelectRecordWrite({
      draft_gate,
      multi_pack,
      operator_ack: true,
    });
    ...
    expect(formatDraftBeforeMergeMultiSelectRecordWriteSummary(c)).toMatch(
      /draft_written=false/,
    );
  });
  ...
  it("parent mismatch blocks", () => {
    const c = composeDraftBeforeMergeMultiSelectRecordWrite({
      draft_gate: {
        ...
        sources: draft_gate.sources.map((s) => ({
          ...s,
          parent_asset_id: "book-other",
        })),
      },
