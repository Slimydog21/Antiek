...
describe("composeMoUnattendedFullscreenDraftCollective", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("MO unattended + fullscreen draft collective pack ready", () => {
    const c = composeMoUnattendedFullscreenDraftCollective({
      mo: mo_unattended,
      fullscreen_pack,
      operator_ack: true,
    });
    ...
    expect(
      formatMoUnattendedFullscreenDraftCollectiveSummary(c),
  ...
  it("spend_consent false blocks mo readiness", () => {
    const c = composeMoUnattendedFullscreenDraftCollective({
      mo: { ...mo_unattended, spend_consent: false },
      fullscreen_pack,
      operator_ack: true,
    });
  ...
  it("unattended_ack false blocks", () => {
    const c = composeMoUnattendedFullscreenDraftCollective({
      mo: { ...mo_unattended, unattended_ack: false },
      fullscreen_pack,
      operator_ack: true,
    });
    expect(c.mo.unattended_package_ready).toBe(false);
