...
describe("composeMarketplaceFreeCompetitionDrSettingsBenchMo", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("marketplace free + competition DR settings bench MO pack ready", () => {
    const c = composeMarketplaceFreeCompetitionDrSettingsBenchMo({
      market: market_input,
      competition_pack: {
    ...
    expect(c.market.purchase_executed).toBe(false);
    expect(c.market.hosted).toBe(false);
    ...
    expect(
      formatMarketplaceFreeCompetitionDrSettingsBenchMoSummary(c),
  ...
  it("no free sha blocks market port and overall under require_both", () => {
    const c = composeMarketplaceFreeCompetitionDrSettingsBenchMo({
      market: {
        ...market_input,
  ...
  it("would_exceed competition blocks overall", () => {
    const c = composeMarketplaceFreeCompetitionDrSettingsBenchMo({
      market: market_input,
      competition_pack: {
        competition: {
