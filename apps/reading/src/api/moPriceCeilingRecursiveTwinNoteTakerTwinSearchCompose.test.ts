...
describe("composeMoPriceCeilingRecursiveTwinNoteTakerTwinSearch", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("MO price-ceiling + recursive twin note-taker twin search pack ready", () => {
    const c = composeMoPriceCeilingRecursiveTwinNoteTakerTwinSearch({
      mo: pack_outer_mo_price,
      twin_pack: pack_outer_twin_pack,
      operator_ack: true,
    });
    expect(c.mo.pack_ready).toBe(true);
    expect(c.mo.ceiling_approved).toBe(true);
    expect(c.mo.live_execution_authorized).toBe(false);
    expect(c.mo.charge_executed).toBe(false);
    expect(c.twin_pack.pack_ready).toBe(true);
    ...
    expect(
      formatMoPriceCeilingRecursiveTwinNoteTakerTwinSearchSummary(c),
  ...
  it("below-recommended ceiling without override blocks mo and overall", () => {
    const c = composeMoPriceCeilingRecursiveTwinNoteTakerTwinSearch({
      mo: {
        ...pack_outer_mo_price,
    ...
    expect(c.mo.ceiling_approved).toBe(false);
  ...
  it("zero twin-search hits nested blocks overall", () => {
    const c = composeMoPriceCeilingRecursiveTwinNoteTakerTwinSearch({
      mo: pack_outer_mo_price,
      twin_pack: {
        ...pack_outer_twin_pack,
        twin_search_pack: {
          ...pack_outer_twin_search_for_note,
          search_query: "zzzznonexistenttoken",
        },
      },
      operator_ack: true,
    });
    expect(c.twin_pack.twin_search_pack.hit_count).toBe(0);
    expect(c.pack_ready).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.twin_written).toBe(false);
    expect(c.remote_index_queried).toBe(false);
