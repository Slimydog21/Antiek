...
import {
  ...
  formatAntiekBenchRecommendMoUnattendedSummary,
...
describe("composeAntiekBenchRecommendMoUnattended", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("bench recommend + MO unattended pack ready", () => {
    const c = composeAntiekBenchRecommendMoUnattended({
      bench,
      mo_pack: {
        mo,
        research_pack: {
    ...
    expect(
      formatAntiekBenchRecommendMoUnattendedSummary(c),
  ...
  it("insufficient bench events still allows decision_tree path; overall needs both", () => {
    const c = composeAntiekBenchRecommendMoUnattended({
      bench: {
        ...bench,
        events: [
    ...
    // recommendation may be null but decision_tree can still be ready
    expect(c.live_router_authorized).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.charge_executed).toBe(false);
    // pack_ready depends on bench.pack_ready (decision_tree) + mo
    if (c.bench.pack_ready && c.mo_pack.pack_ready) {
      expect(c.pack_ready).toBe(true);
    }
  });

  it("session mismatch in research blocks mo_pack and overall", () => {
    const c = composeAntiekBenchRecommendMoUnattended({
      bench,
      mo_pack: {
        mo,
