...
describe("composeSettingsAddModelDraftFullscreenWeeklyNdMo", () => {
  it("settings + research pack ready", () => {
    const c = composeSettingsAddModelDraftFullscreenWeeklyNdMo({
      settings: SETTINGS,
    ...
    expect(
      formatSettingsAddModelDraftFullscreenWeeklyNdMoSummary(c),
  ...
  it("secret-like pending id throws", () => {
    expect(() =>
      composeSettingsAddModelDraftFullscreenWeeklyNdMo({
        settings: {
          ...SETTINGS,
  ...
  it("propose_add still never mutates inventory", () => {
    const c = composeSettingsAddModelDraftFullscreenWeeklyNdMo({
      settings: {
        ...SETTINGS,
        action: "propose_add",
    ...
    expect(c.settings.proposed_new_count).toBe(2);
    expect(c.pack_ready).toBe(true);
    expect(c.inventory_mutated).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.merge_executed).toBe(false);
