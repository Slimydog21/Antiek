 ...
 * settings add-model + ND shadow + twin presentation pack (pure).
 *
 * Operator vision: every information asset viewed as HTML — never PDF-primary —
 * while free-before-buy marketplace, BYOK settings, NotDiamond REJECT, and
 * recursive twin presentation honesty remain pure.
 *
...
import {
  composeMarketplaceFreeSettingsAddModelNdShadowTwin,
  type MarketplaceFreeSettingsAddModelNdShadowTwinCompose,
  type MarketplaceFreeSettingsAddModelNdShadowTwinInput,
} from "./marketplaceFreeSettingsAddModelNdShadowTwinCompose";

export interface HtmlNativeMarketplaceFreeSettingsNdTwinInput {
  html_view: Omit<HtmlNativeViewSessionAuthorityInput, "operator_ack">;
  market_pack: Omit<
    MarketplaceFreeSettingsAddModelNdShadowTwinInput,
    "operator_ack"
  >;
...
export interface HtmlNativeMarketplaceFreeSettingsNdTwinCompose {
  session_id: string;
  ...
  notes: string[];
  authority: "html_native_marketplace_free_settings_nd_twin_compose_advisory";
}
...
/**
 * HTML-native view authority stacked on marketplace free settings ND twin pack.
 * Never authorizes PDF primary; never purchases/hosts; ND REJECT.
 */
export function composeHtmlNativeMarketplaceFreeSettingsNdTwin(
  input: HtmlNativeMarketplaceFreeSettingsNdTwinInput,
  ...
  const market_pack = composeMarketplaceFreeSettingsAddModelNdShadowTwin({
    ...input.market_pack,
    operator_ack: input.operator_ack,
  });
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — HTML-native view + marketplace free settings ND twin pack ready; still pure",
    );
  } else {
    notes.push(
...
export function formatHtmlNativeMarketplaceFreeSettingsNdTwinSummary(
