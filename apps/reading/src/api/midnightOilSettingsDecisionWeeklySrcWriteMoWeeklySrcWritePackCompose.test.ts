...
describe("composeMidnightOilSettingsDecisionWeeklySrcWriteMoWeeklySrcWritePack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("Midnight Oil + settings decision competition weekly pack ready", () => {
    const c = composeMidnightOilSettingsDecisionWeeklySrcWriteMoWeeklySrcWritePack({
    ...
    expect(
      formatMidnightOilSettingsDecisionWeeklySrcWriteMoWeeklySrcWritePackSummary(c),
  ...
  it("price_ceiling_ack false blocks MO unattended pack", () => {
    const c = composeMidnightOilSettingsDecisionWeeklySrcWriteMoWeeklySrcWritePack({
      mo: {
        ...residual_mo2_outer_mo,
  ...
  it("recommend_only stage still pure (no live execution)", () => {
    const c = composeMidnightOilSettingsDecisionWeeklySrcWriteMoWeeklySrcWritePack({
      mo: {
        ...residual_mo2_outer_mo,
        stage: "recommend_only" as const,
    ...
    expect(c.charge_executed).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
    // recommend_only may still compose; pack_ready depends on MO gates
    expect(c.secrets_stored).toBe(false);
  });
});
