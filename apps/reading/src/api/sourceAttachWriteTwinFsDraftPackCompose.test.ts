...
describe("composeSourceAttachWriteTwinFsDraftPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("source attach + write twin fs draft pack ready", () => {
    const c = composeSourceAttachWriteTwinFsDraftPack({
    ...
    expect(c.authority).toBe("source_attach_write_twin_fs_draft_pack_compose_advisory");
    expect(formatSourceAttachWriteTwinFsDraftPackSummary(c)).toMatch(/remote_fetched=false|pdf_primary=false|REJECT/);
  });
