...
import {
  ...
  formatCollectiveMultiselectFloatingDrWorkstationMoWeeklyPackSummary,
} from "./collectiveMultiselectFloatingDrWorkstationMoWeeklyPackCompose";

describe("composeCollectiveMultiselectFloatingDrWorkstationMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_cm_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_cm_outer_decision_pack = {
    decision: residual_cm_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_cm_outer_twin_records,
      html_pack: residual_cm_outer_html_pack,
    },
  };

  const residual_cm_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_cm_outer_record_pack = {
    session_id: "sess-1",
    items: residual_cm_outer_items,
    decision_pack: residual_cm_outer_decision_pack,
  };

  const residual_cm_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_cm_outer_floating_pack = {
    highlight_launch: residual_cm_outer_highlight_launch,
    record_pack: residual_cm_outer_record_pack,
  };
  ...
  it("collective multiselect + floating DR workstation MO weekly pack ready", () => {
    const c = composeCollectiveMultiselectFloatingDrWorkstationMoWeeklyPack({
    ...
    expect(
      formatCollectiveMultiselectFloatingDrWorkstationMoWeeklyPackSummary(c),
  ...
  it("parent mismatch blocks pack_ready", () => {
    const c = composeCollectiveMultiselectFloatingDrWorkstationMoWeeklyPack({
      multiselect: {
        ...
        members: residual_cm_outer_multiselect.members.map((m) => ({
          ...m,
          parent_asset_id: "book-other",
        })),
      },
