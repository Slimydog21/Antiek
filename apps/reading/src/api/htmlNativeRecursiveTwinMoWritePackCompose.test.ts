...
describe("composeHtmlNativeRecursiveTwinMoWritePack", () => {
  it("html view + twin mo pack ready", () => {
    const c = composeHtmlNativeRecursiveTwinMoWritePack({
      html_view: HTML_VIEW,
      twin_mo: {
        twin: TWIN,
        mo_write: {
    ...
    expect(c.twin_mo.pack_ready).toBe(true);
    ...
    expect(formatHtmlNativeRecursiveTwinMoWritePackSummary(c)).toMatch(
      /pdf_primary=false/,
    );
  });
  ...
  it("pdf claim never authorizes pdf primary", () => {
    const c = composeHtmlNativeRecursiveTwinMoWritePack({
      html_view: {
        ...HTML_VIEW,
        claimed_format: "pdf",
