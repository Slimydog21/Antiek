/**
 * Twin intelligent search residual over HTML-native marketplace MO weekly pack
 * (short residual: TwinSearchHtmlNativeMarketplaceMoWeeklyPack — FS 255).
 *
...
import {
  composeHtmlNativeMarketplaceMoWeeklyPack,
  type HtmlNativeMarketplaceMoWeeklyPackCompose,
  type HtmlNativeMarketplaceMoWeeklyPackInput,
} from "./htmlNativeMarketplaceMoWeeklyPackCompose";

export interface TwinSearchHtmlNativeMarketplaceMoWeeklyPackInput {
  search_query: string;
  twin_records: TwinSearchRecord[];
  search_limit?: number;
  html_pack: Omit<HtmlNativeMarketplaceMoWeeklyPackInput, "operator_ack">;
...
export interface TwinSearchHtmlNativeMarketplaceMoWeeklyPackCompose {
  week_id: string;
  session_id: string;
  ...
  notes: string[];
  authority: "twin_search_html_native_marketplace_mo_weekly_pack_compose_advisory";
}
...
/**
 * Twin substrate search stacked on HTML-native marketplace MO weekly pack.
 * Never remote-indexes; never purchases; never PDF-primary; ND REJECT.
 */
export function composeTwinSearchHtmlNativeMarketplaceMoWeeklyPack(
  input: TwinSearchHtmlNativeMarketplaceMoWeeklyPackInput,
  ...
  const html_pack = composeHtmlNativeMarketplaceMoWeeklyPack({
    ...input.html_pack,
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — twin search + HTML-native marketplace MO weekly ready; still pure",
    );
  } else {
    notes.push(
...
export function formatTwinSearchHtmlNativeMarketplaceMoWeeklyPackSummary(
