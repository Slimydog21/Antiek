...
import {
  ...
  formatDraftBeforeMergeCollectiveFloatingDrMoWeeklyPackSummary,
} from "./draftBeforeMergeCollectiveFloatingDrMoWeeklyPackCompose";

describe("composeDraftBeforeMergeCollectiveFloatingDrMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const residual_db_outer_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_db_outer_decision_pack = {
    decision: residual_db_outer_decision,
    twin_search_pack: {
      search_query: "scaling noise",
      twin_records: residual_db_outer_twin_records,
      html_pack: residual_db_outer_html_pack,
    },
  };

  const residual_db_outer_items = [
    {
      record_id: "r1",
      kind: "insight" as const,
      text: "scaling holds under noise in compute-optimal regimes",
  ...
  const residual_db_outer_record_pack = {
    session_id: "sess-1",
    items: residual_db_outer_items,
    decision_pack: residual_db_outer_decision_pack,
  };

  const residual_db_outer_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_db_outer_floating_pack = {
    highlight_launch: residual_db_outer_highlight_launch,
    record_pack: residual_db_outer_record_pack,
  };

  const residual_db_outer_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_db_outer_collective_pack = {
    multiselect: residual_db_outer_multiselect,
    floating_pack: residual_db_outer_floating_pack,
  };

  it("draft-before-merge + collective floating DR MO weekly pack ready", () => {
    const c = composeDraftBeforeMergeCollectiveFloatingDrMoWeeklyPack({
    ...
    expect(c.draft_gate.draft_written).toBe(false);
    expect(c.draft_gate.merge_executed).toBe(false);
    ...
    expect(
      formatDraftBeforeMergeCollectiveFloatingDrMoWeeklyPackSummary(c),
  ...
  it("session mismatch on draft_gate blocks pack_ready", () => {
    const c = composeDraftBeforeMergeCollectiveFloatingDrMoWeeklyPack({
      draft_gate: {
        ...residual_db_outer_draft_gate,
  ...
  it("parent mismatch on draft sources blocks pack_ready", () => {
    const c = composeDraftBeforeMergeCollectiveFloatingDrMoWeeklyPack({
      draft_gate: {
        ...residual_db_outer_draft_gate,
        parent_asset_id: "book-other",
        sources: residual_db_outer_draft_gate.sources.map((s) => ({
          ...s,
          parent_asset_id: "book-other",
        })),
      },
