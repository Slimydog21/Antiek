...
import {
  ...
  formatNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpackSummary,
...
describe("composeNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack", () => {
const WEEKLY_ND = {
  weekly_learn: {
    week_id: "2026-W28",
...
const residual_ts10_tip_twin_records = [
  ...
  const residual_sd_competition_pack = {
  ...
  const residual_mo_settings_pack = {
    settings: residual_sd_settings,
    competition_pack: residual_sd_competition_pack,
  };

  const residual_mkt_tip_market = {
    title: "Scaling Laws Book",
    account_id: "acct-1",
    free_copy_available: true as boolean | null,
    free_html_projection_sha: "sha-free-1",
  ...
  const residual_mkt_tip_mo_pack = {
    mo: residual_mo_tip,
    settings_pack: residual_mo_settings_pack,
  };

  const residual_hn_tip_html_view = {
    session_id: "sess-1",
    asset_id: "book-1",
    html_projection_sha: "sha-free-1",
    view_requested: true,
  ...
  const residual_hn_tip_market_pack = {
    market: residual_mkt_tip_market,
    mo_pack: residual_mkt_tip_mo_pack,
  };

  const residual_ts_tip_html_pack = {
    html_view: residual_hn_tip_html_view,
    market_pack: residual_hn_tip_market_pack,
  };

  const residual_ts_tip_twin_records = [
    {
      twin_id: "twin-book-1",
      parent_asset_id: "book-1",
      insights: ["Scaling laws hold under noise in compute-optimal regimes"],
  ...
  const residual_md_tip_decision = {
    models: [
      {
        model_id: "gpt-5.5",
        tier: "frontier",
  ...
  const residual_md_tip_twin_search_pack = {
    search_query: "scaling laws",
    twin_records: residual_ts_tip_twin_records,
    html_pack: residual_ts_tip_html_pack,
  };

  const residual_fdr_tip_items = [
    {
      record_id: "r-ws-1",
      kind: "insight" as const,
      text: "Workstation residual records scaling laws under noise",
    },
    {
      record_id: "r-ws-2",
      kind: "question" as const,
      text: "What residual gaps remain in the research loop?",
    },
  ];

  const residual_fdr_tip_decision_pack = {
    decision: residual_md_tip_decision,
    twin_search_pack: residual_md_tip_twin_search_pack,
  };

  const residual_fdr_tip_record_pack = {
    session_id: "sess-1",
    items: residual_fdr_tip_items,
    decision_pack: residual_fdr_tip_decision_pack,
  };

  const residual_fdr_tip_highlight_launch = {
    parent_asset_id: "book-1",
    highlight: "scaling laws claim under noise",
    gated: false,
    preferred_view_mode: "floating" as const,
  ...
  const residual_col_tip_floating_pack = {
    highlight_launch: residual_fdr_tip_highlight_launch,
    record_pack: residual_fdr_tip_record_pack,
  };

  const residual_col_tip_multiselect = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const residual_dbm_tip_collective_pack = {
    multiselect: residual_col_tip_multiselect,
    floating_pack: residual_col_tip_floating_pack,
  };

  const residual_dbm_tip_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent body on scaling laws</p>",
    sources: [
  ...
  const residual_fs_tip_draft_pack = {
    draft_gate: residual_dbm_tip_draft_gate,
    collective_pack: residual_dbm_tip_collective_pack,
  };

  const residual_fs_tip_fullscreen = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    highlight: "scaling laws claim under noise",
    gated: false,
  ...
  const residual_wt_tip_fullscreen_pack = {
    fullscreen: residual_fs_tip_fullscreen,
    draft_pack: residual_fs_tip_draft_pack,
  };

  const residual_wt_tip_write = {
    session_id: "sess-1",
    draft_id: "draft-1",
    parent_asset_id: "book-1",
    twin_slices: [
  ...
  const residual_sa_tip_write_pack = {
    write: residual_wt_tip_write,
    fullscreen_pack: residual_wt_tip_fullscreen_pack,
  };

  const residual_sa_tip_sources = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    requested_families: ["arxiv", "substack"] as const,
    sources: [
  ...
  const residual_ab_tip_source_pack = {
    sources: residual_sa_tip_sources,
    write_pack: residual_sa_tip_write_pack,
  };

  const residual_ab_tip_weekly_learn = {
    week_id: "2026-W28",
    min_events_per_task: 2,
    events: [
      {
  ...
  const residual_rt_tip_weekly_pack = {
    weekly_learn: residual_ab_tip_weekly_learn,
    source_pack: residual_ab_tip_source_pack,
  };

  const residual_rt_tip_twin = {
    parent_asset_id: "book-1",
    source_excerpt:
      "<p>Scaling laws hold under noise in compute-optimal regimes.</p>",
    focus_questions: ["Where does it break?", "What residual gaps?"],
  ...
  const residual_rt_tip_presentation = {
    view_mode: "side_panel" as const,
    open_requested: true,
    merge_to_parent_preview: false,
    presented_insights: [
  ...
  const residual_nds_tip_twin_presentation = {
    twin: residual_rt_tip_twin,
    presentation: residual_rt_tip_presentation,
    weekly_pack: residual_rt_tip_weekly_pack,
  };

  const residual_nds_tip_shadow = {
    selected_model_id: "gpt-5.5",
    nd_recommended_model_id: "claude-opus",
    kill_switch_on: true,
    confidence: 0.72,
  ...
  it("ND shadow REJECT + recursive twin presentation pack ready", () => {
    const c = composeNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
      nd_shadow: residual_nds_tip_shadow,
      twin_presentation: residual_nds_tip_twin_presentation,
      operator_ack: true,
    });
    ...
    expect(c.twin_presentation.pack_ready).toBe(true);
    ...
    expect(c.authority).toBe("nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_mkt_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_ts_hn_market_mo_sd_cd_nds_rt_ab_sa_wt_fs_dbm_col_fdr_ws_md_twin_search_html_native_mow12_mpack_compose_advisory");
    expect(formatNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpackSummary(c)).toMatch(/REJECT|pack_ready=true/);
  });
  ...
  it("session misalign blocks pack under require_both", () => {
    const c = composeNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMktMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTsHnMarketMoSdCdNdsRtAbSaWtFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
      nd_shadow: residual_nds_tip_shadow,
      twin_presentation: {
        ...residual_nds_tip_twin_presentation,
        weekly_pack: {
          ...residual_rt_tip_weekly_pack,
          source_pack: {
            ...residual_ab_tip_source_pack,
            sources: { ...residual_sa_tip_sources, session_id: "sess-OTHER" },
          },
        },
      },
      operator_ack: true,
    });
    expect(c.twin_presentation.session_aligned).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.twin_written).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.backlog_mutated).toBe(false);
