...
describe("composeSourceAttachSettingsDecisionMo", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("source attach + settings decision MO pack ready", () => {
    const c = composeSourceAttachSettingsDecisionMo({
    ...
    expect(c.settings_mo.pack_ready).toBe(true);
    ...
    expect(
      formatSourceAttachSettingsDecisionMoSummary(c),
  ...
  it("would_exceed on settings blocks overall", () => {
    const c = composeSourceAttachSettingsDecisionMo({
      sources: sources_input,
      settings_mo: {
        ...settings_mo,
    ...
    expect(c.settings_mo.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.live_router_authorized).toBe(false);
  });
