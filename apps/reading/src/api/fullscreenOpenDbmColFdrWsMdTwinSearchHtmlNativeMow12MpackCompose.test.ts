...
describe("composeFullscreenOpenDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack", () => {
const WEEKLY_ND = {
  weekly_learn: {
    week_id: "2026-W28",
...
const residual_ts10_tip_twin_records = [
  ...
  it("fullscreen + draft-before-merge pack ready", () => {
    const c = composeFullscreenOpenDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
    ...
    expect(formatFullscreenOpenDbmColFdrWsMdTwinSearchHtmlNativeMow12MpackSummary(c)).toMatch(/REJECT|pack_ready=true/);
  });
  ...
  it("session misalign blocks pack under require_both", () => {
    const c = composeFullscreenOpenDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
      fullscreen: { ...residual_fs_fullscreen, session_id: "sess-OTHER" },
      draft_pack: residual_fs_draft_pack,
      operator_ack: true,
    });
