...
describe("composeWriteModeTwinFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack", () => {
const WEEKLY_ND = {
  weekly_learn: {
    week_id: "2026-W28",
...
const residual_ts10_tip_twin_records = [
  ...
  it("write twin + fullscreen pack ready", () => {
    const c = composeWriteModeTwinFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
    ...
    expect(formatWriteModeTwinFsDbmColFdrWsMdTwinSearchHtmlNativeMow12MpackSummary(c)).toMatch(/REJECT|pack_ready=true/);
  });
  ...
  it("session misalign blocks pack under require_both", () => {
    const c = composeWriteModeTwinFsDbmColFdrWsMdTwinSearchHtmlNativeMow12Mpack({
      write: { ...residual_wt_write, session_id: "sess-OTHER" },
      fullscreen_pack: residual_wt_fullscreen_pack,
      operator_ack: true,
    });
