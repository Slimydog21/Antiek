...
describe("composeWriteModeTwinCollectiveFullscreenMoUnattendedNdTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("write twin collective + fullscreen MO unattended ND twin pack ready", () => {
    const c = composeWriteModeTwinCollectiveFullscreenMoUnattendedNdTwin({
    ...
    expect(
      formatWriteModeTwinCollectiveFullscreenMoUnattendedNdTwinSummary(c),
  ...
  it("spend_consent false blocks fullscreen pack overall", () => {
    const c = composeWriteModeTwinCollectiveFullscreenMoUnattendedNdTwin({
      write: pack_outer_write,
      fullscreen_pack: {
        ...pack_outer_fullscreen_pack,
        mo_pack: {
          ...pack_outer_mo_pack,
          mo: { ...pack_outer_mo, spend_consent: false },
        },
      },
      operator_ack: true,
    });
    expect(c.fullscreen_pack.mo_pack.mo.unattended_package_ready).toBe(false);
    expect(c.fullscreen_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.draft_written).toBe(false);
    expect(c.analysis_written).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
