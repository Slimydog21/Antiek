...
import {
  ...
  formatAntiekBenchSourceAttachSettingsMoSummary,
...
describe("composeAntiekBenchSourceAttachSettingsMo", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("bench recommend + source attach settings MO pack ready", () => {
    const c = composeAntiekBenchSourceAttachSettingsMo({
      bench: bench_input,
      source_pack,
      operator_ack: true,
    });
    expect(c.bench.pack_ready).toBe(true);
    expect(c.bench.recommendation?.recommended_model_id).toBe("gpt-5.5");
    ...
    expect(
      formatAntiekBenchSourceAttachSettingsMoSummary(c),
  ...
  it("source session mismatch blocks overall", () => {
    const c = composeAntiekBenchSourceAttachSettingsMo({
      bench: bench_input,
      source_pack: {
        ...source_pack,
        sources: { ...sources_input, session_id: "sess-other" },
      },
      operator_ack: true,
    });
    expect(c.source_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.suite_rewritten).toBe(false);
  });

  it("high min_events_for_recommendation yields null rec and still pure", () => {
    const c = composeAntiekBenchSourceAttachSettingsMo({
      bench: {
        ...bench_input,
    ...
    expect(c.bench.recommendation).toBeNull();
    // pack_ready may still be true if decision_tree path allows no-data
    expect(c.live_router_authorized).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
