...
import {
  ...
  formatNdShadowRecursiveTwinWeeklyPackSummary,
} from "./ndShadowRecursiveTwinWeeklyPackCompose";

describe("composeNdShadowRecursiveTwinWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("ND shadow REJECT + twin presentation weekly pack ready", () => {
    const c = composeNdShadowRecursiveTwinWeeklyPack({
    ...
    expect(c.authority).toBe("nd_shadow_recursive_twin_weekly_pack_compose_advisory");
    expect(formatNdShadowRecursiveTwinWeeklyPackSummary(c)).toMatch(/live_router_authorized=false/);
  });
  ...
  it("open_requested false blocks twin presentation and pack", () => {
    const c = composeNdShadowRecursiveTwinWeeklyPack({
      nd_shadow: residual_nd_outer_nd_shadow,
      twin_presentation: {
        ...residual_nd_outer_twin_presentation,
        presentation: {
          ...residual_rtp_outer_presentation,
          open_requested: false,
        },
      },
      operator_ack: true,
    });
    expect(c.twin_presentation.presentation.presentation_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });

  it("kill switch on still allows pack when twin presentation ready (shadow advisory)", () => {
    const c = composeNdShadowRecursiveTwinWeeklyPack({
      nd_shadow: {
        ...residual_nd_outer_nd_shadow,
        kill_switch_on: true,
      },
      twin_presentation: residual_nd_outer_twin_presentation,
      operator_ack: true,
