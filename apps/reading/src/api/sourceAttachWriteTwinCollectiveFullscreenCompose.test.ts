...
import {
  ...
  formatSourceAttachWriteTwinCollectiveFullscreenSummary,
} from "./sourceAttachWriteTwinCollectiveFullscreenCompose";

describe("composeSourceAttachWriteTwinCollectiveFullscreen", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("source attach + write twin collective fullscreen pack ready", () => {
    const c = composeSourceAttachWriteTwinCollectiveFullscreen({
    ...
    expect(
      formatSourceAttachWriteTwinCollectiveFullscreenSummary(c),
  ...
  it("empty sources blocks attach_ready and pack", () => {
    const c = composeSourceAttachWriteTwinCollectiveFullscreen({
      sources: {
        ...residual_sources,
        sources: [],
    ...
    expect(c.attach_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.pdf_primary).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
