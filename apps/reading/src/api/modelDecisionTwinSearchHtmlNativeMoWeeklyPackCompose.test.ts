...
describe("composeModelDecisionTwinSearchHtmlNativeMoWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("model decision + twin search HTML-native marketplace MO weekly pack ready", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMoWeeklyPack({
    ...
    expect(
      formatModelDecisionTwinSearchHtmlNativeMoWeeklyPackSummary(c),
  ...
  it("operator_ack false blocks pack_ready", () => {
    ...
    expect(c.decision.decision_ready).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.secrets_stored).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });
