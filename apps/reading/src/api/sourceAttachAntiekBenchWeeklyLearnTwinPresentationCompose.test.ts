...
describe("composeSourceAttachAntiekBenchWeeklyLearnTwinPresentation", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("arxiv/substack attach + weekly learn twin presentation pack ready", () => {
    const c = composeSourceAttachAntiekBenchWeeklyLearnTwinPresentation({
      sources: pack_outer_sources,
      weekly_pack: pack_outer_weekly_pack,
      operator_ack: true,
    });
    expect(c.sources.pack_ready).toBe(true);
    ...
    expect(
      formatSourceAttachAntiekBenchWeeklyLearnTwinPresentationSummary(c),
  ...
  it("would_exceed on source quality blocks sources pack", () => {
    const c = composeSourceAttachAntiekBenchWeeklyLearnTwinPresentation({
      sources: {
        ...pack_outer_sources,
        would_exceed: true,
    ...
    expect(c.sources.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.backlog_mutated).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
