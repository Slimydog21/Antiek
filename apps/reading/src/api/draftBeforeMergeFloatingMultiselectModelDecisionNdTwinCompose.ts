 ...
 * + twin intelligent search HTML-native free marketplace + settings + ND REJECT + twin presentation remain pure.
 *
...
import {
  ...
  type FloatingMultiselectModelDecisionNdTwinInput,
} from "./floatingMultiselectModelDecisionNdTwinCompose";

export interface DraftBeforeMergeFloatingMultiselectModelDecisionNdTwinInput {
  draft_gate: Omit<FloatingDraftBeforeFullMergeGateInput, "operator_ack">;
  multi_pack: Omit<
    FloatingMultiselectModelDecisionNdTwinInput,
    "operator_ack"
  >;
...
export interface DraftBeforeMergeFloatingMultiselectModelDecisionNdTwinCompose {
  session_id: string;
  ...
  notes: string[];
  authority: "draft_before_merge_floating_multiselect_model_decision_nd_twin_compose_advisory";
}
 ...
 */
export function composeDraftBeforeMergeFloatingMultiselectModelDecisionNdTwin(
  input: DraftBeforeMergeFloatingMultiselectModelDecisionNdTwinInput,
  ...
  const multi_pack = composeFloatingMultiselectModelDecisionNdTwin({
    ...input.multi_pack,
    operator_ack: input.operator_ack,
  });
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — draft-before-merge + floating multi-select model decision ND twin ready; still pure",
    );
  } else {
    notes.push(
...
export function formatDraftBeforeMergeFloatingMultiselectModelDecisionNdTwinSummary(
