...
describe("composeMoUnattendedSourceAttachAntiekBenchRewrite", () => {
  ...
  it("MO unattended + source attach rewrite pack ready", () => {
    const c = composeMoUnattendedSourceAttachAntiekBenchRewrite({
      mo: mo_ready,
      research_pack,
      operator_ack: true,
    });
    ...
    expect(
      formatMoUnattendedSourceAttachAntiekBenchRewriteSummary(c),
  ...
  it("low ceiling blocks MO under unattended_pack", () => {
    const c = composeMoUnattendedSourceAttachAntiekBenchRewrite({
      mo: {
        ...mo_ready,
  ...
  it("zero rewrite proposals blocks overall", () => {
    ...
    expect(c.research_pack.rewrite_pack.proposal_count).toBe(0);
    expect(c.pack_ready).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.charge_executed).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
