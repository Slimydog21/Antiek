...
import {
  ...
  formatNdShadowRecursiveTwinWeeklySrcWriteMoWeeklySrcWritePackSummary,
} from "./ndShadowRecursiveTwinWeeklySrcWriteMoWeeklySrcWritePackCompose";

describe("composeNdShadowRecursiveTwinWeeklySrcWriteMoWeeklySrcWritePack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("ND shadow REJECT + recursive twin weekly MO weekly src write pack ready", () => {
    const c = composeNdShadowRecursiveTwinWeeklySrcWriteMoWeeklySrcWritePack({
    ...
    expect(
      formatNdShadowRecursiveTwinWeeklySrcWriteMoWeeklySrcWritePackSummary(c),
  ...
  it("kill_switch off still REJECT and blocks pack under require_both honesty", () => {
    const c = composeNdShadowRecursiveTwinWeeklySrcWriteMoWeeklySrcWritePack({
      nd_shadow: {
        ...residual_nds_outer_nd_shadow,
        kill_switch_on: false,
      },
      twin_presentation: residual_nds_outer_twin_presentation,
      operator_ack: true,
    });
    // Platform §16: production router always REJECT even if kill switch off
    expect(c.production_router_verdict).toBe("REJECT");
    expect(c.live_router_authorized).toBe(false);
    expect(c.nd_shadow.production_router_verdict).toBe("REJECT");
  });
