...
describe("composeMoPriceCeilingWriteTwinSettingsDraft", () => {
  it("mo recommend + research write ready", () => {
    const c = composeMoPriceCeilingWriteTwinSettingsDraft({
      mo: MO,
      research_write: {
        write: WRITE,
        settings_research: {
    ...
    expect(c.research_write.pack_ready).toBe(true);
    ...
    expect(formatMoPriceCeilingWriteTwinSettingsDraftSummary(c)).toMatch(
  ...
  it("approve_ceiling stage still never charges", () => {
    const rec = composeMoPriceCeilingWriteTwinSettingsDraft({
    ...
    const recommended = rec.mo.recommend.recommended_ceiling_usd!;
    const c = composeMoPriceCeilingWriteTwinSettingsDraft({
      mo: {
        ...MO,
