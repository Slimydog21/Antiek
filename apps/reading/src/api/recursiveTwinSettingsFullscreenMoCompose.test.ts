...
describe("composeRecursiveTwinSettingsFullscreenMo", () => {
  const settings_pack = {
    settings: {
      models: [
        { model_id: "gpt-5.5", provider: "openai" },
  ...
  it("recursive twin + settings fullscreen MO pack ready", () => {
    const c = composeRecursiveTwinSettingsFullscreenMo({
      twin,
      settings_pack,
      operator_ack: true,
    });
    ...
    expect(formatRecursiveTwinSettingsFullscreenMoSummary(c)).toMatch(
      /twin_written=false/,
    );
  });
  ...
  it("empty source_excerpt fails closed", () => {
    expect(() =>
      composeRecursiveTwinSettingsFullscreenMo({
