 ...
 * settings add-model + ND shadow + competition DR + MO pack (pure).
 *
 * Operator vision: every information asset viewed as HTML — never PDF-primary —
 * while free-before-buy marketplace, BYOK settings, NotDiamond REJECT, competition
 * DR quality, and Midnight Oil unattended honesty remain pure.
 *
 * pdf_view_authorized / pdf_primary always false.
 * purchase_executed / hosted / twin_written always false.
...
import {
  ...
  type MarketplaceFreeSettingsAddModelNdShadowInput,
} from "./marketplaceFreeSettingsAddModelNdShadowCompose";

export interface HtmlNativeMarketplaceFreeSettingsNdShadowInput {
  html_view: Omit<HtmlNativeViewSessionAuthorityInput, "operator_ack">;
  market_pack: Omit<
    MarketplaceFreeSettingsAddModelNdShadowInput,
    "operator_ack"
  >;
  operator_ack: boolean;
  /**
   * When true (default), require html_view.pack_ready AND market_pack.pack_ready,
   * session/parent alignment, and PDF honesty flags.
   */
  require_both?: boolean;
}

export interface HtmlNativeMarketplaceFreeSettingsNdShadowCompose {
  session_id: string;
  ...
  market_pack: MarketplaceFreeSettingsAddModelNdShadowCompose;
  ...
  notes: string[];
  authority: "html_native_marketplace_free_settings_nd_shadow_compose_advisory";
}
...
/**
 * HTML-native view authority stacked on marketplace free settings ND pack.
 * Never authorizes PDF primary; never purchases/hosts; ND REJECT.
 */
export function composeHtmlNativeMarketplaceFreeSettingsNdShadow(
  input: HtmlNativeMarketplaceFreeSettingsNdShadowInput,
  ...
  const notes: string[] = [
    ...
    "purchase_executed=false · hosted=false · twin_written=false",
    "production_router_verdict=REJECT",
  ];
  ...
  const market_pack = composeMarketplaceFreeSettingsAddModelNdShadow({
    ...input.market_pack,
    operator_ack: input.operator_ack,
  });
  ...
  // Parent alignment: HTML asset under view should match market pack parent/asset.
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — HTML-native view + marketplace free settings ND pack ready; still pure",
    );
  } else {
    notes.push(
...
export function formatHtmlNativeMarketplaceFreeSettingsNdShadowSummary(
  c: HtmlNativeMarketplaceFreeSettingsNdShadowCompose,
): string {
  return (
    ...
    `path=${c.market_pack.market.path} · week=${c.week_id} · ` +
