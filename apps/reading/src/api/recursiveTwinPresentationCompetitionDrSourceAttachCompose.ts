 ...
 * source-attach + Antiek-bench recommend + MO unattended pack (pure).
 *
 ...
 * competition DR + source-attach Antiek-bench recommend residual pack.
 *
...
import {
  ...
  type CompetitionDrSourceAttachAntiekBenchRecommendInput,
} from "./competitionDrSourceAttachAntiekBenchRecommendCompose";
...
export interface RecursiveTwinPresentationCompetitionDrSourceAttachInput {
  twin: Omit<RecursiveTwinNoteTakerInput, "operator_ack">;
  presentation: TwinPresentationSurfaceInput;
  competition_pack: Omit<
    CompetitionDrSourceAttachAntiekBenchRecommendInput,
    "operator_ack"
  >;
  operator_ack: boolean;
  /**
   * When true (default), require presentation_ready AND competition_pack.pack_ready
   * and parent_asset_id alignment.
   */
  require_both?: boolean;
}

export interface RecursiveTwinPresentationCompetitionDrSourceAttachCompose {
  ...
  competition_pack: CompetitionDrSourceAttachAntiekBenchRecommendCompose;
  ...
  notes: string[];
  authority: "recursive_twin_presentation_competition_dr_source_attach_compose_advisory";
}
...
/**
 * Twin note-taker presentation stacked on competition DR source-attach pack.
 * Never writes twin; never merges; never purchases; ND REJECT.
 */
export function composeRecursiveTwinPresentationCompetitionDrSourceAttach(
  input: RecursiveTwinPresentationCompetitionDrSourceAttachInput,
  ...
  const competition_pack = composeCompetitionDrSourceAttachAntiekBenchRecommend(
    {
  ...
  const operator_id = requireNonEmpty(
    competition_pack.operator_id,
    "operator_id",
  );
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — twin presentation + competition DR source-attach pack ready; still pure",
    );
  } else {
    notes.push(
...
export function formatRecursiveTwinPresentationCompetitionDrSourceAttachSummary(
  c: RecursiveTwinPresentationCompetitionDrSourceAttachCompose,
): string {
  return (
    ...
    `twin_written=false · merge_executed=false · purchase_executed=false · remote_fetched=false`
  );
}
