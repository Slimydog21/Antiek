...
describe("composeAntiekBenchWeeklySrcWritePack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("weekly learn + source attach write twin fs draft pack ready", () => {
    const c = composeAntiekBenchWeeklySrcWritePack({
    ...
    expect(c.authority).toBe("antiek_bench_weekly_src_write_pack_compose_advisory");
    expect(formatAntiekBenchWeeklySrcWritePackSummary(c)).toMatch(/backlog_mutated=false/);
  });
  ...
  it("session mismatch blocks pack_ready", () => {
    const c = composeAntiekBenchWeeklySrcWritePack({
      weekly_learn: residual_bench_outer_weekly_learn,
      source_pack: {
        ...residual_bench_outer_source_pack,
        sources: {
          ...residual_src_outer_sources,
