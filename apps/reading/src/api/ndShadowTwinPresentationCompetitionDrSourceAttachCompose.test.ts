...
describe("composeNdShadowTwinPresentationCompetitionDrSourceAttach", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("ND shadow REJECT + twin presentation source-attach pack ready", () => {
    const c = composeNdShadowTwinPresentationCompetitionDrSourceAttach({
    ...
    expect(
      formatNdShadowTwinPresentationCompetitionDrSourceAttachSummary(c),
    ).toMatch(/REJECT/);
  });
  ...
  it("open_requested false blocks twin presentation readiness", () => {
    const c = composeNdShadowTwinPresentationCompetitionDrSourceAttach({
      nd_shadow: outer_nd_shadow_input,
      twin_presentation: {
        ...twin_presentation,
        presentation: {
          ...outer_presentation_input,
          open_requested: false,
        },
      },
      operator_ack: true,
    });
    expect(c.twin_presentation.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });

  it("parent mismatch blocks twin presentation pack", () => {
    const c = composeNdShadowTwinPresentationCompetitionDrSourceAttach({
      nd_shadow: outer_nd_shadow_input,
      twin_presentation: {
        ...twin_presentation,
