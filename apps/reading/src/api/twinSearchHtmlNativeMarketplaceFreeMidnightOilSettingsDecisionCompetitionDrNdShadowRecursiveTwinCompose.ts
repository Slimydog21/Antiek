 ...
 * Midnight Oil + settings decision pack (pure).
 *
 ...
 * marketplace + MO honesty — without remote index, twin writes, or purchase.
 *
...
import {
  ...
  type HtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinInput,
} from "./htmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose";

export interface TwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinInput {
  search_query: string;
  twin_records: TwinSearchRecord[];
  search_limit?: number;
  html_pack: Omit<
    HtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinInput,
    "operator_ack"
  >;
...
export interface TwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinCompose {
  week_id: string;
  session_id: string;
  ...
  notes: string[];
  authority: "twin_search_html_native_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_compose_advisory";
}
...
/**
 * Twin substrate search stacked on HTML-native marketplace free MO settings.
 * Never remote-indexes; never purchases; never PDF-primary; ND REJECT.
 */
export function composeTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin(
  input: TwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinInput,
  ...
  const html_pack = composeHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
    ...input.html_pack,
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — twin search + HTML-native marketplace free MO ready; still pure",
    );
  } else {
    notes.push(
...
export function formatTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinSummary(
