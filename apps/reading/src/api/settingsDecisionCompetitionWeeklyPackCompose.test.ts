...
describe("composeSettingsDecisionCompetitionWeeklyPack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("settings decision + competition DR ND shadow pack ready", () => {
    const c = composeSettingsDecisionCompetitionWeeklyPack({
    ...
    expect(c.authority).toBe("settings_decision_competition_weekly_pack_compose_advisory");
    expect(formatSettingsDecisionCompetitionWeeklyPackSummary(c)).toMatch(/secrets_stored=false/);
  });
  ...
  it("week mismatch blocks pack_ready", () => {
    const c = composeSettingsDecisionCompetitionWeeklyPack({
      settings: {
        ...residual_settings_outer_settings,
        week_id: "2026-W99",
      },
      competition_pack: residual_settings_outer_competition_pack,
      operator_ack: true,
  ...
  it("would_exceed on competition blocks overall pack_ready", () => {
    const c = composeSettingsDecisionCompetitionWeeklyPack({
      settings: residual_settings_outer_settings,
      competition_pack: {
        competition: {
          ...residual_comp_outer_competition,
          would_exceed: true,
        },
        nd_pack: residual_comp_outer_nd_pack,
      },
