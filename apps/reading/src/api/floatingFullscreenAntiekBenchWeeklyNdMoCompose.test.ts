...
describe("composeFloatingFullscreenAntiekBenchWeeklyNdMo", () => {
  it("fullscreen + weekly ND multi-select ready", () => {
    const c = composeFloatingFullscreenAntiekBenchWeeklyNdMo({
      fullscreen: {
        session_id: "sess-1",
        parent_asset_id: "book-1",
    ...
    expect(c.weekly_nd.pack_ready).toBe(true);
    ...
    expect(formatFloatingFullscreenAntiekBenchWeeklyNdMoSummary(c)).toMatch(
  ...
  it("sparse weekly events block pack", () => {
    const c = composeFloatingFullscreenAntiekBenchWeeklyNdMo({
      fullscreen: {
        session_id: "sess-1",
        parent_asset_id: "book-1",
    ...
    expect(c.weekly_nd.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.backlog_mutated).toBe(false);
  });
});
