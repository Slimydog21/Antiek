...
describe("composeSourceAttachAntiekBenchRewriteModelDecision", () => {
  ...
  it("source attach + rewrite model decision pack ready", () => {
    const c = composeSourceAttachAntiekBenchRewriteModelDecision({
    ...
    expect(c.sources.html_ready_count).toBe(2);
    expect(c.rewrite_pack.pack_ready).toBe(true);
    expect(c.rewrite_pack.proposal_count).toBeGreaterThanOrEqual(1);
    ...
    expect(
      formatSourceAttachAntiekBenchRewriteModelDecisionSummary(c),
  ...
  it("session mismatch blocks pack_ready", () => {
    const c = composeSourceAttachAntiekBenchRewriteModelDecision({
      sources: { ...attach_sources_input, session_id: "sess-other" },
      rewrite_pack,
      operator_ack: true,
    });
    expect(c.pack_ready).toBe(false);
  ...
  it("zero rewrite proposals blocks overall", () => {
    const c = composeSourceAttachAntiekBenchRewriteModelDecision({
      sources: attach_sources_input,
      rewrite_pack: {
    ...
    expect(c.rewrite_pack.proposal_count).toBe(0);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.applied).toBe(false);
