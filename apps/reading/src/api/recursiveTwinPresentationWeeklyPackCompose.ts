 ...
 * (short residual: RecursiveTwinPresentationWeeklyPack — FS 255 policy).
...
import {
  composeAntiekBenchWeeklySrcWritePack,
  type AntiekBenchWeeklySrcWritePackCompose,
  type AntiekBenchWeeklySrcWritePackInput,
} from "./antiekBenchWeeklySrcWritePackCompose";

export interface RecursiveTwinPresentationAntiekBenchWeeklySrcWritePackInput {
  twin: Omit<RecursiveTwinNoteTakerInput, "operator_ack">;
  presentation: TwinPresentationSurfaceInput;
  weekly_pack: Omit<
    AntiekBenchWeeklySrcWritePackInput,
    "operator_ack"
  >;
...
export interface RecursiveTwinPresentationAntiekBenchWeeklySrcWritePackCompose {
  week_id: string;
  session_id: string;
  ...
  weekly_pack: AntiekBenchWeeklySrcWritePackCompose;
  ...
  notes: string[];
  authority: "recursive_twin_presentation_weekly_pack_compose_advisory";
}
 ...
 */
export function composeRecursiveTwinPresentationWeeklyPack(
  input: RecursiveTwinPresentationAntiekBenchWeeklySrcWritePackInput,
): RecursiveTwinPresentationAntiekBenchWeeklySrcWritePackCompose {
  ...
  const weekly_pack = composeAntiekBenchWeeklySrcWritePack({
    ...input.weekly_pack,
    operator_ack: input.operator_ack,
  });
  ...
  return {
    ...
    notes,
    authority:
      "recursive_twin_presentation_weekly_pack_compose_advisory",
  };
}

export function formatRecursiveTwinPresentationWeeklyPackSummary(
  c: RecursiveTwinPresentationAntiekBenchWeeklySrcWritePackCompose,
): string {
  return (
