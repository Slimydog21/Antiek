...
describe("composeFullscreenMoPriceCeilingDraftMulti", () => {
  const mo_pack = {
    mo: {
      operator_id: "op-1",
      work_minutes: 120,
  ...
  it("fullscreen + MO price-ceiling pack ready", () => {
    const c = composeFullscreenMoPriceCeilingDraftMulti({
      fullscreen,
      mo_pack,
      operator_ack: true,
    });
    ...
    expect(formatFullscreenMoPriceCeilingDraftMultiSummary(c)).toMatch(
