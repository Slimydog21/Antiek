...
import {
  ...
  formatRecursiveTwinPresentationCompetitionDrSourceAttachSummary,
} from "./recursiveTwinPresentationCompetitionDrSourceAttachCompose";

describe("composeRecursiveTwinPresentationCompetitionDrSourceAttach", () => {
const WEEKLY_ND = {
  weekly_learn: {
    week_id: "2026-W28",
...
const COMPETITION_CITATIONS = [
  ...
  it("twin presentation + competition DR source-attach pack ready", () => {
    const c = composeRecursiveTwinPresentationCompetitionDrSourceAttach({
    ...
    expect(
      formatRecursiveTwinPresentationCompetitionDrSourceAttachSummary(c),
  ...
  it("open_requested false blocks presentation", () => {
    const c = composeRecursiveTwinPresentationCompetitionDrSourceAttach({
      twin: outer_twin_input,
      presentation: { ...outer_presentation_input, open_requested: false },
      competition_pack: outer_competition_pack,
      operator_ack: true,
    });
    expect(c.presentation.presentation_ready).toBe(false);
  ...
  it("parent_asset_id mismatch blocks pack_ready", () => {
    const c = composeRecursiveTwinPresentationCompetitionDrSourceAttach({
      twin: { ...outer_twin_input, parent_asset_id: "book-other" },
      presentation: outer_presentation_input,
      competition_pack: outer_competition_pack,
      operator_ack: true,
    });
