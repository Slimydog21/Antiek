...
describe("composeMoPriceCeilingDraftMultiSelectRecordWrite", () => {
  const draft_multi = {
    draft_gate: {
      session_id: "sess-1",
      parent_asset_id: "book-1",
        ...
        write_pack: {
          ...
          highlight_pack: {
            highlight: {
              ...
              source_families: ["arxiv", "substack"] as ("arxiv" | "substack")[],
            },
            twin_search_pack: {
              competition_pack: {
  ...
  it("MO price-ceiling + draft multi pack ready", () => {
    const c = composeMoPriceCeilingDraftMultiSelectRecordWrite({
      mo,
      draft_multi,
      operator_ack: true,
    });
    expect(c.mo.pack_ready).toBe(true);
    expect(c.draft_multi.pack_ready).toBe(true);
    ...
    expect(formatMoPriceCeilingDraftMultiSelectRecordWriteSummary(c)).toMatch(
  ...
  it("approve_ceiling without price_ceiling_ack blocks MO path", () => {
    const c = composeMoPriceCeilingDraftMultiSelectRecordWrite({
      mo: {
        ...mo,
        stage: "approve_ceiling",
        approved_ceiling_usd: 60,
  ...
  it("draft multi parent gate still pure when blocked", () => {
    const c = composeMoPriceCeilingDraftMultiSelectRecordWrite({
      mo,
      draft_multi: {
        ...draft_multi,
        draft_gate: {
          ...draft_multi.draft_gate,
          parent_asset_id: "book-other",
          sources: draft_multi.draft_gate.sources.map((s) => ({
            ...s,
            parent_asset_id: "book-other",
          })),
        },
    ...
    expect(c.draft_multi.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.charge_executed).toBe(false);
  });
