...
describe("composeNdShadowRecursiveTwinPresentationWeeklySourceAttach", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("ND shadow REJECT + twin presentation weekly source attach pack ready", () => {
    const c = composeNdShadowRecursiveTwinPresentationWeeklySourceAttach({
    ...
    expect(
      formatNdShadowRecursiveTwinPresentationWeeklySourceAttachSummary(c),
  ...
  it("kill switch on still allows pack when twin presentation ready (shadow advisory)", () => {
    ...
    // kill switch suppresses shadow_visible but REJECT + twin ready still packs
    expect(c.nd_shadow.production_router_verdict).toBe("REJECT");
    expect(c.nd_shadow.live_router_authorized).toBe(false);
    expect(c.pack_ready).toBe(true);
    expect(c.twin_written).toBe(false);
