...
describe("composeTwinSearchAntiekBenchWeeklyHtmlNative", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("twin search + weekly HTML-native pack ready", () => {
    const c = composeTwinSearchAntiekBenchWeeklyHtmlNative({
    ...
    expect(c.weekly_html.pack_ready).toBe(true);
    ...
    expect(
      formatTwinSearchAntiekBenchWeeklyHtmlNativeSummary(c),
  ...
  it("zero hits blocks pack_ready when require_both", () => {
    const c = composeTwinSearchAntiekBenchWeeklyHtmlNative({
      search_query: "zzzznotpresenttokenxyz",
      twin_records,
      weekly_html: {
        weekly_learn,
        html_pack: { html_view, twin_pack },
  ...
  it("session mismatch in weekly html blocks", () => {
    const c = composeTwinSearchAntiekBenchWeeklyHtmlNative({
      search_query: "scaling laws noise",
      twin_records,
      weekly_html: {
