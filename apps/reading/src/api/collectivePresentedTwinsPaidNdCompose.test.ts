...
describe("composeCollectivePresentedTwinsPaidNd", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("collective presented twins + paid ND pack ready", () => {
    const c = composeCollectivePresentedTwinsPaidNd({
      collective,
      paid_nd,
      operator_ack: true,
    });
    expect(c.collective.pack_ready).toBe(true);
    expect(c.paid_nd.pack_ready).toBe(true);
    ...
    expect(
      formatCollectivePresentedTwinsPaidNdSummary(c),
  ...
  it("parent mismatch blocks", () => {
    const c = composeCollectivePresentedTwinsPaidNd({
      collective: {
        ...
        members: collective.members.map((m) => ({
          ...m,
          parent_asset_id: "book-other",
        })),
      },
