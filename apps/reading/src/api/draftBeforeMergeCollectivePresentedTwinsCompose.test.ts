...
describe("composeDraftBeforeMergeCollectivePresentedTwins", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("draft-before-merge + collective pack ready", () => {
    const c = composeDraftBeforeMergeCollectivePresentedTwins({
      draft_gate,
      collective_pack,
      operator_ack: true,
    });
    ...
    expect(
      formatDraftBeforeMergeCollectivePresentedTwinsSummary(c),
  ...
  it("promote stage still never merges", () => {
    const c = composeDraftBeforeMergeCollectivePresentedTwins({
      draft_gate: {
        ...draft_gate,
