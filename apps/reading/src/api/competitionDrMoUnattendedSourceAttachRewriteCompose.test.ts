...
describe("composeCompetitionDrMoUnattendedSourceAttachRewrite", () => {
  ...
  it("competition DR + MO unattended source-attach rewrite pack ready", () => {
    const c = composeCompetitionDrMoUnattendedSourceAttachRewrite({
    ...
    expect(
      formatCompetitionDrMoUnattendedSourceAttachRewriteSummary(c),
    ).toMatch(/live_dispatch_authorized=false|pack_ready=true/);
  });
  ...
  it("session mismatch blocks", () => {
    const c = composeCompetitionDrMoUnattendedSourceAttachRewrite({
      competition: { ...competition_dr_input, session_id: "sess-other" },
      mo_pack: mo_pack_input,
      operator_ack: true,
    });
    expect(c.pack_ready).toBe(false);
  ...
  it("would_exceed on competition blocks overall", () => {
    const c = composeCompetitionDrMoUnattendedSourceAttachRewrite({
      competition: {
        ...competition_input,
        would_exceed: true,
