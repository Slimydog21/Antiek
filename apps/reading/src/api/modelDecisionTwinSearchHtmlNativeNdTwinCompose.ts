 ...
 * intelligent search + HTML-native marketplace free + settings ND twin presentation pack (pure).
 *
 ...
 * BYOK settings + NotDiamond REJECT + twin presentation honesty remain pure — never live-route,
...
import {
  ...
  type TwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinInput,
} from "./twinSearchHtmlNativeMarketplaceFreeSettingsNdTwinCompose";

export interface ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinInput {
  decision: Omit<SettingsDecisionTreeUsageBarInput, "operator_ack">;
  twin_search_pack: Omit<
    TwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinInput,
    "operator_ack"
  >;
...
export interface ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinCompose {
  week_id: string;
  session_id: string;
  ...
  notes: string[];
  authority: "model_decision_twin_search_html_native_nd_twin_compose_advisory";
}
...
/**
 * Model decision/usage/projection stacked on twin search HTML-native ND twin pack.
 * Never live-routes; never mutates bench; never PDF-primary; ND REJECT.
 */
export function composeModelDecisionTwinSearchHtmlNativeNdTwin(
  input: ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinInput,
): ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinCompose {
  ...
  const twin_search_pack = composeTwinSearchHtmlNativeMarketplaceFreeSettingsNdTwin({
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — model decision + twin search HTML-native ND twin ready; still pure",
    );
  } else {
    notes.push(
...
export function formatModelDecisionTwinSearchHtmlNativeNdTwinSummary(
  c: ModelDecisionTwinSearchHtmlNativeMarketplaceFreeSettingsNdTwinCompose,
): string {
  const budget =
