...
describe("composeModelDecisionTwinSearchHtmlNativeMoWeeklySrcWriteMoWeeklySrcWritePack", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("model decision + twin search HTML-native marketplace MO weekly src write pack ready", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMoWeeklySrcWriteMoWeeklySrcWritePack({
    ...
    expect(
      formatModelDecisionTwinSearchHtmlNativeMoWeeklySrcWriteMoWeeklySrcWritePackSummary(c),
    ).toMatch(/live_router_authorized=false/);
  });
  ...
  it("would_exceed blocks pack under block_on_budget_exceed", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMoWeeklySrcWriteMoWeeklySrcWritePack({
      decision: {
        ...residual_md2_outer_decision,
  ...
  it("empty twin hits block twin_search_pack and pack under require_both", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMoWeeklySrcWriteMoWeeklySrcWritePack({
      decision: residual_md2_outer_decision,
      twin_search_pack: {
        ...residual_md2_outer_twin_search_pack,
        twin_records: [],
      },
      operator_ack: true,
    });
