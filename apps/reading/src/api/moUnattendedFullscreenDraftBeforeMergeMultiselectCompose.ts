/**
 * Midnight Oil unattended package residual over fullscreen + draft-before-merge
 * floating multi-select model decision pack (pure).
 *
 * Operator vision: set time + goals + price ceiling for unattended deep research
 * while the workstation holds fullscreen draft + multi-select + model decision
 * budget + twin search free settings — without authorizing live workers.
 *
...
import {
  composeFullscreenDraftBeforeMergeFloatingMultiselect,
  type FullscreenDraftBeforeMergeFloatingMultiselectCompose,
  type FullscreenDraftBeforeMergeFloatingMultiselectInput,
} from "./fullscreenDraftBeforeMergeFloatingMultiselectCompose";

export interface MoUnattendedFullscreenDraftBeforeMergeMultiselectInput {
  mo: Omit<MidnightOilUnattendedPackageInput, "operator_ack">;
  fullscreen_pack: Omit<
    FullscreenDraftBeforeMergeFloatingMultiselectInput,
    "operator_ack"
  >;
...
export interface MoUnattendedFullscreenDraftBeforeMergeMultiselectCompose {
  session_id: string;
  ...
  notes: string[];
  authority: "mo_unattended_fullscreen_draft_before_merge_multiselect_compose_advisory";
}
...
/**
 * MO unattended package stacked on fullscreen draft multi-select model decision.
 * Never live-executes; ND REJECT.
 */
export function composeMoUnattendedFullscreenDraftBeforeMergeMultiselect(
  input: MoUnattendedFullscreenDraftBeforeMergeMultiselectInput,
  ...
  const fullscreen_pack = composeFullscreenDraftBeforeMergeFloatingMultiselect({
    ...input.fullscreen_pack,
    operator_ack: input.operator_ack,
  });
  notes.push(...fullscreen_pack.notes.map((n) => `[fullscreen_pack] ${n}`));

  const session_id = requireNonEmpty(fullscreen_pack.session_id, "session_id");
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — MO unattended + fullscreen draft multi-select ready; still pure",
    );
  } else {
    notes.push(
...
export function formatMoUnattendedFullscreenDraftBeforeMergeMultiselectSummary(
