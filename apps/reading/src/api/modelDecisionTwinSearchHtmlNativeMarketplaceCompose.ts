 ...
 * HTML-native recursive twin marketplace free pack (pure).
 *
 ...
 * while twin intelligent search + HTML-native + recursive twin + free-first
 * marketplace honesty remain pure — never live-route, store secrets, or
 * PDF-primary.
 *
...
import {
  composeTwinSearchHtmlNativeRecursiveTwinMarketplace,
  type TwinSearchHtmlNativeRecursiveTwinMarketplaceCompose,
  type TwinSearchHtmlNativeRecursiveTwinMarketplaceInput,
} from "./twinSearchHtmlNativeRecursiveTwinMarketplaceCompose";

export interface ModelDecisionTwinSearchHtmlNativeMarketplaceInput {
  decision: Omit<SettingsDecisionTreeUsageBarInput, "operator_ack">;
  twin_search_pack: Omit<
    TwinSearchHtmlNativeRecursiveTwinMarketplaceInput,
    "operator_ack"
  >;
...
export interface ModelDecisionTwinSearchHtmlNativeMarketplaceCompose {
  week_id: string;
  session_id: string;
  ...
  twin_search_pack: TwinSearchHtmlNativeRecursiveTwinMarketplaceCompose;
  pack_ready: boolean;
  ...
  notes: string[];
  authority: "model_decision_twin_search_html_native_marketplace_compose_advisory";
}
...
/**
 * Model decision/usage/projection stacked on twin search HTML-native marketplace.
 * Never live-routes; never mutates bench; never PDF-primary; ND REJECT.
 */
export function composeModelDecisionTwinSearchHtmlNativeMarketplace(
  input: ModelDecisionTwinSearchHtmlNativeMarketplaceInput,
  ...
  const twin_search_pack = composeTwinSearchHtmlNativeRecursiveTwinMarketplace({
  ...
  const focus_task = requireNonEmpty(twin_search_pack.focus_task, "focus_task");
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — model decision + twin search HTML-native marketplace ready; still pure",
    );
  } else {
    notes.push(
...
export function formatModelDecisionTwinSearchHtmlNativeMarketplaceSummary(
  c: ModelDecisionTwinSearchHtmlNativeMarketplaceCompose,
): string {
  const budget =
