...
} from "./floatingMultiselectModelDecisionNdTwinCompose";

describe("composeFloatingMultiselectModelDecisionNdTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("floating multi-select + model decision ND twin pack ready", () => {
    const c = composeFloatingMultiselectModelDecisionNdTwin({
    ...
    expect(
      formatFloatingMultiselectModelDecisionNdTwinSummary(c),
  ...
  it("would_exceed on nested decision blocks overall", () => {
    ...
    expect(c.decision_pack.decision.would_exceed).toBe(true);
    expect(c.decision_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_dispatched).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
