...
describe("composeMoUnattendedDraftBeforeMergeFloatingMultiselectNdTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  const outer_twin_search_pack = {
    search_query: "scaling noise",
    twin_records: outer_twin_records,
    html_pack: outer_html_pack,
  };

  const outer_multiselect_input = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    members: [
      {
  ...
  const outer_decision_pack = {
    decision: outer_decision_input,
    twin_search_pack: outer_twin_search_pack,
  };

  const outer_multi_pack = {
    multiselect: outer_multiselect_input,
    decision_pack: outer_decision_pack,
  };

  const outer_draft_gate = {
    session_id: "sess-1",
    parent_asset_id: "book-1",
    parent_excerpt: "<p>Parent scaling laws body</p>",
    sources: [
  ...
  const outer_draft_pack = {
    draft_gate: outer_draft_gate,
    multi_pack: outer_multi_pack,
  };

  // approved_ceiling ≥ recommended (work_minutes/60 * usd_per_hour * intensity)
  const outer_mo = {
    operator_id: "op-1",
    work_minutes: 120,
    goals: [
  ...
  it("MO unattended + draft-before-merge multiselect ND twin pack ready", () => {
    const c = composeMoUnattendedDraftBeforeMergeFloatingMultiselectNdTwin({
      mo: outer_mo,
      draft_pack: outer_draft_pack,
      operator_ack: true,
    });
    expect(c.mo.unattended_package_ready).toBe(true);
    ...
    expect(
      formatMoUnattendedDraftBeforeMergeFloatingMultiselectNdTwinSummary(c),
    ).toMatch(/live_execution_authorized=false/);
  });
  ...
  it("spend_consent false blocks mo readiness", () => {
    const c = composeMoUnattendedDraftBeforeMergeFloatingMultiselectNdTwin({
      mo: { ...outer_mo, spend_consent: false },
      draft_pack: outer_draft_pack,
      operator_ack: true,
    });
    expect(c.mo.unattended_package_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
  });

  it("would_exceed on nested decision blocks overall", () => {
    const c = composeMoUnattendedDraftBeforeMergeFloatingMultiselectNdTwin({
      mo: outer_mo,
      draft_pack: {
        ...outer_draft_pack,
    ...
    expect(
      c.draft_pack.multi_pack.decision_pack.decision.would_exceed,
    ).toBe(true);
    expect(c.draft_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
    expect(c.draft_written).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
