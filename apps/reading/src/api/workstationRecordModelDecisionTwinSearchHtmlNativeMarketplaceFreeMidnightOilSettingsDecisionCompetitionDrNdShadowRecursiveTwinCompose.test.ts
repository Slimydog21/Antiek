...
describe("composeWorkstationRecordModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("workstation records + model decision twin search pack ready", () => {
    const c = composeWorkstationRecordModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      session_id: "sess-1",
      items: residual_workstation_outer_items,
      decision_pack: residual_workstation_outer_decision_pack,
      operator_ack: true,
    });
    expect(c.records.pack_ready).toBe(true);
    expect(c.records.item_count).toBe(2);
    expect(c.decision_pack.pack_ready).toBe(true);
    ...
    expect(
      formatWorkstationRecordModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinSummary(c),
    ).toMatch(/record_persisted=false|prompts_injected=false|REJECT/);
  });
  ...
  it("empty items blocks records pack and overall", () => {
    const c = composeWorkstationRecordModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      session_id: "sess-1",
      items: [],
      decision_pack: residual_workstation_outer_decision_pack,
    ...
    expect(c.records.pack_ready).toBe(false);
