/**
 * Write-mode twin collective analysis residual over fullscreen + Midnight Oil
 * unattended draft multiselect model decision ND twin pack (pure).
 *
 ...
 * MO unattended + draft-before-merge + multi-select + model decision ND twin
...
import {
  ...
  type FullscreenMoUnattendedDraftMultiselectNdTwinInput,
} from "./fullscreenMoUnattendedDraftMultiselectNdTwinCompose";

export interface WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinInput {
  write: Omit<WriteModeTwinCollectiveAnalysisInput, "operator_ack">;
  fullscreen_pack: Omit<
    FullscreenMoUnattendedDraftMultiselectNdTwinInput,
    "operator_ack"
  >;
...
export interface WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinCompose {
  session_id: string;
  ...
  notes: string[];
  authority: "write_mode_twin_collective_fullscreen_mo_unattended_nd_twin_compose_advisory";
}
...
/**
 * Write twin collective analysis stacked on fullscreen MO unattended ND twin pack.
 * Never writes analysis/draft; never live-dispatches.
 */
export function composeWriteModeTwinCollectiveFullscreenMoUnattendedNdTwin(
  input: WriteModeTwinCollectiveFullscreenMoUnattendedNdTwinInput,
  ...
  const fullscreen_pack = composeFullscreenMoUnattendedDraftMultiselectNdTwin({
  ...
  if (pack_ready) {
    notes.push(
      "pack_ready=true — write twin collective + fullscreen MO unattended ND twin ready; still pure",
    );
  } else {
    notes.push(
...
export function formatWriteModeTwinCollectiveFullscreenMoUnattendedNdTwinSummary(
