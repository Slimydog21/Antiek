...
describe("composeSettingsDecisionCompetitionDrNdShadowTwinWeekly", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("settings decision + competition DR ND shadow twin weekly pack ready", () => {
    const c = composeSettingsDecisionCompetitionDrNdShadowTwinWeekly({
    ...
    expect(
      formatSettingsDecisionCompetitionDrNdShadowTwinWeeklySummary(c),
  ...
  it("week_id mismatch blocks pack_ready", () => {
    const c = composeSettingsDecisionCompetitionDrNdShadowTwinWeekly({
      settings: {
        ...residual_settings,
        week_id: "2026-W99",
  ...
  it("budget projection would_exceed is advisory (still pure)", () => {
    const c = composeSettingsDecisionCompetitionDrNdShadowTwinWeekly({
      settings: {
        ...residual_settings,
        daily_cap_usd: 1,
    ...
    expect(c.settings.bench_rec.decision_tree.would_exceed).toBe(true);
    expect(c.secrets_stored).toBe(false);
    expect(c.inventory_mutated).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
    // projection is advisory — pack may still be ready when both subpacks ready
    expect(c.competition_pack.pack_ready).toBe(true);
  });
});
