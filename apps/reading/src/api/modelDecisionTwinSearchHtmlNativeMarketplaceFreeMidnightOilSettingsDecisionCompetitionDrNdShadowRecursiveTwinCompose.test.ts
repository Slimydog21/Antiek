...
describe("composeModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("model decision + twin search HTML-native marketplace free pack ready", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
    ...
    expect(c.decision.would_exceed).toBe(false);
    ...
    expect(
      formatModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwinSummary(c),
    ).toMatch(/live_router_authorized=false|REJECT|would_exceed=false/);
  });
  ...
  it("would_exceed blocks pack_ready under block_on_budget_exceed", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      decision: {
        ...residual_model_outer_decision,
  ...
  it("empty twin_records blocks twin_search pack and overall", () => {
    const c = composeModelDecisionTwinSearchHtmlNativeMarketplaceFreeMidnightOilSettingsDecisionCompetitionDrNdShadowRecursiveTwin({
      decision: residual_model_outer_decision,
      twin_search_pack: {
        ...residual_model_outer_twin_search_pack,
        twin_records: [],
      },
      operator_ack: true,
    });
    expect(c.twin_search_pack.hit_count).toBe(0);
    expect(c.twin_search_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_index_queried).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
