...
describe("composeMarketplaceFreeAntiekBenchRecommendMo", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("free-before-buy port + bench recommend MO pack ready", () => {
    const c = composeMarketplaceFreeAntiekBenchRecommendMo({
      market,
      bench_mo: {
        bench,
        mo_pack: {
    ...
    expect(c.bench_mo.pack_ready).toBe(true);
    ...
    expect(
      formatMarketplaceFreeAntiekBenchRecommendMoSummary(c),
  ...
  it("unknown free availability blocks port", () => {
    const c = composeMarketplaceFreeAntiekBenchRecommendMo({
      market: {
        ...market,
  ...
  it("port_requested false blocks overall pack_ready", () => {
    const c = composeMarketplaceFreeAntiekBenchRecommendMo({
      market: { ...market, port_requested: false },
      bench_mo: {
        bench,
        mo_pack: {
          mo,
