...
describe("composeCompetitionDrNdShadowSourceAttachWeeklyLearn", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("competition DR + ND shadow source-attach weekly learn pack ready", () => {
    const c = composeCompetitionDrNdShadowSourceAttachWeeklyLearn({
    ...
    expect(c.nd_pack.nd_shadow.production_router_verdict).toBe("REJECT");
    expect(c.authority).toBe(
      "competition_dr_nd_shadow_source_attach_weekly_learn_compose_advisory",
    );
    expect(
      formatCompetitionDrNdShadowSourceAttachWeeklyLearnSummary(c),
  ...
  it("would_exceed on competition quality blocks competition pack", () => {
    const c = composeCompetitionDrNdShadowSourceAttachWeeklyLearn({
      competition: {
        ...pack_outer_competition,
        would_exceed: true,
