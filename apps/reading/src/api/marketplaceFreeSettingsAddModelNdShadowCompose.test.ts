...
describe("composeMarketplaceFreeSettingsAddModelNdShadow", () => {
  ...
  it("marketplace free + settings add-model ND shadow pack ready", () => {
    const c = composeMarketplaceFreeSettingsAddModelNdShadow({
      market: market_free,
      settings_pack,
      operator_ack: true,
    });
    expect(c.market.port_ready).toBe(true);
    expect(c.market.path).toMatch(/prefer_free/);
    ...
    expect(
      formatMarketplaceFreeSettingsAddModelNdShadowSummary(c),
  ...
  it("unknown free availability blocks port_ready under free-first", () => {
    const c = composeMarketplaceFreeSettingsAddModelNdShadow({
      market: {
        ...market_free,
  ...
  it("would_exceed on nested competition blocks overall", () => {
    ...
    expect(c.settings_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.purchase_executed).toBe(false);
    expect(c.hosted).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
