...
import {
  composeMidnightOilSettingsDecisionCompetitionDr,
  formatMidnightOilSettingsDecisionCompetitionDrSummary,
} from "./midnightOilSettingsDecisionCompetitionDrCompose";

describe("composeMidnightOilSettingsDecisionCompetitionDr", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("Midnight Oil + settings decision competition DR pack ready", () => {
    const c = composeMidnightOilSettingsDecisionCompetitionDr({
      mo: residual_mo,
    ...
    expect(
      formatMidnightOilSettingsDecisionCompetitionDrSummary(c),
  ...
  it("recommend_only stage ready without operator_ack on MO alone still needs both", () => {
    const c = composeMidnightOilSettingsDecisionCompetitionDr({
      mo: {
        operator_id: "op-1",
        work_minutes: 120,
    ...
    // recommend_only can be mo.pack_ready without ack, but require_both needs settings too
    expect(c.mo.live_execution_authorized).toBe(false);
    expect(c.charge_executed).toBe(false);
    expect(c.production_router_verdict).toBe("REJECT");
    // settings ready + mo recommend_only ready + operator_ack true → pack may be ready
    expect(c.settings_pack.pack_ready).toBe(true);
    expect(c.mo.pack_ready).toBe(true);
    expect(c.pack_ready).toBe(true);
  });
