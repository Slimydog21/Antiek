...
describe("composeHtmlNativeRecursiveTwinSettingsFullscreenMo", () => {
  const twin_pack = {
    twin: {
      parent_asset_id: "book-1",
      source_excerpt:
  ...
  it("HTML-native + recursive twin settings MO pack ready", () => {
    const c = composeHtmlNativeRecursiveTwinSettingsFullscreenMo({
      html_view,
      twin_pack,
      operator_ack: true,
    });
    ...
    expect(formatHtmlNativeRecursiveTwinSettingsFullscreenMoSummary(c)).toMatch(
  ...
  it("parent/asset mismatch blocks", () => {
    const c = composeHtmlNativeRecursiveTwinSettingsFullscreenMo({
