...
describe("composeDraftBeforeMergeMoPriceCeilingRecursiveTwin", () => {
...
const COMPETITION_CITATIONS = [
  ...
  it("draft-before-merge + MO price-ceiling recursive twin pack ready", () => {
    const c = composeDraftBeforeMergeMoPriceCeilingRecursiveTwin({
    ...
    expect(
      formatDraftBeforeMergeMoPriceCeilingRecursiveTwinSummary(c),
  ...
  it("below-ceiling MO nested blocks overall", () => {
    const c = composeDraftBeforeMergeMoPriceCeilingRecursiveTwin({
      draft_gate: residual_draft_gate,
      mo_pack: {
        ...residual_mo_pack,
    ...
    expect(c.mo_pack.mo.ceiling_approved).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.draft_written).toBe(false);
    expect(c.merge_executed).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
