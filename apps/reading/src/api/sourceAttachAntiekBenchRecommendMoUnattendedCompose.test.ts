...
describe("composeSourceAttachAntiekBenchRecommendMoUnattended", () => {
  ...
  it("source attach + Antiek-bench recommend MO unattended pack ready", () => {
    const c = composeSourceAttachAntiekBenchRecommendMoUnattended({
      sources: outer_sources_input,
      recommend_pack,
      operator_ack: true,
    });
    expect(c.sources.attach_ready).toBe(true);
    expect(c.sources.source_count).toBe(2);
    expect(c.sources.html_ready_count).toBe(1);
    expect(c.recommend_pack.pack_ready).toBe(true);
    ...
    expect(
      formatSourceAttachAntiekBenchRecommendMoUnattendedSummary(c),
  ...
  it("empty sources blocks attach and overall", () => {
    const c = composeSourceAttachAntiekBenchRecommendMoUnattended({
      sources: {
        ...outer_sources_input,
        sources: [],
    ...
    expect(c.sources.attach_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.remote_fetched).toBe(false);
    expect(c.suite_rewritten).toBe(false);
    expect(c.live_execution_authorized).toBe(false);
