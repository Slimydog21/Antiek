...
describe("composeNdShadowSourceAttachWeeklyLearnTwinPresentation", () => {
...
const COMPETITION_CITATIONS = [
  ...
  // kill_switch_on true is honest production posture; shadow suppressed but REJECT held
  ...
  it("ND shadow REJECT + source-attach weekly learn twin presentation pack ready", () => {
    const c = composeNdShadowSourceAttachWeeklyLearnTwinPresentation({
    ...
    expect(
      formatNdShadowSourceAttachWeeklyLearnTwinPresentationSummary(c),
  ...
  it("would_exceed on sources blocks source_pack overall", () => {
    const c = composeNdShadowSourceAttachWeeklyLearnTwinPresentation({
      nd_shadow: pack_outer_nd_shadow,
      source_pack: {
        ...pack_outer_source_pack,
        sources: {
          ...pack_outer_sources,
          would_exceed: true,
        },
    ...
    expect(c.source_pack.sources.pack_ready).toBe(false);
  ...
  it("session mismatch on sources blocks overall", () => {
    const c = composeNdShadowSourceAttachWeeklyLearnTwinPresentation({
      nd_shadow: pack_outer_nd_shadow,
      source_pack: {
        ...pack_outer_source_pack,
    ...
    expect(c.source_pack.session_aligned).toBe(false);
    expect(c.source_pack.pack_ready).toBe(false);
    expect(c.pack_ready).toBe(false);
    expect(c.live_router_authorized).toBe(false);
    expect(c.nd_shadow.production_router_verdict).toBe("REJECT");
