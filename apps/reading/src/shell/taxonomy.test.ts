...
function discoverModeIds(): Set<ModeId> {
  ...
  // Speak SPR-08 preserves Interview as component-mode capability folded into
  // /speak. It has no index.tsx because the standalone route was retired.
  const interviewModules = import.meta.glob("../modes/Interview/InterviewTranscript.tsx");
  if (Object.keys(interviewModules).length > 0) {
    ids.add("Interview");
  }
...
describe("Sprint 25+ economics dashboard route integrity", () => {
  ...
  it("keeps deleted ad-economics orphans out of taxonomy until contracts exist", () => {
    expect(modeById("AdvertiserConsole")).toBeUndefined();
    expect(modeById("CreatorPayouts")).toBeUndefined();
  });
});
