/**
 * MascotStation.station.test.tsx — the fixed-station behaviours that need the
 * WERNER-ICE flag ON and a mounted rAF (2026-07-02 rework).
 *
 * This REPLACES the deleted MascotStation.iceFishing.integration.test.tsx,
 * whose central pin was "a bare pointermove MOVES the mascot" (the reel). That
 * behaviour is exactly what the operator removed, so its inverse is now the
 * load-bearing regression guard:
 *
 *   (1) THE MASCOT DOES NOT FOLLOW THE CURSOR — a pointermove (anywhere, any
 *       distance) leaves Brain's position untouched. This is the whole ask.
...
describe("MascotStation — the fixed station (flag on)", () => {
  ...
  it("stays calm for any pointer state (no fishing gag, no bait class)", () => {
    ...
    expect(container.querySelector(".mascot-fishing")).toBeNull();
    expect(bob?.className).not.toContain("mascot-fishing");
...
it("does not wander off on its own (fixed station)", () => {
  ...
  // waddleToEl) with a mocked on-screen rect, and assert Brain walks TO the
