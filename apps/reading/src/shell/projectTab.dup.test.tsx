/**
 * projectTab.dup.test.tsx — SPR-05 M4 regression: the Research "+"
 * (now the Mascot) yields EXACTLY ONE project tab, never two.
 *
 ...
 * Mascot, whose float path is idempotent by construction:
 ...
 *     duplicates;
 *   - MascotStation.floatProjectTab() (MascotStation.tsx:127-137) further
 ...
 *   1. Many Mascot single-clicks (each elapsing its window) → exactly ONE
 *      ProjectTree panel (the Research "+" yields one tab).
 *   2. The same id opened via the ⌘B docked path AND then floated via the
 *      Mascot is still ONE panel (the two openers share the id; they don't
 *      stack).
 ...
 *      panels). The Mascot only ever touches the project-tree id.
 */
import { afterAll, afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { act, cleanup, fireEvent, render, screen } from "@testing-library/react";
import { MemoryRouter, Route, Routes } from "react-router-dom";

import { MascotStation, PROJECT_TREE_PANEL_ID } from "./MascotStation";
...
function mountStation() {
  return render(
    <MemoryRouter initialEntries={["/"]}>
      <Routes>
        <Route path="*" element={<MascotStation />} />
...
describe("Research + (Mascot) yields exactly one project tab (SPR-05 M4 regression)", () => {
  ...
  it("the ⌘B docked opener and the Mascot float share the one id (no second tab)", () => {
    ...
    // Now the Mascot floats it — it flips the existing panel, never adds one.
    mountStation();
  ...
  it("an unrelated panel coexists — the Mascot only touches the project-tree id (other surfaces unaffected)", () => {
