...
/**
 * The palette itself, mounted only while it is open. The ⌘K shell
 * (CommandPalette.tsx) owns open/close, the global shortcut and focus
 * return, and loads this module on demand so none of it rides in the entry
 * chunk. Closing unmounts it, which also clears the query.
 */
export default function CommandPalettePanel({ onClose }: { onClose: () => void }) {
  ...
  // The palette is a modal dialog: it traps Tab (below); the shell gives
  // focus back to whatever held it when ⌘K (or the Search door) opened it.
