 ...
 *   • `data-akb-chunk-id` identifies each graph-backed region only after the
 *     ordered regions reconstruct the complete page. A mismatch keeps the page
 *     asset-level so attribution never claims more precision than the body can
 *     prove.
 *
...
export interface ReadingColumnProps {
  ...
  text: string;
  /** Ordered authoritative chunks owned by this page. Empty means the page is
   * rendered honestly without a chunk anchor. */
  chunks?: ReaderChunk[];
}
...
export const ReadingColumn = forwardRef<HTMLElement, ReadingColumnProps>(
  function ReadingColumn({ assetId, text, chunks = [] }, ref) {
    return (
      <article
        ref={ref}
        ...
        // servable book reading session. A null assetId
        ...
        // page stays asset-level because it has no tagged child region.
        {...(assetId ? { "data-akb-asset-id": assetId } : {})}
        className="flex-1 font-serif text-[15px] leading-[1.7] text-ink dark:text-bright"
      >
        {chunks.length > 0 ? (
          chunks.map((chunk) => (
            <section key={chunk.chunk_id} data-akb-chunk-id={chunk.chunk_id}>
              {renderBlocks(chunk.text)}
            </section>
          ))
        ) : text.trim() ? (
          renderBlocks(text)
