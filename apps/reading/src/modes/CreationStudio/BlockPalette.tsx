...
export default function BlockPalette() {
  ...
  return (
    <div className="bg-white border border-stone-200 rounded-md p-3 flex flex-col gap-2 min-h-0">
      ...
      <ul className="flex-1 overflow-y-auto space-y-1 min-h-0">
        {hits.length === 0 && !loading && (
          <li className="text-xs text-stone-400 italic px-1 py-2">
        ...
        {hits.map((h) => (
          <li key={`${h.block_kind}:${h.block_id}`}>
            <div
              ...
              className="px-2 py-1.5 border border-stone-200 hover:border-stone-400 hover:bg-stone-50 rounded cursor-grab active:cursor-grabbing"
              title={h.body}
            >
