...
const artifactProviderRecord: MultimediaAssetRecord = {
  ...
  jobs: [
    ...queuedProviderRecord.jobs,
    {
      ...
      execution_mode: "live",
      provider_family: "krea",
      live_request_max_budget_usd: 88,
      live_request_route_policy: "highest_quality",
      live_request_dry_run_revision_id: "rev-live-1",
...
describe("Multimedia workstation", () => {
  ...
  it("reviews a plan before render approval and then opens playback", async () => {
    ...
    expect(
      screen.getByText(
        "Edit brief and Steer outline update local planning state only; they do not call providers or spend budget. Approve render creates a draft render package only; it does not call providers, export, or publish media.",
      ),
    ).toBeTruthy();
  ...
  it("surfaces attached artifact previews and rejected retry actions in persisted rows", async () => {
    ...
    expect(within(persistedAssets).getByText("Evidence only; does not clear manual review or rights")).toBeTruthy();
    expect(
      within(persistedAssets).getByText(
        "Export review packets are evidence only; copying one does not complete manual review, clear rights, export, publish, or call providers.",
      ),
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(writeText).toHaveBeenCalledWith(
        [
          ...
          "Activation audit: Not run in app",
          "Activation proof: Worker logs + manual artifact attach",
          "Provenance packet: Evidence only; does not verify rights or media truth",
          "Copy action: Read-only; no provider worker triggered",
        ].join("\n"),
      ),
    );
    ...
    await waitFor(() =>
      expect(writeText).toHaveBeenCalledWith(
        [
          ...
          "Review packet: Evidence only; does not clear manual review or rights",
  ...
  it("surfaces attached provider artifacts with open, download, and copy actions", async () => {
    ...
    expect(within(jobPanel).getByText("Evidence only; does not clear manual review or rights")).toBeTruthy();
    expect(
      within(jobPanel).getByText(
        "Export review packets are evidence only; copying one does not complete manual review, clear rights, export, publish, or call providers.",
      ),
    ...
    await waitFor(() =>
      expect(writeText).toHaveBeenCalledWith(
        [
          ...
          "Provider route: Highest quality / krea",
          "Dry-run revision: rev-live-1",
          "Activation boundary: Separate worker activation required",
          "Manual review: Required before export",
          "Rights review: Required before public use",
          "Export action: No export or publish action available",
