    ...
    <div className="flex flex-col h-screen">
      ...
      <main className="flex-1 overflow-y-auto bg-white">
        <div className="max-w-5xl mx-auto px-8 py-10 space-y-8">
          ...
          {loading && (
            <p className="text-sm text-stone-500">Loading publishers…</p>
          )}
          {error && (
    ...
    <section className="border border-stone-200 rounded-md p-5 space-y-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {headlineKeys.map(([k, label]) => (
          <div
            key={k}
            className="border border-stone-100 rounded-md px-2 py-2 text-center"
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="border border-stone-100 rounded-md px-3 py-2">
          ...
          <p className="text-lg font-serif text-stone-900">
            ...
            {snapshot.pendingDeletions > 0 && (
              <Link
                to="/privacy"
                className="ml-2 text-xs font-mono text-stone-500 hover:underline"
              >
                review →
              </Link>
            )}
        ...
        <div className="border border-stone-100 rounded-md px-3 py-2">
          ...
          {snapshot.recentPayouts.length === 0 ? (
            <p className="text-sm italic text-stone-500">No transfers yet.</p>
          ) : (
            <ul className="text-xs font-mono text-stone-700 space-y-0.5">
  ...
  return (
    <section className="border border-stone-200 rounded-md p-5 space-y-3">
      <div className="space-y-1">
        <h2 className="text-base font-serif text-stone-900">{title}</h2>
        <p className="text-xs text-stone-600">{description}</p>
      </div>
      {publishers.length === 0 ? (
        <p className="text-xs italic text-stone-500">No publishers in this bucket.</p>
      ) : (
        <ul className="divide-y divide-stone-200">
          {publishers.map((p) => (
            <li key={p.ip_holder_id} className="py-2 flex items-center justify-between gap-4">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-serif text-stone-900 truncate">
                  {p.display_name}
                </p>
              ...
              {actionLabel && onAction && (
                <button
                  ...
                  className="px-2.5 py-1 rounded-md bg-stone-900 text-white text-xs font-medium hover:bg-stone-800 transition-colors shrink-0"
                >
                  {actionLabel}
                </button>
              )}
