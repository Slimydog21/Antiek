...
export default function Settings() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "decision">(
    "overview",
  );
    ...
    <div className="h-full overflow-y-auto bg-ice-2 dark:bg-space-2">
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        <header>
          ...
          <div
            className="mt-5 flex gap-1 border-b border-ink/15 dark:border-bright/15"
            role="tablist"
                ...
                {budget && (
                  <>
                    <div className="font-mono text-[13px] space-y-2">
                      ...
                      {budget.cap_env && (
                        <Row
                          label="Display cap source"
                          value={budget.cap_env}
                        />
                      )}
      ...
      <div className="grid gap-3 border-y border-ink/15 py-4 dark:border-bright/15 sm:grid-cols-3">
        ...
        <label className="text-xs font-semibold text-ink-soft dark:text-starlight">
          ...
          <input
            ...
            onChange={(event) => {
              invalidateDecision();
              setInputChars(Number(event.target.value) || 0);
            }}
            className="mt-1 block h-10 w-full border border-ink/20 bg-transparent px-2 text-sm text-ink dark:border-bright/20 dark:text-bright"
          />
        </label>
        <label className="text-xs font-semibold text-ink-soft dark:text-starlight">
          ...
          <input
            ...
            onChange={(event) => {
              invalidateDecision();
              setOutputTokens(Number(event.target.value) || 0);
            }}
            className="mt-1 block h-10 w-full border border-ink/20 bg-transparent px-2 text-sm text-ink dark:border-bright/20 dark:text-bright"
          />
        </label>
      ...
      {decision && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-baseline justify-between gap-3 border-b border-ink/15 pb-3 dark:border-bright/15">
            <p className="text-sm text-ink dark:text-bright">
              Recommended tier:{" "}
              <strong>{decision.recommended_tier ?? "none available"}</strong>
            </p>
            <p className="font-mono text-xs text-ink-soft dark:text-starlight">
              {decision.benchmark_status === "measured"
                ? "Measured evidence"
            ...
            <table className="w-full min-w-[680px] border-collapse text-left text-sm">
              <thead className="border-b-2 border-ink/70 text-xs text-ink-soft dark:border-bright/70 dark:text-starlight">
                <tr>
                  ...
                  <th className="py-2 pr-3">Estimate high</th>
              ...
              <tbody>
                {decision.candidates.map((candidate) => (
                  <tr
                    ...
                    className="border-b border-ink/10 dark:border-bright/10"
                  >
                    ...
                    <td className="py-3 pr-3">
                      <span className="block">{candidate.model}</span>
...
function PasskeySettings() {
  ...
  async function forgetPasskey(passkey: SavedPasskey) {
    if (
      !window.confirm(
        `Forget “${passkey.label}”? Email recovery will still work.`,
      )
    )
      return;
    setWorking(true);
  ...
  return (
    <LemonCard title="Unlock & recovery" elevation="z1" colour="glacial">
      <div className="p-4 space-y-4">
        <div>
          <p className="text-sm text-ink dark:text-bright font-semibold">
            Passkeys
          </p>
          <p className="text-xs text-ink-soft dark:text-starlight mt-1">
            Face ID, Touch ID, or your device PIN unlocks Antiek. Email stays
        ...
        {passkeys === null ? (
          ...
          <ul className="space-y-2">
            {passkeys.map((passkey) => (
              <li
                key={passkey.id}
                className="flex items-center justify-between gap-3 rounded-hog border border-rule dark:border-slate-2 bg-ice-0 dark:bg-charcoal-2 px-3 py-2"
              >
                <span>
                  <strong className="block text-sm text-ink dark:text-bright">
                    {passkey.label}
                  </strong>
                  <small className="text-[11px] text-ink-soft dark:text-starlight">
                    ...
                    {passkey.last_used_at
                      ? ` · used ${new Date(passkey.last_used_at * 1000).toLocaleDateString()}`
                      : ""}
                  </small>
                </span>
                <button
                  type="button"
                  disabled={working}
                  onClick={() => void forgetPasskey(passkey)}
