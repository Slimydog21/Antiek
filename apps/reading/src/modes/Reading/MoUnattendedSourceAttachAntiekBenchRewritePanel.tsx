/**
 * MoUnattendedSourceAttachAntiekBenchRewritePanel — free-file.
 * Midnight Oil unattended over source-attach + Antiek-bench rewrite pack.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMoUnattendedSourceAttachAntiekBenchRewrite,
  formatMoUnattendedSourceAttachAntiekBenchRewriteSummary,
  type MoUnattendedSourceAttachAntiekBenchRewriteCompose,
} from "../../api/moUnattendedSourceAttachAntiekBenchRewriteCompose";

export default function MoUnattendedSourceAttachAntiekBenchRewritePanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MoUnattendedSourceAttachAntiekBenchRewriteCompose | null>(null);

  function onCompose() {
    setError(null);
    setResult(null);
    try {
      // Compact demo — full nest proven in pure tests (may fail on incomplete nest).
      setResult(
        composeMoUnattendedSourceAttachAntiekBenchRewrite({
          mo: {
            operator_id: "op-demo",
            work_minutes: 120,
            goals: [
              { goal_id: "g1", title: "Deep research arxiv gaps" },
              { goal_id: "g2", title: "Twin notes synthesis" },
            ],
            usd_per_hour: 15,
  ...
  return (
    <div className="flex flex-col gap-3 p-4 max-w-3xl">
      <h2 className="text-lg font-semibold">
        Midnight Oil · source-attach · Antiek-bench rewrite
      </h2>
      <p className="text-sm text-muted">
        Pure residual: unattended MO price-ceiling pack over arxiv/substack
        attach + recursive rewrite + model decision. live_execution_authorized=false
        · charge_executed=false · ND REJECT.
      </p>
      ...
      <LemonButton type="primary" onClick={onCompose}>
        Compose MO unattended residual pack
      </LemonButton>
      {error && (
      ...
      {result && (
        <LemonCard className="p-3 text-sm space-y-1">
          <div>
            {formatMoUnattendedSourceAttachAntiekBenchRewriteSummary(result)}
          </div>
          ...
          <div>charge_executed={String(result.charge_executed)}</div>
          <div>
