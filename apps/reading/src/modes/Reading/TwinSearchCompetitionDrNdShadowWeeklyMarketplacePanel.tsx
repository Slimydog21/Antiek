/**
 * TwinSearchCompetitionDrNdShadowWeeklyMarketplacePanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeTwinSearchCompetitionDrNdShadowWeeklyMarketplace,
  formatTwinSearchCompetitionDrNdShadowWeeklyMarketplaceSummary,
  type TwinSearchCompetitionDrNdShadowWeeklyMarketplaceCompose,
} from "../../api/twinSearchCompetitionDrNdShadowWeeklyMarketplaceCompose";

export default function TwinSearchCompetitionDrNdShadowWeeklyMarketplacePanel() {
  const [ack, setAck] = useState(true);
  const [query, setQuery] = useState("scaling orchestration residual");
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<TwinSearchCompetitionDrNdShadowWeeklyMarketplaceCompose | null>(
      null,
    );
  ...
  return (
    <div data-testid="twin-search-competition-dr-nd-weekly-panel">
      <LemonCard title="Research · twin intelligent search over competition DR + ND weekly">
        <p className="text-sm opacity-80">
          Intelligent search over twin substrate derived from competition gaps +
          arxiv/substack citations, stacked on ND shadow REJECT + Antiek-bench
          weekly free marketplace. Pure — never remote-indexes or writes twins.
        </p>
