/**
 * WriteTwinCollectiveHighlightFloatTwinSearchPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeWriteTwinCollectiveHighlightFloatTwinSearch,
  formatWriteTwinCollectiveHighlightFloatTwinSearchSummary,
  type WriteTwinCollectiveHighlightFloatTwinSearchCompose,
} from "../../api/writeTwinCollectiveHighlightFloatTwinSearchCompose";

export default function WriteTwinCollectiveHighlightFloatTwinSearchPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<WriteTwinCollectiveHighlightFloatTwinSearchCompose | null>(null);
  ...
  return (
    <div data-testid="write-twin-collective-highlight-float-twin-search-panel">
      <LemonCard title="Research · write twin collective + highlight float twin-search">
        <p className="text-sm opacity-80">
          Write-mode twin draft + collective analysis over highlight float DR +
          twin intelligent search competition pack. Pure — never writes assets.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatWriteTwinCollectiveHighlightFloatTwinSearchSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
