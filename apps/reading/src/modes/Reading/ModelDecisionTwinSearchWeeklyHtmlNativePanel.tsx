/**
 * ModelDecisionTwinSearchWeeklyHtmlNativePanel — free-file.
 * Model decision-tree usage bar + projection over twin search weekly HTML-native.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeModelDecisionTwinSearchWeeklyHtmlNative,
  formatModelDecisionTwinSearchWeeklyHtmlNativeSummary,
  type ModelDecisionTwinSearchWeeklyHtmlNativeCompose,
} from "../../api/modelDecisionTwinSearchWeeklyHtmlNativeCompose";

export default function ModelDecisionTwinSearchWeeklyHtmlNativePanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<ModelDecisionTwinSearchWeeklyHtmlNativeCompose | null>(null);

  function onCompose() {
    setError(null);
    setResult(null);
    try {
      // Deep pack fixture lives in tests; panel demos decision + search surface.
      // Call compose with a deliberately minimal twin_search_pack may throw or
      // fail pack_ready — wrap so operators see honesty gates.
      setResult(
        composeModelDecisionTwinSearchWeeklyHtmlNative({
          decision: {
  ...
  return (
    <div data-testid="model-decision-twin-search-weekly-html-native-panel">
      <LemonCard title="Settings · model decision + twin search weekly">
        <p className="text-sm opacity-80">
          Decision-tree usage bar + prompt projection over twin intelligent
          search + Antiek-bench weekly HTML-native pack. Pure — no live route.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatModelDecisionTwinSearchWeeklyHtmlNativeSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
