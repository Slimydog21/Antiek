/**
 * AntiekBenchWeeklyHtmlNativeRecursiveTwinPanel — free-file.
 * Weekly Antiek-bench learn surface over HTML-native recursive twin settings MO.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeAntiekBenchWeeklyHtmlNativeRecursiveTwin,
  formatAntiekBenchWeeklyHtmlNativeRecursiveTwinSummary,
  type AntiekBenchWeeklyHtmlNativeRecursiveTwinCompose,
} from "../../api/antiekBenchWeeklyHtmlNativeRecursiveTwinCompose";

export default function AntiekBenchWeeklyHtmlNativeRecursiveTwinPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<AntiekBenchWeeklyHtmlNativeRecursiveTwinCompose | null>(null);
  ...
  return (
    <div data-testid="antiek-bench-weekly-html-native-recursive-twin-panel">
      <LemonCard title="Antiek-bench · weekly learn + HTML-native twin pack">
        <p className="text-sm opacity-80">
          Weekly usage-learn rewrite proposals over HTML-native recursive twin
          settings fullscreen MO. Pure — backlog/store/suite never mutated.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatAntiekBenchWeeklyHtmlNativeRecursiveTwinSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
