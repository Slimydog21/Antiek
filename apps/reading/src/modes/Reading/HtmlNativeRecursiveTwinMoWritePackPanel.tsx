/**
 * HtmlNativeRecursiveTwinMoWritePackPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeHtmlNativeRecursiveTwinMoWritePack,
  formatHtmlNativeRecursiveTwinMoWritePackSummary,
  type HtmlNativeRecursiveTwinMoWritePackCompose,
} from "../../api/htmlNativeRecursiveTwinMoWritePackCompose";

export default function HtmlNativeRecursiveTwinMoWritePackPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<HtmlNativeRecursiveTwinMoWritePackCompose | null>(null);
  ...
  return (
    <div data-testid="html-native-recursive-twin-mo-write-pack-panel">
      <LemonCard title="Reading · HTML-native + recursive twin MO write pack">
        <p className="text-sm opacity-80">
          HTML-native view authority over recursive twin + MO price-ceiling
          write pack. Pure — never PDF primary, never writes twin, never charges.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatHtmlNativeRecursiveTwinMoWritePackSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
