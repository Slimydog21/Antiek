/**
 * MarketplaceFreeAntiekBenchRecommendMoPanel — free-file.
 * Free-before-buy HTML port over Antiek-bench recommend + MO unattended.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMarketplaceFreeAntiekBenchRecommendMo,
  formatMarketplaceFreeAntiekBenchRecommendMoSummary,
  type MarketplaceFreeAntiekBenchRecommendMoCompose,
} from "../../api/marketplaceFreeAntiekBenchRecommendMoCompose";

export default function MarketplaceFreeAntiekBenchRecommendMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MarketplaceFreeAntiekBenchRecommendMoCompose | null>(null);

  function onCompose() {
    setError(null);
    setResult(null);
    try {
      // Free market path only — full nested bench_mo proven in pure tests.
      // Panel demos free-before-buy honesty; pack_ready may be false if
      // nested MO pack is incomplete (intentional free-file surface).
      setResult(
        composeMarketplaceFreeAntiekBenchRecommendMo({
          market: {
  ...
  return (
    <div data-testid="marketplace-free-antiek-bench-recommend-mo-panel">
      <LemonCard title="Marketplace · free-before-buy + bench MO">
        <p className="text-sm opacity-80">
          Prefer free HTML copy before buy; port intent over Antiek-bench
          task→model rec + Midnight Oil unattended. Pure — never purchases or
          hosts.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatMarketplaceFreeAntiekBenchRecommendMoSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
