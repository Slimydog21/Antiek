/**
 * AntiekBenchRecommendMoUnattendedPanel — free-file.
 * Antiek-bench task→model rec over MO unattended + source attach pack.
 */
...
} from "../../api/antiekBenchRecommendMoUnattendedCompose";

export default function AntiekBenchRecommendMoUnattendedPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<AntiekBenchRecommendMoUnattendedCompose | null>(null);
  ...
  return (
    <div data-testid="antiek-bench-recommend-mo-unattended-panel">
      <LemonCard title="Antiek-bench · task model rec + MO unattended">
        <p className="text-sm opacity-80">
          Weekly usage → task→model recommendation into the decision tree,
          stacked on Midnight Oil unattended + source attach pack. Pure —
          never live-routes.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatAntiekBenchRecommendMoUnattendedSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
