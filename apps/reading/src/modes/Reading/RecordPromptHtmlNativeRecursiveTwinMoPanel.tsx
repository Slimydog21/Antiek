/**
 * RecordPromptHtmlNativeRecursiveTwinMoPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeRecordPromptHtmlNativeRecursiveTwinMo,
  formatRecordPromptHtmlNativeRecursiveTwinMoSummary,
  type RecordPromptHtmlNativeRecursiveTwinMoCompose,
} from "../../api/recordPromptHtmlNativeRecursiveTwinMoCompose";

export default function RecordPromptHtmlNativeRecursiveTwinMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<RecordPromptHtmlNativeRecursiveTwinMoCompose | null>(null);
  ...
  return (
    <div data-testid="record-prompt-html-native-recursive-twin-mo-panel">
      <LemonCard title="Workstation · record→prompt + HTML-native recursive twin MO">
        <p className="text-sm opacity-80">
          Records inform prompts with model budget projection over the
          HTML-native recursive twin MO pack. Pure — never injects prompts.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatRecordPromptHtmlNativeRecursiveTwinMoSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
