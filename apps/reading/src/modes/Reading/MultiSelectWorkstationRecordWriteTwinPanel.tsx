/**
 * MultiSelectWorkstationRecordWriteTwinPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMultiSelectWorkstationRecordWriteTwin,
  formatMultiSelectWorkstationRecordWriteTwinSummary,
  type MultiSelectWorkstationRecordWriteTwinCompose,
} from "../../api/multiSelectWorkstationRecordWriteTwinCompose";

export default function MultiSelectWorkstationRecordWriteTwinPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MultiSelectWorkstationRecordWriteTwinCompose | null>(null);
  ...
  return (
    <div data-testid="multiselect-workstation-record-write-twin-panel">
      <LemonCard title="Research · multi-select + workstation record write twin">
        <p className="text-sm opacity-80">
          Floating multi-select cohesive unit over workstation record→prompt +
          write twin highlight pack. Pure — never live-dispatches packs.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatMultiSelectWorkstationRecordWriteTwinSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
