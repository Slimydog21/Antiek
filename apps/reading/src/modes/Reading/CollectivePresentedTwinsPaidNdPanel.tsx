/**
 * CollectivePresentedTwinsPaidNdPanel — free-file.
 * Multi-select collective cohesive of presented twins over free-first paid
 * purchase + ND shadow twin presentation pack. Pure advisory only.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeCollectivePresentedTwinsPaidNd,
  formatCollectivePresentedTwinsPaidNdSummary,
  type CollectivePresentedTwinsPaidNdCompose,
} from "../../api/collectivePresentedTwinsPaidNdCompose";

export default function CollectivePresentedTwinsPaidNdPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<CollectivePresentedTwinsPaidNdCompose | null>(null);
      ...
      setResult(
        composeCollectivePresentedTwinsPaidNd({
          collective: {
            ...
            cohesive_prompt: "Synthesize presented twin instances as one unit",
          },
          paid_nd: {
            purchase: {
  ...
  return (
    <LemonCard
      ...
      data-testid="collective-presented-twins-paid-nd-panel"
    >
      <h2 className="text-lg font-semibold">
        Collective presented twins · paid ND pack
      </h2>
      <p className="text-sm text-muted">
        Pure advisory — live_dispatched / merge_executed / purchase_executed
      ...
      <LemonButton type="primary" onClick={onCompose}>
        Compose collective pack
      </LemonButton>
      {error && (
        <pre className="text-xs text-danger whitespace-pre-wrap">{error}</pre>
      )}
      {result && (
        <div className="text-sm" data-testid="summary">
          {formatCollectivePresentedTwinsPaidNdSummary(result)}
        </div>
      )}
    </LemonCard>
  );
