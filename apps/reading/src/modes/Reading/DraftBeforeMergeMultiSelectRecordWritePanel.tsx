/**
 * DraftBeforeMergeMultiSelectRecordWritePanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeDraftBeforeMergeMultiSelectRecordWrite,
  formatDraftBeforeMergeMultiSelectRecordWriteSummary,
  type DraftBeforeMergeMultiSelectRecordWriteCompose,
} from "../../api/draftBeforeMergeMultiSelectRecordWriteCompose";

export default function DraftBeforeMergeMultiSelectRecordWritePanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<DraftBeforeMergeMultiSelectRecordWriteCompose | null>(null);
  ...
  return (
    <div data-testid="draft-before-merge-multiselect-record-write-panel">
      <LemonCard title="Research · draft-before-merge + multi-select record write">
        <p className="text-sm opacity-80">
          Provisional combined draft before full merge, over multi-select
          cohesive + workstation record write twin pack. Pure — never merges.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatDraftBeforeMergeMultiSelectRecordWriteSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
