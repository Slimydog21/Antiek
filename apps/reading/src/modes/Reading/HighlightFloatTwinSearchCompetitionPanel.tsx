/**
 * HighlightFloatTwinSearchCompetitionPanel — free-file.
 */
...
} from "../../api/highlightFloatTwinSearchCompetitionCompose";

export default function HighlightFloatTwinSearchCompetitionPanel() {
  const [ack, setAck] = useState(true);
  const [highlight, setHighlight] = useState(
    "scaling orchestration residual under noise",
  );
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<HighlightFloatTwinSearchCompetitionCompose | null>(null);
  ...
  return (
    <div data-testid="highlight-float-twin-search-competition-panel">
      <LemonCard title="Research · highlight float → twin search competition">
        <p className="text-sm opacity-80">
          From a highlight, float deep research then twin-search the competition
          DR + ND weekly pack. Pure — never live-dispatches or remote-indexes.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatHighlightFloatTwinSearchCompetitionSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
