/**
 * SettingsAddModelFullscreenMoDraftMultiPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeSettingsAddModelFullscreenMoDraftMulti,
  formatSettingsAddModelFullscreenMoDraftMultiSummary,
  type SettingsAddModelFullscreenMoDraftMultiCompose,
} from "../../api/settingsAddModelFullscreenMoDraftMultiCompose";

export default function SettingsAddModelFullscreenMoDraftMultiPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<SettingsAddModelFullscreenMoDraftMultiCompose | null>(null);
  ...
  return (
    <div data-testid="settings-add-model-fullscreen-mo-draft-multi-panel">
      <LemonCard title="Research · settings add-model + fullscreen MO draft multi">
        <p className="text-sm opacity-80">
          Settings add-model inventory (BYOK honesty + usage bar) over
          fullscreen DR + MO price-ceiling draft multi pack. Pure — never
          stores secrets.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatSettingsAddModelFullscreenMoDraftMultiSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
