/**
 * AntiekBenchSourceAttachSettingsMoPanel — free-file.
 * Antiek-bench task→model recommendation over source attach + settings + MO.
 */
...
} from "../../api/antiekBenchSourceAttachSettingsMoCompose";

export default function AntiekBenchSourceAttachSettingsMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<AntiekBenchSourceAttachSettingsMoCompose | null>(null);
  ...
  return (
    <LemonCard
      ...
      data-testid="antiek-bench-source-attach-settings-mo-panel"
    >
      <h2 className="text-lg font-semibold">
        Antiek-bench recommend · source attach · settings · MO
      </h2>
      <p className="text-sm text-muted">
        Pure advisory — live_router / suite_rewritten / remote_fetched always
        false.
      </p>
      ...
      {result && (
        <div className="space-y-1 text-sm">
          <div data-testid="summary">
            {formatAntiekBenchSourceAttachSettingsMoSummary(result)}
          </div>
          <div>
            rec=
            <strong>
              {result.bench.recommendation?.recommended_model_id ?? "null"}
            </strong>{" "}
