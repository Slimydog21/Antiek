/**
 * AntiekBenchWeeklyNdMultiselectMoPanel — free-file.
 */
...
} from "../../api/antiekBenchWeeklyNdMultiselectMoCompose";

export default function AntiekBenchWeeklyNdMultiselectMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<AntiekBenchWeeklyNdMultiselectMoCompose | null>(null);
  ...
  return (
    <div data-testid="antiek-bench-weekly-nd-multiselect-mo-panel">
      <LemonCard title="Settings · Antiek-bench weekly learn + ND multi-select MO">
        <p className="text-sm opacity-80">
          Weekly usage-learn proposes bench rewrites while ND shadow REJECT
          multi-select research pack stays ready. Pure — no bench store mutate,
          no live router.
        </p>
        ...
        <LemonButton
          ...
          onClick={onCompose}
          className="mt-2"
          data-testid="abwnm-compose"
        >
          Compose weekly learn + ND multi-select pack
        </LemonButton>
        {error && <p className="text-sm text-danger mt-2">{error}</p>}
        {result && (
          <div className="mt-3 text-sm" data-testid="abwnm-result">
            <p>{formatAntiekBenchWeeklyNdMultiselectMoSummary(result)}</p>
            <ul className="list-disc pl-5">
              <li>pack_ready={String(result.pack_ready)}</li>
              <li>proposals={result.weekly_learn.proposal_count}</li>
              <li>backlog_mutated={String(result.backlog_mutated)}</li>
              <li>
