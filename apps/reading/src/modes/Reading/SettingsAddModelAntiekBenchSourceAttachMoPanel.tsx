/**
 * SettingsAddModelAntiekBenchSourceAttachMoPanel — free-file.
 * Settings BYOK add-model inventory over Antiek-bench + source attach + MO pack.
 */
...
} from "../../api/settingsAddModelAntiekBenchSourceAttachMoCompose";

export default function SettingsAddModelAntiekBenchSourceAttachMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<SettingsAddModelAntiekBenchSourceAttachMoCompose | null>(null);
  ...
  return (
    <LemonCard
      ...
      data-testid="settings-add-model-antiek-bench-source-attach-mo-panel"
    >
      <h2 className="text-lg font-semibold">
        Settings add-model · Antiek-bench · source attach · MO
      </h2>
      <p className="text-sm text-muted">
        Pure advisory — secrets_stored / inventory_mutated / live_router /
      ...
      {result && (
        <div className="space-y-1 text-sm">
          <div data-testid="summary">
            {formatSettingsAddModelAntiekBenchSourceAttachMoSummary(result)}
          </div>
          <div>
            proposed_new=
            <strong>{result.settings.proposed_new_count}</strong> · vs=
            <strong>{result.inventory_vs_bench}</strong> · rec=
            <strong>
              {result.bench_pack.bench.recommendation?.recommended_model_id ??
                "null"}
            </strong>{" "}
