/**
 * MoPriceCeilingWriteTwinSettingsDraftPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMoPriceCeilingWriteTwinSettingsDraft,
  formatMoPriceCeilingWriteTwinSettingsDraftSummary,
  type MoPriceCeilingWriteTwinSettingsDraftCompose,
} from "../../api/moPriceCeilingWriteTwinSettingsDraftCompose";

export default function MoPriceCeilingWriteTwinSettingsDraftPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MoPriceCeilingWriteTwinSettingsDraftCompose | null>(null);

  function onCompose() {
    setError(null);
    setResult(null);
    try {
      // Demo uses recommend_only + minimal research_write via nested pure modules
      setResult(
        composeMoPriceCeilingWriteTwinSettingsDraft({
          mo: {
            operator_id: "op-demo",
  ...
  return (
    <div data-testid="mo-price-ceiling-write-twin-settings-draft-panel">
      <LemonCard title="Midnight Oil · price ceiling + write twin settings pack">
        <p className="text-sm opacity-80">
          Recommended price ceiling over write twin collective + settings draft
          fullscreen ND. Pure — never charges or live-executes.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatMoPriceCeilingWriteTwinSettingsDraftSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
