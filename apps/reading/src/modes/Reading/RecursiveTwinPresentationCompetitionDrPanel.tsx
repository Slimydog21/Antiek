/**
 * RecursiveTwinPresentationCompetitionDrPanel — free-file.
 * Twin note-taker presentation (side-panel/overlay/fullscreen/inline)
 * over competition DR quality + free marketplace bench MO pack.
...
} from "../../api/recursiveTwinPresentationCompetitionDrCompose";
...
export default function RecursiveTwinPresentationCompetitionDrPanel() {
  ...
  const [result, setResult] =
    useState<RecursiveTwinPresentationCompetitionDrCompose | null>(null);
  ...
  return (
    <LemonCard className="p-4 space-y-3" data-testid="twin-presentation-competition-dr-panel">
      <h2 className="text-lg font-semibold">
        Recursive twin presentation · competition DR
      </h2>
      ...
      {result && (
        <div className="space-y-2 text-sm">
          <div data-testid="summary">
            {formatRecursiveTwinPresentationCompetitionDrSummary(result)}
          </div>
          <div>
            pack_ready=<strong>{String(result.pack_ready)}</strong> ·
            presentation_ready=
            <strong>{String(result.presentation.presentation_ready)}</strong> ·
            view_mode=<strong>{result.presentation.view_mode}</strong>
          </div>
          <div>
          ...
          <details>
            <summary>presentation_sections ({result.presentation.presentation_sections.length})</summary>
            <pre className="text-xs overflow-auto max-h-48">
              {result.presentation.presentation_sections.join("\n")}
            </pre>
          </details>
        </div>
      )}
