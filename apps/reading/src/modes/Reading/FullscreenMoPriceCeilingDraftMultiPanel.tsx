/**
 * FullscreenMoPriceCeilingDraftMultiPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeFullscreenMoPriceCeilingDraftMulti,
  formatFullscreenMoPriceCeilingDraftMultiSummary,
  type FullscreenMoPriceCeilingDraftMultiCompose,
} from "../../api/fullscreenMoPriceCeilingDraftMultiCompose";

export default function FullscreenMoPriceCeilingDraftMultiPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<FullscreenMoPriceCeilingDraftMultiCompose | null>(null);
  ...
  return (
    <div data-testid="fullscreen-mo-price-ceiling-draft-multi-panel">
      <LemonCard title="Research · fullscreen + MO price-ceiling draft multi">
        <p className="text-sm opacity-80">
          Fullscreen floating DR open over MO price-ceiling + draft multi-select
          pack. Pure — never live-dispatches.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatFullscreenMoPriceCeilingDraftMultiSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
