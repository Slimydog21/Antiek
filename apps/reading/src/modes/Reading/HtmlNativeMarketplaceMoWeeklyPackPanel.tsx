/**
 * HtmlNativeMarketplaceMoWeeklyPackPanel — free-file.
 */
...
export default function HtmlNativeMarketplaceMoWeeklyPackPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<HtmlNativeMarketplaceMoWeeklyPackCompose | null>(null);
  ...
  function onCompose() {
    ...
    try {
      ...
      void ack;
      void composeHtmlNativeMarketplaceMoWeeklyPack;
      void formatHtmlNativeMarketplaceMoWeeklyPackSummary;
    } catch (e) {
    ...
    <div className="flex flex-col gap-3 p-4 max-w-3xl">
      ...
      {result && (
        <LemonCard className="p-3 text-sm">
          {formatHtmlNativeMarketplaceMoWeeklyPackSummary(result)}
        </LemonCard>
      )}
    </div>
  );
