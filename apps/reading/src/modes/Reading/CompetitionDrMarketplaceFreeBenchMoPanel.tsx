/**
 * CompetitionDrMarketplaceFreeBenchMoPanel — free-file.
 * Competition DR quality over free-before-buy + bench recommend MO pack.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeCompetitionDrMarketplaceFreeBenchMo,
  formatCompetitionDrMarketplaceFreeBenchMoSummary,
  type CompetitionDrMarketplaceFreeBenchMoCompose,
} from "../../api/competitionDrMarketplaceFreeBenchMoCompose";

export default function CompetitionDrMarketplaceFreeBenchMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<CompetitionDrMarketplaceFreeBenchMoCompose | null>(null);

  function onCompose() {
    setError(null);
    setResult(null);
    try {
      // Full nested free_pack proven in pure tests; panel demos competition
      // surface + free market honesty. Uses compact free market + minimal
      // competition for UI craft; deep nest may yield pack_ready via tests.
      setResult(
        composeCompetitionDrMarketplaceFreeBenchMo({
          competition: {
  ...
  return (
    <div data-testid="competition-dr-marketplace-free-bench-mo-panel">
      <LemonCard title="Research · competition DR + free market bench MO">
        <p className="text-sm opacity-80">
          Competition gap + arxiv/substack quality pack over free-before-buy +
          Antiek-bench recommend + Midnight Oil. Pure — never live-dispatches.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatCompetitionDrMarketplaceFreeBenchMoSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
