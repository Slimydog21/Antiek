/**
 * MoUnattendedSourceAttachModelDecisionPanel — free-file.
 * Midnight Oil unattended package over source attach + model decision pack.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMoUnattendedSourceAttachModelDecision,
  formatMoUnattendedSourceAttachModelDecisionSummary,
  type MoUnattendedSourceAttachModelDecisionCompose,
} from "../../api/moUnattendedSourceAttachModelDecisionCompose";

export default function MoUnattendedSourceAttachModelDecisionPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MoUnattendedSourceAttachModelDecisionCompose | null>(null);
  ...
  return (
    <div data-testid="mo-unattended-source-attach-model-decision-panel">
      <LemonCard title="Midnight Oil · unattended + source attach decision">
        <p className="text-sm opacity-80">
          Time + goals + price ceiling approval for unattended deep research
          over HTML-native source attach + model decision pack. Pure — never
          live-executes.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatMoUnattendedSourceAttachModelDecisionSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
