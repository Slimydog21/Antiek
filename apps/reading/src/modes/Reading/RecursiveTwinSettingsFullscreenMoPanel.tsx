/**
 * RecursiveTwinSettingsFullscreenMoPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeRecursiveTwinSettingsFullscreenMo,
  formatRecursiveTwinSettingsFullscreenMoSummary,
  type RecursiveTwinSettingsFullscreenMoCompose,
} from "../../api/recursiveTwinSettingsFullscreenMoCompose";

export default function RecursiveTwinSettingsFullscreenMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<RecursiveTwinSettingsFullscreenMoCompose | null>(null);
  ...
  return (
    <div data-testid="recursive-twin-settings-fullscreen-mo-panel">
      <LemonCard title="Research · recursive twin + settings fullscreen MO">
        <p className="text-sm opacity-80">
          Recursive twin note-taker (insights + questions twin) over settings
          add-model + fullscreen MO draft multi pack. Pure — never writes twins.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatRecursiveTwinSettingsFullscreenMoSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
