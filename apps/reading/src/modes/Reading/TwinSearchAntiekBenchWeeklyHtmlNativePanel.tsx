/**
 * TwinSearchAntiekBenchWeeklyHtmlNativePanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeTwinSearchAntiekBenchWeeklyHtmlNative,
  formatTwinSearchAntiekBenchWeeklyHtmlNativeSummary,
  type TwinSearchAntiekBenchWeeklyHtmlNativeCompose,
} from "../../api/twinSearchAntiekBenchWeeklyHtmlNativeCompose";

export default function TwinSearchAntiekBenchWeeklyHtmlNativePanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<TwinSearchAntiekBenchWeeklyHtmlNativeCompose | null>(null);
  ...
  return (
    <div data-testid="twin-search-antiek-bench-weekly-html-native-panel">
      <LemonCard title="Research · twin search + weekly HTML-native">
        <p className="text-sm opacity-80">
          Intelligent twin substrate search over Antiek-bench weekly learn +
          HTML-native recursive twin pack. Pure — no remote index, no suite
          rewrite.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatTwinSearchAntiekBenchWeeklyHtmlNativeSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
