/**
 * AntiekBenchWeeklyMarketplaceFreeSourcePanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeAntiekBenchWeeklyMarketplaceFreeSource,
  formatAntiekBenchWeeklyMarketplaceFreeSourceSummary,
  type AntiekBenchWeeklyMarketplaceFreeSourceCompose,
} from "../../api/antiekBenchWeeklyMarketplaceFreeSourceCompose";

export default function AntiekBenchWeeklyMarketplaceFreeSourcePanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<AntiekBenchWeeklyMarketplaceFreeSourceCompose | null>(null);
  ...
  return (
    <div data-testid="antiek-bench-weekly-marketplace-free-source-panel">
      <LemonCard title="Settings · Antiek-bench weekly + free marketplace research">
        <p className="text-sm opacity-80">
          Weekly usage-learn rewrite proposals over free-before-buy marketplace
          + arxiv/substack research pack. Pure — never mutates bench store.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatAntiekBenchWeeklyMarketplaceFreeSourceSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
