/**
 * SettingsAddModelNdShadowTwinPresentationPanel — free-file.
 * Settings add-model (BYOK ids) over ND shadow REJECT + twin presentation pack.
 * Pure advisory — secrets_stored/inventory_mutated always false.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeSettingsAddModelNdShadowTwinPresentation,
  formatSettingsAddModelNdShadowTwinPresentationSummary,
  type SettingsAddModelNdShadowTwinPresentationCompose,
} from "../../api/settingsAddModelNdShadowTwinPresentationCompose";

export default function SettingsAddModelNdShadowTwinPresentationPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<SettingsAddModelNdShadowTwinPresentationCompose | null>(null);
          ...
          nd_pack: {
            ...
            twin_presentation: {
              ...
              presentation: {
                ...
                presented_insights: ["scaling holds"],
  ...
  return (
    <LemonCard
      ...
      data-testid="settings-add-model-nd-shadow-twin-presentation-panel"
    >
      <h2 className="text-lg font-semibold">
        Settings add-model · ND shadow twin presentation
      </h2>
      <p className="text-sm text-muted">
        Pure advisory — secrets_stored / inventory_mutated always false. ND
        production REJECT.
      </p>
      ...
      {result && (
        <div className="space-y-2 text-sm">
          <div data-testid="summary">
            {formatSettingsAddModelNdShadowTwinPresentationSummary(result)}
          </div>
          <div>
            pack_ready=<strong>{String(result.pack_ready)}</strong> ·
            settings_ready=
            <strong>{String(result.settings.pack_ready)}</strong> · proposed=
            <strong>{result.settings.proposed_new_count}</strong>
          </div>
          <div>
            secrets_stored={String(result.secrets_stored)} · inventory_mutated=
            {String(result.inventory_mutated)} · live_router=
            {String(result.live_router_authorized)} · verdict=
