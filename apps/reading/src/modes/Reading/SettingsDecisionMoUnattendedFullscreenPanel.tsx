/**
 * SettingsDecisionMoUnattendedFullscreenPanel — free-file.
 * Settings decision-tree usage bar + budget projection over MO unattended
 * fullscreen draft collective pack. Pure advisory only.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeSettingsDecisionMoUnattendedFullscreen,
  formatSettingsDecisionMoUnattendedFullscreenSummary,
  type SettingsDecisionMoUnattendedFullscreenCompose,
} from "../../api/settingsDecisionMoUnattendedFullscreenCompose";

export default function SettingsDecisionMoUnattendedFullscreenPanel() {
  const [ack, setAck] = useState(true);
  const [spent, setSpent] = useState(10);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<SettingsDecisionMoUnattendedFullscreenCompose | null>(null);
  ...
  return (
    <LemonCard
      ...
      data-testid="settings-decision-mo-unattended-fullscreen-panel"
    >
      <h2 className="text-lg font-semibold">
        Settings decision tree · MO unattended fullscreen
      </h2>
      <p className="text-sm text-muted">
        Pure advisory — live_router_authorized / live_execution_authorized
        always false. Budget projection blocks pack when would_exceed.
      </p>
      ...
      {result && (
        <div className="space-y-1 text-sm">
          <div data-testid="summary">
            {formatSettingsDecisionMoUnattendedFullscreenSummary(result)}
          </div>
          <div>
            ...
            usage_percent=
            <strong>
              {result.decision.usage_percent === null
                ? "null"
                : result.decision.usage_percent.toFixed(1)}
            </strong>
          </div>
        </div>
      )}
