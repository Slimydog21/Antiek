/**
 * HtmlNativeRecursiveTwinSettingsFullscreenMoPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeHtmlNativeRecursiveTwinSettingsFullscreenMo,
  formatHtmlNativeRecursiveTwinSettingsFullscreenMoSummary,
  type HtmlNativeRecursiveTwinSettingsFullscreenMoCompose,
} from "../../api/htmlNativeRecursiveTwinSettingsFullscreenMoCompose";

export default function HtmlNativeRecursiveTwinSettingsFullscreenMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<HtmlNativeRecursiveTwinSettingsFullscreenMoCompose | null>(null);
  ...
  return (
    <div data-testid="html-native-recursive-twin-settings-fullscreen-mo-panel">
      <LemonCard title="Research · HTML-native + recursive twin settings MO">
        <p className="text-sm opacity-80">
          HTML-native view session authority (never PDF-primary) over recursive
          twin note-taker + settings fullscreen MO pack. Pure doctrine.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatHtmlNativeRecursiveTwinSettingsFullscreenMoSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
