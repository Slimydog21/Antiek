/**
 * MarketplaceFreeSettingsAddModelNdShadowTwinPanel — free-file.
 * Free-before-buy HTML port over settings add-model + ND twin presentation.
 * Pure advisory — purchase_executed/hosted always false.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMarketplaceFreeSettingsAddModelNdShadowTwin,
  formatMarketplaceFreeSettingsAddModelNdShadowTwinSummary,
  type MarketplaceFreeSettingsAddModelNdShadowTwinCompose,
} from "../../api/marketplaceFreeSettingsAddModelNdShadowTwinCompose";

export default function MarketplaceFreeSettingsAddModelNdShadowTwinPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MarketplaceFreeSettingsAddModelNdShadowTwinCompose | null>(null);

  function onCompose() {
    setError(null);
    setResult(null);
    try {
      // Full nest proven in pure tests; demo uses free market + compact settings/nd.
      setResult(
        composeMarketplaceFreeSettingsAddModelNdShadowTwin({
          market: {
  ...
  return (
    <LemonCard
      ...
      data-testid="marketplace-free-settings-nd-twin-panel"
    >
      <h2 className="text-lg font-semibold">
        Marketplace free · settings ND twin
      </h2>
      <p className="text-sm text-muted">
        Pure advisory — purchase_executed / hosted always false. ND REJECT.
      </p>
      ...
      {result && (
        <div className="space-y-2 text-sm">
          <div data-testid="summary">
            {formatMarketplaceFreeSettingsAddModelNdShadowTwinSummary(result)}
          </div>
          <div>
            pack_ready=<strong>{String(result.pack_ready)}</strong> ·
            port_ready=<strong>{String(result.market.port_ready)}</strong>
          </div>
          <div>
            purchase_executed={String(result.purchase_executed)} · hosted=
            {String(result.hosted)} · verdict=
