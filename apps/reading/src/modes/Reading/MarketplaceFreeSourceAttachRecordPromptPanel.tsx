/**
 * MarketplaceFreeSourceAttachRecordPromptPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMarketplaceFreeSourceAttachRecordPrompt,
  formatMarketplaceFreeSourceAttachRecordPromptSummary,
  type MarketplaceFreeSourceAttachRecordPromptCompose,
} from "../../api/marketplaceFreeSourceAttachRecordPromptCompose";

export default function MarketplaceFreeSourceAttachRecordPromptPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MarketplaceFreeSourceAttachRecordPromptCompose | null>(null);
  ...
  return (
    <div data-testid="marketplace-free-source-attach-record-prompt-panel">
      <LemonCard title="Marketplace · free HTML + source-attach research pack">
        <p className="text-sm opacity-80">
          Free-before-buy HTML port stacked with arxiv/substack attach and
          record→prompt pack. Pure — never purchases or hosts.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatMarketplaceFreeSourceAttachRecordPromptSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
