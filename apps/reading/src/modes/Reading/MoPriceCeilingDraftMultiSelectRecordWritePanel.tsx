/**
 * MoPriceCeilingDraftMultiSelectRecordWritePanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeMoPriceCeilingDraftMultiSelectRecordWrite,
  formatMoPriceCeilingDraftMultiSelectRecordWriteSummary,
  type MoPriceCeilingDraftMultiSelectRecordWriteCompose,
} from "../../api/moPriceCeilingDraftMultiSelectRecordWriteCompose";

export default function MoPriceCeilingDraftMultiSelectRecordWritePanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<MoPriceCeilingDraftMultiSelectRecordWriteCompose | null>(null);
            ...
            multi_pack: {
              ...
              record_write: {
                record_prompt: {
                  ...
                  parent_asset_id: "book-demo",
                  records: [
                    { record_id: "r1", kind: "insight", body: "holds" },
                    { record_id: "r2", kind: "question", body: "gaps?" },
                  ],
  ...
  return (
    <div data-testid="mo-price-ceiling-draft-multiselect-record-write-panel">
      <LemonCard title="Research · MO price-ceiling + draft multi-select pack">
        <p className="text-sm opacity-80">
          Midnight Oil recommended price ceiling over draft-before-merge +
          multi-select record write twin pack. Pure — never charges or launches.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatMoPriceCeilingDraftMultiSelectRecordWriteSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
