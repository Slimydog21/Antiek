/**
 * SourceAttachRecordPromptHtmlNativeMoPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeSourceAttachRecordPromptHtmlNativeMo,
  formatSourceAttachRecordPromptHtmlNativeMoSummary,
  type SourceAttachRecordPromptHtmlNativeMoCompose,
} from "../../api/sourceAttachRecordPromptHtmlNativeMoCompose";

export default function SourceAttachRecordPromptHtmlNativeMoPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<SourceAttachRecordPromptHtmlNativeMoCompose | null>(null);
        ...
        composeSourceAttachRecordPromptHtmlNativeMo({
          ...
          record_html: {
            record_prompt: {
              ...
              user_prompt: "Summarize open questions with arxiv/substack context",
  ...
  return (
    <div data-testid="source-attach-record-prompt-html-native-mo-panel">
      <LemonCard title="Research · arxiv/substack attach + record→prompt HTML pack">
        <p className="text-sm opacity-80">
          HTML-native arxiv/substack attach with citations and quality gate over
          record→prompt recursive twin MO pack. Pure — never remote-fetches.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatSourceAttachRecordPromptHtmlNativeMoSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
