/**
 * WorkstationRecordWriteTwinHighlightPanel — free-file.
 */
import { useState } from "react";
import { LemonButton, LemonCard } from "../../components/lemon";
import {
  composeWorkstationRecordWriteTwinHighlight,
  formatWorkstationRecordWriteTwinHighlightSummary,
  type WorkstationRecordWriteTwinHighlightCompose,
} from "../../api/workstationRecordWriteTwinHighlightCompose";

export default function WorkstationRecordWriteTwinHighlightPanel() {
  const [ack, setAck] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] =
    useState<WorkstationRecordWriteTwinHighlightCompose | null>(null);
      ...
      setResult(
        composeWorkstationRecordWriteTwinHighlight({
          record_prompt: {
            ...
            records: [
              { record_id: "r1", kind: "insight", body: "scaling holds" },
              { record_id: "r2", kind: "question", body: "failure mode?" },
            ],
            user_prompt: "Summarize with twin context",
  ...
  return (
    <div data-testid="workstation-record-write-twin-highlight-panel">
      <LemonCard title="Research · workstation record→prompt + write twin highlight">
        <p className="text-sm opacity-80">
          Records inject into model decision (usage bar + projection) over write
          twin collective + highlight float twin-search pack. Pure — never
          injects prompts or persists records.
        </p>
        ...
        {result && (
          <pre className="text-xs mt-2 p-2 bg-bg-light rounded overflow-auto max-h-64">
            {formatWorkstationRecordWriteTwinHighlightSummary(result)}
          </pre>
        )}
      </LemonCard>
    </div>
