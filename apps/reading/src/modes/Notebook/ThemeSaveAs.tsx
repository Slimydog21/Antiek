// SPR-11 / M5 — Save-as menu for the per-theme notebook surface.
//
// Renamed from `SaveAs` during integration: SPR-09 already shipped a
// `SaveAs.tsx` in this directory for the per-document notebook surface
// (different prop shape — takes `documentId` and calls
// `savePerDocNotebook`). The two components share no contract, so they
// live as siblings rather than as a discriminated union.
//
...
// and Markdown are also server-rendered for now. The /api/themes/
// :id/export endpoint is a follow-up PR; today the fetch will 404
// and the component surfaces a clean error state.

import { useCallback, useState } from "react";

import type { ThemeResponse } from "../../../api/themes/by-slug";

export interface ThemeSaveAsProps {
  theme: ThemeResponse;
}

type SaveFormat = "antiek" | "pdf" | "markdown";

export default function ThemeSaveAs(props: ThemeSaveAsProps): JSX.Element {
  ...
  return (
    <div className="relative inline-flex">
      <button
        ...
        data-testid="theme-save-as-toggle"
        aria-expanded={open}
      >
        Save as…
      </button>
      {open && (
        <ul
          ...
          data-testid="theme-save-as-menu"
        >
          <li>
            <button
              type="button"
              ...
              data-testid="theme-save-as-antiek"
            >
          ...
          <li>
            <button
              ...
              className="w-full text-left px-3 py-1.5 hover:bg-stone-50"
              data-testid="theme-save-as-markdown"
            >
