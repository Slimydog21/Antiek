...
describe("invent class product-map densify", () => {
  ...
  it("invent polish wave complete note pins densify gate + Flipbook NO-GO honesty", () => {
    ...
    expect(summary).toMatch(
      /29\/25[01]|32\/268|36\/297|37\/298|38\/299|39\/30[23]|42\/3[5-6][0-9]|43\/37[0-9]|44\/37[0-9]|45\/38[0-9]/,
    );
    expect(flipbook).toMatch(
      /39 files \/ 30[23] tests|38 files \/ 299 tests|42 files \/ 3[5-6][0-9] tests|43 files \/ 37[0-9] tests|44 files \/ 37[0-9] tests|45 files \/ 38[0-9] tests/,
    );
