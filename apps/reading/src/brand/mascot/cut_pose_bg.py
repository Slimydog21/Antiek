"""
Cut the baked near-white BACKGROUND out of the Brain emote poses (SPR-06 M2).

THE WHITE-BOX CRUX (anchor[mascot], v1 complaint #5)
-----------------------------------------------------
...
the corner pixels paint an ugly opaque WHITE BOX behind the mascot. We do NOT
...
--------------------------------------------------------------------------
Brain's BELLY and FACE are themselves near-white. A global "make every
near-white pixel transparent" pass would punch holes through his belly and
leave a ghost mascot. The background, by contrast, is the connected region of
neutral near-white reachable from the image EDGES — and it is SEPARATED from the
white belly by Brain's closed black/ink OUTLINE. So we FLOOD-FILL inward from
...
(poses/anchor/mascot_default_v5_transparent.png, produced by the same family of
script) already uses; this mirrors it for the emote poses.

EDGES (anti-aliasing): a near-white pixel adjacent to a kept (mascot) pixel is
...
Output: poses/<name>_transparent.png (e.g. mascot_thinking_v1_transparent.png).
...
def cut(image_path: Path, out_path: Path | None = None) -> Path:
    ...
    # neutral near-white pixels and is halted by the mascot's outline, so it
...
def main() -> None:
    ...
    targets = sorted(poses_dir.glob("mascot_*_v1_corrected.png"))
