...
describe("semantic motion proof sheet", () => {
  it("covers all four reactions and all four named review frames", () => {
    for (const reaction of ["curious", "happy", "dizzy", "hit"]) {
      expect(storySource).toContain(`${reaction}: Mascot`);
    }
  ...
  it("derives settle frames from the canonical duration table", () => {
    expect(storySource).toContain("MASCOT_SEMANTIC_DURATION_MS[reaction]");
  ...
  it("freezes every animated descendant and retains explicit reduced stills", () => {
    ...
    expect(cssSource).toContain(
      "animation-delay: var(--mascot-review-delay) !important",
    );
    ...
    for (const animation of [
      "mascot-curious-beat",
      "mascot-evidence-arrive",
      "mascot-proud-lift",
      "mascot-verified-beat",
      "mascot-paperclip-orbit",
      "mascot-hit-rebound",
      "mascot-tab-impact",
    ]) {
