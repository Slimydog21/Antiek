...
const meta = {
  title: "Brand / Mascot / Semantic motion proof",
  parameters: {
    layout: "fullscreen",
...
export const ContactSheet: Story = {
  render: () => (
    <main className="min-h-screen bg-ice-2 p-4 text-ink dark:bg-space-2 dark:text-bright md:p-8">
      <h1 className="font-serif text-xl">Brain — semantic motion proof</h1>
      ...
      <div className="grid grid-cols-[4.5rem_repeat(4,minmax(0,1fr))] gap-2 md:gap-3">
        ...
        {reactionOrder.map((reaction) => {
          ...
          const frames = [
            ...
            {
              name: "settle",
              elapsedMs: MASCOT_SEMANTIC_DURATION_MS[reaction],
              reduced: false,
            },
          ...
          return (
            <div key={reaction} className="contents">
              ...
              {frames.map((frame) => {
                const style = {
                  "--mascot-review-delay": `-${frame.elapsedMs}ms`,
                } as CSSProperties;
                return (
                  <div
                    key={frame.name}
                    className="mascot-motion-proof__frame rounded-hog border-edge border-sun bg-ice-0 p-3 dark:bg-charcoal-2 md:p-5"
