...
/**
 * The Antiek brain, caught a fish — one-shot completion celebration.
 *
 ...
 *
 * Core mark delegated to <BrainMascot mood="celebrate" /> + --mascot-* tokens
 * for any remaining accents. No more parallel geometry fork.
 */
type Props = { size?: number; label?: string };

export default function BrainCaughtAFish({
  size = 48,
  ...
  return (
    ...
    >
      {/* Core mark now single-source from canonical at celebrate fidelity.
          ...
          positioned to sit on the delegated BrainMascot. */}
      <BrainMascot mood="celebrate" size={size} />
      {/* Pose chrome overlays — minimal absolute SVGs for fish + sparkle only.
          The mascot body itself is fully delegated; no geometry duplication. */}
      <svg
        viewBox="0 0 64 64"
        width={size}
        height={size}
        ...
        aria-hidden="true"
      >
        <g className="mascot-fish-flipper" transform="translate(12 -4) scale(0.55)">
          <path d="M44 32 Q52 22 50 16 Q48 18 47 22 Q45 26 44 32 Z" fill="var(--mascot-coat)" />
