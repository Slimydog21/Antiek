...
/* LOSTPIXEL PIXEL-FREEZE: the visible h1/body strings below still say
   "Werner" ON PURPOSE — the brand-werner-animations--* baselines (now
   brand-mascot-animations--*) were renamed, not re-minted, so canvas pixels
   must stay byte-identical. Purge the rendered wording in the same PR that
   re-mints these baselines. */
const meta = {
  title: "Brand / Mascot / Animations",
    ...
    <div className="p-8 bg-ice-2 dark:bg-space-2 min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <Cell
          ...
          pose={<BrainTobogganSpinner size={64} />}
        />
        <Cell
          label="Thinking"
          hint="AISidecar 'thinking…' state"
          pose={<BrainThinking size={56} />}
        />
        <Cell
          label="Caught a fish"
          hint="investigation-complete toast · one-shot"
          pose={<BrainCaughtAFish size={72} />}
        />
        <Cell
          label="Sleeping"
          hint="idle screen · empty-state"
          pose={<BrainSleeping size={120} />}
        />
        <Cell
          label="Waddle"
          hint="route-navigation transition"
          pose={<BrainWaddle size={48} />}
        />
      </div>
    </div>
  ),
        ...
        {[24, 40, 64].map((size) => (
          <Cell
            ...
            pose={
              <BrainThinking
                size={size}
                label={`Brain thinking at ${size} pixels`}
              />
            }
          />
        ))}
        ...
        {[48, 96, 120].map((size) => (
          <Cell
            ...
            pose={
              <BrainSleeping
                size={size}
                label={`Brain sleeping at ${size} pixels`}
                reduced
              />
            }
          />
    ...
    <div className="p-8 bg-ice-2 dark:bg-space-2 min-h-screen">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Cell
          ...
          pose={<MascotCurious size={80} reduced={false} />}
        />
        <Cell
          label="Happy"
          hint="archival verification · 800ms"
          pose={<MascotHappy size={80} reduced={false} />}
        />
        <Cell
          label="Dizzy"
          hint="one paperclip orbit · 1300ms"
          pose={<MascotDizzy size={80} reduced={false} />}
        />
        <Cell
          label="Hit"
          hint="brass-tab rebound · 800ms"
          pose={<MascotHit size={80} reduced={false} />}
        />
      </div>
    </div>
  ),
...
export const CanonicalMoods: Story = {
  render: () => (
    <div className="p-8 bg-ice-2 dark:bg-space-2 min-h-screen">
      ...
      <p className="text-sm font-serif italic text-ink-soft dark:text-starlight mb-6">
        Single source. Rail mark must read "cute mascot" at 28 px. Restraint:
