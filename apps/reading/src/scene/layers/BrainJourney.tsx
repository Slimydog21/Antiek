...
/**
 * BrainJourney (SPR-04, milestone 3 — "the mascot running off into the
 * unknown").
 *
 ...
 * interactive <MascotStation/> (SPR-05 / SPR-12):
 *
 *   | aspect      | BrainJourney (this)        | MascotStation (SPR-05)        |
 *   |-------------|------------------------------|-------------------------------|
 ...
 * matrix above and the SPR-05 mascot contract. This mascot never reacts; it
 * only journeys.
 * ─────────────────────────────────────────────────────────────────────────
 *
 ...
 *
 * COLOUR: token fills only (mascot-coat / mascot-belly via CSS vars) — no hex.
 */

export interface BrainJourneyProps {
  mood: SceneMood;
}

export function BrainJourney({ mood }: BrainJourneyProps) {
  const night = mood.dayPart === "night" || mood.dayPart === "dusk";
  // A scenery silhouette reads as a darker shape by day, a faint one by night.
  const coat = night ? "var(--mascot-coat)" : "var(--shadow-2)";
  const belly = "var(--mascot-belly)";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          animation: "akb-brain-journey 38s linear infinite",
          willChange: "transform",
        }}
      >
        <div
          className="h-full w-full"
          style={{ animation: "akb-brain-bob 0.9s ease-in-out infinite" }}
        >
      ...
      {/* The walk (akb-brain-journey) drifts the mascot across the horizon
          and shrinks it ("into the unknown"); the bob (akb-brain-bob) is a
