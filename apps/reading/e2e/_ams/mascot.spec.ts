/**
 * mascot.spec.ts — the SPR-06 EXPERIENCE GATE for Brain the mascot.
 *
 ...
 * MascotStation stories (title "Shell / MascotStation (SPR-06)" → ids verified
 ...
 *     measures WALK MOTION, not TRANSLATION: the mascot is PINNED (left/top
...
const ROAMING = "shell-mascotstation-spr-06--roaming";
const REDUCED = "shell-mascotstation-spr-06--reduced-motion-note";
...
async function loadMascot(page: Page, id: string): Promise<void> {
  ...
  await expect(mascot, "mascot not found in the story").toBeVisible({
    timeout: 10_000,
  });
}
...
test.describe("SPR-06 — Brain is ALIVE (real Chromium pixels)", () => {
  ...
  test("M1 walk-cycle: the mascot art changes across two mid-stroll frames with position pinned", async ({
    ...
    // and activate it; the mascot starts waddling toward it with the gait on.
    await page.evaluate(() => {
    ...
    // `.mascot-rig-foot-*`; the single-source BrainMascot art walks with the
    ...
    expect(pin, "could not pin the mascot for the M1 walk-motion sample").not.toBeNull();
    const box = pin!;
    ...
    expect(
      diff,
      `mascot square barely changed (mean abs diff ${diff.toFixed(2)}) with the mascot ` +
  ...
  for (const theme of ["light", "dark"] as const) {
    test(`M2 white box: the emote region has no opaque white backdrop (${theme} theme)`, async ({
      ...
      // Most of the square is mascot + transparent surround (= story bg). A
  ...
  test("M4 waddle-to-button: the mascot moves measurably TOWARD the activated control + plays a hit emote", async ({
    page,
  }) => {
    await loadMascot(page, ROAMING);
    // Inject a control at a known FAR-side rect (right edge) carrying the
    // data-product-id the choreography resolves; record the mascot's start.
    ...
    expect(
      distAfter,
      `mascot did not move toward the activated control: distance ${distBefore.toFixed(0)}→` +
    ...
    //     the mascot-hit-bump wrapper on the hit kind.
    await expect(
      page.locator('[data-testid="brain-mascot"] .mascot-hit-bump'),
