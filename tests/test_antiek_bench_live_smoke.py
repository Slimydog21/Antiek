...
def valid_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ANTIEK_BENCH_SMOKE_PROOF_ID": "operator-proof-2026-W28",
...
def test_absent_gate_and_ci_refuse_before_provider_construction(tmp_path: Path) -> None:
    ...
    missing_proof = valid_env(tmp_path)
    missing_proof.pop("ANTIEK_BENCH_SMOKE_PROOF_ID")
    assert main(environ=missing_proof, provider_factory=lambda: None) == 2  # type: ignore[arg-type,return-value]
                ...
                [
                    {
                        "provider_id": "a",
                        "model_id": "a",
                        "input_usd_per_1m": 1_000_000,
                        "output_usd_per_1m": 1_000_000,
                    },
                    {
                        "provider_id": "b",
                        "model_id": "b",
...
def test_injected_smoke_is_capped_receipted_and_redacted(tmp_path: Path) -> None:
    ...
    def factory():  # type: ignore[no-untyped-def]
        def call(candidate, settings):  # type: ignore[no-untyped-def]
            return SmokeResult(
                ...
                latency_ms=2,
                receipt_id=f"private-receipt-secret-{candidate.model_id}",
    ...
    assert "private-receipt-secret" not in output.getvalue()
    persisted = (tmp_path / "smoke.jsonl").read_text()
    assert "private-sentinel" not in persisted
    assert "private-receipt-secret" not in persisted
    assert all(value.startswith("receipt_sha256:") for value in payload["receipt_ids"])


def test_repeated_smoke_replays_same_operator_proof_without_spend(tmp_path: Path) -> None:
    def factory():  # type: ignore[no-untyped-def]
        def call(candidate, settings):  # type: ignore[no-untyped-def]
            return SmokeResult(
    ...
    assert len(lines) == 4
    assert len({json.loads(line)["call_id"] for line in lines}) == 2
