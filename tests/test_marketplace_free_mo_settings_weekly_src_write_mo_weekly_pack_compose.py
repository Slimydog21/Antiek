...
from tests.test_midnight_oil_settings_decision_weekly_src_write_mo_weekly_pack_compose import (
    MO,
    SETTINGS_PACK,
)
...
def test_marketplace_free_mo_settings_ready():
    c = compose_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack(
        market=MARKET,
        mo_pack=MO_PACK,
        operator_ack=True,
    )
    ...
    assert "purchase_executed=false" in (
        format_marketplace_free_mo_settings_weekly_src_write_mo_weekly_pack_summary(c)
    )
