"""Pure tests for marketplace free + source-attach record→prompt pack."""
...
def test_free_port_research_ready():
    c = compose_marketplace_free_source_attach_record_prompt(
        market=MARKET,
        research=RESEARCH,
        operator_ack=True,
    )
    ...
    assert "purchase_executed=false" in (
        format_marketplace_free_source_attach_record_prompt_summary(c)
    )
    assert c.to_dict()["hosted"] is False
...
def test_unknown_free_blocks_market():
    c = compose_marketplace_free_source_attach_record_prompt(
        market={
            **MARKET,
...
def test_require_operator_ack_type():
    with pytest.raises(MarketplaceFreeSourceAttachRecordPromptComposeError):
        compose_marketplace_free_source_attach_record_prompt(
            market=MARKET,
            research=RESEARCH,
