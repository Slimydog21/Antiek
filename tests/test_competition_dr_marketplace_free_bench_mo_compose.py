"""Pure tests for competition DR + free marketplace bench MO pack."""
...
def test_competition_free_bench_mo_ready():
    c = compose_competition_dr_marketplace_free_bench_mo(
        competition=COMPETITION,
        free_pack=FREE_PACK,
        operator_ack=True,
    )
    assert c.competition.pack_ready is True
    assert c.free_pack.pack_ready is True
    ...
    assert "live_dispatch_authorized=false" in (
        format_competition_dr_marketplace_free_bench_mo_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(CompetitionDrMarketplaceFreeBenchMoComposeError):
        compose_competition_dr_marketplace_free_bench_mo(
            competition=COMPETITION,
            free_pack=FREE_PACK,
