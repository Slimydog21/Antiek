"""Pure tests for source attach + model decision twin search pack."""
...
def test_source_attach_model_decision_ready():
    c = compose_source_attach_model_decision_twin_search(
        sources=SOURCES,
    ...
    assert "remote_fetched=false" in (
        format_source_attach_model_decision_twin_search_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(SourceAttachModelDecisionTwinSearchComposeError):
        compose_source_attach_model_decision_twin_search(
            sources=SOURCES,
            decision_pack=DECISION_PACK,
