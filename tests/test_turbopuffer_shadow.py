...
def test_benchmark_artifact_requires_twenty_and_redacts_content(graph, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(search_module, "SentenceTransformerEmbedding", lambda _name: HashEmbedding())
    con = connect_write(graph, purpose="tpuf-cli-model-identity")
    try:
        con.execute("UPDATE embeddings_meta SET provider='sentence_transformers'")
    finally:
        con.close()
