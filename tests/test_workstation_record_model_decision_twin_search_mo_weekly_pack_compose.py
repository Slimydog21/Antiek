"""Pure tests for workstation records over model decision twin search MO weekly pack."""
...
from tests.test_model_decision_twin_search_html_native_mo_weekly_pack_compose import (
    DECISION,
    TWIN_SEARCH_PACK,
)
...
def test_workstation_records_model_decision_ready():
    c = compose_workstation_record_model_decision_twin_search_mo_weekly_pack(
        session_id="sess-1",
        items=ITEMS,
    ...
    assert "record_persisted=false" in (
        format_workstation_record_model_decision_twin_search_mo_weekly_pack_summary(c)
    )
