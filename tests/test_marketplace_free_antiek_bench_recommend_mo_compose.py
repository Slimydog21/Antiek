"""Pure tests for marketplace free + Antiek-bench recommend MO pack."""
...
def test_marketplace_free_bench_mo_ready():
    c = compose_marketplace_free_antiek_bench_recommend_mo(
        market=MARKET,
        bench_mo=BENCH_MO,
        operator_ack=True,
    )
    ...
    assert c.bench_mo.pack_ready is True
    ...
    assert "purchase_executed=false" in (
        format_marketplace_free_antiek_bench_recommend_mo_summary(c)
    )
...
def test_port_requested_false_blocks():
    c = compose_marketplace_free_antiek_bench_recommend_mo(
...
def test_require_operator_ack_type():
    with pytest.raises(MarketplaceFreeAntiekBenchRecommendMoComposeError):
        compose_marketplace_free_antiek_bench_recommend_mo(
            market=MARKET,
            bench_mo=BENCH_MO,
