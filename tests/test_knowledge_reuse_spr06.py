    ...
    try:
        ...
        for i, (text, content_class) in enumerate(units):
            ...
            claim_id = insert_node(
                con,
                canonical_label=f"claim {i}: {text}",
                node_type="claim",
                graph_scope="depth",
                investigation_id=prior_investigation,
...
def test_m3_event_emitted_with_units_and_scores(seeded_quantum_db):
    ...
    assert [item["unit_id"] for item in payload["source_qualifications"]] == payload[
        "reused_unit_ids"
    ]
    assert {item["state"] for item in payload["source_qualifications"]} == {"unknown"}
...
def test_m4_select_within_budget_is_deterministic():
    ...
    a, da = kr.select_units_within_budget(
        units, budget=500, counter=counter, header_text=kr._REUSE_HEADER
    )
    b, db_ = kr.select_units_within_budget(
        units, budget=500, counter=counter, header_text=kr._REUSE_HEADER
    )
...
def test_m5_relevant_injects(seeded_quantum_db):
    ...
    try:
        units = kr.retrieve_prior_units(
            sub,
            question_text="neutral atom qubit error rate suppression scaling",
        )
        result = kr.assemble_context_pack_with_reuse(
            role="user_agent",
            investigation_id="inv-pos",
            layers=[],
            units=units,
        )
    finally:
...
def test_m5_unrelated_injects_none_but_event_still_emitted(seeded_quantum_db):
    ...
    try:
        units = kr.retrieve_prior_units(
            sub,
            question_text="medieval cathedral architecture flying buttress stained glass",
        )
        result = kr.assemble_context_pack_with_reuse(
            role="user_agent",
            investigation_id="inv-neg",
            layers=[],
            units=units,
        )
    finally:
...
async def test_runner_start_without_substrate_is_unchanged(seeded_quantum_db):
    ...
    db, emb, _ = seeded_quantum_db
    runner = HostLocalRunner(make_demo_loop(steps=1), claims=operator_claims())
