"""Pure tests for MO unattended + source attach model decision pack."""
...
def test_mo_unattended_source_attach_ready():
    c = compose_mo_unattended_source_attach_model_decision(
        mo=MO,
        research_pack=RESEARCH_PACK,
        operator_ack=True,
    )
    ...
    assert "live_execution_authorized=false" in (
        format_mo_unattended_source_attach_model_decision_summary(c)
    )
...
def test_low_ceiling_without_override_blocks_mo():
    c = compose_mo_unattended_source_attach_model_decision(
        mo={**MO, "approved_ceiling_usd": 1},
        research_pack=RESEARCH_PACK,
        operator_ack=True,
    )
...
def test_require_operator_ack_type():
    with pytest.raises(MoUnattendedSourceAttachModelDecisionComposeError):
        compose_mo_unattended_source_attach_model_decision(
            mo=MO,
            research_pack=RESEARCH_PACK,
