...
def test_remaining_read_sprints_stay_planned_until_promoted() -> None:
    assert {lock.resolve_read_sprint(n).status for n in range(5, 10)} == {"planned"}
...
def test_roadmap_consumes_read_sprint_status_and_focus_advances() -> None:
    ...
    assert by_id["read:5"].status.value == "planned"
    assert roadmap.execution_focus() is not None
    assert roadmap.execution_focus().node_id == "read:5"


def test_read_lock_version_present() -> None:
    assert isinstance(lock.READ_LOCK_VERSION, int) and lock.READ_LOCK_VERSION >= 4
