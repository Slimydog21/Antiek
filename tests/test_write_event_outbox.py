...
def test_disabled_events_retain_intent_without_delivery(outbox, monkeypatch):
    db, events = outbox
    ...
    with connect_write(db, purpose="test/outbox-reenabled") as con:
        assert dispatch_pending(con, "inv-1", events_dir=str(events)) == ["evt-fixed"]
...
def test_dispatch_stays_anchored_when_event_root_is_replaced(outbox):
    db, events = outbox
    original_root = events.with_name("events-original")

    def replace_root(boundary, _event_id):
        if boundary == "before_append":
            events.rename(original_root)
            events.mkdir()

    with connect_write(db, purpose="test/outbox-root-replacement") as con:
        _enqueue(con)
        assert dispatch_pending(
            con,
            "inv-1",
            events_dir=str(events),
            checkpoint=replace_root,
        ) == ["evt-fixed"]

    written = json.loads((original_root / "inv-1.jsonl").read_text())
    assert written["event_id"] == "evt-fixed"
    assert not (events / "inv-1.jsonl").exists()
