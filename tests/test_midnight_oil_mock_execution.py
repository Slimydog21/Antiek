...
def _request(preflight) -> MidnightOilMockExecutionRequest:
    return MidnightOilMockExecutionRequest(
...
def test_mock_execution_emits_canonical_zero_cost_receipts_and_html_twins() -> None:
    preflight = _accepted_preflight(goal="Compare <engine> evidence & counterclaims.")

    receipt = execute_mock_midnight_oil(_request(preflight))
    ...
    for output in receipt.role_outputs:
        assert output.route_receipt.route_receipt_id in preflight.launch_packet.role_route_receipt_ids
    ...
    assert "Compare &lt;engine&gt; evidence &amp; counterclaims." not in receipt.html_information_asset
...
def test_mock_execution_rejects_route_lineage_drift() -> None:
...
def test_mock_execution_rejects_unacknowledged_or_drifted_approval() -> None:
    preflight = _accepted_preflight()
...
def test_mock_execution_never_reflects_free_form_goal_text() -> None:
    preflight = _accepted_preflight(
...
def test_mock_execution_api_contract() -> None:
    preflight = _accepted_preflight()

    with TestClient(create_app()) as client:
        response = client.post(
            "/research/midnight-oil/mock-execution",
