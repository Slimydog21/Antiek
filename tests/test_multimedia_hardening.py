...
def _audio_asset(status: str = "ready") -> MultimediaAssetContract:
    audio = assemble_audio_experience(_plan(), asset_id="mm-hard-audio", revision_id="rev-audio")
...
def _video_asset() -> MultimediaAssetContract:
    plan = _plan()
    audio = assemble_audio_experience(plan, asset_id="mm-hard-video", revision_id="rev-audio")
...
def test_video_without_transcript_fails_accessibility_even_with_captions():
    asset = _video_asset()
    scenes = build_video_scenes(_plan(), assemble_audio_experience(_plan(), asset_id="x", revision_id="r"))
