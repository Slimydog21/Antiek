"""Pure tests for fullscreen + MO price-ceiling draft multi pack."""
...
def test_fullscreen_mo_ready():
    c = compose_fullscreen_mo_price_ceiling_draft_multi(
    ...
    assert "live_dispatched=false" in (
        format_fullscreen_mo_price_ceiling_draft_multi_summary(c)
    )
...
def test_gated_highlight_fails_closed():
    with pytest.raises(FullscreenMoPriceCeilingDraftMultiComposeError, match="gated"):
        compose_fullscreen_mo_price_ceiling_draft_multi(
...
def test_require_operator_ack_type():
    with pytest.raises(FullscreenMoPriceCeilingDraftMultiComposeError):
        compose_fullscreen_mo_price_ceiling_draft_multi(
            fullscreen=FULLSCREEN,
            mo_pack=MO_PACK,
