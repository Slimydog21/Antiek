...
def test_factory_returns_reuse_consuming_loop_by_default(monkeypatch):
    """ACV SPR-06 (merge with ANT-DRL exa seam): env unset → the prod default
    is the reuse-CONSUMING loop, never exa. Identity is hard to assert across
    factories, so we assert by constructing the loop and comparing the qualname
    of the produced coroutine function — both come from
    make_reuse_consuming_loop, never from exa."""
    ...
    reuse = make_reuse_consuming_loop(steps=3, cost_per_step=0.01, emit_note=True)
    assert loop_fn.__qualname__ == reuse.__qualname__
    assert "exa_gather_loop" not in loop_fn.__qualname__


def test_factory_returns_stub_when_env_set(monkeypatch):
    """Main's reuse-blind contract stub stays explicitly selectable."""
