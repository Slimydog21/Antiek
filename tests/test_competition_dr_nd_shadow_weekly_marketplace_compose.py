"""Pure tests for competition DR + ND shadow weekly marketplace pack."""
...
def test_competition_nd_weekly_ready():
    c = compose_competition_dr_nd_shadow_weekly_marketplace(
        competition=COMPETITION,
        nd_weekly=ND_WEEKLY,
        operator_ack=True,
    )
    assert c.competition.pack_ready is True
    assert c.nd_weekly.pack_ready is True
    ...
    assert "live_dispatch_authorized=false" in (
        format_competition_dr_nd_shadow_weekly_marketplace_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(CompetitionDrNdShadowWeeklyMarketplaceComposeError):
        compose_competition_dr_nd_shadow_weekly_marketplace(
            competition=COMPETITION,
            nd_weekly=ND_WEEKLY,
