"""Pure tests for floating DR residual over workstation MD mow12."""
...
from tests.test_ws_md_twin_search_html_native_mow12_mpk_compose import (
    DECISION_PACK,
    ITEMS,
)
...
def test_fdr_ws_md_mow12_ready():
    c = compose_fdr_ws_md_twin_search_html_native_mow12_mpk(
    ...
    assert "live_dispatched=false" in (
        format_fdr_ws_md_twin_search_html_native_mow12_mpk_summary(c)
    )
...
def test_gated_highlight_fail_closed():
    with pytest.raises(FdrWsMdTwinSearchHtmlNativeMow12MpkComposeError, match="gated"):
        compose_fdr_ws_md_twin_search_html_native_mow12_mpk(
