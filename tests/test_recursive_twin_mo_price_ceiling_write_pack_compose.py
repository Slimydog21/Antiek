"""Pure tests for recursive twin + MO price-ceiling write pack."""
...
def test_twin_mo_write_ready():
    c = compose_recursive_twin_mo_price_ceiling_write_pack(
        twin=TWIN,
        mo_write=MO_WRITE,
        operator_ack=True,
    )
    assert c.twin.twin_propose_ready is True
    assert c.mo_write.pack_ready is True
    ...
    assert "twin_written=false" in (
        format_recursive_twin_mo_price_ceiling_write_pack_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(RecursiveTwinMoPriceCeilingWritePackComposeError):
        compose_recursive_twin_mo_price_ceiling_write_pack(
            twin=TWIN,
            mo_write=MO_WRITE,
