"""Pure tests for highlight float → twin search competition pack."""
...
def test_highlight_float_twin_search_ready():
    c = compose_highlight_float_twin_search_competition(
        highlight={
            "parent_asset_id": "book-1",
    ...
    assert "live_dispatched=false" in (
        format_highlight_float_twin_search_competition_summary(c)
    )
...
def test_gated_highlight_fails_closed():
    with pytest.raises(HighlightFloatTwinSearchCompetitionComposeError, match="gated"):
        compose_highlight_float_twin_search_competition(
            highlight={
                "parent_asset_id": "book-1",
...
def test_require_operator_ack_type():
    with pytest.raises(HighlightFloatTwinSearchCompetitionComposeError):
        compose_highlight_float_twin_search_competition(
            highlight={
                "parent_asset_id": "book-1",
                "highlight": "scaling",
