...
from tests.test_write_twin_fs_draft_pack_compose import (
    FULLSCREEN_PACK,
    WRITE,
)
...
def test_source_attach_write_twin_ready():
    c = compose_source_attach_write_twin_fs_draft_pack(
        sources=SOURCES,
    ...
    assert "remote_fetched=false" in (
        format_source_attach_write_twin_fs_draft_pack_summary(c)
    )
