"""Pure tests for ND shadow + Antiek-bench weekly marketplace pack."""
...
def test_nd_reject_weekly_market_ready():
    c = compose_nd_shadow_antiek_bench_weekly_marketplace(
        nd_shadow=ND_SHADOW,
    ...
    assert c.weekly_market.pack_ready is True
    ...
    assert "live_router_authorized=false" in (
        format_nd_shadow_antiek_bench_weekly_marketplace_summary(c)
    )
...
def test_kill_switch_off_still_never_routes():
    c = compose_nd_shadow_antiek_bench_weekly_marketplace(
...
def test_require_operator_ack_type():
    with pytest.raises(NdShadowAntiekBenchWeeklyMarketplaceComposeError):
        compose_nd_shadow_antiek_bench_weekly_marketplace(
            nd_shadow=ND_SHADOW,
            weekly_market=WEEKLY_MARKET,
