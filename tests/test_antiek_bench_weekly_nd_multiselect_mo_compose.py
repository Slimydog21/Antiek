"""Pure tests for Antiek-bench weekly learn + ND multi-select pack."""
...
def test_weekly_nd_ready():
    c = compose_antiek_bench_weekly_nd_multiselect_mo(
        weekly_learn=WEEKLY,
        nd_research=ND_RESEARCH,
        operator_ack=True,
    )
    assert c.weekly_learn.learn_ready is True
    assert c.weekly_learn.proposal_count == 2
    assert c.nd_research.pack_ready is True
    ...
    assert "backlog_mutated=false" in (
        format_antiek_bench_weekly_nd_multiselect_mo_summary(c)
    )
...
def test_unattended_blocks_nd():
    ...
    assert c.nd_research.pack_ready is False
    assert c.pack_ready is False
    assert c.production_router_verdict == "REJECT"
