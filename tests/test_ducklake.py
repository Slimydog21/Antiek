...
class _FakePostgresConnection:
    ...
    def execute(self, sql, params=None):
        ...
        if compact.startswith("INSERT INTO catalog_entries"):
            assert "%s" in sql
            assert "ON CONFLICT(user_id) DO UPDATE" in compact
...
def test_postgres_backend_crud_uses_postgres_upsert_contract():
    conn = _FakePostgresConnection()
    backend = PostgresCatalogBackend(conn=conn)
    cat = DuckLakeCatalog(backend=backend)

    entry = cat.register(
        user_id="u-1",
        db_path="/pg/u-1.duckdb",
        encryption_key_ref="alias/antiek-graph-u-1",
        shard_id="a",
        last_size_bytes=42,
    )

    assert cat.lookup("u-1") == entry
    assert cat.list_all() == [entry]
    assert cat.deregister("u-1") is True
    assert cat.lookup("u-1") is None
    assert any("TIMESTAMPTZ" in sql for sql in conn.sql)
