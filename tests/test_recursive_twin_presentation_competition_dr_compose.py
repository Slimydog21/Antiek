"""Pure tests for recursive twin presentation + competition DR free-market pack."""
...
from tests.test_competition_dr_marketplace_free_bench_mo_compose import (
    COMPETITION,
    FREE_PACK,
)
...
def test_twin_presentation_competition_ready():
    c = compose_recursive_twin_presentation_competition_dr(
        twin=TWIN,
    ...
    assert "twin_written=false" in (
        format_recursive_twin_presentation_competition_dr_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(RecursiveTwinPresentationCompetitionDrComposeError):
        compose_recursive_twin_presentation_competition_dr(
            twin=TWIN,
            presentation=PRESENTATION,
            competition_pack=COMPETITION_PACK,
