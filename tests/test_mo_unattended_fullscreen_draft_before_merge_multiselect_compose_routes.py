"""Route tests for MO unattended + fullscreen draft multi-select pack."""

from __future__ import annotations

from fastapi import FastAPI
from fastapi.testclient import TestClient

from interfaces.research.api.mo_unattended_fullscreen_draft_before_merge_multiselect_compose_routes import (
    register_mo_unattended_fullscreen_draft_before_merge_multiselect_compose_routes,
)
from tests.test_mo_unattended_fullscreen_draft_before_merge_multiselect_compose import (
    FULLSCREEN_PACK,
    MO,
)


def _client() -> TestClient:
    app = FastAPI()
    register_mo_unattended_fullscreen_draft_before_merge_multiselect_compose_routes(
        app
    )
    return TestClient(app)


def _payload(*, operator_ack: bool = True) -> dict:
    return {
        "mo": MO,
        "fullscreen_pack": FULLSCREEN_PACK,
        "operator_ack": operator_ack,
    }


_PATH = "/research/mo-unattended-fullscreen-draft-before-merge-multiselect/compose"


def test_compose_route():
    c = _client()
    r = c.post(_PATH, json=_payload(operator_ack=True))
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["pack_ready"] is True
    assert body["mo"]["unattended_package_ready"] is True
    assert body["fullscreen_pack"]["pack_ready"] is True
    assert body["live_execution_authorized"] is False
    assert body["live_dispatched"] is False
    assert body["merge_executed"] is False
    assert body["draft_written"] is False
    assert body["live_router_authorized"] is False
    assert body["secrets_stored"] is False
    assert body["remote_index_queried"] is False
    assert body["pdf_primary"] is False
    assert body["production_router_verdict"] == "REJECT"
    assert (
        body["authority"]
        == "mo_unattended_fullscreen_draft_before_merge_multiselect_compose_advisory"
    )


def test_compose_route_ack_false():
    c = _client()
    r = c.post(_PATH, json=_payload(operator_ack=False))
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["pack_ready"] is False
    assert body["live_execution_authorized"] is False


def test_compose_route_spend_consent_false():
    c = _client()
    payload = _payload(operator_ack=True)
    payload["mo"] = {**MO, "spend_consent": False}
    r = c.post(_PATH, json=payload)
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["mo"]["unattended_package_ready"] is False
    assert body["pack_ready"] is False
    assert body["live_execution_authorized"] is False


def test_compose_route_missing_operator_ack_422():
    c = _client()
    payload = _payload()
    del payload["operator_ack"]
    r = c.post(_PATH, json=payload)
    assert r.status_code == 422
