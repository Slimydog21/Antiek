...
the 2026-06-12 leaked-ledger incident):
  ...
  - STDOUT SNAPSHOTS: every captured stdout is scanned for token-like
    ...
    server deliberately embeds in image_url, the planted
    PLANTEDLEAKMARKER credential shape, and any "Bearer <...>" echo.
    The mock server separately RECORDS that the Authorization header
    arrived, proving the secret was used yet never printed.
...
# Credential-shaped planted marker for red-proofing the leak scanner.
_PLANTED_LEAK_MARKER = "PLANTEDLEAKMARKER_fakeid:FAKESECRETVALUE_0123456789"
...
def _assert_no_secrets(stdout: str) -> None:
    ...
    assert _PLANTED_LEAK_MARKER not in stdout
    assert "FAKESIGNEDVALUE" not in stdout, "signed query param leaked"
    token_match = _TOKEN_LIKE.search(stdout)
    assert token_match is None, f"token-like pattern in stdout: {token_match.group(0)!r}"
...
class _MockProxy:
    ...
    def __init__(
        self,
        scene_responses: list[tuple[int, dict[str, Any]]],
    ) -> None:
...
def _run_smoke(port: int, *extra_args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ...
        env={
            ...
            "KREA_API_TOKEN": _PLANTED_LEAK_MARKER,
        },
        cwd=str(_REPO),
    )
...
def _load_smoke_module(name: str) -> Any:
...
def test_200_path_fresh_then_cached_exits_0_and_leaks_nothing() -> None:
...
def test_non_str_image_url_prints_type_name_only_never_the_value() -> None:
...
def test_repeat_not_cached_is_a_rebill_failure_exit_1() -> None:
...
def test_503_no_key_exits_3_with_decision_row() -> None:
...
def test_503_no_api_balance_points_at_the_money_step() -> None:
...
def test_unknown_reason_is_admitted_not_guessed() -> None:
...
def test_planted_marker_absent_from_all_subprocess_output_channels() -> None:
    """Red-proof target: if the script prints env or exception text carrying
    the planted Krea-shaped marker, this fails on stdout or stderr."""
    with _MockProxy([(503, _disabled("no_key"))]) as mock:
        proc = _run_smoke(mock.port)
    combined = proc.stdout + proc.stderr
    assert proc.returncode == 3
    assert _PLANTED_LEAK_MARKER not in combined
    assert not _TOKEN_LIKE.search(combined)
    assert not _BEARER_ECHO.search(combined)


# ── --help works offline + carries the full decision table ──────────────


def test_help_offline_lists_full_post_spr01_vocabulary() -> None:
    proc = subprocess.run(
...
def test_static_all_output_routes_through_the_scrubber() -> None:
    src = _SCRIPT.read_text()
...
def test_build_server_env_forces_cap_3_unless_overridden() -> None:
    mod = _load_smoke_module("krea_smoke2")
...
def test_strip_query_drops_signed_params() -> None:
    mod = _load_smoke_module("krea_smoke3")
...
def test_unreachable_base_exits_2_cleanly() -> None:
...
def test_401_explains_operator_auth_without_echoing_anything() -> None:
