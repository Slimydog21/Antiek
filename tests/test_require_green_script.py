...
def test_deploy_pins_the_exact_sha_verified_by_the_gate():
    ...
    # The pull checks out antiek_target_sha only through the fact the gate
    # sets once it cleared, so no tag selection can pull an ungated SHA
    # (tests/test_deploy_playbook_gate_tags.py executes both properties).
    version = git_pull["ansible.builtin.git"]["version"]
    assert version.strip().startswith("{{ antiek_gate_cleared_ref | mandatory("), version
