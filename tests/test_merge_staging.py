...
def test_merge_window_is_bounded(env):
    """The connect_write-held merge window is seconds-scale and recorded,
    distinct from the (here, instantaneous) ingest. A generous ceiling guards
    against an accidental per-row-write regression that would balloon the
    window; the recorded number is reported in the handoff."""
    live = os.path.join(env["tmpdir"], "live.duckdb")
    staging = os.path.join(env["tmpdir"], "staging.duckdb")
    init_database_at_path(live)
    _stage_books(
