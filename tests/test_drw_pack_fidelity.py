...
@pytest.mark.asyncio
async def test_synthesis_tail_error_surfaces_in_terminal_status(env):
    funnel = PromotionFunnel(db_path=env["db"], embedding_provider=_FakeEmbedding())
    runner = HostLocalRunner(
        make_parallel_gather_loop(
            top_k=0,
    ...
    session.record_synthesis_tail_error(
        RuntimeError("synthesis exploded"), stage="synthesis_tail"
    )

    assert session.is_complete() is True
    status = session.terminal_status()
    assert status["deep_research_complete"] is False
    assert status["synthesis_tail_error"] == (
        "[synthesis_tail] RuntimeError: synthesis exploded"
    )

    events = trajectory("session-err", events_dir=env["events"])
    assert any(
        e.get("action_type") == "cascade.synthesis_tail.failed" for e in events
    )