"""Pure tests for MO price-ceiling + write twin settings draft pack."""
...
def test_mo_research_write_ready():
    c = compose_mo_price_ceiling_write_twin_settings_draft(
        mo=MO,
        research_write=RESEARCH_WRITE,
        operator_ack=True,
    )
    assert c.mo.pack_ready is True
    assert c.research_write.pack_ready is True
    ...
    assert "charge_executed=false" in (
        format_mo_price_ceiling_write_twin_settings_draft_summary(c)
    )
...
def test_approve_ceiling_never_charges():
    rec = compose_mo_price_ceiling_write_twin_settings_draft(
        mo={**MO, "stage": "recommend_only", "price_ceiling_ack": False},
        research_write=RESEARCH_WRITE,
        operator_ack=True,
    )
    recommended = rec.mo.recommend.recommended_ceiling_usd
    assert recommended is not None and recommended > 0
    c = compose_mo_price_ceiling_write_twin_settings_draft(
        mo={
            **MO,
...
def test_require_operator_ack_type():
    with pytest.raises(MoPriceCeilingWriteTwinSettingsDraftComposeError):
        compose_mo_price_ceiling_write_twin_settings_draft(
            mo=MO,
            research_write=RESEARCH_WRITE,
