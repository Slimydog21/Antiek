"""Pure tests for write twin collective + settings draft fullscreen ND."""
...
def test_write_settings_ready():
    c = compose_write_twin_collective_settings_draft_fullscreen_nd_mo(
        write=WRITE,
        settings_research=SETTINGS_RESEARCH,
        operator_ack=True,
    )
    assert c.write.pack_ready is True
    assert c.settings_research.pack_ready is True
    ...
    assert "analysis_written=false" in (
        format_write_twin_collective_settings_draft_fullscreen_nd_mo_summary(c)
    )
...
def test_full_analysis_never_writes():
    c = compose_write_twin_collective_settings_draft_fullscreen_nd_mo(
        write={**WRITE, "analysis_kind": "full_analysis"},
        settings_research=SETTINGS_RESEARCH,
        operator_ack=True,
    )
    assert c.write.collective_analysis.analysis.kind == "full_analysis"
    assert c.pack_ready is True
    assert c.analysis_written is False
    assert c.draft_written is False
...
def test_require_operator_ack_type():
    with pytest.raises(WriteTwinCollectiveSettingsDraftFullscreenNdMoComposeError):
        compose_write_twin_collective_settings_draft_fullscreen_nd_mo(
            write=WRITE,
            settings_research=SETTINGS_RESEARCH,
