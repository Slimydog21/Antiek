"""Pure tests for Antiek-bench recommend + MO unattended pack."""
...
def test_bench_recommend_mo_ready():
    c = compose_antiek_bench_recommend_mo_unattended(
        bench=BENCH,
        mo_pack=MO_PACK,
        operator_ack=True,
    )
    ...
    assert "live_router_authorized=false" in (
        format_antiek_bench_recommend_mo_unattended_summary(c)
    )
...
def test_session_mismatch_blocks_mo():
    c = compose_antiek_bench_recommend_mo_unattended(
        bench=BENCH,
        mo_pack={
            "mo": MO,
...
def test_low_mo_ceiling_blocks():
    c = compose_antiek_bench_recommend_mo_unattended(
        bench=BENCH,
        mo_pack={
...
def test_require_operator_ack_type():
    with pytest.raises(AntiekBenchRecommendMoUnattendedComposeError):
        compose_antiek_bench_recommend_mo_unattended(
            bench=BENCH,
            mo_pack=MO_PACK,
