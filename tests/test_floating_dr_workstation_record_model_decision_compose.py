"""Pure tests for floating DR over workstation record model decision pack."""
...
from tests.test_workstation_record_model_decision_twin_search_compose import (
    DECISION_PACK,
    ITEMS,
)
...
def test_floating_dr_workstation_record_ready():
    c = compose_floating_dr_workstation_record_model_decision(
    ...
    assert "live_dispatched=false" in (
        format_floating_dr_workstation_record_model_decision_summary(c)
    )
...
def test_gated_highlight_fail_closed():
    with pytest.raises(
        FloatingDrWorkstationRecordModelDecisionComposeError, match="gated"
    ):
        compose_floating_dr_workstation_record_model_decision(
