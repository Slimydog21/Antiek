"""Route tests for recursive twin presentation + competition DR source-attach pack."""
...
_PATH = (
    "/research/recursive-twin-presentation-competition-dr-source-attach/compose"
)
...
def _payload(
    *, operator_ack: bool = True, open_requested: bool = True
) -> dict:
    return {
        "twin": {
...
def test_compose_route_open_false_blocks():
    c = _client()
    r = c.post(
        _PATH, json=_payload(operator_ack=True, open_requested=False)
    )
    assert r.status_code == 200, r.text
    body = r.json()
