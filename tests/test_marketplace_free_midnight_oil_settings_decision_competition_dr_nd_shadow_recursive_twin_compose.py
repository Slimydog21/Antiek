...
from tests.test_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_compose import (
    MO,
    SETTINGS_PACK,
)
...
def test_marketplace_free_mo_settings_ready():
    c = compose_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin(
        market=MARKET,
        mo_pack=MO_PACK,
        operator_ack=True,
    )
    ...
    assert "purchase_executed=false" in (
        format_marketplace_free_midnight_oil_settings_decision_competition_dr_nd_shadow_recursive_twin_summary(c)
    )
