"""Pure tests for draft-before-full-merge + fullscreen weekly ND pack."""
...
def test_draft_only_fullscreen_weekly_nd_ready():
    c = compose_floating_draft_before_full_merge_fullscreen_weekly_nd_mo(
    ...
    assert "merge_executed=false" in (
        format_floating_draft_before_full_merge_fullscreen_weekly_nd_mo_summary(c)
    )
...
def test_session_parent_mismatch_blocks():
    pack = {
        **FULLSCREEN_PACK,
        "fullscreen": {
...
def test_promote_full_merge_never_merges():
    c = compose_floating_draft_before_full_merge_fullscreen_weekly_nd_mo(
        draft_gate={
            **DRAFT_GATE,
    ...
    assert c.draft_gate.full_merge_intent_ready is True
    assert c.pack_ready is True
    assert c.draft_written is False
    assert c.merge_executed is False
    assert c.live_dispatched is False
...
def test_gated_highlight_raises():
    pack = {
        **FULLSCREEN_PACK,
        "fullscreen": {
            **FULLSCREEN_PACK["fullscreen"],
    ...
    with pytest.raises(FloatingDraftBeforeFullMergeFullscreenWeeklyNdMoComposeError):
        compose_floating_draft_before_full_merge_fullscreen_weekly_nd_mo(
            draft_gate=DRAFT_GATE,
            fullscreen_pack=pack,
            operator_ack=True,
