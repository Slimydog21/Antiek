"""Pure tests for model decision + twin search weekly HTML-native pack."""
...
def test_model_decision_twin_search_ready():
    c = compose_model_decision_twin_search_weekly_html_native(
        decision=DECISION,
    ...
    assert "live_router_authorized=false" in (
        format_model_decision_twin_search_weekly_html_native_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(ModelDecisionTwinSearchWeeklyHtmlNativeComposeError):
        compose_model_decision_twin_search_weekly_html_native(
            decision=DECISION,
            twin_search_pack=TWIN_SEARCH_PACK,
