"""Pure tests for draft-before-merge + multi-select record write pack."""
...
def test_draft_multi_ready():
    c = compose_draft_before_merge_multi_select_record_write(
        draft_gate=DRAFT_GATE,
    ...
    assert "draft_written=false" in (
        format_draft_before_merge_multi_select_record_write_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(DraftBeforeMergeMultiSelectRecordWriteComposeError):
        compose_draft_before_merge_multi_select_record_write(
            draft_gate=DRAFT_GATE,
            multi_pack=MULTI_PACK,
