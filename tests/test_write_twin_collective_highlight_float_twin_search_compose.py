"""Pure tests for write twin collective + highlight float twin-search pack."""
...
def test_write_highlight_ready():
    c = compose_write_twin_collective_highlight_float_twin_search(
        write=WRITE,
    ...
    assert "draft_written=false" in (
        format_write_twin_collective_highlight_float_twin_search_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(WriteTwinCollectiveHighlightFloatTwinSearchComposeError):
        compose_write_twin_collective_highlight_float_twin_search(
            write=WRITE,
            highlight_pack=HIGHLIGHT_PACK,
