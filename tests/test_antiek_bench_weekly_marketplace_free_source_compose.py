"""Pure tests for Antiek-bench weekly + marketplace free source pack."""
...
def test_weekly_market_ready():
    c = compose_antiek_bench_weekly_marketplace_free_source(
        weekly_learn=WEEKLY_LEARN,
        market_research=MARKET_RESEARCH,
        operator_ack=True,
    )
    assert c.weekly_learn.learn_ready is True
    assert c.market_research.pack_ready is True
    ...
    assert "backlog_mutated=false" in (
        format_antiek_bench_weekly_marketplace_free_source_summary(c)
    )
    assert c.to_dict()["store_mutated"] is False
...
def test_sparse_events_not_learn_ready():
    c = compose_antiek_bench_weekly_marketplace_free_source(
        weekly_learn={
            "week_id": "2026-W28",
            "min_events_per_task": 10,
            "events": [
                {
                    "event_id": "e1",
                    "task": "deep_research",
...
def test_require_operator_ack_type():
    with pytest.raises(AntiekBenchWeeklyMarketplaceFreeSourceComposeError):
        compose_antiek_bench_weekly_marketplace_free_source(
            weekly_learn=WEEKLY_LEARN,
            market_research=MARKET_RESEARCH,
