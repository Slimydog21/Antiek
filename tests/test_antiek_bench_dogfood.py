...
def test_dogfood_fixture_payload_includes_shannon_turing_lovelace_v12() -> None:
    ...
    assert payload["suite_version"] == "suite-competitive-dogfood-v26"
    assert payload["item_count"] >= 41
    ...
    assert (
        "v26" in html or "moil" in html or "collective-unit-twin" in html or "v25" in html or "domain-aware" in html or "v24" in html or "budget-foresight" in html or "purchase" in html or "v23" in html or "quick-call" in html or "publication" in html
...
def test_payload_and_api_html():
    ...
    assert r1.json()["suite_version"] == "suite-competitive-dogfood-v26"
    assert r1.json()["item_count"] == r2.json()["item_count"]
    assert r1.json()["item_count"] >= 41
    assert r1.json()["by_task_class"]["book_qa"] == 9
    assert r1.json()["by_task_class"]["wrestle"] == 28
