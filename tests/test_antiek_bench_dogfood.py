...
def test_dogfood_fixture_payload_includes_shannon_turing_lovelace_v12() -> None:
    ...
    assert payload["suite_version"] == "suite-competitive-dogfood-v17"
    assert payload["item_count"] >= 27
    ...
    assert (
        "v17" in html or "select-recent" in html or "workstation" in html
...
def test_payload_and_api_html():
    ...
    assert r1.json()["suite_version"] == "suite-competitive-dogfood-v17"
    assert r1.json()["item_count"] == r2.json()["item_count"]
    assert r1.json()["item_count"] >= 27
    assert r1.json()["by_task_class"]["book_qa"] == 7
    assert r1.json()["by_task_class"]["wrestle"] == 16
