...
def test_dogfood_fixture_payload_includes_shannon_turing_lovelace_v12() -> None:
    ...
    assert payload["suite_version"] == "suite-competitive-dogfood-v31"
    assert payload["item_count"] >= 47
    ...
    assert (
        "v31" in html or "evidence-write-multi-hop" in html or "v30" in html or "domain-aware-stem-expanded" in html or "v29" in html or "citation-chain-hops" in html or "multi-hop" in html or "v28" in html or "citation-chain" in html or "scorecard" in html or "v27" in html or "foresight-chrome" in html or "v26" in html or "moil" in html or "collective-unit-twin" in html or "v25" in html or "domain-aware" in html or "v24" in html or "budget-foresight" in html or "purchase" in html or "v23" in html or "quick-call" in html or "publication" in html
...
def test_payload_and_api_html():
    ...
    assert r1.json()["suite_version"] == "suite-competitive-dogfood-v31"
    assert r1.json()["item_count"] == r2.json()["item_count"]
    assert r1.json()["item_count"] >= 47
    assert r1.json()["by_task_class"]["book_qa"] == 9
    assert r1.json()["by_task_class"]["wrestle"] == 34
