"""Pure tests for HTML-native + recursive twin MO write pack."""
...
def test_html_twin_mo_ready():
    c = compose_html_native_recursive_twin_mo_write_pack(
        html_view=HTML_VIEW,
        twin_mo=TWIN_MO,
        operator_ack=True,
    )
    assert c.html_view.pack_ready is True
    assert c.twin_mo.pack_ready is True
    ...
    assert "pdf_primary=false" in (
        format_html_native_recursive_twin_mo_write_pack_summary(c)
    )
...
def test_pdf_claim_never_authorizes_pdf():
    c = compose_html_native_recursive_twin_mo_write_pack(
        html_view={
            **HTML_VIEW,
            "claimed_format": "pdf",
...
def test_require_operator_ack_type():
    with pytest.raises(HtmlNativeRecursiveTwinMoWritePackComposeError):
        compose_html_native_recursive_twin_mo_write_pack(
            html_view=HTML_VIEW,
            twin_mo=TWIN_MO,
