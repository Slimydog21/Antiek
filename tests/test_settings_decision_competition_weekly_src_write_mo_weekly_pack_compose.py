...
from tests.test_competition_dr_nd_shadow_weekly_src_write_mo_weekly_pack_compose import (
    COMPETITION,
    ND_PACK,
)
...
def test_settings_decision_competition_pack_ready():
    c = compose_settings_decision_competition_weekly_src_write_mo_weekly_pack(
        settings=SETTINGS,
    ...
    assert "secrets_stored=false" in (
        format_settings_decision_competition_weekly_src_write_mo_weekly_pack_summary(c)
    )
