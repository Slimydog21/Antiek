...
from substrate.notebooks.authority import operator_notebook_account_authority

OPERATOR = operator_notebook_account_authority()
...
def test_create_notebook_defaults(db):
    authority = OPERATOR.notebook("notebook-defaults")
    create_notebook(db, authority, title="My research notebook")
    nb = get_notebook(db, authority)
...
def test_create_notebook_rejects_invalid_content_class(db):
    with pytest.raises(ValueError):
        create_notebook(db, OPERATOR.notebook("invalid-class"), title="X", content_class="garbage")


def test_append_blocks_preserves_order(db):
    authority = OPERATOR.notebook("ordered-blocks")
    create_notebook(db, authority, title="Test")
    b1 = append_block(db, authority, block_type="prose", content={"text": "Intro"})
    b2 = append_block(db, authority, block_type="claim_card", content={}, ref_id="claim-A")
    b3 = append_block(db, authority, block_type="note", content={}, ref_id="note-B")
...
def test_append_block_rejects_invalid_type(db):
    authority = OPERATOR.notebook("invalid-block")
    create_notebook(db, authority, title="Test")
    with pytest.raises(ValueError):
        append_block(db, authority, block_type="bogus", content={})


def test_block_index_starts_at_zero(db):
    authority = OPERATOR.notebook("zero-index")
    create_notebook(db, authority, title="Test")
    append_block(db, authority, block_type="prose", content={})
    nb = get_notebook(db, authority)
    assert nb is not None
...
def test_promote_to_public_changes_content_class(db):
    authority = OPERATOR.notebook("promotion")
    create_notebook(db, authority, title="Public-bound notebook")
    nb = get_notebook(db, authority)
    assert nb and nb.content_class == "user_owned"

    promote_to_public(db, authority)
    nb = get_notebook(db, authority)
    assert nb and nb.content_class == "user_public_contribution"


def test_list_notebooks_filtered_by_investigation(db):
    nb1 = create_notebook(db, OPERATOR.notebook("list-a"), title="A", investigation_id="inv-1")
    nb2 = create_notebook(db, OPERATOR.notebook("list-b"), title="B", investigation_id="inv-1")
    nb3 = create_notebook(db, OPERATOR.notebook("list-c"), title="C", investigation_id="inv-2")
    listed = list_notebooks(db, OPERATOR, investigation_id="inv-1")
...
def test_get_notebook_returns_none_for_missing(db):
    assert get_notebook(db, OPERATOR.notebook("nonexistent-id")) is None


def test_block_content_json_round_trips(db):
    authority = OPERATOR.notebook("round-trip")
    create_notebook(db, authority, title="Test")
    content = {
    ...
    block_id = append_block(db, authority, block_type="prose", content=content)
    nb = get_notebook(db, authority)
    assert nb is not None
