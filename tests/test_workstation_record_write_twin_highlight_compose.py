"""Pure tests for workstation record→prompt + write twin highlight pack."""
...
def test_record_write_ready():
    c = compose_workstation_record_write_twin_highlight(
        record_prompt=RECORD_PROMPT,
    ...
    assert "prompts_injected=false" in (
        format_workstation_record_write_twin_highlight_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(WorkstationRecordWriteTwinHighlightComposeError):
        compose_workstation_record_write_twin_highlight(
            record_prompt=RECORD_PROMPT,
            write_pack=WRITE_PACK,
