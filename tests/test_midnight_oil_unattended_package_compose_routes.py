"""Route tests for MO unattended package compose."""
...
def _client() -> TestClient:
    app = FastAPI()
    register_midnight_oil_unattended_package_compose_routes(app)
...
def test_compose_ok():
    c = _client()
    r = c.post(
        "/research/midnight-oil-unattended-package/compose",
        json={
            "operator_id": "op-1",
            "work_minutes": 120,
            "goals": [
    ...
    body = r.json()
    assert body["unattended_package_ready"] is True
...
def test_compose_unattended_false():
    ...
    assert r.status_code == 200
    assert r.json()["unattended_package_ready"] is False
