"""Pure tests for HTML-native + recursive twin settings fullscreen MO pack."""
...
def test_html_twin_ready():
    c = compose_html_native_recursive_twin_settings_fullscreen_mo(
        html_view=HTML_VIEW,
        twin_pack=TWIN_PACK,
        operator_ack=True,
    )
    ...
    assert "pdf_view_authorized=false" in (
        format_html_native_recursive_twin_settings_fullscreen_mo_summary(c)
    )
...
def test_parent_asset_mismatch_blocks():
    c = compose_html_native_recursive_twin_settings_fullscreen_mo(
...
def test_require_operator_ack_type():
    with pytest.raises(HtmlNativeRecursiveTwinSettingsFullscreenMoComposeError):
        compose_html_native_recursive_twin_settings_fullscreen_mo(
            html_view=HTML_VIEW,
            twin_pack=TWIN_PACK,
