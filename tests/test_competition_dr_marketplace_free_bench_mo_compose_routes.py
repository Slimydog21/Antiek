"""Route tests for competition DR + free marketplace bench MO pack."""
...
from tests.test_marketplace_free_antiek_bench_recommend_mo_compose_routes import (
    _payload as _fp_payload,
)


def _client() -> TestClient:
    app = FastAPI()
    register_competition_dr_marketplace_free_bench_mo_compose_routes(app)
    return TestClient(app)


def _payload(*, operator_ack: bool = True, session_id: str = "sess-1") -> dict:
    fp = _fp_payload(operator_ack=operator_ack, session_id=session_id)
    return {
        "competition": {
...
def test_compose_route():
    c = _client()
    r = c.post(
        "/research/competition-dr-marketplace-free-bench-mo/compose",
