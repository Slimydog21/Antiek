"""Pure tests for settings add-model + fullscreen MO draft multi pack."""
...
def test_settings_fullscreen_mo_ready():
    c = compose_settings_add_model_fullscreen_mo_draft_multi(
        settings=SETTINGS,
        fullscreen_mo=FULLSCREEN_MO,
        operator_ack=True,
    )
    assert c.settings.pack_ready is True
    assert c.fullscreen_mo.pack_ready is True
    ...
    assert "secrets_stored=false" in (
        format_settings_add_model_fullscreen_mo_draft_multi_summary(c)
    )
...
def test_propose_add_remains_pure():
    c = compose_settings_add_model_fullscreen_mo_draft_multi(
        settings={
            **SETTINGS,
            "action": "propose_add",
            "pending_add_model_ids": ["mimo-v2", "claude-opus"],
        },
        fullscreen_mo=FULLSCREEN_MO,
        operator_ack=True,
    )
...
def test_fullscreen_gated_fails_closed():
    with pytest.raises(
        SettingsAddModelFullscreenMoDraftMultiComposeError, match="gated"
    ):
        compose_settings_add_model_fullscreen_mo_draft_multi(
            settings=SETTINGS,
            fullscreen_mo={
...
def test_require_operator_ack_type():
    with pytest.raises(SettingsAddModelFullscreenMoDraftMultiComposeError):
        compose_settings_add_model_fullscreen_mo_draft_multi(
            settings=SETTINGS,
            fullscreen_mo=FULLSCREEN_MO,
