"""Pure tests for settings add-model + draft fullscreen weekly ND pack."""
...
def test_settings_research_ready():
    c = compose_settings_add_model_draft_fullscreen_weekly_nd_mo(
        settings=SETTINGS,
    ...
    assert "inventory_mutated=false" in (
        format_settings_add_model_draft_fullscreen_weekly_nd_mo_summary(c)
    )
...
def test_secret_like_pending_raises():
    with pytest.raises(SettingsAddModelDraftFullscreenWeeklyNdMoComposeError):
        compose_settings_add_model_draft_fullscreen_weekly_nd_mo(
            settings={**SETTINGS, "pending_add_model_ids": ["sk-abc123secret"]},
            research_pack=RESEARCH_PACK,
            operator_ack=True,
        )


def test_propose_add_never_mutates():
    c = compose_settings_add_model_draft_fullscreen_weekly_nd_mo(
        settings={
            **SETTINGS,
            "action": "propose_add",
    ...
    assert c.settings.proposed_new_count == 2
    assert c.pack_ready is True
    assert c.inventory_mutated is False
    assert c.secrets_stored is False
    assert c.merge_executed is False
