...
from tests.test_write_mode_twin_collective_fullscreen_collective_multiselect_compose import (
    FULLSCREEN_PACK,
    WRITE,
)
...
def test_source_attach_write_twin_ready():
    c = compose_source_attach_write_twin_collective_fullscreen(
        sources=SOURCES,
    ...
    assert "remote_fetched=false" in (
        format_source_attach_write_twin_collective_fullscreen_summary(c)
    )
