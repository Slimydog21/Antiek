...
from tests.test_write_mode_twin_collective_fullscreen_draft_before_merge_compose import (
    FULLSCREEN_PACK,
    WRITE,
)
...
def test_source_attach_write_twin_ready():
    c = compose_source_attach_write_twin_fullscreen_draft_before_merge(
        sources=SOURCES,
    ...
    assert "remote_fetched=false" in (
        format_source_attach_write_twin_fullscreen_draft_before_merge_summary(c)
    )
