"""Pure tests for twin search over competition DR ND weekly marketplace pack."""
...
def test_twin_search_competition_nd_ready():
    c = compose_twin_search_competition_dr_nd_shadow_weekly_marketplace(
        competition_pack=COMPETITION_PACK,
        search_query="scaling orchestration residual",
        operator_ack=True,
    )
    ...
    assert len(c.twin_corpus) > 0
    assert (
        c.authority
        == "twin_search_competition_dr_nd_shadow_weekly_marketplace_compose_advisory"
    )
    assert "remote_index_queried=false" in (
        format_twin_search_competition_dr_nd_shadow_weekly_marketplace_summary(c)
    )
    assert c.to_dict()["remote_index_queried"] is False
...
def test_extra_twins_expand_corpus():
    c = compose_twin_search_competition_dr_nd_shadow_weekly_marketplace(
        competition_pack=COMPETITION_PACK,
        search_query="attention efficiency",
        extra_twin_records=[
            {
                "twin_id": "twin-extra-1",
                "parent_asset_id": "asset-extra",
                "insights": ["attention efficiency tradeoffs under noise"],
                "questions": ["Where does attention break?"],
                "source_label": "extra",
            }
        ],
        operator_ack=True,
    )
    assert any(r["twin_id"] == "twin-extra-1" for r in c.twin_corpus)
    assert c.pack_ready is True
    assert c.remote_index_queried is False
    assert c.merge_executed is False
    assert c.twin_written is False
...
def test_require_operator_ack_type():
    with pytest.raises(TwinSearchCompetitionDrNdShadowWeeklyMarketplaceComposeError):
        compose_twin_search_competition_dr_nd_shadow_weekly_marketplace(
            competition_pack=COMPETITION_PACK,
            search_query="scaling",
