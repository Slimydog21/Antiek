...
def test_dispatch_authority_mismatch_does_not_charge(tmp_path):
    budget = BudgetManager()
    budget.register("expected", 1.0)
    event = RemoteStepEvent(seq=1, kind="step", cost_usd=0.25, tokens=10)
    authority = InvestigationAuthority("account", "foreign", tmp_path)
    with pytest.raises(ValueError, match="crosses authorized"):
        record_remote_dispatch(
            investigation_id="expected",
            event=event,
            budget=budget,
            authority=authority,
        )
    assert budget.spent("expected") == 0.0
...
def test_dispatch_call_emitted_in_host_local_shape(events_dir):
    ...
    ev = RemoteStepEvent(
        ...
        cost_usd=0.02,
        tokens=11,
        provider="fake-daytona",
        model="leaf",
        data={"input_tokens": 5, "output_tokens": 11, "latency_ms": 30, "tier": "pro"},
    )
    eid = record_remote_dispatch(
        investigation_id="inv-0",
        event=ev,
        budget=budget,
        events_dir=events_dir,
        authority=InvestigationAuthority(
            operator_claims().user_id, "inv-0", Path(events_dir)
        ),
    )
    assert eid
...
async def test_aggregate_cap_halts_new_launches(events_dir):
    ...
    r = RemoteResearchRunner(
        prov, claims=operator_claims(), budget=budget, events_dir=events_dir, seal_on_complete=False
    )
    ...
    rows = _trajectory("inv-2", events_dir)
    halted = [x for x in rows if x["action_type"] == ActionType.INVESTIGATION_CHASE_HALTED.value]
