...
from tests.test_recursive_twin_presentation_weekly_pack_compose import (
    PRESENTATION,
    TWIN,
    WEEKLY_PACK,
)
...
def test_nd_shadow_twin_presentation_weekly_ready():
    c = compose_nd_shadow_recursive_twin_weekly_pack(
        nd_shadow=ND_SHADOW,
    ...
    assert "live_router_authorized=false" in (
        format_nd_shadow_recursive_twin_weekly_pack_summary(c)
    )
