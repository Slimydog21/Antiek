...
def test_unauthenticated_local_keeps_the_legacy_namespace(env: Path) -> None:
    """Local dev (no operator-auth configured) is provably single-operator:
    the bare app — no auth middleware, exactly like the existing settings
    suites — keeps the legacy sentinel so local dev and those tests are
    unchanged. The namespace split begins only when auth exists."""
    app = FastAPI()
    register_settings_budget_routes(app)
    with TestClient(app) as local:
        assert local.post("/settings/models/user", json=_ADD_BODY).status_code == 201
        inventory = local.get("/settings/models/user").json()
        assert inventory["count"] == 1
    registry = json.loads((env / "settings" / "user_models.json").read_text())
    assert [record["owner_user_id"] for record in registry.values()] == ["__operator__"]
    reset_provider_registry()
