...
def test_write_to_read_outline_block_hop_stays_in_canonical_thread() -> None:
    """The real Write trace seam carries an outline_block id, not the insight
    node id. It still belongs to the insight thread by provenance lineage."""
    node = "node-flywheel"
    events = [
        _ev("e1", "seam.research_to_read", node, "2026-05-25T10:00:00"),
        _ev("e2", "seam.read_to_write", node, "2026-05-25T10:01:00"),
    ...
    assert [h.workflow for h in thread.hops] == ["research", "read", "write", "read"]
    assert [(h.entity_id, h.entity_kind) for h in thread.hops] == [
        (node, "insight_node"),
        (node, "insight_node"),
        (node, "insight_node"),
        ("oblk-1", "outline_block"),
    ]
...
def test_write_to_speak_hop_is_committed() -> None:
    """The write→speak seam is reconstructable as a committed hop."""
    node = "node-3"
    events = [
