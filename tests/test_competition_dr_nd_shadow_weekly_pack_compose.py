...
from tests.test_nd_shadow_recursive_twin_weekly_pack_compose import (
    ND_SHADOW,
    TWIN_PRESENTATION,
)
...
def test_competition_nd_shadow_twin_presentation_weekly_ready():
    c = compose_competition_dr_nd_shadow_weekly_pack(
    ...
    assert "live_dispatch_authorized=false" in (
        format_competition_dr_nd_shadow_weekly_pack_summary(c)
    )
