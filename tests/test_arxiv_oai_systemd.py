...
def test_arxiv_oai_sync_service_runs_incremental_with_live_state_paths():
    ...
    assert (
        ...
        'arxiv_throttle.json.governor.lock"'
    ) in service
    assert (
    ...
    assert "ExecStartPre=/usr/bin/mkdir -p {{ antiek_state_dir }}/reports" in service
    assert (
        "ExecStartPost={{ antiek_install_dir }}/.venv/bin/python -m "
        "tools.source_census --source arxiv --db-path "
...
def test_ansible_setup_and_deploy_render_and_enable_oai_timer():
    ...
    assert "state: restarted" in deploy
