"""Route tests for marketplace HTML view + twin session compose."""
...
from interfaces.research.api.marketplace_html_view_twin_session_compose_routes import (
...
def _client() -> TestClient:
    app = FastAPI()
    register_marketplace_html_view_twin_session_compose_routes(app)
...
def test_compose_paid():
    c = _client()
    r = c.post(
        "/research/marketplace-html-view-twin-session/compose",
        json={
            "session_id": "sess-1",
            "asset_id": "book-1",
            "title": "Deep Learning",
...
def test_compose_free():
    ...
    assert r.status_code == 200
    assert r.json()["session_ready"] is True
