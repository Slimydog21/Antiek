"""Pure tests for MO price-ceiling + draft multi-select record write pack."""
...
def test_mo_draft_multi_ready():
    c = compose_mo_price_ceiling_draft_multi_select_record_write(
        mo=MO,
        draft_multi=DRAFT_MULTI,
        operator_ack=True,
    )
    assert c.mo.pack_ready is True
    assert c.draft_multi.pack_ready is True
    ...
    assert "charge_executed=false" in (
        format_mo_price_ceiling_draft_multi_select_record_write_summary(c)
    )
...
def test_approve_ceiling_without_ack_blocks_mo():
    c = compose_mo_price_ceiling_draft_multi_select_record_write(
        mo={
            **MO,
...
def test_draft_parent_mismatch_blocks():
    dm = {
        **DRAFT_MULTI,
        "draft_gate": {
            **DRAFT_GATE,
            ...
            "sources": [
                {**s, "parent_asset_id": "book-other"}
                for s in DRAFT_GATE["sources"]
            ],
        },
    }
    c = compose_mo_price_ceiling_draft_multi_select_record_write(
    ...
    assert c.draft_multi.pack_ready is False
    assert c.pack_ready is False
    assert c.live_execution_authorized is False
    assert c.charge_executed is False
...
def test_require_operator_ack_type():
    with pytest.raises(MoPriceCeilingDraftMultiSelectRecordWriteComposeError):
        compose_mo_price_ceiling_draft_multi_select_record_write(
            mo=MO,
            draft_multi=DRAFT_MULTI,
