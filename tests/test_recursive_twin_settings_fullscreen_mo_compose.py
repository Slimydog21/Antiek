"""Pure tests for recursive twin + settings fullscreen MO pack."""
...
def test_twin_settings_ready():
    c = compose_recursive_twin_settings_fullscreen_mo(
        twin=TWIN,
    ...
    assert "twin_written=false" in (
        format_recursive_twin_settings_fullscreen_mo_summary(c)
    )
...
def test_empty_source_excerpt_fails_closed():
    with pytest.raises(
        RecursiveTwinSettingsFullscreenMoComposeError, match="source_excerpt"
    ):
        compose_recursive_twin_settings_fullscreen_mo(
            twin={**TWIN, "source_excerpt": "   "},
            settings_pack=SETTINGS_PACK,
            operator_ack=True,
        )
...
def test_require_operator_ack_type():
    with pytest.raises(RecursiveTwinSettingsFullscreenMoComposeError):
        compose_recursive_twin_settings_fullscreen_mo(
            twin=TWIN,
            settings_pack=SETTINGS_PACK,
