...
def _isolate_state(tmp_path, monkeypatch):
    ...
    The cached canonical throttle is cleared too. ``_check_endpoint_health`` now
    routes its probe through the host-global governor on
    ``canonical_arxiv_throttle()``, and that throttle binds its state path ONCE
    at construction — so without this reset the first test to build it would pin
    every later test (and, worse, a real run in the same process) to one
    tmp_path, and the >=3s spacing would leak across tests."""
...
# ── Endpoint health is GOVERNED (SWARM: arXiv lane) ──
#
# The verifier probes oaipmh.arxiv.org — the SAME host production harvests — and
# arXiv bans by IP. An ungoverned probe competes with a running harvest for the
# one-per-three-seconds budget and can trip the very ban it exists to report.


def test_endpoint_health_refuses_to_probe_while_the_ban_sentinel_is_armed(tmp_path):
    """With a live ban recorded in the shared throttle state, the check must
    report the ban and NOT egress. Probing during a ban extends it."""
    ...
    def fake_urlopen(req, timeout=None):
        ...
        raise AssertionError("egressed while the ban sentinel was armed")
    ...
    assert called == []
    assert not result.passed
    assert result.detail == "ban sentinel armed, endpoint not probed"


def test_endpoint_health_429_arms_the_ban_sentinel(monkeypatch):
    """A 429 from the probe must be recorded in the SHARED ban sentinel, so the
    next arXiv request anywhere on this box refuses before egressing.

    ``urlopen`` RAISES on a 4xx rather than returning, so a naive governed
    wrapper would let the 429 escape before ``note_response`` ever saw it and
    the sentinel would stay disarmed. This is the regression guard for that."""
    def fake_urlopen(req, timeout=None):
        raise urllib.error.HTTPError(
            str(req.full_url), 429, "Too Many Requests", {"Retry-After": "600"}, None
        )
    ...
    state = json.loads(Path(os.environ["ANTIEK_ARXIV_THROTTLE_PATH"]).read_text())
    assert state["banned_until"] > time.time(), (
        "a 429 from the verifier did not arm the host-global ban sentinel"
    )


def test_endpoint_health_probe_records_the_request_in_the_shared_state(monkeypatch):
    """A successful probe still passes through the governor: the shared
    ``last_request_at`` advances, which is what spaces the NEXT arXiv request on
    this box (a harvest page, a PDF fetch) three seconds behind this one."""
    identify = (
        b'<OAI-PMH xmlns="http://www.openarchives.org/OAI/2.0/">'
        b"<Identify><repositoryName>arXiv</repositoryName></Identify></OAI-PMH>"
    )
    ...
    assert result.passed
    state = json.loads(Path(os.environ["ANTIEK_ARXIV_THROTTLE_PATH"]).read_text())
    assert state["last_request_at"] > 0


def test_endpoint_health_sends_the_arxiv_contact_user_agent(monkeypatch):
    """arXiv policy asks for a reachable contact in the User-Agent. The probe
    must send the project's canonical one, not an anonymous verifier string —
    an unattributable request is the kind that gets blocked first."""
    ...
    monkeypatch.setattr("urllib.request.urlopen", fake_urlopen)
    _check_endpoint_health("https://oaipmh.arxiv.org/oai")

    assert seen["ua"] == default_user_agent()
    assert "Antiek" in seen["ua"]
