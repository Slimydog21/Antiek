...
@pytest.fixture
def book_server(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def _read(server: AntiekMemoryServer, uri: str) -> dict:
...
def test_withheld_book_returns_licensing_error_without_body_or_holder(
    book_server, name: str, servability: str,
) -> None:
...
def test_private_book_is_not_found(book_server) -> None:
...
def test_public_book_serves_enveloped_body_with_attribution(book_server, identifier: str) -> None:
    server, _handler, markers = book_server
...
def test_mismatched_identity_or_malformed_uri_is_not_found(book_server, uri: str) -> None:
    server, _handler, markers = book_server
    response = _read(server, uri)
    assert response["error"]["code"] == -32602
