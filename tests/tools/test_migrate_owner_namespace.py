...
def _seed_legacy_state(env: Path) -> None:
    ...
    # 1. user-model registry + model_provider credential: the bare app (no
    #    auth middleware) is exactly the pre-Option-A production posture.
    app = FastAPI()
...
# ---------------------------------------------------------------------------
# Connected BYO tools — the store the migration did not cover
# ---------------------------------------------------------------------------


def test_a_connected_tool_survives_the_migration_and_becomes_readable(env: Path) -> None:
    """End to end: connect as the sentinel, migrate, read as the derived owner.

    Re-owning the registry row alone is not enough. ``_metadata_matches``
    requires ``metadata.owner_user_id == record.owner_user_id``, and
    ``_byok_reown_candidates`` deliberately skips ``connector_*`` pipeline
    kinds, so the credential has to be re-sealed with the row or the connection
    reads as tampered.
    """
    from runtime.connectors.registry import connect_tool, list_tool_connections

    artifact = str(env / "byok" / "credentials.enc")
    # No explicit key_bytes: the migration reads through the configured key
    # file exactly as production does, so seeding with an ad-hoc key would test
    # a path that cannot occur.
    connect_tool(LEGACY_OWNER, "youtube", "AIza" + "a" * 24, artifact_path=artifact)
    # Invisible to the person who will own it, which is the live defect.
    assert [r.vendor for r in list_tool_connections(TARGET, artifact_path=artifact)
            if r.credential_present] == []

    report = run_migration(OPERATOR, apply=True)

    assert ("tool_connections.json", "registry") in {(m.store, m.table) for m in report.moves}
    after = {r.vendor: r for r in list_tool_connections(TARGET, artifact_path=artifact)}
    assert after["youtube"].credential_present is True, "credential did not survive re-sealing"
    # And it is gone from the sentinel's namespace — one-way, no duplication.
    assert [r.vendor for r in list_tool_connections(LEGACY_OWNER, artifact_path=artifact)
            if r.credential_present] == []


def test_a_connected_tool_is_listed_in_the_dry_run_without_being_moved(env: Path) -> None:
    from runtime.connectors.registry import connect_tool, list_tool_connections
    ...
    assert ("tool_connections.json", "registry") in {(m.store, m.table) for m in report.moves}
    assert report.applied is False
    still = [r.vendor for r in list_tool_connections(LEGACY_OWNER, artifact_path=artifact)
             if r.credential_present]
    assert still == ["youtube"], "dry run moved data"
