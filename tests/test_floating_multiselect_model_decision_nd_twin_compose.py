"""Pure tests for floating multi-select over model decision ND twin pack."""
...
MULTISELECT = {
    ...
    "cohesive_prompt": "Synthesize A and B as one unit over model decision ND twin",
}
...
def test_floating_multiselect_model_decision_nd_twin_ready():
    c = compose_floating_multiselect_model_decision_nd_twin(
    ...
    assert "live_dispatched=false" in (
        format_floating_multiselect_model_decision_nd_twin_summary(c)
    )
