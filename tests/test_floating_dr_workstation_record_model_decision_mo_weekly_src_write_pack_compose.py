...
from tests.test_workstation_record_model_decision_twin_search_mo_weekly_src_write_pack_compose import (
    DECISION_PACK,
    ITEMS,
)
...
def test_floating_dr_workstation_record_ready():
    c = compose_floating_dr_workstation_record_model_decision_mo_weekly_src_write_pack(
    ...
    assert "live_dispatched=false" in (
        format_floating_dr_workstation_record_model_decision_mo_weekly_src_write_pack_summary(c)
    )
...
def test_gated_highlight_fail_closed():
    with pytest.raises(
        FloatingDrWorkstationRecordModelDecisionMoWeeklySrcWritePackComposeError, match="gated"
    ):
        compose_floating_dr_workstation_record_model_decision_mo_weekly_src_write_pack(
