"""Pure tests for twin search + Antiek-bench weekly HTML-native pack."""
...
def test_twin_search_weekly_ready():
    c = compose_twin_search_antiek_bench_weekly_html_native(
    ...
    assert c.weekly_html.pack_ready is True
    ...
    assert "remote_index_queried=false" in (
        format_twin_search_antiek_bench_weekly_html_native_summary(c)
    )
...
def test_require_operator_ack_type():
    with pytest.raises(TwinSearchAntiekBenchWeeklyHtmlNativeComposeError):
        compose_twin_search_antiek_bench_weekly_html_native(
            search_query="scaling laws noise",
            twin_records=TWIN_RECORDS,
            weekly_html=WEEKLY_HTML,
