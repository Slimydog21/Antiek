"""Pure tests for Antiek-bench weekly + HTML-native recursive twin pack."""
...
def test_weekly_html_twin_ready():
    c = compose_antiek_bench_weekly_html_native_recursive_twin(
        weekly_learn=WEEKLY_LEARN,
        html_pack=HTML_PACK,
        operator_ack=True,
    )
    ...
    assert "suite_rewritten=false" in (
        format_antiek_bench_weekly_html_native_recursive_twin_summary(c)
    )
...
def test_insufficient_events_blocks_learn():
    c = compose_antiek_bench_weekly_html_native_recursive_twin(
        weekly_learn={
            "week_id": "2026-W28",
...
def test_require_operator_ack_type():
    with pytest.raises(AntiekBenchWeeklyHtmlNativeRecursiveTwinComposeError):
        compose_antiek_bench_weekly_html_native_recursive_twin(
            weekly_learn=WEEKLY_LEARN,
            html_pack=HTML_PACK,
