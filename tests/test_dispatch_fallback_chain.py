...
PRIMARY_TIERS_WITH_FALLBACK = ("flash", "pro", "synthesis", "verify")
ROLES_ROUTED_TO_PRIMARY_TIERS = (
    "decomposer",          # pro
...
def test_every_hermes_primary_tier_has_a_fallback(production_config):
    """The chain only works if every Hermes-primary tier carries a
    fallback. A tier without a fallback is a substrate-dark hazard
    the moment the bridge fails."""
    missing_fallbacks: list[str] = []
    for tier_name in PRIMARY_TIERS_WITH_FALLBACK:
        tier = production_config.tiers.get(tier_name)
        assert tier is not None, f"missing tier {tier_name!r}"
        if tier.provider != "hermes":
            # Already not Hermes-primary; not the case we insure.
            continue
        if tier.fallback is None:
            missing_fallbacks.append(tier_name)
    assert not missing_fallbacks, (
        f"Hermes-primary tiers without a fallback: {missing_fallbacks}. "
        f"A bridge outage will brick every dispatch routed to these "
        f"tiers. Add a fallback in substrate/dispatch/config.yaml."
    )


def test_fallbacks_all_route_through_openrouter(production_config):
    """Architectural posture: Hermes-primary, OpenRouter as the
    single fallback provider. Mixing fallback providers (e.g. some
    tiers fall to anthropic, others to openrouter) would break the
    single-degradation-path mental model the chaos test insures."""
    for tier_name in PRIMARY_TIERS_WITH_FALLBACK:
        tier = production_config.tiers[tier_name]
        if tier.fallback is None:
            continue
        assert tier.fallback.provider == "openrouter", (
            f"tier {tier_name!r} falls back to {tier.fallback.provider!r}; "
            f"production posture is openrouter-only for fallback"
        )
...
@pytest.mark.parametrize("role", ROLES_ROUTED_TO_PRIMARY_TIERS)
def test_dispatch_falls_through_to_openrouter_when_hermes_dies(
    role, production_config, _events_dir,
):
    """For every role that routes to a Hermes-primary tier, when the
    Hermes provider raises, the dispatch must complete via the
    OpenRouter fallback. The successful event must have
