...
def bootstrap() -> bool:
    ...
    value, when certifi is missing or broken (leave the interpreter default
    alone rather than point at nothing), or when certifi names a bundle that is
    not a real file. Never raises: every entrypoint calls this at import, so a
    soft dependency must not crash a CLI. Idempotent: a second call is a no-op
