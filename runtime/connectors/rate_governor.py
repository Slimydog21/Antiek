...
def _state_file(state_root: Path, vendor: str, *, owner: str | None) -> Path:
    ...
    case and is deliberate, not a fallback: it is for the operator-run
    acquisition lanes, where one key serves the whole host —
    ``acquisition/twitter/api_client`` (the operator's own BYOK bearer, driven
